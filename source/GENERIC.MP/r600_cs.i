...
static int r600_cs_track_validate_cb(struct radeon_cs_parser *p, int i)
{
 ...
 if (!r600_fmt_is_valid_color(format)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d cb invalid format %d for %d (0x%08X)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 366, format, i, track->cb_color_info[i]);
  return -22;
 }
 ...
 if (!(((pitch) & ((pitch_align) - 1)) == 0)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d cb pitch (%d, 0x%x, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 413, pitch, pitch_align, array_mode);
  return -22;
 }
 if (!(((height) & ((height_align) - 1)) == 0)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d cb height (%d, 0x%x, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 418, height, height_align, array_mode);
  return -22;
 }
...
static int r600_cs_track_validate_db(struct radeon_cs_parser *p)
{
 ...
 if ((track->db_depth_size & 0xFFFFFC00) == 0xFFFFFC00) {
  ...
  if (!(((pitch) & ((pitch_align) - 1)) == 0)) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d db pitch (%d, 0x%x, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 604, pitch, pitch_align, array_mode);
   return -22;
  }
  if (!(((height) & ((height_align) - 1)) == 0)) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d db height (%d, 0x%x, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 609, height, height_align, array_mode);
   return -22;
  }
  if (!(((base_offset) & ((base_align) - 1)) == 0)) {
 ...
 if ((((track->db_depth_info) >> 25) & 0x1)) {
  ...
  if (track->htile_bo == ((void *)0)) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d htile enabled without htile surface 0x%08x\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 637, track->db_depth_info);
   return -22;
  }
  if ((track->db_depth_size & 0xFFFFFC00) == 0xFFFFFC00) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d htile can't be enabled with bogus db_depth_size 0x%08x\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 642, track->db_depth_size);
   return -22;
  }
  nbx = pitch;
  nby = height;
  if ((((track->htile_surface) >> 2) & 0x1)) {
   ...
   case 1:
    ...
    break;
   default:
    printf("drm:pid%d:%s *WARNING* " "%s:%d invalid num pipes %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 681, track->npipes);
    return -22;
   }
  }
  nbx = nbx >> 3;
  ...
  if (size > radeon_bo_size(track->htile_bo)) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d htile surface too small %ld for %ld (%d %d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 695, radeon_bo_size(track->htile_bo), size, nbx, nby);
   return -22;
  }
 }
 track->db_dirty = 0;
  ...
  for (i = 0; i < 8; i++) {
   ...
   if (format != 0x00000000 &&
    ...
    if (track->cb_color_bo[i] == ((void *)0)) {
     printf("drm:pid%d:%s *WARNING* " "%s:%d mask 0x%08X | 0x%08X no cb for %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 758, track->cb_target_mask, track->cb_shader_mask, i);
     return -22;
    }
...
{
 static uint32_t vline_start_end[2] = {0x6538,
           0x6d38};
 static uint32_t vline_status[2] = {0x653c,
        0x6d3c};
          ...
          uint32_t *vline_status)
{
 ...
 if (wait_reg_mem_info & 0x10) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "vline WAIT_REG_MEM waiting on MEM instead of REG\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return -22;
 }
 if (wait_reg_mem_info & 0x100) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "vline WAIT_REG_MEM waiting on PFP instead of ME\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return -22;
 }
 ...
 } else if (reg64 == vline_start_end[0]) {
  header &= ~0x0000ffff;
...
static int r600_cs_check_reg(struct radeon_cs_parser *p, u32 reg64, u32 idx)
{
 ...
 case 0x0280BC:
  if (!(p->cs_flags & 0x01) &&
   ...
   } else if (reloc->tiling_flags & 0x2) {
           ...
           u32 tiling_flags)
{
 ...
 if (!r600_fmt_is_valid_texture(format, p->family)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d texture invalid format %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1550, format);
  return -22;
 }
 if (r600_get_array_mode_alignment(&array_check,
       &pitch_align, &height_align, &depth_align, &base_align)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d tex array mode (%d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1557, (((word0) >> 3) & 0xF));
  return -22;
 }
 if (!(((pitch) & ((pitch_align) - 1)) == 0)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d tex pitch (%d, 0x%x, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1565, pitch, pitch_align, (((word0) >> 3) & 0xF));
  return -22;
 }
 if (!(((base_offset) & ((base_align) - 1)) == 0)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d tex base offset (0x%llx, 0x%llx, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1570, base_offset, base_align, (((word0) >> 3) & 0xF));
  return -22;
 }
 if (!(((mip_offset) & ((base_align) - 1)) == 0)) {
  printf("drm:pid%d:%s *WARNING* " "%s:%d tex mip offset (0x%llx, 0x%llx, %d) invalid\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1575, mip_offset, base_align, (((word0) >> 3) & 0xF));
  return -22;
 }
 if (blevel > llevel) {
    ...
    struct radeon_cs_packet *pkt)
{
 ...
 case 0x2B:
 {
  ...
  if (r) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d invalid cmd stream\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1722);
   return r;
  }
  break;
 }
 case 0x2D:
  ...
  r = r600_cs_track_check(p);
  if (r) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d invalid cmd stream %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1734, idx);
   return r;
  }
  break;
 case 0x29:
 case 0x2E:
  ...
  r = r600_cs_track_check(p);
  if (r) {
   printf("drm:pid%d:%s *WARNING* " "%s:%d invalid cmd stream\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , __func__, 1746);
   return r;
  }
  break;
 case 0x3C:
  ...
  case 0x3:
   ...
   if (tiled) {
    ...
    if (idx_value & (1 << 31)) {
     ...
     ib[idx+6] += ((u32)(((dst_reloc->gpu_offset) >> 16) >> 16)) & 0xff;
    } else {
    ...
    if (p->family >= CHIP_RV770) {
     ...
     ib[idx+1] += (u32)(dst_reloc->gpu_offset & 0xfffffffc);
     ib[idx+2] += (u32)(src_reloc->gpu_offset & 0xfffffffc);
     ib[idx+3] += ((u32)(((src_reloc->gpu_offset) >> 16) >> 16)) & 0xff;
     ib[idx+3] += (((u32)(((dst_reloc->gpu_offset) >> 16) >> 16)) & 0xff) << 16;
     p->idx += 4;
    }
   }
