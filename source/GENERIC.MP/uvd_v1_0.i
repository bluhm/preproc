...
uint32_t uvd_v1_0_get_rptr(struct radeon_device *rdev,
      struct radeon_ring *ring)
{
 return r100_mm_rreg(rdev, (0xf690), 0);
}
uint32_t uvd_v1_0_get_wptr(struct radeon_device *rdev,
      struct radeon_ring *ring)
{
 return r100_mm_rreg(rdev, (0xf694), 0);
}
void uvd_v1_0_set_wptr(struct radeon_device *rdev,
         struct radeon_ring *ring)
{
 r100_mm_wreg(rdev, (0xf694), (ring->wptr), 0);
}
void uvd_v1_0_fence_emit(struct radeon_device *rdev,
    struct radeon_fence *fence)
{
...
int uvd_v1_0_resume(struct radeon_device *rdev)
{
 ...
 size = (((rdev->uvd_fw->size) + (4096 - 1)) & ~(4096 - 1)) >> 3;
 r100_mm_wreg(rdev, (0xf608), (addr), 0);
 r100_mm_wreg(rdev, (0xf60c), (size), 0);
 addr += size;
 size = (1024*1024) >> 3;
 r100_mm_wreg(rdev, (0xf610), (addr), 0);
 r100_mm_wreg(rdev, (0xf614), (size), 0);
 addr += size;
 size = (1024*1024) >> 3;
 r100_mm_wreg(rdev, (0xf618), (addr), 0);
 r100_mm_wreg(rdev, (0xf61c), (size), 0);
 addr = (rdev->uvd.gpu_addr >> 28) & 0xF;
 r100_mm_wreg(rdev, (0xf594), ((addr << 12) | (addr << 0)), 0);
 addr = (rdev->uvd.gpu_addr >> 32) & 0xFF;
 r100_mm_wreg(rdev, (0xf498), (addr | (0x9 << 16) | (0x1 << 31)), 0);
 r100_mm_wreg(rdev, (0xf51C), (*((uint32_t*)rdev->uvd.cpu_addr)), 0);
 return 0;
}
int uvd_v1_0_init(struct radeon_device *rdev)
{
 struct radeon_ring *ring = &rdev->ring[5];
 uint32_t tmp;
 int r;
 if (rdev->family < CHIP_RV740)
  (rdev)->asic->pm.set_uvd_clocks((rdev), (10000), (10000));
 else
 ...
 if (r)
  goto done;
 ring->ready = 1;
 r = (rdev)->asic->ring[(5)]->ring_test((rdev), (ring));
 if (r) {
  ring->ready = 0;
  goto done;
 }
 ...
 if (r) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "radeon: ring failed to lock UVD ring (%d).\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
  goto done;
 }
 tmp = ((0 << 30) | (((0xf6c8) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16);
 radeon_ring_write(ring, tmp);
 radeon_ring_write(ring, 0xFFFFF);
 tmp = ((0 << 30) | (((0xf6c4) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16);
 radeon_ring_write(ring, tmp);
 radeon_ring_write(ring, 0xFFFFF);
 tmp = ((0 << 30) | (((0xf6cc) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16);
 radeon_ring_write(ring, tmp);
 radeon_ring_write(ring, 0xFFFFF);
 radeon_ring_write(ring, ((0 << 30) | (((0xf6c0) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16));
 radeon_ring_write(ring, 0x8);
 radeon_ring_write(ring, ((0 << 30) | (((0xf400) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16));
 ...
 if (!r) {
  ...
  case CHIP_RV630:
  case CHIP_RV620:
   r100_mm_wreg(rdev, (0x2000), (0), 0);
   r100_mm_wreg(rdev, (0x2000), (1 << 4), 0);
   r100_mm_wreg(rdev, (0x2348), (0x3f), 0);
   r100_mm_wreg(rdev, (0x2000), (0x1f), 0);
  case CHIP_RV670:
  case CHIP_RV635:
   do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf660), 0); tmp_ &= (~0x10); tmp_ |= ((0x10) & ~(~0x10)); r100_mm_wreg(rdev, (0xf660), (tmp_), 0); } while (0);
   break;
  default:
   break;
  }
...
int uvd_v1_0_start(struct radeon_device *rdev)
{
 ...
 u32 lmi_swap_cntl = 0;
 u32 mp_swap_cntl = 0;
 r100_mm_wreg(rdev, (0xf4a8), (0), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf500), 0); tmp_ &= (~(1 << 1)); tmp_ |= ((0) & ~(~(1 << 1))); r100_mm_wreg(rdev, (0xf500), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf4f4), 0); tmp_ &= (~(1 << 8)); tmp_ |= ((1 << 8) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0xf4f4), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf480), 0); tmp_ &= (~(1 << 3)); tmp_ |= ((1 << 3) & ~(~(1 << 3))); r100_mm_wreg(rdev, (0xf480), (tmp_), 0); } while (0);
 mdelay(1);
 r100_mm_wreg(rdev, (0xf680), ((1<<2) | (1<<3) | (1<<1) | (1<<0) | (1<<5) | (1<<6) | (1<<7) | (1<<13)), 0);
 mdelay(5);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xe60), 0); tmp_ &= (~(1 << 18)); tmp_ |= ((0) & ~(~(1 << 18))); r100_mm_wreg(rdev, (0xe60), (tmp_), 0); } while (0);
 mdelay(5);
 r100_mm_wreg(rdev, (0xf598), (0x40 | (1 << 8) | (1 << 13) | (1 << 21) | (1 << 9) | (1 << 20)), 0);
 lmi_swap_cntl = 0xa;
 mp_swap_cntl = 0;
 r100_mm_wreg(rdev, (0xf5b4), (lmi_swap_cntl), 0);
 r100_mm_wreg(rdev, (0xf5bC), (mp_swap_cntl), 0);
 r100_mm_wreg(rdev, (0xf5e4), (0x40c2040), 0);
 r100_mm_wreg(rdev, (0xf5e8), (0x0), 0);
 r100_mm_wreg(rdev, (0xf5eC), (0x40c2040), 0);
 r100_mm_wreg(rdev, (0xf5f0), (0x0), 0);
 r100_mm_wreg(rdev, (0xf5f8), (0), 0);
 r100_mm_wreg(rdev, (0xf5f4), (0x88), 0);
 r100_mm_wreg(rdev, (0xf680), ((1<<3)), 0);
 mdelay(5);
 r100_mm_wreg(rdev, (0xf660), (1 << 9), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf4f4), 0); tmp_ &= (~(1 << 8)); tmp_ |= ((0) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0xf4f4), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf480), 0); tmp_ &= (~(1 << 3)); tmp_ |= ((0) & ~(~(1 << 3))); r100_mm_wreg(rdev, (0xf480), (tmp_), 0); } while (0);
 r100_mm_wreg(rdev, (0xf680), (0), 0);
 mdelay(10);
 for (i = 0; i < 10; ++i) {
  uint32_t status;
  for (j = 0; j < 100; ++j) {
   status = r100_mm_rreg(rdev, (0xf6bc), 0);
   if (status & 2)
    break;
   mdelay(10);
  }
  ...
  if (status & 2)
   break;
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "UVD not responding, trying to reset the VCPU!!!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf680), 0); tmp_ &= (~(1<<3)); tmp_ |= (((1<<3)) & ~(~(1<<3))); r100_mm_wreg(rdev, (0xf680), (tmp_), 0); } while (0);
  mdelay(10);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf680), 0); tmp_ &= (~(1<<3)); tmp_ |= ((0) & ~(~(1<<3))); r100_mm_wreg(rdev, (0xf680), (tmp_), 0); } while (0);
  mdelay(10);
  r = -1;
 }
 if (r) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "UVD not responding, giving up!!!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return r;
 }
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf500), 0); tmp_ &= (~(3 << 1)); tmp_ |= ((3<<1) & ~(~(3 << 1))); r100_mm_wreg(rdev, (0xf500), (tmp_), 0); } while (0);
 r100_mm_wreg(rdev, (0xf6a4), (0x11010101), 0);
 r100_mm_wreg(rdev, (0xf698), (0), 0);
 r100_mm_wreg(rdev, (0xf498), (((u32)(((ring->gpu_addr) >> 16) >> 16)) | (0x7 << 16) | (0x1 << 31)), 0);
 r100_mm_wreg(rdev, (0xf690), (0x0), 0);
 ring->wptr = r100_mm_rreg(rdev, (0xf690), 0);
 r100_mm_wreg(rdev, (0xf694), (ring->wptr), 0);
 r100_mm_wreg(rdev, (0xf68c), (ring->gpu_addr), 0);
 rb_bufsz = drm_order(ring->ring_size);
 rb_bufsz = (0x1 << 8) | rb_bufsz;
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf6a4), 0); tmp_ &= (~0x11f1f); tmp_ |= ((rb_bufsz) & ~(~0x11f1f)); r100_mm_wreg(rdev, (0xf6a4), (tmp_), 0); } while (0);
 return 0;
}
void uvd_v1_0_stop(struct radeon_device *rdev)
{
 ...
 r100_mm_wreg(rdev, (0xf680), ((1<<3)), 0);
 mdelay(5);
 r100_mm_wreg(rdev, (0xf660), (0x0), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf4f4), 0); tmp_ &= (~(1 << 8)); tmp_ |= ((0) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0xf4f4), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0xf480), 0); tmp_ &= (~(1 << 3)); tmp_ |= ((0) & ~(~(1 << 3))); r100_mm_wreg(rdev, (0xf480), (tmp_), 0); } while (0);
}
int uvd_v1_0_ring_test(struct radeon_device *rdev, struct radeon_ring *ring)
{
 ...
 unsigned i;
 int r;
 r100_mm_wreg(rdev, (0xf6f4), (0xCAFEDEAD), 0);
 ...
 for (i = 0; i < rdev->usec_timeout; i++) {
  tmp = r100_mm_rreg(rdev, (0xf6f4), 0);
  if (tmp == 0xDEADBEEF)
   break;
  delay(1);
 }
...
void uvd_v1_0_ib_execute(struct radeon_device *rdev, struct radeon_ib *ib)
{
 ...
 radeon_ring_write(ring, ((0 << 30) | (((0xf684) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16));
 radeon_ring_write(ring, ib->gpu_addr);
 radeon_ring_write(ring, ((0 << 30) | (((0xf688) >> 2) & 0xFFFF) | ((0) & 0x3FFF) << 16));
...
int uvd_v1_0_ib_test(struct radeon_device *rdev, struct radeon_ring *ring)
{
 ...
 int r;
 if (rdev->family < CHIP_RV740)
  r = (rdev)->asic->pm.set_uvd_clocks((rdev), (10000), (10000));
 else
  r = (rdev)->asic->pm.set_uvd_clocks((rdev), (53300), (40000));
 if (r) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "radeon: failed to raise UVD clocks (%d).\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
  return r;
 }
