       ...
       struct drm_atomic_state *state)
{
 ...
 for ((i) = 0; (i) < (state)->dev->mode_config.num_total_plane && ((plane) = (state)->planes[i], (plane_state) = (state)->plane_states[i], 1); (i)++) if (plane_state) {
  ...
  fence_wait(plane->state->fence, 0);
  ...
  struct drm_atomic_state *old_state)
{
 ...
 for ((i) = 0; (i) < (old_state)->dev->mode_config.num_crtc && ((crtc) = (old_state)->crtcs[i], (old_crtc_state) = (old_state)->crtc_states[i], 1); (i)++) if (old_crtc_state) {
  if (!old_crtc_state->enable)
   continue;
  ret = ({ long __ret = (((uint64_t)(50)) * hz / 1000); if (!(old_crtc_state->last_vblank_count != drm_crtc_vblank_count(crtc))) __ret = ({ long ret = (((uint64_t)(50)) * hz / 1000); do { int deadline, __error; ((!cold) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_atomic_helper.c", 990, "!cold")); __mtx_enter(&sch_mtx ); ((void)_atomic_add_int_nv((&(dev->vblank[i].queue).count), 1)); deadline = ticks + ret; __error = msleep(&dev->vblank[i].queue, &sch_mtx, 0, "drmweti", ret); ret = deadline - ticks; ((void)_atomic_sub_int_nv((&(dev->vblank[i].queue).count), 1)); if (__error == -1 || __error == 4) { ret = -4; __mtx_leave(&sch_mtx ); break; } if ((((uint64_t)(50)) * hz / 1000) && (ret <= 0 || __error == 35)) { __mtx_leave(&sch_mtx ); ret = ((old_crtc_state->last_vblank_count != drm_crtc_vblank_count(crtc))) ? 1 : 0; break; } __mtx_leave(&sch_mtx ); } while (ret > 0 && !(old_crtc_state->last_vblank_count != drm_crtc_vblank_count(crtc))); ret; }); __ret; });
...
retry:
 ...
 for (assert_drm_connector_list_read_locked(&(connector->dev)->mode_config), tmp_connector = ({ const __typeof( ((struct drm_connector *)0)->head ) *__mptr = ((&(connector->dev)->mode_config.connector_list)->next); (struct drm_connector *)( (char *)__mptr - __builtin_offsetof(struct drm_connector, head) );}); &tmp_connector->head != (&(connector->dev)->mode_config.connector_list); tmp_connector = ({ const __typeof( ((typeof(*(tmp_connector)) *)0)->head ) *__mptr = (((tmp_connector)->head.next)); (typeof(*(tmp_connector)) *)( (char *)__mptr - __builtin_offsetof(typeof(*(tmp_connector)), head) );})) {
      ...
      struct drm_modeset_acquire_ctx *ctx)
{
 ...
 for (assert_drm_connector_list_read_locked(&(dev)->mode_config), conn = ({ const __typeof( ((struct drm_connector *)0)->head ) *__mptr = ((&(dev)->mode_config.connector_list)->next); (struct drm_connector *)( (char *)__mptr - __builtin_offsetof(struct drm_connector, head) );}); &conn->head != (&(dev)->mode_config.connector_list); conn = ({ const __typeof( ((typeof(*(conn)) *)0)->head ) *__mptr = (((conn)->head.next)); (typeof(*(conn)) *)( (char *)__mptr - __builtin_offsetof(typeof(*(conn)), head) );})) {
  struct drm_connector_state *conn_state;
