...
struct eso_softc {
 ...
 unsigned int sc_revision;
 struct pci_attach_args sc_pa;
 bus_size_t sc_vcsize;
 bus_dma_tag_t sc_dmat;
 ...
 int sc_rdrain;
 uint8_t sc_a2c2;
 uint8_t sc_gain[(17 + 1)][2];
 unsigned int sc_recsrc;
 unsigned int sc_monooutsrc;
 unsigned int sc_monoinbypass;
 unsigned int sc_recmon;
 unsigned int sc_preamp;
 unsigned int sc_spatializer;
 unsigned int sc_mvmute;
};
struct eso_dma {
...
struct cfattach eso_ca = {
 sizeof (struct eso_softc), eso_match, eso_attach, ((void *)0),
 eso_activate
};
...
eso_match(struct device *parent, void *match, void *aux)
{
 ...
 if ((((pa->pa_id) >> 0) & 0xffff) == 0x125d &&
     (((pa->pa_id) >> 16) & 0xffff) == 0x1969)
  return (1);
 return (0);
}
void
eso_attach(struct device *parent, struct device *self, void *aux)
{
 ...
 if (pci_mapreg_map(pa, 0x14, 0x00000001, 0,
     &sc->sc_sb_iot, &sc->sc_sb_ioh, ((void *)0), ((void *)0), 0)) {
  printf(": can't map SB I/O space\n");
  return;
 }
 if (pci_mapreg_map(pa, 0x18, 0x00000001, 0,
     &sc->sc_dmac_iot, &sc->sc_dmac_ioh, &vcbase, &sc->sc_vcsize, 0)) {
  vcbase = 0;
  sc->sc_vcsize = 0x10;
 }
 if (pci_mapreg_map(pa, 0x1c, 0x00000001, 0,
     &sc->sc_mpu_iot, &sc->sc_mpu_ioh, ((void *)0), ((void *)0), 0)) {
 ...
 printf(", %s\n", intrstring);
 if ((((vcbase) & 0x03ff) != 0)) {
  pci_conf_write(pa->pa_pc, pa->pa_tag, 0x60,
          vcbase | 0x01);
 ...
 if (sc->sc_mpudev != ((void *)0)) {
  mvctl = eso_read_mixreg(sc, 0x64);
  mvctl |= 0x40;
  eso_write_mixreg(sc, 0x64, mvctl);
 }
}
void
eso_setup(struct eso_softc *sc, int verbose, int resuming)
{
 ...
 pci_conf_write(pa->pa_pc, pa->pa_tag, 0x50,
         pci_conf_read(pa->pa_pc, pa->pa_tag, 0x50) &
         ~(0x0006000 | 0x0000700));
 bus_space_write_1(sc->sc_iot, sc->sc_ioh, 0x07,
     0x10 | 0x20 | 0x40 |
     0x80);
 a2mode = eso_read_mixreg(sc, 0x71);
 a2mode |= 0x20 | 0x02;
 eso_write_mixreg(sc, 0x71, a2mode);
 tmp = eso_read_mixreg(sc, 0x64);
 tmp &= ~0x80;
 tmp |= 0x02;
 eso_write_mixreg(sc, 0x64, tmp);
 if (!resuming) {
  sc->sc_recmon = sc->sc_spatializer = sc->sc_mvmute = 0;
  eso_set_monooutsrc(sc, 0x00);
  eso_set_monoinbypass(sc, 0);
  eso_set_preamp(sc, 1);
  for (idx = 0; idx < (17 + 1); idx++) {
   int v;
   switch (idx) {
    case 1:
   case 2:
  ...
  tmp = eso_read_ctlreg(sc, 0xa8);
  if (sc->sc_recmon)
   tmp |= 0x08;
  else
   tmp &= ~0x08;
  eso_write_ctlreg(sc, 0xa8, tmp);
  tmp = eso_read_mixreg(sc, 0x50);
  if (sc->sc_spatializer)
   tmp |= 0x08;
  else
   tmp &= ~0x08;
  eso_write_mixreg(sc, 0x50,
      tmp | 0x04);
  if (sc->sc_mvmute) {
   eso_write_mixreg(sc, 0x60,
       eso_read_mixreg(sc, 0x60) |
       0x40);
   eso_write_mixreg(sc, 0x62,
       eso_read_mixreg(sc, 0x62) |
       0x40);
  } else {
   eso_write_mixreg(sc, 0x60,
       eso_read_mixreg(sc, 0x60) &
...
eso_write_cmd(struct eso_softc *sc, uint8_t cmd)
{
 int i;
 for (i = 0; i < 5000; i++) {
  if ((bus_space_read_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x0c)
      & 0x80) == 0) {
...
void
eso_write_ctlreg(struct eso_softc *sc, uint8_t reg64, uint8_t val)
{
 eso_write_cmd(sc, reg64);
...
uint8_t
eso_read_ctlreg(struct eso_softc *sc, uint8_t reg64)
{
 eso_write_cmd(sc, 0xc0);
 eso_write_cmd(sc, reg64);
 return (eso_read_rdr(sc));
}
void
eso_write_mixreg(struct eso_softc *sc, uint8_t reg64, uint8_t val)
{
 bus_space_write_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x04, reg64);
 bus_space_write_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x05, val);
}
uint8_t
eso_read_mixreg(struct eso_softc *sc, uint8_t reg64)
{
 uint8_t val;
 bus_space_write_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x04, reg64);
 val = bus_space_read_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x05);
 return (val);
}
int
eso_intr(void *hdl)
{
 ...
 if (irqctl & 0x20) {
  eso_write_mixreg(sc, 0x7a, sc->sc_a2c2);
  if (sc->sc_pintr)
...
{
 int i;
 bus_space_write_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x06,
     0x01 | 0x02);
 (void)bus_space_read_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x00);
 bus_space_write_1(sc->sc_sb_iot, sc->sc_sb_ioh, 0x06, 0);
 for (i = 0; i < 5000; i++) {
  if ((bus_space_read_1(sc->sc_sb_iot, sc->sc_sb_ioh,
      0x0e) & 0x80) != 0 &&
      bus_space_read_1(sc->sc_sb_iot, sc->sc_sb_ioh,
   0x0a) == 0xaa) {
   eso_write_cmd(sc, 0xc6);
   eso_write_mixreg(sc, 0x00,
       0x00);
   return (0);
  } else {
   delay(1000);
    ...
    struct audio_params *play, struct audio_params *rec)
{
 ...
 for (mode = 0x02; mode != -1;
  ...
  p->msb = 1;
  r[0] = 793800L /
      (128 - (rd[0] = 128 - 793800L / p->sample_rate));
  r[1] = 768000L /
      (128 - (rd[1] = 128 - 768000L / p->sample_rate));
  ar[0] = p->sample_rate - r[0];
  ar[1] = p->sample_rate - r[1];
  clk = ((ar[0]) < 0 ? -(ar[0]) : (ar[0])) > ((ar[1]) < 0 ? -(ar[1]) : (ar[1])) ? 1 : 0;
  srg = rd[clk] | (clk == 1 ? 0x80 : 0x00);
  fltdiv = 256 - 200279L / r[clk];
  p->sample_rate = r[clk];
  if (mode == 0x02) {
   ;
   eso_write_ctlreg(sc, 0xa1, srg);
   eso_write_ctlreg(sc, 0xa2, fltdiv);
  } else {
   ;
   eso_write_mixreg(sc, 0x70, srg);
   eso_write_mixreg(sc, 0x72, fltdiv);
  }
 }
 return (0);
}
...
eso_halt_input(void *hdl)
{
 ...
 eso_write_ctlreg(sc, 0xb8,
     0x02 | 0x08 |
     0x01);
 bus_space_write_1(sc->sc_dmac_iot, sc->sc_dmac_ioh, 0x0b,
     0x04 | 0x00);
 sc->sc_rintr = ((void *)0);
...
eso_set_port(void *hdl, mixer_ctrl_t *cp)
{
 ...
 case 17:
  ...
  case 1:
   lgain = rgain = ((cp->un.value.level[0]) & 0xf0);
   break;
  case 2:
   lgain = ((cp->un.value.level[0]) & 0xf0);
   rgain = ((cp->un.value.level[1]) & 0xf0);
   break;
  default:
   goto error;
  }
 ...
 case 7:
  ...
  switch (cp->un.value.num_channels) {
  case 1:
   lgain = rgain = ((cp->un.value.level[0]) & 0xfc);
   break;
  case 2:
   lgain = ((cp->un.value.level[0]) & 0xfc);
   rgain = ((cp->un.value.level[1]) & 0xfc);
   break;
  default:
   goto error;
  }
...
eso_query_devinfo(void *hdl, mixer_devinfo_t *dip)
{
 ...
 case 19:
  ...
  dip->type = 1;
  dip->un.e.num_mem = 3;
  strlcpy(dip->un.e.member[0].label.name, "mute",
      sizeof dip->un.e.member[0].label.name);
  dip->un.e.member[0].ord = 0x00;
  strlcpy(dip->un.e.member[1].label.name, "dac",
      sizeof dip->un.e.member[1].label.name);
  dip->un.e.member[1].ord = 0x04;
  strlcpy(dip->un.e.member[2].label.name, "mixerout",
      sizeof dip->un.e.member[2].label.name);
  dip->un.e.member[2].ord = 0x06;
  break;
 case 20:
  dip->mixer_class = 30;
  dip->next = dip->prev = -1;
 ...
 case 9:
  ...
  dip->prev = -1;
  dip->next = 23;
  strlcpy(dip->label.name, "spatial",
      sizeof dip->label.name);
  dip->type = 3;
  dip->un.v.num_channels = 1;
 ...
 case 28:
  dip->mixer_class = 28;
  dip->next = dip->prev = -1;
  strlcpy(dip->label.name, "monitor",
      sizeof dip->label.name);
  dip->type = 0;
  break;
 case 10:
 ...
 case 18:
  ...
  dip->type = 1;
  dip->un.e.num_mem = 4;
  strlcpy(dip->un.e.member[0].label.name, "mic",
      sizeof dip->un.e.member[0].label.name);
  dip->un.e.member[0].ord = 0x00;
  strlcpy(dip->un.e.member[1].label.name, "line",
      sizeof dip->un.e.member[1].label.name);
  dip->un.e.member[1].ord = 0x06;
  strlcpy(dip->un.e.member[2].label.name, "cd",
      sizeof dip->un.e.member[2].label.name);
  dip->un.e.member[2].ord = 0x02;
  strlcpy(dip->un.e.member[3].label.name, "mixerout",
      sizeof dip->un.e.member[3].label.name);
  dip->un.e.member[3].ord = 0x05;
  break;
 case 11:
  dip->mixer_class = 29;
  dip->next = dip->prev = -1;
...
{
 int error, wait;
 wait = (flags & 0x0002) ? 0x0001 : 0x0000;
 ed->ed_size = size;
 ...
 error = bus_dmamem_map(ed->ed_dmat, ed->ed_segs, ed->ed_nsegs,
     ed->ed_size, &ed->ed_addr, wait | 0x0004);
 if (error)
  goto free;
 ...
 error = bus_dmamap_load(ed->ed_dmat, ed->ed_map, ed->ed_addr,
     ed->ed_size, ((void *)0), wait |
     ((direction == 0x02) ? 0x0200 : 0x0400));
 if (error)
  goto destroy;
 return (0);
 destroy:
...
eso_allocm(void *hdl, int direction, size_t size, int type, int flags)
{
 ...
 size_t boundary;
 int error;
 if ((ed = malloc(sizeof (*ed), type, flags)) == ((void *)0))
  return (((void *)0));
 if (direction == 0x02)
  boundary = 0x10000;
 else
...
eso_freem(void *hdl, void *addr, int type)
{
 struct eso_softc *sc = hdl;
 struct eso_dma *p, **pp;
 for (pp = &sc->sc_dmas; (p = *pp) != ((void *)0); pp = &p->ed_next) {
  if (((void *)(p)->ed_addr) == addr) {
   eso_freemem(p);
   *pp = p->ed_next;
   free(p, type, 0);
   return;
...
{
 size_t maxsize;
 maxsize = (direction == 0x01) ? 0xfff0 : 0x10000;
 if (bufsize > maxsize)
  bufsize = maxsize;
 return (bufsize);
}
    ...
    void (*intr)(void *), void *arg, struct audio_params *param)
{
 ...
 ;
 ;
 for (ed = sc->sc_dmas; ed != ((void *)0) && ((void *)(ed)->ed_addr) != start;
      ed = ed->ed_next)
  ;
 if (ed == ((void *)0)) {
 ...
 blksize = -(blksize >> 1);
 eso_write_mixreg(sc, 0x74, blksize & 0xff);
 eso_write_mixreg(sc, 0x76, blksize >> 8);
 ...
 sc->sc_a2c2 |= 0x40;
 eso_write_mixreg(sc, 0x7a, sc->sc_a2c2);
 bus_space_write_4(sc->sc_iot, sc->sc_ioh, 0x00, ((ed)->ed_map->dm_segs[0].ds_addr));
 bus_space_write_2(sc->sc_iot, sc->sc_ioh, 0x04,
    ...
    void (*intr)(void *), void *arg, struct audio_params *param)
{
 ...
 eso_write_ctlreg(sc, 0xa8, actl);
 eso_write_ctlreg(sc, 0xb9, 0x02);
 blksize = -blksize;
 eso_write_ctlreg(sc, 0xa4, blksize & 0xff);
 eso_write_ctlreg(sc, 0xa5, blksize >> 8);
 a1c1 = 0x10 | 0x80;
 if (param->precision == 16)
  a1c1 |= 0x04;
 if (param->channels == 2)
 ...
 if (param->encoding == 7 ||
     param->encoding == 6)
  a1c1 |= 0x20;
 eso_write_ctlreg(sc, 0xb7, a1c1);
 eso_write_ctlreg(sc, 0xb1,
     0x10 | 0x40);
 eso_write_ctlreg(sc, 0xb2,
     0x10 | 0x40);
 bus_space_write_1(sc->sc_dmac_iot, sc->sc_dmac_ioh, 0x0d, 0);
 bus_space_write_1(sc->sc_dmac_iot, sc->sc_dmac_ioh, 0x0f,
     0x01);
 bus_space_write_1(sc->sc_dmac_iot, sc->sc_dmac_ioh, 0x0b,
     0x04 | 0x10 | 0x00);
 bus_space_write_4(sc->sc_dmac_iot, sc->sc_dmac_ioh, 0x00,
     ((ed)->ed_map->dm_segs[0].ds_addr));
...
eso_set_recsrc(struct eso_softc *sc, u_int recsrc)
{
 ...
 for (i = 0; i < di.un.e.num_mem; i++) {
  if (recsrc == di.un.e.member[i].ord) {
   eso_write_mixreg(sc, 0x1c, recsrc);
   sc->sc_recsrc = recsrc;
   return (0);
  }
 }
...
eso_set_monooutsrc(struct eso_softc *sc, uint monooutsrc)
{
 ...
 for (i = 0; i < di.un.e.num_mem; i++) {
  if (monooutsrc == di.un.e.member[i].ord) {
   mpm = eso_read_mixreg(sc, 0x7d);
   mpm &= ~0x06;
   mpm |= monooutsrc;
   eso_write_mixreg(sc, 0x7d, mpm);
...
{
 uint8_t mv;
 mv = eso_read_mixreg(sc, 0x60);
 sc->sc_gain[7][0] =
     (mv & ~0x40) << 2;
 mv = eso_read_mixreg(sc, 0x60);
 sc->sc_gain[7][1] =
     (mv & ~0x40) << 2;
 sc->sc_mvmute = (mv & 0x40) != 0;
}
void
eso_set_gain(struct eso_softc *sc, uint port)
{
 ...
 case 15:
  ...
  break;
 case 8:
  tmp = eso_read_mixreg(sc, 0x3c);
  tmp &= 0xf8;
  tmp |= (sc->sc_gain[port][0] >> 5);
  eso_write_mixreg(sc, 0x3c, tmp);
  return;
 case 7:
  eso_write_mixreg(sc, 0x60,
      (sc->sc_gain[port][0] >> 2) |
      (sc->sc_mvmute ? 0x40 : 0x00));
  eso_write_mixreg(sc, 0x62,
      (sc->sc_gain[port][1] >> 2) |
      (sc->sc_mvmute ? 0x40 : 0x00));
  return;
 case 9:
  eso_write_mixreg(sc, 0x52,
      sc->sc_gain[port][0]);
  return;
 case 10:
  eso_write_ctlreg(sc, 0xb4,((sc->sc_gain[port][0]) | ((sc->sc_gain[port][1]) >> 4)));
  return;
 default:
  printf("eso_set_gain: bad port %u", port);
  return;
  }
 eso_write_mixreg(sc, mixreg, ((sc->sc_gain[port][0]) | ((sc->sc_gain[port][1]) >> 4)));
}
int
eso_activate(struct device *self, int act)
{
 ...
 case 2:
  ...
  tmp = bus_space_read_1(sc->sc_iot, sc->sc_ioh, 0x07);
  tmp &= ~(0xf0);
  bus_space_write_1(sc->sc_iot, sc->sc_ioh, 0x07, tmp);
  break;
 case 3:
  bus_space_write_1(sc->sc_iot, sc->sc_ioh, 0x06, 0);
