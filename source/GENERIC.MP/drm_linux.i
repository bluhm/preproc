# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c"
# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c"
# 19 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c"
...
# 20 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c" 2
# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/ppbreg.h" 1
# 21 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c" 2
...
flush_workqueue(struct workqueue_struct *wq)
{
 ...
 while (!barrier) {
  sleep_setup(&sls, &barrier, 32, "flwqbar");
...
flush_work(struct work_struct *work)
{
 ...
 while (!barrier) {
  sleep_setup(&sls, &barrier, 32, "flwkbar");
  sleep_finish(&sls, !barrier);
 }
}
...
flush_delayed_work(struct delayed_work *dwork)
{
 ...
 while (((&dwork->to)->to_flags & 2))
  tsleep(&barrier, 32, "fldwto", 1);

 task_set(&task, flush_barrier, &barrier);
 task_add(dwork->tq, &task);
 while (!barrier) {
  sleep_setup(&sls, &barrier, 32, "fldwbar");
  sleep_finish(&sls, !barrier);
 }
}
...
{
 return ((int64_t)tv->tv_sec * 1000000000L) +
  tv->tv_usec * 1000L;
}
...
dmi_found(const struct dmi_system_id *dsi)
{
 ...
 for (i = 0; i < (sizeof((dsi->matches)) / sizeof((dsi->matches)[0])); i++) {
...
__free_pages(struct vm_page *page, unsigned int order)
{
 ...
 do { (&mlist)->tqh_first = ((void *)0); (&mlist)->tqh_last = &(&mlist)->tqh_first; } while (0);
 for (i = 0; i < (1 << order); i++)
  do { (&page[i])->pageq.tqe_next = ((void *)0); (&page[i])->pageq.tqe_prev = (&mlist)->tqh_last; *(&mlist)->tqh_last = (&page[i]); (&mlist)->tqh_last = &(&page[i])->pageq.tqe_next; } while (0);
...
kmap(struct vm_page *pg)
{
 ...
 va = uvm_km_valloc_wait(phys_map, (1 << 13));
...
kunmap(void *addr)
{
 ...
 ;
 uvm_km_free_wakeup(phys_map, va, (1 << 13));
     ...
     pgprot_t prot)
{
 ...
 va = uvm_km_valloc(kernel_map, (1 << 13) * npages);
 if (va == 0)
  return ((void *)0);
 for (i = 0; i < npages; i++) {
  pa = ((pages[i])->phys_addr) | prot;
  pmap_enter((&kernel_pmap_), va + (i * (1 << 13)), pa,
      0x01 | 0x02,
...
vunmap(void *addr, size_t size)
{
 ...
 pmap_remove((&kernel_pmap_), va, va + size);
 ;
...
print_hex_dump(const char *level, const char *prefix_str, int prefix_type,
    int rowsize, int groupsize, const void *buf, size_t len, _Bool ascii)
{
...
panic_cmp(struct rb_node *a, struct rb_node *b)
{
 ...
 void linux_root_RB_INSERT_COLOR(struct linux_root *head, struct rb_node *elm) { struct rb_node *parent, *gparent, *tmp; while ((parent = (elm)->__entry.rbe_parent) && (parent)->__entry.rbe_color == 1) { gparent = (parent)->__entry.rbe_parent; if (parent == (gparent)->__entry.rbe_left) { tmp = (gparent)->__entry.rbe_right; if (tmp && (tmp)->__entry.rbe_color == 1) { (tmp)->__entry.rbe_color = 0; do { (parent)->__entry.rbe_color = 0; (gparent)->__entry.rbe_color = 1; } while (0); elm = gparent; continue; } if ((parent)->__entry.rbe_right == elm) { do { (tmp) = (parent)->__entry.rbe_right; if (((parent)->__entry.rbe_right = (tmp)->__entry.rbe_left)) { ((tmp)->__entry.rbe_left)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_left = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = parent; parent = elm; elm = tmp; } do { (parent)->__entry.rbe_color = 0; (gparent)->__entry.rbe_color = 1; } while (0); do { (tmp) = (gparent)->__entry.rbe_left; if (((gparent)->__entry.rbe_left = (tmp)->__entry.rbe_right)) { ((tmp)->__entry.rbe_right)->__entry.rbe_parent = (gparent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (gparent)->__entry.rbe_parent)) { if ((gparent) == ((gparent)->__entry.rbe_parent)->__entry.rbe_left) ((gparent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((gparent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_right = (gparent); (gparent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); } else { tmp = (gparent)->__entry.rbe_left; if (tmp && (tmp)->__entry.rbe_color == 1) { (tmp)->__entry.rbe_color = 0; do { (parent)->__entry.rbe_color = 0; (gparent)->__entry.rbe_color = 1; } while (0); elm = gparent; continue; } if ((parent)->__entry.rbe_left == elm) { do { (tmp) = (parent)->__entry.rbe_left; if (((parent)->__entry.rbe_left = (tmp)->__entry.rbe_right)) { ((tmp)->__entry.rbe_right)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_right = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = parent; parent = elm; elm = tmp; } do { (parent)->__entry.rbe_color = 0; (gparent)->__entry.rbe_color = 1; } while (0); do { (tmp) = (gparent)->__entry.rbe_right; if (((gparent)->__entry.rbe_right = (tmp)->__entry.rbe_left)) { ((tmp)->__entry.rbe_left)->__entry.rbe_parent = (gparent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (gparent)->__entry.rbe_parent)) { if ((gparent) == ((gparent)->__entry.rbe_parent)->__entry.rbe_left) ((gparent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((gparent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_left = (gparent); (gparent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); } } (head->rbh_root)->__entry.rbe_color = 0; } void linux_root_RB_REMOVE_COLOR(struct linux_root *head, struct rb_node *parent, struct rb_node *elm) { struct rb_node *tmp; while ((elm == ((void *)0) || (elm)->__entry.rbe_color == 0) && elm != (head)->rbh_root) { if ((parent)->__entry.rbe_left == elm) { tmp = (parent)->__entry.rbe_right; if ((tmp)->__entry.rbe_color == 1) { do { (tmp)->__entry.rbe_color = 0; (parent)->__entry.rbe_color = 1; } while (0); do { (tmp) = (parent)->__entry.rbe_right; if (((parent)->__entry.rbe_right = (tmp)->__entry.rbe_left)) { ((tmp)->__entry.rbe_left)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_left = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = (parent)->__entry.rbe_right; } if (((tmp)->__entry.rbe_left == ((void *)0) || ((tmp)->__entry.rbe_left)->__entry.rbe_color == 0) && ((tmp)->__entry.rbe_right == ((void *)0) || ((tmp)->__entry.rbe_right)->__entry.rbe_color == 0)) { (tmp)->__entry.rbe_color = 1; elm = parent; parent = (elm)->__entry.rbe_parent; } else { if ((tmp)->__entry.rbe_right == ((void *)0) || ((tmp)->__entry.rbe_right)->__entry.rbe_color == 0) { struct rb_node *oleft; if ((oleft = (tmp)->__entry.rbe_left)) (oleft)->__entry.rbe_color = 0; (tmp)->__entry.rbe_color = 1; do { (oleft) = (tmp)->__entry.rbe_left; if (((tmp)->__entry.rbe_left = (oleft)->__entry.rbe_right)) { ((oleft)->__entry.rbe_right)->__entry.rbe_parent = (tmp); } do {} while (0); if (((oleft)->__entry.rbe_parent = (tmp)->__entry.rbe_parent)) { if ((tmp) == ((tmp)->__entry.rbe_parent)->__entry.rbe_left) ((tmp)->__entry.rbe_parent)->__entry.rbe_left = (oleft); else ((tmp)->__entry.rbe_parent)->__entry.rbe_right = (oleft); } else (head)->rbh_root = (oleft); (oleft)->__entry.rbe_right = (tmp); (tmp)->__entry.rbe_parent = (oleft); do {} while (0); if (((oleft)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = (parent)->__entry.rbe_right; } (tmp)->__entry.rbe_color = (parent)->__entry.rbe_color; (parent)->__entry.rbe_color = 0; if ((tmp)->__entry.rbe_right) ((tmp)->__entry.rbe_right)->__entry.rbe_color = 0; do { (tmp) = (parent)->__entry.rbe_right; if (((parent)->__entry.rbe_right = (tmp)->__entry.rbe_left)) { ((tmp)->__entry.rbe_left)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_left = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); elm = (head)->rbh_root; break; } } else { tmp = (parent)->__entry.rbe_left; if ((tmp)->__entry.rbe_color == 1) { do { (tmp)->__entry.rbe_color = 0; (parent)->__entry.rbe_color = 1; } while (0); do { (tmp) = (parent)->__entry.rbe_left; if (((parent)->__entry.rbe_left = (tmp)->__entry.rbe_right)) { ((tmp)->__entry.rbe_right)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_right = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = (parent)->__entry.rbe_left; } if (((tmp)->__entry.rbe_left == ((void *)0) || ((tmp)->__entry.rbe_left)->__entry.rbe_color == 0) && ((tmp)->__entry.rbe_right == ((void *)0) || ((tmp)->__entry.rbe_right)->__entry.rbe_color == 0)) { (tmp)->__entry.rbe_color = 1; elm = parent; parent = (elm)->__entry.rbe_parent; } else { if ((tmp)->__entry.rbe_left == ((void *)0) || ((tmp)->__entry.rbe_left)->__entry.rbe_color == 0) { struct rb_node *oright; if ((oright = (tmp)->__entry.rbe_right)) (oright)->__entry.rbe_color = 0; (tmp)->__entry.rbe_color = 1; do { (oright) = (tmp)->__entry.rbe_right; if (((tmp)->__entry.rbe_right = (oright)->__entry.rbe_left)) { ((oright)->__entry.rbe_left)->__entry.rbe_parent = (tmp); } do {} while (0); if (((oright)->__entry.rbe_parent = (tmp)->__entry.rbe_parent)) { if ((tmp) == ((tmp)->__entry.rbe_parent)->__entry.rbe_left) ((tmp)->__entry.rbe_parent)->__entry.rbe_left = (oright); else ((tmp)->__entry.rbe_parent)->__entry.rbe_right = (oright); } else (head)->rbh_root = (oright); (oright)->__entry.rbe_left = (tmp); (tmp)->__entry.rbe_parent = (oright); do {} while (0); if (((oright)->__entry.rbe_parent)) do {} while (0); } while (0); tmp = (parent)->__entry.rbe_left; } (tmp)->__entry.rbe_color = (parent)->__entry.rbe_color; (parent)->__entry.rbe_color = 0; if ((tmp)->__entry.rbe_left) ((tmp)->__entry.rbe_left)->__entry.rbe_color = 0; do { (tmp) = (parent)->__entry.rbe_left; if (((parent)->__entry.rbe_left = (tmp)->__entry.rbe_right)) { ((tmp)->__entry.rbe_right)->__entry.rbe_parent = (parent); } do {} while (0); if (((tmp)->__entry.rbe_parent = (parent)->__entry.rbe_parent)) { if ((parent) == ((parent)->__entry.rbe_parent)->__entry.rbe_left) ((parent)->__entry.rbe_parent)->__entry.rbe_left = (tmp); else ((parent)->__entry.rbe_parent)->__entry.rbe_right = (tmp); } else (head)->rbh_root = (tmp); (tmp)->__entry.rbe_right = (parent); (parent)->__entry.rbe_parent = (tmp); do {} while (0); if (((tmp)->__entry.rbe_parent)) do {} while (0); } while (0); elm = (head)->rbh_root; break; } } } if (elm) (elm)->__entry.rbe_color = 0; } struct rb_node * linux_root_RB_REMOVE(struct linux_root *head, struct rb_node *elm) { struct rb_node *child, *parent, *old = elm; int color; if ((elm)->__entry.rbe_left == ((void *)0)) child = (elm)->__entry.rbe_right; else if ((elm)->__entry.rbe_right == ((void *)0)) child = (elm)->__entry.rbe_left; else { struct rb_node *left; elm = (elm)->__entry.rbe_right; while ((left = (elm)->__entry.rbe_left)) elm = left; child = (elm)->__entry.rbe_right; parent = (elm)->__entry.rbe_parent; color = (elm)->__entry.rbe_color; if (child) (child)->__entry.rbe_parent = parent; if (parent) { if ((parent)->__entry.rbe_left == elm) (parent)->__entry.rbe_left = child; else (parent)->__entry.rbe_right = child; do {} while (0); } else (head)->rbh_root = child; if ((elm)->__entry.rbe_parent == old) parent = elm; (elm)->__entry = (old)->__entry; if ((old)->__entry.rbe_parent) { if (((old)->__entry.rbe_parent)->__entry.rbe_left == old) ((old)->__entry.rbe_parent)->__entry.rbe_left = elm; else ((old)->__entry.rbe_parent)->__entry.rbe_right = elm; do {} while (0); } else (head)->rbh_root = elm; ((old)->__entry.rbe_left)->__entry.rbe_parent = elm; if ((old)->__entry.rbe_right) ((old)->__entry.rbe_right)->__entry.rbe_parent = elm; if (parent) { left = parent; do { do {} while (0); } while ((left = (left)->__entry.rbe_parent)); } goto color; } parent = (elm)->__entry.rbe_parent; color = (elm)->__entry.rbe_color; if (child) (child)->__entry.rbe_parent = parent; if (parent) { if ((parent)->__entry.rbe_left == elm) (parent)->__entry.rbe_left = child; else (parent)->__entry.rbe_right = child; do {} while (0); } else (head)->rbh_root = child; color: if (color == 0) linux_root_RB_REMOVE_COLOR(head, parent, child); return (old); } struct rb_node * linux_root_RB_INSERT(struct linux_root *head, struct rb_node *elm) { struct rb_node *tmp; struct rb_node *parent = ((void *)0); int comp = 0; tmp = (head)->rbh_root; while (tmp) { parent = tmp; comp = (panic_cmp)(elm, parent); if (comp < 0) tmp = (tmp)->__entry.rbe_left; else if (comp > 0) tmp = (tmp)->__entry.rbe_right; else return (tmp); } do { (elm)->__entry.rbe_parent = parent; (elm)->__entry.rbe_left = (elm)->__entry.rbe_right = ((void *)0); (elm)->__entry.rbe_color = 1; } while (0); if (parent != ((void *)0)) { if (comp < 0) (parent)->__entry.rbe_left = elm; else (parent)->__entry.rbe_right = elm; do {} while (0); } else (head)->rbh_root = elm; linux_root_RB_INSERT_COLOR(head, elm); return (((void *)0)); } struct rb_node * linux_root_RB_FIND(struct linux_root *head, struct rb_node *elm) { struct rb_node *tmp = (head)->rbh_root; int comp; while (tmp) { comp = panic_cmp(elm, tmp); if (comp < 0) tmp = (tmp)->__entry.rbe_left; else if (comp > 0) tmp = (tmp)->__entry.rbe_right; else return (tmp); } return (((void *)0)); } struct rb_node * linux_root_RB_NFIND(struct linux_root *head, struct rb_node *elm) { struct rb_node *tmp = (head)->rbh_root; struct rb_node *res = ((void *)0); int comp; while (tmp) { comp = panic_cmp(elm, tmp); if (comp < 0) { res = tmp; tmp = (tmp)->__entry.rbe_left; } else if (comp > 0) tmp = (tmp)->__entry.rbe_right; else return (tmp); } return (res); } struct rb_node * linux_root_RB_NEXT(struct rb_node *elm) { if ((elm)->__entry.rbe_right) { elm = (elm)->__entry.rbe_right; while ((elm)->__entry.rbe_left) elm = (elm)->__entry.rbe_left; } else { if ((elm)->__entry.rbe_parent && (elm == ((elm)->__entry.rbe_parent)->__entry.rbe_left)) elm = (elm)->__entry.rbe_parent; else { while ((elm)->__entry.rbe_parent && (elm == ((elm)->__entry.rbe_parent)->__entry.rbe_right)) elm = (elm)->__entry.rbe_parent; elm = (elm)->__entry.rbe_parent; } } return (elm); } struct rb_node * linux_root_RB_PREV(struct rb_node *elm) { if ((elm)->__entry.rbe_left) { elm = (elm)->__entry.rbe_left; while ((elm)->__entry.rbe_right) elm = (elm)->__entry.rbe_right; } else { if ((elm)->__entry.rbe_parent && (elm == ((elm)->__entry.rbe_parent)->__entry.rbe_right)) elm = (elm)->__entry.rbe_parent; else { while ((elm)->__entry.rbe_parent && (elm == ((elm)->__entry.rbe_parent)->__entry.rbe_left)) elm = (elm)->__entry.rbe_parent; elm = (elm)->__entry.rbe_parent; } } return (elm); } struct rb_node * linux_root_RB_MINMAX(struct linux_root *head, int val) { struct rb_node *tmp = (head)->rbh_root; struct rb_node *parent = ((void *)0); while (tmp) { parent = tmp; if (val < 0) tmp = (tmp)->__entry.rbe_left; else tmp = (tmp)->__entry.rbe_right; } return (parent); };
# 337 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c"
int idr_cmp(struct idr_entry *, struct idr_entry *);
void idr_tree_SPLAY(struct idr_tree *, struct idr_entry *); void idr_tree_SPLAY_MINMAX(struct idr_tree *, int); struct idr_entry *idr_tree_SPLAY_INSERT(struct idr_tree *, struct idr_entry *); struct idr_entry *idr_tree_SPLAY_REMOVE(struct idr_tree *, struct idr_entry *); static __inline struct idr_entry * idr_tree_SPLAY_FIND(struct idr_tree *head, struct idr_entry *elm) { if (((head)->sph_root == ((void *)0))) return(((void *)0)); idr_tree_SPLAY(head, elm); if ((idr_cmp)(elm, (head)->sph_root) == 0) return (head->sph_root); return (((void *)0)); } static __inline struct idr_entry * idr_tree_SPLAY_NEXT(struct idr_tree *head, struct idr_entry *elm) { idr_tree_SPLAY(head, elm); if ((elm)->entry.spe_right != ((void *)0)) { elm = (elm)->entry.spe_right; while ((elm)->entry.spe_left != ((void *)0)) { elm = (elm)->entry.spe_left; } } else elm = ((void *)0); return (elm); } static __inline struct idr_entry * idr_tree_SPLAY_MIN_MAX(struct idr_tree *head, int val) { idr_tree_SPLAY_MINMAX(head, val); return ((head)->sph_root); };
...
idr_init(struct idr *idr)
{
 ...
 if (!initialized) {
  pool_init(&idr_pool, sizeof(struct idr_entry), 0, 6, 0,
      "idrpl", ((void *)0));
  initialized = 1;
 }
 do { (&idr->tree)->sph_root = ((void *)0); } while (0);
}
...
idr_destroy(struct idr *idr)
{
 ...
 while ((id = (((&idr->tree)->sph_root == ((void *)0)) ? ((void *)0) : idr_tree_SPLAY_MIN_MAX(&idr->tree, -1)))) {
  idr_tree_SPLAY_REMOVE(&idr->tree, id);
    ...
    unsigned int gfp_mask)
{
 ...
 while (idr_tree_SPLAY_INSERT(&idr->tree, id)) {
  if (++id->id == end)
   id->id = start;
  if (id->id == begin) {
   pool_put(&idr_pool, id);
...
idr_replace(struct idr *idr, void *ptr, int id)
{
 ...
 find.id = id;
 res = idr_tree_SPLAY_FIND(&idr->tree, &find);
 if (res == ((void *)0))
  return ERR_PTR(-2);
 old = res->ptr;
 res->ptr = ptr;
...
idr_remove(struct idr *idr, int id)
{
 ...
 res = idr_tree_SPLAY_FIND(&idr->tree, &find);
 if (res) {
  idr_tree_SPLAY_REMOVE(&idr->tree, res);
...
idr_get_next(struct idr *idr, int *id)
{
 ...
 res = idr_find(idr, *id);
 if (res == ((void *)0))
  res = (((&idr->tree)->sph_root == ((void *)0)) ? ((void *)0) : idr_tree_SPLAY_MIN_MAX(&idr->tree, -1));
 else
  res = idr_tree_SPLAY_NEXT(&idr->tree, res);
 if (res == ((void *)0))
  return ((void *)0);
 *id = res->id;
 return res->ptr;
...
idr_for_each(struct idr *idr, int (*func)(int, void *, void *), void *data)
{
 ...
 for ((id) = (((&idr->tree)->sph_root == ((void *)0)) ? ((void *)0) : idr_tree_SPLAY_MIN_MAX(&idr->tree, -1)); (id) != ((void *)0); (id) = idr_tree_SPLAY_NEXT(&idr->tree, id)) {
...
struct idr_entry * idr_tree_SPLAY_INSERT(struct idr_tree *head, struct idr_entry *elm) { if (((head)->sph_root == ((void *)0))) { (elm)->entry.spe_left = (elm)->entry.spe_right = ((void *)0); } else { int __comp; idr_tree_SPLAY(head, elm); __comp = (idr_cmp)(elm, (head)->sph_root); if(__comp < 0) { (elm)->entry.spe_left = ((head)->sph_root)->entry.spe_left; (elm)->entry.spe_right = (head)->sph_root; ((head)->sph_root)->entry.spe_left = ((void *)0); } else if (__comp > 0) { (elm)->entry.spe_right = ((head)->sph_root)->entry.spe_right; (elm)->entry.spe_left = (head)->sph_root; ((head)->sph_root)->entry.spe_right = ((void *)0); } else return ((head)->sph_root); } (head)->sph_root = (elm); return (((void *)0)); } struct idr_entry * idr_tree_SPLAY_REMOVE(struct idr_tree *head, struct idr_entry *elm) { struct idr_entry *__tmp; if (((head)->sph_root == ((void *)0))) return (((void *)0)); idr_tree_SPLAY(head, elm); if ((idr_cmp)(elm, (head)->sph_root) == 0) { if (((head)->sph_root)->entry.spe_left == ((void *)0)) { (head)->sph_root = ((head)->sph_root)->entry.spe_right; } else { __tmp = ((head)->sph_root)->entry.spe_right; (head)->sph_root = ((head)->sph_root)->entry.spe_left; idr_tree_SPLAY(head, elm); ((head)->sph_root)->entry.spe_right = __tmp; } return (elm); } return (((void *)0)); } void idr_tree_SPLAY(struct idr_tree *head, struct idr_entry *elm) { struct idr_entry __node, *__left, *__right, *__tmp; int __comp; (&__node)->entry.spe_left = (&__node)->entry.spe_right = ((void *)0); __left = __right = &__node; while ((__comp = (idr_cmp)(elm, (head)->sph_root))) { if (__comp < 0) { __tmp = ((head)->sph_root)->entry.spe_left; if (__tmp == ((void *)0)) break; if ((idr_cmp)(elm, __tmp) < 0){ do { ((head)->sph_root)->entry.spe_left = (__tmp)->entry.spe_right; (__tmp)->entry.spe_right = (head)->sph_root; (head)->sph_root = __tmp; } while (0); if (((head)->sph_root)->entry.spe_left == ((void *)0)) break; } do { (__right)->entry.spe_left = (head)->sph_root; __right = (head)->sph_root; (head)->sph_root = ((head)->sph_root)->entry.spe_left; } while (0); } else if (__comp > 0) { __tmp = ((head)->sph_root)->entry.spe_right; if (__tmp == ((void *)0)) break; if ((idr_cmp)(elm, __tmp) > 0){ do { ((head)->sph_root)->entry.spe_right = (__tmp)->entry.spe_left; (__tmp)->entry.spe_left = (head)->sph_root; (head)->sph_root = __tmp; } while (0); if (((head)->sph_root)->entry.spe_right == ((void *)0)) break; } do { (__left)->entry.spe_right = (head)->sph_root; __left = (head)->sph_root; (head)->sph_root = ((head)->sph_root)->entry.spe_right; } while (0); } } do { (__left)->entry.spe_right = ((head)->sph_root)->entry.spe_left; (__right)->entry.spe_left = ((head)->sph_root)->entry.spe_right; ((head)->sph_root)->entry.spe_left = (&__node)->entry.spe_right; ((head)->sph_root)->entry.spe_right = (&__node)->entry.spe_left; } while (0); } void idr_tree_SPLAY_MINMAX(struct idr_tree *head, int __comp) { struct idr_entry __node, *__left, *__right, *__tmp; (&__node)->entry.spe_left = (&__node)->entry.spe_right = ((void *)0); __left = __right = &__node; while (1) { if (__comp < 0) { __tmp = ((head)->sph_root)->entry.spe_left; if (__tmp == ((void *)0)) break; if (__comp < 0){ do { ((head)->sph_root)->entry.spe_left = (__tmp)->entry.spe_right; (__tmp)->entry.spe_right = (head)->sph_root; (head)->sph_root = __tmp; } while (0); if (((head)->sph_root)->entry.spe_left == ((void *)0)) break; } do { (__right)->entry.spe_left = (head)->sph_root; __right = (head)->sph_root; (head)->sph_root = ((head)->sph_root)->entry.spe_left; } while (0); } else if (__comp > 0) { __tmp = ((head)->sph_root)->entry.spe_right; if (__tmp == ((void *)0)) break; if (__comp > 0) { do { ((head)->sph_root)->entry.spe_right = (__tmp)->entry.spe_left; (__tmp)->entry.spe_left = (head)->sph_root; (head)->sph_root = __tmp; } while (0); if (((head)->sph_root)->entry.spe_right == ((void *)0)) break; } do { (__left)->entry.spe_right = (head)->sph_root; __left = (head)->sph_root; (head)->sph_root = ((head)->sph_root)->entry.spe_right; } while (0); } } do { (__left)->entry.spe_right = ((head)->sph_root)->entry.spe_left; (__right)->entry.spe_left = ((head)->sph_root)->entry.spe_right; ((head)->sph_root)->entry.spe_left = (&__node)->entry.spe_right; ((head)->sph_root)->entry.spe_right = (&__node)->entry.spe_left; } while (0); };
...
i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
 ...
 while (num > 2) {
  ...
  err = iic_exec(&adap->ic, op, msgs->addr, ((void *)0), 0,
      msgs->buf, msgs->len, 0);
  if (err) {
   ret = -err;
   goto fail;
...
# 700 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c"
void
    ...
    const struct backlight_ops *ops, struct backlight_properties *props)
{
 ...
 bd = malloc(sizeof(*bd), 145, 0x0001);
 bd->ops = ops;
 bd->props = *props;
 bd->data = data;
