...
struct mutex sch_mtx = { ((void *)0), ((((14)) > 0 && ((14)) < 12) ? 12 : ((14))), 0 };
void *sch_ident;
int sch_priority;
void
...
void idr_tree_SPLAY(struct idr_tree *, struct idr_entry *); void idr_tree_SPLAY_MINMAX(struct idr_tree *, int); struct idr_entry *idr_tree_SPLAY_INSERT(struct idr_tree *, struct idr_entry *); struct idr_entry *idr_tree_SPLAY_REMOVE(struct idr_tree *, struct idr_entry *); static __attribute__((__unused__)) __inline struct idr_entry * idr_tree_SPLAY_FIND(struct idr_tree *head, struct idr_entry *elm) { if (((head)->sph_root == ((void *)0))) return(((void *)0)); idr_tree_SPLAY(head, elm); if ((idr_cmp)(elm, (head)->sph_root) == 0) return (head->sph_root); return (((void *)0)); } static __attribute__((__unused__)) __inline struct idr_entry * idr_tree_SPLAY_NEXT(struct idr_tree *head, struct idr_entry *elm) { idr_tree_SPLAY(head, elm); if ((elm)->entry.spe_right != ((void *)0)) { elm = (elm)->entry.spe_right; while ((elm)->entry.spe_left != ((void *)0)) { elm = (elm)->entry.spe_left; } } else elm = ((void *)0); return (elm); } static __attribute__((__unused__)) __inline struct idr_entry * idr_tree_SPLAY_MIN_MAX(struct idr_tree *head, int val) { idr_tree_SPLAY_MINMAX(head, val); return ((head)->sph_root); };
...
{
 int flags = (gfp_mask & 0x0002) ? 0x0002 : 0x0001;
 ((_kernel_lock_held()) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c", 391, "_kernel_lock_held()"));
    ...
    unsigned int gfp_mask)
{
 ...
 struct idr_entry *id;
 int begin;
 ((_kernel_lock_held()) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_linux.c", 405, "_kernel_lock_held()"));
 if (idr_entry_cache) {
  id = idr_entry_cache;
