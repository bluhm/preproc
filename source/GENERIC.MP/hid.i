...
struct hid_data {
 ...
 const uint8_t *end;
 const uint8_t *p;
 struct hid_item cur[4];
 struct hid_pos_data last_pos[16];
 int32_t usages_min[64];
 int32_t usages_max[64];
 int32_t usage_last;
 uint32_t loc_size;
 uint32_t loc_count;
 enum hid_kind kind;
...
{
 struct hid_data *s;
 s = malloc(sizeof(*s), 127, 0x0001 | 0x0008);
 s->start = s->p = d;
