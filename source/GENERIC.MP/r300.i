...
uint32_t rv370_pcie_rreg(struct radeon_device *rdev, uint32_t reg64)
{
 unsigned long flags;
 uint32_t r;
 _spin_lock_irqsave(&rdev->pcie_idx_lock, flags );
 r100_mm_wreg(rdev, (0x0030), (((reg64) & rdev->pcie_reg_mask)), 0);
 r = r100_mm_rreg(rdev, (0x0034), 0);
 _spin_unlock_irqrestore(&rdev->pcie_idx_lock, flags );
 return r;
}
void rv370_pcie_wreg(struct radeon_device *rdev, uint32_t reg64, uint32_t v)
{
...
void rv370_pcie_gart_set_page(struct radeon_device *rdev, unsigned i,
         uint64_t entry)
{
 void *ptr = rdev->gart.ptr;
 iowrite32(entry, ((void *)ptr) + (i * 4));
}
 ...
 case 0x4540 +60:
  ...
  if (p->cs_flags & 0x01) {
   ...
   if (reloc->tiling_flags & 0x1)
    tile_flags |= (1 << 2);
   if (reloc->tiling_flags & 0x2)
    tile_flags |= (1 << 3);
   else if (reloc->tiling_flags & 0x20)
    tile_flags |= (2 << 3);
