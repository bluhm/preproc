...
{
 struct radeon_bo *robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
 if (robj) {
...
retry:
 ...
 if (r) {
  if (r != -4) {
   ...
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to allocate GEM object (%ld, %d, %u, %d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , size, initial_domain, alignment, r);
  }
  return r;
 }
 *obj = &robj->gem_base;
 robj->pid = (__curcpu->ci_self)->ci_curproc->p_p->ps_pid;
     ...
     uint32_t rdomain, uint32_t wdomain)
{
 ...
 uint32_t domain;
 long r;
 robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
 domain = wdomain;
 if (!domain) {
  domain = rdomain;
 }
 ...
 if (domain == 0x1) {
  r = reservation_object_wait_timeout_rcu(robj->tbo.resv, 1, 1, 30 * hz);
  if (!r)
   r = -16;
  if (r < 0 && r != -4) {
   printf("" "Failed to wait for object: %li\n", r);
   return r;
  }
 }
 return 0;
...
{
 struct radeon_bo *rbo = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((obj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
     ...
     struct drm_file *filp)
{
 ...
 robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
 r = reservation_object_test_signaled_rcu(robj->tbo.resv, 1);
 if (r == 0)
  r = -16;
 else
  r = 0;
 cur_placement = (robj->tbo.mem.mem_type);
         ...
         struct drm_file *filp)
{
 ...
 robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
 ret = reservation_object_wait_timeout_rcu(robj->tbo.resv, 1, 1, 30 * hz);
 if (ret == 0)
  r = -16;
 else if (ret < 0)
  r = ret;
 ...
 if (rdev->asic->mmio_hdp_flush &&
     radeon_mem_type_to_domain(cur_placement) == 0x4)
        ...
        struct radeon_bo_va *bo_va)
{
 ...
 if (!vm_bos)
  return;
 r = ttm_eu_reserve_buffers(&ticket, &list, 1, ((void *)0));
 if (r)
  goto error_free;
...
error_free:
 ...
 if (r && r != -4)
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Couldn't update BO_VA (%d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
}
int radeon_gem_va_ioctl(struct drm_device *dev, void *data,
     struct drm_file *filp)
{
 ...
 case 1:
  if (bo_va->it.start) {
   args->operation = 2;
   args->offset = bo_va->it.start * 4096;
   radeon_bo_unreserve(rbo);
   goto out;
  }
   ...
   struct drm_file *filp)
{
 ...
 case 0:
  ...
  break;
 case 1:
  robj->initial_domain = args->value & (0x4 |
            0x2 |
            0x1);
  break;
 default:
