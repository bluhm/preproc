# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c"
# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c"
# 46 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c"
...
# 47 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c" 2
...
# 48 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c" 2
...
uint64_t SipHash(const SIPHASH_KEY *, int, int, const void *, size_t)
      __attribute__ ((__bounded__ (__buffer__, 4, 5) ));
# 50 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../crypto/siphash.c" 2
...
SipHash_Init(SIPHASH_CTX *ctx, const SIPHASH_KEY *key)
{
 ...
 __builtin_memset((ctx->buf), (0), (sizeof(ctx->buf)));
 ctx->bytes = 0;
}
...
SipHash_Update(SIPHASH_CTX *ctx, int rc, int rf, const void *src, size_t len)
{
 ...
 if (used > 0) {
  ...
  if (len >= left) {
   __builtin_memcpy((&ctx->buf[used]), (ptr), (left));
 ...
 while (len >= sizeof(ctx->buf)) {
  __builtin_memcpy((ctx->buf), (ptr), (sizeof(ctx->buf)));
  SipHash_CRounds(ctx, rc);
  len -= sizeof(ctx->buf);
  ptr += sizeof(ctx->buf);
 }
...
SipHash_Final(void *dst, SIPHASH_CTX *ctx, int rc, int rf)
{
 ...
 __swapm64((&r), (SipHash_End(ctx, rc, rf)));
 __builtin_memcpy((dst), (&r), (sizeof r));
}
...
SipHash_End(SIPHASH_CTX *ctx, int rc, int rf)
{
 ...
 used = ctx->bytes % sizeof(ctx->buf);
 left = sizeof(ctx->buf) - used;
 __builtin_memset((&ctx->buf[used]), (0), (left - 1));
 ctx->buf[7] = ctx->bytes;
...
SipHash_Rounds(SIPHASH_CTX *ctx, int rounds)
{
 while (rounds--) {
  ctx->v[0] += ctx->v[1];
  ctx->v[2] += ctx->v[3];
  ctx->v[1] = ((ctx->v[1]) << (13)) | ( (ctx->v[1]) >> (64 - (13)));
  ctx->v[3] = ((ctx->v[3]) << (16)) | ( (ctx->v[3]) >> (64 - (16)));

  ctx->v[1] ^= ctx->v[0];
  ctx->v[3] ^= ctx->v[2];
  ctx->v[0] = ((ctx->v[0]) << (32)) | ( (ctx->v[0]) >> (64 - (32)));

  ctx->v[2] += ctx->v[1];
  ctx->v[0] += ctx->v[3];
  ctx->v[1] = ((ctx->v[1]) << (17)) | ( (ctx->v[1]) >> (64 - (17)));
  ctx->v[3] = ((ctx->v[3]) << (21)) | ( (ctx->v[3]) >> (64 - (21)));

  ctx->v[1] ^= ctx->v[2];
  ctx->v[3] ^= ctx->v[0];
  ctx->v[2] = ((ctx->v[2]) << (32)) | ( (ctx->v[2]) >> (64 - (32)));
 }
}
...
{
 uint64_t m = __mswap64((uint64_t *)ctx->buf);
