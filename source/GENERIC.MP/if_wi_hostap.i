...
{
 u_int16_t s = (__uint16_t)(__builtin_constant_p(* (u_int16_t *) *ppkt) ? (__uint16_t)(((__uint16_t)(* (u_int16_t *) *ppkt) & 0xffU) << 8 | ((__uint16_t)(* (u_int16_t *) *ppkt) & 0xff00U) >> 8) : __swap16md(* (u_int16_t *) *ppkt));
...
take_tlv(caddr_t *ppkt, int *plen, int id_expect, void *dst, int maxlen)
{
 ...
 if (id != id_expect || *plen < len+2 || maxlen < len)
  return -1;
 __builtin_bcopy((*ppkt + 2), (dst), (len));
 *plen -= 2 + len;
 *ppkt += 2 + len;
 return (len);
}
...
{
 * (u_int16_t *) *ppkt = (__uint16_t)(__builtin_constant_p(s) ? (__uint16_t)(((__uint16_t)(s) & 0xffU) << 8 | ((__uint16_t)(s) & 0xff00U) >> 8) : __swap16md(s));
 *ppkt += sizeof(u_int16_t);
}
static void
put_tlv(caddr_t *ppkt, u_int8_t id, void *src, u_int8_t len)
{
 (*ppkt)[0] = id;
 (*ppkt)[1] = len;
 __builtin_bcopy((src), ((*ppkt) + 2), (len));
 *ppkt += 2 + len;
}
static int
...
wihap_init(struct wi_softc *sc)
{
 ...
 __builtin_bzero((whi), (sizeof(struct wihap_info)));
 if (sc->wi_ptype != 0x6)
  return;
 whi->apflags = 0x0001;
 do { (&whi->sta_list)->tqh_first = ((void *)0); (&whi->sta_list)->tqh_last = &(&whi->sta_list)->tqh_first; } while (0);
 for (i = 0; i < 113; i++)
  do { ((&whi->sta_hash[i])->lh_first) = ((void *)0); } while (0);
 whi->inactivity_time = (120);
...
wihap_sta_disassoc(struct wi_softc *sc, u_int8_t sta_addr[], u_int16_t reason)
{
 ...
 resp_hdr->frame_ctl = 0x0000 | 0x00A0;
...
wihap_sta_deauth(struct wi_softc *sc, u_int8_t sta_addr[], u_int16_t reason)
{
 ...
 resp_hdr = (struct wi_80211_hdr *)sc->wi_txbuf;
 __builtin_bzero((resp_hdr), (sizeof(struct wi_80211_hdr)));
 resp_hdr->frame_ctl = (__uint16_t)(__builtin_constant_p(0x0000 | 0x00C0) ? (__uint16_t)(((__uint16_t)(0x0000 | 0x00C0) & 0xffU) << 8 | ((__uint16_t)(0x0000 | 0x00C0) & 0xff00U) >> 8) : __swap16md(0x0000 | 0x00C0));
 pkt = (caddr_t)&sc->wi_txbuf + sizeof(struct wi_80211_hdr);
 __builtin_bcopy((sta_addr), (resp_hdr->addr1), (6));
 __builtin_bcopy((sc->sc_ic.ic_myaddr), (resp_hdr->addr2), (6));
 __builtin_bcopy((sc->sc_ic.ic_myaddr), (resp_hdr->addr3), (6));
...
wihap_shutdown(struct wi_softc *sc)
{
 ...
 if (!(whi->apflags & 0x0001))
  return;
 whi->apflags = 0;
 s = _splraise(6);
 timeout_del(&whi->tmo);
 for (sta = ((&whi->sta_list)->tqh_first); sta != ((void *)0); sta = next) {
  timeout_del(&sta->tmo);
  if (sc->sc_ic.ic_ac.ac_if.if_flags & 0x4)
   printf("wihap_shutdown: free(sta=%p)\n", sta);
  next = ((sta)->list.tqe_next);
  if (sta->challenge)
   free(sta->challenge, 127, 0);
  free(sta, 2, 0);
 }
 do { (&whi->sta_list)->tqh_first = ((void *)0); (&whi->sta_list)->tqh_last = &(&whi->sta_list)->tqh_first; } while (0);
...
{
 return ((addr[3] + addr[4] + addr[5]) % 113);
}
static __inline int
...
{
 do { if (((sta)->list.tqe_next) != ((void *)0)) (sta)->list.tqe_next->list.tqe_prev = (sta)->list.tqe_prev; else (&whi->sta_list)->tqh_last = (sta)->list.tqe_prev; *(sta)->list.tqe_prev = (sta)->list.tqe_next; ((sta)->list.tqe_prev) = ((void *)-1); ((sta)->list.tqe_next) = ((void *)-1); } while (0);
 sta->flags &= ~0x1000;
 do { (sta)->list.tqe_next = ((void *)0); (sta)->list.tqe_prev = (&whi->sta_list)->tqh_last; *(&whi->sta_list)->tqh_last = (sta); (&whi->sta_list)->tqh_last = &(sta)->list.tqe_next; } while (0);
}
void
wihap_timeout(void *v)
{
 ...
 int i, s;
 s = _splraise(6);
 for (i = 10, sta = ((&whi->sta_list)->tqh_first);
     i != 0 && sta != ((void *)0) && (sta->flags & 0x1000);
     i--, sta = next) {
  next = ((sta)->list.tqe_next);
  if (((&sta->tmo)->to_flags & 2)) {
   wihap_sta_movetail(whi, sta);
  } else if (sta->flags & 0x0002) {
   if (sc->sc_ic.ic_ac.ac_if.if_flags & 0x4)
 ...
 sta = ((&whi->sta_list)->tqh_first);
 if (sta != ((void *)0) && (sta->flags & 0x1000))
  timeout_add(&whi->tmo, 1);
 _splx(s);
}
void
wihap_sta_timeout(void *v)
{
 ...
 do { if (((sta)->list.tqe_next) != ((void *)0)) (sta)->list.tqe_next->list.tqe_prev = (sta)->list.tqe_prev; else (&whi->sta_list)->tqh_last = (sta)->list.tqe_prev; *(sta)->list.tqe_prev = (sta)->list.tqe_next; ((sta)->list.tqe_prev) = ((void *)-1); ((sta)->list.tqe_next) = ((void *)-1); } while (0);
 sta->flags |= 0x1000;
 do { if (((sta)->list.tqe_next = (&whi->sta_list)->tqh_first) != ((void *)0)) (&whi->sta_list)->tqh_first->list.tqe_prev = &(sta)->list.tqe_next; else (&whi->sta_list)->tqh_last = &(sta)->list.tqe_next; (&whi->sta_list)->tqh_first = (sta); (sta)->list.tqe_prev = &(&whi->sta_list)->tqh_first; } while (0);
 if (!((&whi->tmo)->to_flags & 2))
...
wihap_sta_delete(struct wihap_sta_info *sta)
{
 ...
 do { if (((sta)->list.tqe_next) != ((void *)0)) (sta)->list.tqe_next->list.tqe_prev = (sta)->list.tqe_prev; else (&whi->sta_list)->tqh_last = (sta)->list.tqe_prev; *(sta)->list.tqe_prev = (sta)->list.tqe_next; ((sta)->list.tqe_prev) = ((void *)-1); ((sta)->list.tqe_next) = ((void *)-1); } while (0);
 do { if ((sta)->hash.le_next != ((void *)0)) (sta)->hash.le_next->hash.le_prev = (sta)->hash.le_prev; *(sta)->hash.le_prev = (sta)->hash.le_next; ((sta)->hash.le_prev) = ((void *)-1); ((sta)->hash.le_next) = ((void *)-1); } while (0);
 if (sta->challenge)
  free(sta->challenge, 127, 0);
 free(sta, 2, 0);
 whi->n_stations--;
...
wihap_sta_alloc(struct wi_softc *sc, u_int8_t *addr)
{
 ...
 sta = malloc(sizeof(*sta), 2, 0x0002 | 0x0008);
 if (sta == ((void *)0))
  return (((void *)0));
 i=hash<<4;
 ...
 sta->asid = 0xc001 + i;
 do { (sta)->list.tqe_next = ((void *)0); (sta)->list.tqe_prev = (&whi->sta_list)->tqh_last; *(&whi->sta_list)->tqh_last = (sta); (&whi->sta_list)->tqh_last = &(sta)->list.tqe_next; } while (0);
 do { if (((sta)->hash.le_next = (&whi->sta_hash[hash])->lh_first) != ((void *)0)) (&whi->sta_hash[hash])->lh_first->hash.le_prev = &(sta)->hash.le_next; (&whi->sta_hash[hash])->lh_first = (sta); (sta)->hash.le_prev = &(&whi->sta_hash[hash])->lh_first; } while (0);
 sta->sc = sc;
 whi->n_stations++;
 __builtin_bcopy((addr), (&sta->addr), (6));
...
wihap_sta_find(struct wihap_info *whi, u_int8_t *addr)
{
 ...
 for((sta) = ((&whi->sta_hash[i])->lh_first); (sta)!= ((void *)0); (sta) = ((sta)->hash.le_next))
    ...
    caddr_t pkt, int len)
{
 ...
 if (sta == ((void *)0)) {
  if (whi->apflags & 0x0002) {
   status = IEEE80211_STATUS_OTHER;
   goto fail;
  }
  if (whi->n_stations >= 1800) {
 ...
 case 0x0001:
  ...
  case 1:
   if (!sta->challenge) {
    sta->challenge = malloc(128, 127, 0x0002);
    if (!sta->challenge)
     return;
   }
   for (i = 0; i < 32; i++)
  ...
  case 3:
   if (challenge_len != 128 || !sta->challenge ||
       !((__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl)) & 0x4000)) {
    status = IEEE80211_STATUS_CHALLENGE;
    goto fail;
   }
   for (i=0; i<32; i++)
...
fail:
 ...
 resp_hdr = (struct wi_80211_hdr *)&sc->wi_txbuf;
 __builtin_bzero((resp_hdr), (sizeof(struct wi_80211_hdr)));
 resp_hdr->frame_ctl = (__uint16_t)(__builtin_constant_p(0x0000 | 0x00B0) ? (__uint16_t)(((__uint16_t)(0x0000 | 0x00B0) & 0xffU) << 8 | ((__uint16_t)(0x0000 | 0x00B0) & 0xff00U) >> 8) : __swap16md(0x0000 | 0x00B0));
  ...
  caddr_t pkt, int len)
{
 ...
 if ((rxfrm->wi_frame_ctl & (__uint16_t)(__builtin_constant_p(0x00F0) ? (__uint16_t)(((__uint16_t)(0x00F0) & 0xffU) << 8 | ((__uint16_t)(0x00F0) & 0xff00U) >> 8) : __swap16md(0x00F0))) ==
     (__uint16_t)(__builtin_constant_p(0x0020) ? (__uint16_t)(((__uint16_t)(0x0020) & 0xffU) << 8 | ((__uint16_t)(0x0020) & 0xff00U) >> 8) : __swap16md(0x0020))) {
  if (len < 6)
   return;
  take_hword(&pkt, &len);
  take_hword(&pkt, &len);
 ...
 if (sc->wi_net_name.i_len != ssid.i_len ||
     __builtin_memcmp((sc->wi_net_name.i_nwid), (ssid.i_nwid), (ssid.i_len))) {
  if (sc->sc_ic.ic_ac.ac_if.if_flags & 0x4)
 ...
 sta = wihap_sta_find(whi, rxfrm->wi_addr2);
 if (sta == ((void *)0) || !(sta->flags & 0x0001)) {
 ...
 if ((sc->wi_use_wep && !(capinfo & 0x0010)) ||
     (!sc->wi_use_wep && (capinfo & 0x0010))) {
  if (sc->sc_ic.ic_ac.ac_if.if_flags & 0x4)
...
fail:
 ...
 resp_hdr = (struct wi_80211_hdr *)&sc->wi_txbuf;
 __builtin_bzero((resp_hdr), (sizeof(struct wi_80211_hdr)));
 resp_hdr->frame_ctl = (__uint16_t)(__builtin_constant_p(0x0000 | 0x0010) ? (__uint16_t)(((__uint16_t)(0x0000 | 0x0010) & 0xffU) << 8 | ((__uint16_t)(0x0000 | 0x0010) & 0xff00U) >> 8) : __swap16md(0x0000 | 0x0010));
 pkt = (caddr_t)&sc->wi_txbuf + sizeof(struct wi_80211_hdr);
 __builtin_bcopy((rxfrm->wi_addr2), (resp_hdr->addr1), (6));
 __builtin_bcopy((sc->sc_ic.ic_myaddr), (resp_hdr->addr2), (6));
 __builtin_bcopy((sc->sc_ic.ic_myaddr), (resp_hdr->addr3), (6));
...
{
 printf("wihap_mgmt_input: len=%d ", (__uint16_t)(__builtin_constant_p(rxfrm->wi_dat_len) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_dat_len) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_dat_len) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_dat_len)));
 if ((rxfrm->wi_frame_ctl & (__uint16_t)(__builtin_constant_p(0x000C) ? (__uint16_t)(((__uint16_t)(0x000C) & 0xffU) << 8 | ((__uint16_t)(0x000C) & 0xff00U) >> 8) : __swap16md(0x000C))) ==
     (__uint16_t)(__builtin_constant_p(0x0000) ? (__uint16_t)(((__uint16_t)(0x0000) & 0xffU) << 8 | ((__uint16_t)(0x0000) & 0xff00U) >> 8) : __swap16md(0x0000))) {
  printf("MGMT: ");
  switch ((__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl)) & 0x00F0) {
  case 0x0000:
   printf("assoc req: \n");
   break;
  case 0x0010:
  ...
  default:
   printf("unknown (stype=0x%x)\n",
       (__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl)) & 0x00F0);
  }
 }
 else {
  printf("ftype=0x%x (ctl=0x%x)\n",
      (__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl)) & 0x000C,
      (__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl)));
 }
}
void
...
wihap_sta_is_assoc(struct wihap_info *whi, u_int8_t addr[])
{
 ...
 if (sta != ((void *)0) && (sta->flags & 0x0002)) {
  timeout_add_sec(&sta->tmo, whi->inactivity_time);
  return (1);
 }
 return (0);
...
wihap_data_input(struct wi_softc *sc, struct wi_frame *rxfrm, struct mbuf *m)
{
 ...
 int mcast, s;
 u_int16_t fctl;
 fctl = (__uint16_t)(__builtin_constant_p(rxfrm->wi_frame_ctl) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_frame_ctl) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_frame_ctl) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_frame_ctl));
 ...
 timeout_add_sec(&sta->tmo, whi->inactivity_time);
 sta->sig_info = (__uint16_t)(__builtin_constant_p(rxfrm->wi_q_info) ? (__uint16_t)(((__uint16_t)(rxfrm->wi_q_info) & 0xffU) << 8 | ((__uint16_t)(rxfrm->wi_q_info) & 0xff00U) >> 8) : __swap16md(rxfrm->wi_q_info));
 _splx(s);
 mcast = (rxfrm->wi_addr3[0] & 0x01) != 0;
 if (mcast || wihap_sta_is_assoc(whi, rxfrm->wi_addr3)) {
  if (mcast) {
   m = m_copym(m, 0, 1000000000, 0x0002);
   if (m == ((void *)0))
    return (0);
...
wihap_ioctl(struct wi_softc *sc, u_long command, caddr_t data)
{
 ...
 if (!(sc->sc_ic.ic_ac.ac_if.if_flags & 0x40))
  return 19;
 switch (command) {
 ...
 case (((unsigned long)0x80000000|(unsigned long)0x40000000) | ((sizeof(struct ifreq) & 0x1fff) << 16) | ((('i')) << 8) | ((200))):
  ...
  if (sta == ((void *)0))
   ...
   error = copyout(&reqsta, ifr->ifr_ifru.ifru_data,
       sizeof(reqsta));
  }
  _splx(s);
  break;
 ...
 case (((unsigned long)0x80000000|(unsigned long)0x40000000) | ((sizeof(struct ifreq) & 0x1fff) << 16) | ((('i')) << 8) | ((203))):
  ...
  while (sta && reqall.size >= n+sizeof(struct hostap_sta)) {
   __builtin_bcopy((sta->addr), (stabuf.addr), (6));
   ...
   if (error)
    break;
   sta = ((sta)->list.tqe_next);
   n += sizeof(struct hostap_sta);
  }
  _splx(s);
  if (!error)
   error = copyout(&reqall, ifr->ifr_ifru.ifru_data,
       sizeof(reqall));
  break;
 case (((unsigned long)0x80000000|(unsigned long)0x40000000) | ((sizeof(struct ieee80211_nodereq_all) & 0x1fff) << 16) | ((('i')) << 8) | ((214))):
  ...
  while (sta && na->na_size >=
   ...
   __builtin_memcpy((nr.nr_macaddr), (sta->addr), (6));
   __builtin_memcpy((nr.nr_bssid), (&sc->sc_ic.ic_myaddr), (6));
