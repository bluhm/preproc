...
u32 dce6_endpoint_rreg(struct radeon_device *rdev,
         u32 block_offset, u32 reg64)
{
 unsigned long flags;
 u32 r;
 _spin_lock_irqsave(&rdev->end_idx_lock, flags );
 r100_mm_wreg(rdev, (0x5E00 + block_offset), (reg64), 0);
 r = r100_mm_rreg(rdev, (0x5E04 + block_offset), 0);
 _spin_unlock_irqrestore(&rdev->end_idx_lock, flags );
 return r;
}
void dce6_endpoint_wreg(struct radeon_device *rdev,
          u32 block_offset, u32 reg64, u32 v)
{
 unsigned long flags;
 _spin_lock_irqsave(&rdev->end_idx_lock, flags );
 if (((rdev->family >= CHIP_BONAIRE)))
  ...
  r100_mm_wreg(rdev, (0x5E00 + block_offset), ((1 << 8) | (((reg64) & 0xff) << 0)), 0);
 r100_mm_wreg(rdev, (0x5E04 + block_offset), (v), 0);
 _spin_unlock_irqrestore(&rdev->end_idx_lock, flags );
}
static void dce6_afmt_get_connected_pins(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < rdev->audio.num_pins; i++) {
  ...
  tmp = radeon_audio_endpoint_rreg(rdev, (offset), (0x56));
  if (((tmp & (3 << 30)) >> 30) == 1)
...
struct r600_audio_pin *dce6_audio_get_pin(struct radeon_device *rdev)
{
 ...
 struct r600_audio_pin *pin = ((void *)0);
 int i, pin_count;
 ...
 if (!pin)
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "No connected audio pins found!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 return pin;
}
void dce6_afmt_select_pin(struct drm_encoder *encoder)
{
 ...
 if (!dig || !dig->afmt || !dig->pin)
  return;
 r100_mm_wreg(rdev, (0x713c + dig->afmt->offset), ((((dig->pin->id) & 7) << 0)), 0);
}
        ...
        struct drm_display_mode *mode)
{
 ...
 radeon_audio_endpoint_wreg(rdev, (dig->pin->offset), (0x37), (tmp));
}
          ...
          u8 *sadb, int sad_count)
{
 ...
 if (!dig || !dig->afmt || !dig->pin)
  return;
 tmp = radeon_audio_endpoint_rreg(rdev, (dig->pin->offset), (0x25));
 tmp &= ~((1 << 17) | (0x7f << 0));
 tmp |= (1 << 16);
 if (sad_count)
  tmp |= (((sadb[0]) & 0x7f) << 0);
  ...
  tmp |= (((5) & 0x7f) << 0);
 radeon_audio_endpoint_wreg(rdev, (dig->pin->offset), (0x25), (tmp));
}
         ...
         struct cea_sad *sads, int sad_count)
{
 ...
 static const u16 eld_reg_to_type[][2] = {
  { 0x28, HDMI_AUDIO_CODING_TYPE_PCM },
  { 0x29, HDMI_AUDIO_CODING_TYPE_AC3 },
  { 0x2A, HDMI_AUDIO_CODING_TYPE_MPEG1 },
  { 0x2B, HDMI_AUDIO_CODING_TYPE_MP3 },
  { 0x2C, HDMI_AUDIO_CODING_TYPE_MPEG2 },
  { 0x2D, HDMI_AUDIO_CODING_TYPE_AAC_LC },
  { 0x2E, HDMI_AUDIO_CODING_TYPE_DTS },
  { 0x2F, HDMI_AUDIO_CODING_TYPE_ATRAC },
  { 0x31, HDMI_AUDIO_CODING_TYPE_EAC3 },
  { 0x32, HDMI_AUDIO_CODING_TYPE_DTS_HD },
  { 0x33, HDMI_AUDIO_CODING_TYPE_MLP },
  { 0x35, HDMI_AUDIO_CODING_TYPE_WMA_PRO },
 };
 if (!dig || !dig->afmt || !dig->pin)
  return;
 for (i = 0; i < (sizeof((eld_reg_to_type)) / sizeof((eld_reg_to_type)[0])); i++) {
  ...
  radeon_audio_endpoint_wreg(rdev, (dig->pin->offset), (eld_reg_to_type[i][0]), (value));
 }
}
void dce6_audio_enable(struct radeon_device *rdev,
         struct r600_audio_pin *pin,
         u8 enable_mask)
{
 if (!pin)
  return;
 radeon_audio_endpoint_wreg(rdev, (pin->offset), (0x54), (enable_mask ? (1 << 31) : 0));
}
        ...
        struct radeon_crtc *crtc, unsigned int clock)
{
 ...
 r100_mm_wreg(rdev, (0x05b0), (24000), 0);
      ...
      struct radeon_crtc *crtc, unsigned int clock)
{
 ...
 r100_mm_wreg(rdev, (0x05ac), (value), 0);
 if (((rdev->family >= CHIP_BONAIRE))) {
  unsigned int div = (r100_mm_rreg(rdev, (0x0490), 0) &
   (0x7f << 24)) >>
   24;
  div = radeon_audio_decode_dfs_div(div);
  if (div)
   clock = clock * 100 / div;
  r100_mm_wreg(rdev, (0x05b8), (24000), 0);
  r100_mm_wreg(rdev, (0x05bc), (clock), 0);
 } else {
