...
static void vce_v2_0_set_sw_cg(struct radeon_device *rdev, _Bool gated)
{
 u32 tmp;
 if (gated) {
  ...
  tmp = r100_mm_rreg(rdev, (0x207bc), 0);
  tmp |= 0xff000000;
  r100_mm_wreg(rdev, (0x207bc), (tmp), 0);
  tmp = r100_mm_rreg(rdev, (0x207c0), 0);
  tmp &= ~0x3fc;
  r100_mm_wreg(rdev, (0x207c0), (tmp), 0);
  r100_mm_wreg(rdev, (0x207a0), (0), 0);
    } else {
  tmp = r100_mm_rreg(rdev, (0x202fc), 0);
  tmp |= 0xe7;
  tmp &= ~0xe70000;
...
static void vce_v2_0_set_dyn_cg(struct radeon_device *rdev, _Bool gated)
{
 u32 orig, tmp;
 tmp = r100_mm_rreg(rdev, (0x202fc), 0);
 tmp &= ~0x00060006;
 ...
 r100_mm_wreg(rdev, (0x202fc), (tmp), 0);
 orig = tmp = r100_mm_rreg(rdev, (0x207bc), 0);
 tmp &= ~0x1fe000;
 tmp &= ~0xff000000;
 if (tmp != orig)
  r100_mm_wreg(rdev, (0x207bc), (tmp), 0);
 orig = tmp = r100_mm_rreg(rdev, (0x207c0), 0);
 tmp &= ~0x3fc;
 if (tmp != orig)
  r100_mm_wreg(rdev, (0x207c0), (tmp), 0);
 if (gated)
...
{
 r100_mm_wreg(rdev, (0x207a0), (7), 0);
}
void vce_v2_0_enable_mgcg(struct radeon_device *rdev, _Bool enable)
{
 _Bool sw_cg = 0;
...
unsigned vce_v2_0_bo_size(struct radeon_device *rdev)
{
 ({ int __ret = !!(rdev->vce_fw->size > (256 * 1024)); if (__ret) printf("WARNING %s failed at %s:%d\n", "rdev->vce_fw->size > (256 * 1024)", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/vce_v2_0.c", 148); __builtin_expect(!!(__ret), 0); });
 return (256 * 1024) + (64 * 1024) + (23552 * 16);
}
int vce_v2_0_resume(struct radeon_device *rdev)
{
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x207bc), 0); tmp_ &= (~0xFF9FF000); tmp_ |= ((0x1FF000) & ~(~0xFF9FF000)); r100_mm_wreg(rdev, (0x207bc), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x207c0), 0); tmp_ &= (~0x3F); tmp_ |= ((0x3F) & ~(~0x3F)); r100_mm_wreg(rdev, (0x207c0), (tmp_), 0); } while (0);
 r100_mm_wreg(rdev, (0x202fc), (0xf7), 0);
 r100_mm_wreg(rdev, (0x21498), (0x00398000), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x214f4), 0); tmp_ &= (~0x1); tmp_ |= ((0x0) & ~(~0x1)); r100_mm_wreg(rdev, (0x214f4), (tmp_), 0); } while (0);
 r100_mm_wreg(rdev, (0x214b4), (0), 0);
 r100_mm_wreg(rdev, (0x214b8), (0), 0);
 r100_mm_wreg(rdev, (0x214a0), (0), 0);
 r100_mm_wreg(rdev, (0x2145c), (addr >> 8), 0);
 addr &= 0xff;
 size = (256 * 1024);
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x21474), 0); tmp_ &= (~0x100); tmp_ |= ((0x0) & ~(~0x100)); r100_mm_wreg(rdev, (0x21474), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x21300), 0); tmp_ &= (~(1 << 3)); tmp_ |= (((1 << 3)) & ~(~(1 << 3))); r100_mm_wreg(rdev, (0x21300), (tmp_), 0); } while (0);
