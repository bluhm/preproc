...
void dce6_audio_enable(struct radeon_device *rdev, struct r600_audio_pin *pin,
  u8 enable_mask);
u32 dce6_endpoint_rreg(struct radeon_device *rdev, u32 offset, u32 reg64);
...
void r600_set_mute(struct drm_encoder *encoder, u32 offset, _Bool mute);
void dce3_2_set_mute(struct drm_encoder *encoder, u32 offset, _Bool mute);
void dce4_set_mute(struct drm_encoder *encoder, u32 offset, _Bool mute);
static void radeon_audio_hdmi_mode_set(struct drm_encoder *encoder,
 struct drm_display_mode *mode);
static void radeon_audio_dp_mode_set(struct drm_encoder *encoder,
 struct drm_display_mode *mode);
void r600_hdmi_enable(struct drm_encoder *encoder, _Bool enable);
void evergreen_hdmi_enable(struct drm_encoder *encoder, _Bool enable);
void evergreen_dp_enable(struct drm_encoder *encoder, _Bool enable);
...
static u32 radeon_audio_rreg(struct radeon_device *rdev, u32 offset, u32 reg64)
{
 return r100_mm_rreg(rdev, (reg64), 0);
}
static void radeon_audio_wreg(struct radeon_device *rdev, u32 offset,
  u32 reg64, u32 v)
{
 r100_mm_wreg(rdev, (reg64), (v), 0);
}
    ...
    struct r600_audio_pin *pin, u8 enable_mask)
{
 ...
 if (rdev->mode_info.mode_config_initialized) {
  for (encoder = ({ const __typeof( ((__typeof(*encoder) *)0)->head ) *__mptr = ((&rdev->ddev->mode_config.encoder_list)->next); (__typeof(*encoder) *)( (char *)__mptr - __builtin_offsetof(__typeof(*encoder), head) );}); &encoder->head != (&rdev->ddev->mode_config.encoder_list); encoder = ({ const __typeof( ((__typeof(*encoder) *)0)->head ) *__mptr = (encoder->head.next); (__typeof(*encoder) *)( (char *)__mptr - __builtin_offsetof(__typeof(*encoder), head) );})) {
...
{
 return ((rdev->family == CHIP_RS600) || (rdev->family == CHIP_RS690) || (rdev->family == CHIP_RS740) || (rdev->family >= CHIP_R600)) && !((rdev->family == CHIP_HAINAN));
}
int radeon_audio_init(struct radeon_device *rdev)
{
 int i;
 ...
 else if (((rdev->family == CHIP_KAVERI)))
  rdev->audio.num_pins = 7;
 else if (((rdev->family >= CHIP_BONAIRE)))
  rdev->audio.num_pins = 7;
 else if (((rdev->family == CHIP_OLAND)))
  rdev->audio.num_pins = 2;
 else if (((rdev->family >= CHIP_ARUBA) && (rdev->flags & RADEON_IS_IGP)))
...
u32 radeon_audio_endpoint_rreg(struct radeon_device *rdev, u32 offset, u32 reg64)
{
 if (rdev->audio.funcs->endpoint_rreg)
  return rdev->audio.funcs->endpoint_rreg(rdev, offset, reg64);
 return 0;
}
 ...
 u32 reg64, u32 v)
{
 if (rdev->audio.funcs->endpoint_wreg)
  rdev->audio.funcs->endpoint_wreg(rdev, offset, reg64, v);
}
static void radeon_audio_write_sad_regs(struct drm_encoder *encoder)
{
 ...
 if (sad_count <= 0) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Couldn't read SADs: %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , sad_count);
  return;
 }
 ((!(!sads)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_audio.c", 373, "!(!sads)"));
...
static void radeon_audio_set_dto(struct drm_encoder *encoder, unsigned int clock)
{
 struct radeon_device *rdev = encoder->dev->dev_private;
 struct radeon_encoder *radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
 struct radeon_crtc *crtc = ({ const __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (encoder->crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
           ...
           struct drm_display_mode *mode)
{
 ...
 if (err < 0) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "failed to setup AVI infoframe: %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , err);
  return err;
 }
 ...
 if (err < 0) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "failed to pack AVI infoframe: %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , err);
  return err;
 }
...
static void radeon_audio_calc_cts(unsigned int clock, int *CTS, int *N, int freq)
{
 ...
 cts *= mul;
 if (n < (128*freq/1500))
  printf("" "Calculated ACR N value is too small. You may experience audio problems.\n");
 if (n > (128*freq/300))
  printf("" "Calculated ACR N value is too large. You may experience audio problems.\n");
 *N = n;
 *CTS = cts;
 do { } while( 0);
}
static const struct radeon_hdmi_acr* radeon_audio_acr(unsigned int clock)
{
 ...
 for (i = 0; i < (sizeof((hdmi_predefined_acr)) / sizeof((hdmi_predefined_acr)[0])); i++)
...
static void radeon_audio_set_mute(struct drm_encoder *encoder, _Bool mute)
{
 struct radeon_encoder *radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
 struct radeon_encoder_atom_dig *dig = radeon_encoder->enc_priv;
 if (!dig || !dig->afmt)
...
void radeon_audio_dpms(struct drm_encoder *encoder, int mode)
{
 ...
 if (radeon_encoder->audio && radeon_encoder->audio->dpms)
  radeon_encoder->audio->dpms(encoder, mode == 0);
}
unsigned int radeon_audio_decode_dfs_div(unsigned int div)
{
 if (div >= 8 && div < 64)
