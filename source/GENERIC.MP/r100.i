...
_Bool r100_page_flip_pending(struct radeon_device *rdev, int crtc_id)
{
 struct radeon_crtc *radeon_crtc = rdev->mode_info.crtcs[crtc_id];
 return !!(r100_mm_rreg(rdev, (0x0224 + radeon_crtc->crtc_offset), 0) &
  (1<<30));
}
       ...
       uint64_t entry)
{
 ...
 gtt[i] = (__uint32_t)(__builtin_constant_p(((u32)(entry))) ? (__uint32_t)(((__uint32_t)(((u32)(entry))) & 0xff) << 24 | ((__uint32_t)(((u32)(entry))) & 0xff00) << 8 | ((__uint32_t)(((u32)(entry))) & 0xff0000) >> 8 | ((__uint32_t)(((u32)(entry))) & 0xff000000) >> 24) : __swap32md(((u32)(entry))));
}
void r100_pci_gart_fini(struct radeon_device *rdev)
{
...
_Bool r100_semaphore_ring_emit(struct radeon_device *rdev,
         ...
         _Bool emit_wait)
{
 do { panic("BUG at %s:%d", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/r100.c", 873); } while (0);
 return 0;
}
struct radeon_fence *r100_copy_blit(struct radeon_device *rdev,
        uint64_t src_offset,
...
static int r100_cp_init_microcode(struct radeon_device *rdev)
{
 ...
 if ((rdev->family == CHIP_R100) || (rdev->family == CHIP_RV100) ||
  ...
  do { } while( 0);
  fw_name = "radeon/R100_cp.bin";
 } else if ((rdev->family == CHIP_R200) ||
     (rdev->family == CHIP_RV250) ||
     (rdev->family == CHIP_RV280) ||
     (rdev->family == CHIP_RS300)) {
  do { } while( 0);
  fw_name = "radeon/R200_cp.bin";
  ...
  do { } while( 0);
  fw_name = "radeon/R300_cp.bin";
 } else if ((rdev->family == CHIP_R420) ||
     (rdev->family == CHIP_R423) ||
     (rdev->family == CHIP_RV410)) {
  do { } while( 0);
  fw_name = "radeon/R420_cp.bin";
 } else if ((rdev->family == CHIP_RS690) ||
     (rdev->family == CHIP_RS740)) {
  do { } while( 0);
  fw_name = "radeon/RS690_cp.bin";
 } else if (rdev->family == CHIP_RS600) {
  do { } while( 0);
  fw_name = "radeon/RS600_cp.bin";
  ...
  do { } while( 0);
  fw_name = "radeon/R520_cp.bin";
 }
 err = request_firmware(&rdev->me_fw, fw_name, rdev->dev);
 if (err) {
  printf("" "radeon_cp: Failed to load firmware \"%s\"\n", fw_name);
 } else if (rdev->me_fw->size % 8) {
  printf("" "radeon_cp: Bogus length %zu in firmware \"%s\"\n", rdev->me_fw->size, fw_name);
  err = -22;
        ...
        struct radeon_ring *ring)
{
 ...
 if (rdev->wb.enabled)
  ...
  rptr = r100_mm_rreg(rdev, (0x0710), 0);
 return rptr;
}
u32 r100_gfx_get_wptr(struct radeon_device *rdev,
        struct radeon_ring *ring)
{
 u32 wptr;
 wptr = r100_mm_rreg(rdev, (0x0714), 0);
 return wptr;
}
void r100_gfx_set_wptr(struct radeon_device *rdev,
         struct radeon_ring *ring)
{
 ...
 (void)r100_mm_rreg(rdev, (0x0714), 0);
}
static void r100_cp_load_microcode(struct radeon_device *rdev)
{
 ...
 if (rdev->me_fw) {
  ...
  for (i = 0; i < size; i += 2) {
   r100_mm_wreg(rdev, (0x07dc), (((__uint32_t)(*(__uint32_t *)(&fw_data[i])))), 0);
   r100_mm_wreg(rdev, (0x07e0), (((__uint32_t)(*(__uint32_t *)(&fw_data[i + 1])))), 0);
  }
 }
}
int r100_cp_init(struct radeon_device *rdev, unsigned ring_size)
...
uint32_t r100_pll_rreg(struct radeon_device *rdev, uint32_t reg64)
{
 unsigned long flags;
 uint32_t data;
 _spin_lock_irqsave(&rdev->pll_idx_lock, flags );
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0008), (reg64 & 0x3f));
 r100_pll_errata_after_index(rdev);
 data = r100_mm_rreg(rdev, (0x000c), 0);
 r100_pll_errata_after_data(rdev);
 _spin_unlock_irqrestore(&rdev->pll_idx_lock, flags );
 return data;
}
void r100_pll_wreg(struct radeon_device *rdev, uint32_t reg64, uint32_t v)
{
 unsigned long flags;
 _spin_lock_irqsave(&rdev->pll_idx_lock, flags );
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0008), (((reg64 & 0x3f) | (1 << 7))));
 r100_pll_errata_after_index(rdev);
...
void r100_mc_stop(struct radeon_device *rdev, struct r100_mc_save *save)
{
 ...
 save->GENMO_WT = bus_space_read_1(rdev->memt, rdev->rmmio_bsh, (0x0003C2));
 ...
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0003C2), (0xFD & save->GENMO_WT));
...
void r100_mc_resume(struct radeon_device *rdev, struct r100_mc_save *save)
{
 ...
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0003C2), (save->GENMO_WT));
...
{
 u32 tmp;
 tmp = bus_space_read_1(rdev->memt, rdev->rmmio_bsh, (0x0003C2));
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0003C2), (0xFD & tmp));
}
...
uint32_t r100_mm_rreg_slow(struct radeon_device *rdev, uint32_t reg64)
{
 unsigned long flags;
 uint32_t ret;
 _spin_lock_irqsave(&rdev->mmio_idx_lock, flags );
 bus_space_write_4(rdev->memt, rdev->rmmio_bsh,
     0x0000, reg64);
 ret = bus_space_read_4(rdev->memt, rdev->rmmio_bsh,
     0x0004);
 _spin_unlock_irqrestore(&rdev->mmio_idx_lock, flags );
 return ret;
}
void r100_mm_wreg_slow(struct radeon_device *rdev, uint32_t reg64, uint32_t v)
{
