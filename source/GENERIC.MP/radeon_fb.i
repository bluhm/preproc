...
{
 struct radeon_fbdev *rfbdev =
  ({ const __typeof( ((struct radeon_fbdev *)0)->helper ) *__mptr = (helper); (struct radeon_fbdev *)( (char *)__mptr - __builtin_offsetof(struct radeon_fbdev, helper) );});
 struct radeon_device *rdev = rfbdev->rdev;
 struct fb_info *info;
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "failed to initialize framebuffer %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , ret);
