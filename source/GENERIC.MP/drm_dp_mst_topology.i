        ...
        struct drm_dp_sideband_msg_tx *txmsg)
{
 ...
 int ret;
 ret = ({ long __ret = (4 * hz); if (!(check_txmsg_state(mgr, txmsg))) __ret = ({ long ret = (4 * hz); do { int deadline, __error; ((!cold) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/drm_dp_mst_topology.c", 766, "!cold")); __mtx_enter(&sch_mtx ); ((void)_atomic_add_int_nv((&(mgr->tx_waitq).count), 1)); deadline = ticks + ret; __error = msleep(&mgr->tx_waitq, &sch_mtx, 0, "drmweti", ret); ret = deadline - ticks; ((void)_atomic_sub_int_nv((&(mgr->tx_waitq).count), 1)); if (__error == -1 || __error == 4) { ret = -4; __mtx_leave(&sch_mtx ); break; } if ((4 * hz) && (ret <= 0 || __error == 35)) { __mtx_leave(&sch_mtx ); ret = ((check_txmsg_state(mgr, txmsg))) ? 1 : 0; break; } __mtx_leave(&sch_mtx ); } while (ret > 0 && !(check_txmsg_state(mgr, txmsg))); ret; }); __ret; });
