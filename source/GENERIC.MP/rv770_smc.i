          ...
          u16 smc_address, u16 limit)
{
 ...
 addr = smc_address;
 addr |= (1 << 16);
 r100_mm_wreg(rdev, (0x200), (addr), 0);
 return 0;
}
int rv770_copy_bytes_to_smc(struct radeon_device *rdev,
       u16 smc_start_address, const u8 *src,
       u16 byte_count, u16 limit)
{
 ...
 _spin_lock_irqsave(&rdev->smc_idx_lock, flags );
 while (byte_count >= 4) {
  data = (src[0] << 24) | (src[1] << 16) | (src[2] << 8) | src[3];
  ret = rv770_set_smc_sram_address(rdev, addr, limit);
  if (ret)
   goto done;
  r100_mm_wreg(rdev, (0x204), (data), 0);
  src += 4;
  byte_count -= 4;
  addr += 4;
 }
 if (byte_count > 0) {
  ...
  if (ret)
   goto done;
  original_data = r100_mm_rreg(rdev, (0x204), 0);
 ...
 _spin_unlock_irqrestore(&rdev->smc_idx_lock, flags );
 return ret;
}
        ...
        u32 byte_count)
{
 ...
 if (byte_count % 4)
  return -22;
 if (smc_first_vector < 0xFFD8) {
  tmp = 0xFFD8 - smc_first_vector;
  if (tmp > byte_count)
   return 0;
  byte_count -= tmp;
  src += tmp;
  smc_first_vector = 0xFFD8;
 }
 for (i = 0; i < byte_count; i += 4) {
  tmp = (src[i] << 24) | (src[i + 1] << 16) | (src[i + 2] << 8) | src[i + 3];
  r100_mm_wreg(rdev, (0x218 + i), (tmp), 0);
 }
 return 0;
}
void rv770_start_smc(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x208), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x208), (tmp_), 0); } while (0);
}
void rv770_reset_smc(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x208), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x208), (tmp_), 0); } while (0);
}
void rv770_stop_smc_clock(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x208), 0); tmp_ &= (~(1 << 11)); tmp_ |= ((0) & ~(~(1 << 11))); r100_mm_wreg(rdev, (0x208), (tmp_), 0); } while (0);
}
void rv770_start_smc_clock(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x208), 0); tmp_ &= (~(1 << 11)); tmp_ |= (((1 << 11)) & ~(~(1 << 11))); r100_mm_wreg(rdev, (0x208), (tmp_), 0); } while (0);
}
_Bool rv770_is_smc_running(struct radeon_device *rdev)
{
 u32 tmp;
 tmp = r100_mm_rreg(rdev, (0x208), 0);
 if ((tmp & (1 << 0)) && (tmp & (1 << 11)))
  return 1;
 else
  return 0;
...
PPSMC_Result rv770_send_msg_to_smc(struct radeon_device *rdev, PPSMC_Msg msg)
{
 ...
 if (!rv770_is_smc_running(rdev))
  return ((uint8_t)0xFF);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x20c), 0); tmp_ &= (~(0xff << 0)); tmp_ |= ((((msg) << 0)) & ~(~(0xff << 0))); r100_mm_wreg(rdev, (0x20c), (tmp_), 0); } while (0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  tmp = r100_mm_rreg(rdev, (0x20c), 0) & (0xff << 8);
  tmp >>= 8;
  if (tmp != 0)
   break;
  udelay(1);
...
{
 int i;
 PPSMC_Result result = ((uint8_t)0x01);
 if (!rv770_is_smc_running(rdev))
  return result;
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (r100_mm_rreg(rdev, (0x208), 0) & (1 << 2))
   break;
  udelay(1);
 }
 return result;
    ...
    u16 limit)
{
 ...
 switch (rdev->family) {
 case CHIP_RV770:
  ucode_start_address = 0x0100;
  ucode_size = 0x410d;
  int_vect = (const u8 *)&rv770_smc_int_vectors;
  int_vect_start_address = 0xffc0;
  int_vect_size = 0x0040;
  break;
 case CHIP_RV730:
  ucode_start_address = 0x0100;
  ucode_size = 0x412c;
 ...
 case CHIP_CAYMAN:
  ...
  break;
 default:
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "unknown asic in smc ucode loader\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  do { panic("BUG at %s:%d", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/rv770_smc.c", 583); } while (0);
 }
         ...
         u16 smc_address, u32 *value, u16 limit)
{
 ...
 if (ret == 0)
  *value = r100_mm_rreg(rdev, (0x204), 0);
 _spin_unlock_irqrestore(&rdev->smc_idx_lock, flags );
 return ret;
}
int rv770_write_smc_sram_dword(struct radeon_device *rdev,
          u16 smc_address, u32 value, u16 limit)
{
 ...
 ret = rv770_set_smc_sram_address(rdev, smc_address, limit);
 if (ret == 0)
  r100_mm_wreg(rdev, (0x204), (value), 0);
 _spin_unlock_irqrestore(&rdev->smc_idx_lock, flags );
 return ret;
}
