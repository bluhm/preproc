...
irqreturn_t radeon_driver_irq_handler_kms(void *arg)
{
 ...
 if (!rdev->irq.installed)
  return 0;
 ret = (rdev)->asic->irq.process((rdev));
 if (ret == IRQ_HANDLED)
  ;
 return ret;
}
...
{
 struct radeon_device *rdev = ({ const __typeof( ((struct radeon_device *)0)->hotplug_work.work ) *__mptr = (work); (struct radeon_device *)( (char *)__mptr - __builtin_offsetof(struct radeon_device, hotplug_work.work) );});
 struct drm_device *dev = rdev->ddev;
 struct drm_mode_config *mode_config = &dev->mode_config;
 struct drm_connector *connector;
 if (!rdev->mode_info.mode_config_initialized)
  return;
 _rw_enter_write(&mode_config->mutex );
 if (mode_config->num_connector) {
  for (connector = ({ const __typeof( ((__typeof(*connector) *)0)->head ) *__mptr = ((&mode_config->connector_list)->next); (__typeof(*connector) *)( (char *)__mptr - __builtin_offsetof(__typeof(*connector), head) );}); &connector->head != (&mode_config->connector_list); connector = ({ const __typeof( ((__typeof(*connector) *)0)->head ) *__mptr = (connector->head.next); (__typeof(*connector) *)( (char *)__mptr - __builtin_offsetof(__typeof(*connector), head) );}))
   radeon_connector_hotplug(connector);
 }
 _rw_exit_write(&mode_config->mutex );
 drm_helper_hpd_irq_event(dev);
}
static void radeon_dp_work_func(struct work_struct *work)
{
 struct radeon_device *rdev = ({ const __typeof( ((struct radeon_device *)0)->dp_work ) *__mptr = (work); (struct radeon_device *)( (char *)__mptr - __builtin_offsetof(struct radeon_device, dp_work) );});
 struct drm_device *dev = rdev->ddev;
 struct drm_mode_config *mode_config = &dev->mode_config;
 struct drm_connector *connector;
 if (mode_config->num_connector) {
...
{
 int r = 0;
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->irq.lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
...
_Bool radeon_irq_kms_sw_irq_get_delayed(struct radeon_device *rdev, int ring)
{
 return __sync_add_and_fetch((&rdev->irq.ring_int[ring]), 1) == 1;
}
void radeon_irq_kms_sw_irq_put(struct radeon_device *rdev, int ring)
{
 unsigned long irqflags;
