...
static inline void trace_radeon_cs(struct radeon_cs_parser *p) {};
static inline void trace_radeon_vm_grab_id(unsigned vmid, int ring) {};
static inline void trace_radeon_vm_bo_update(struct radeon_bo_va *bo_va) {};
static inline void trace_radeon_vm_set_page(uint64_t pe, uint64_t addr, unsigned count, uint32_t incr, uint32_t flags) {};
static inline void trace_radeon_vm_flush(uint64_t pd_addr, unsigned ring, unsigned id) {};
static inline void trace_radeon_fence_request(struct drm_device *dev, int ring, u32 seqno) {};
static inline void trace_radeon_fence_emit(struct drm_device *dev, int ring, u32 seqno) {};
static inline void trace_radeon_fence_wait_begin(struct drm_device *dev, int ring, u32 seqno) {};
static inline void trace_radeon_fence_wait_end(struct drm_device *dev, int ring, u32 seqno) {};
...
{
 queue_delayed_work(((struct workqueue_struct *)systq),
...
static int radeon_fence_check_signaled(wait_queue_t *wait, unsigned mode, int flags, void *key)
{
 struct radeon_fence *fence;
 u64 seq;
 fence = ({ const __typeof( ((struct radeon_fence *)0)->fence_wake ) *__mptr = (wait); (struct radeon_fence *)( (char *)__mptr - __builtin_offsetof(struct radeon_fence, fence_wake) );});
 seq = (*(&fence->rdev->fence_drv[fence->ring].last_seq));
 if (seq >= fence->seq) {
...
static _Bool radeon_fence_activity(struct radeon_device *rdev, int ring)
{
...
static void radeon_fence_check_lockup(struct work_struct *work)
{
 ...
 struct radeon_device *rdev;
 int ring;
 fence_drv = ({ const __typeof( ((struct radeon_fence_driver *)0)->lockup_work.work ) *__mptr = (work); (struct radeon_fence_driver *)( (char *)__mptr - __builtin_offsetof(struct radeon_fence_driver, lockup_work.work) );});
 rdev = fence_drv->rdev;
 ring = fence_drv - &rdev->fence_drv[0];
 if (!(_rw_enter(&rdev->exclusive_lock, 0x0002UL | 0x0040UL ) == 0)) {
  radeon_fence_schedule_check(rdev, ring);
  return;
 }
 if (fence_drv->delayed_irq && rdev->ddev->irq_enabled) {
  ...
  _spin_lock_irqsave(&rdev->irq.lock, irqflags );
  (rdev)->asic->irq.set((rdev));
  _spin_unlock_irqrestore(&rdev->irq.lock, irqflags );
 }
 if (radeon_fence_activity(rdev, ring))
  _wake_up(&rdev->fence_queue );
 else if ((rdev)->asic->ring[(ring)]->is_lockup((rdev), (&rdev->ring[ring]))) {
  printf("drm:pid%d:%s *WARNING* " "GPU lockup (current fence id " "0x%016llx last fence id 0x%016llx on ring %d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , (uint64_t)(*(&fence_drv->last_seq)), fence_drv->sync_seq[ring], ring);
  rdev->needs_reset = 1;
  _wake_up(&rdev->fence_queue );
 }
 _rw_exit_read(&rdev->exclusive_lock );
}
void radeon_fence_process(struct radeon_device *rdev, int ring)
{
 if (radeon_fence_activity(rdev, ring))
...
static _Bool radeon_fence_is_signaled(struct fence *f)
{
...
static _Bool radeon_fence_enable_signaling(struct fence *f)
{
 struct radeon_fence *fence = to_radeon_fence(f);
 struct radeon_device *rdev = fence->rdev;
       ...
       long timeout)
{
 ...
 if (intr) {
  r = ({ long __ret = timeout; if (!(( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))) __ret = ({ long ret = timeout; do { int deadline, __error; ((!cold) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_fence.c", 503, "!cold")); __mtx_enter(&sch_mtx ); ((void)_atomic_add_int_nv((&(rdev->fence_queue).count), 1)); deadline = ticks + ret; __error = msleep(&rdev->fence_queue, &sch_mtx, 0x100, "drmweti", ret); ret = deadline - ticks; ((void)_atomic_sub_int_nv((&(rdev->fence_queue).count), 1)); if (__error == -1 || __error == 4) { ret = -4; __mtx_leave(&sch_mtx ); break; } if (timeout && (ret <= 0 || __error == 35)) { __mtx_leave(&sch_mtx ); ret = ((( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))) ? 1 : 0; break; } __mtx_leave(&sch_mtx ); } while (ret > 0 && !(( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))); ret; }); __ret; });
 } else {
  r = ({ long __ret = timeout; if (!(( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))) __ret = ({ long ret = timeout; do { int deadline, __error; ((!cold) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_fence.c", 507, "!cold")); __mtx_enter(&sch_mtx ); ((void)_atomic_add_int_nv((&(rdev->fence_queue).count), 1)); deadline = ticks + ret; __error = msleep(&rdev->fence_queue, &sch_mtx, 0, "drmweti", ret); ret = deadline - ticks; ((void)_atomic_sub_int_nv((&(rdev->fence_queue).count), 1)); if (__error == -1 || __error == 4) { ret = -4; __mtx_leave(&sch_mtx ); break; } if (timeout && (ret <= 0 || __error == 35)) { __mtx_leave(&sch_mtx ); ret = ((( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))) ? 1 : 0; break; } __mtx_leave(&sch_mtx ); } while (ret > 0 && !(( radeon_fence_any_seq_signaled(rdev, target_seq) || rdev->needs_reset))); ret; }); __ret; });
 }
 if (rdev->needs_reset)
  r = -11;
 for (i = 0; i < 8; ++i) {
...
int radeon_fence_wait(struct radeon_fence *fence, _Bool intr)
{
 uint64_t seq[8] = {};
 long r;
 if (({ static int __warned; int __ret = !!(!to_radeon_fence(&fence->base)); if (__ret && !__warned) { printf("WARNING %s failed at %s:%d\n", "!to_radeon_fence(&fence->base)", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_fence.c", 546); __warned = 1; } __builtin_expect(!!(__ret), 0); }))
  return fence_wait(&fence->base, intr);
 seq[fence->ring] = fence->seq;
 r = radeon_fence_wait_seq_timeout(fence->rdev, seq, intr, (0x7fffffff));
 if (r < 0) {
  return r;
 }
     ...
     _Bool intr)
{
 ...
 if (num_rings == 0)
  return -2;
 r = radeon_fence_wait_seq_timeout(rdev, seq, intr, (0x7fffffff));
 if (r < 0) {
  return r;
 }
 return 0;
...
int radeon_fence_wait_next(struct radeon_device *rdev, int ring)
{
 uint64_t seq[8] = {};
 long r;
 seq[ring] = (*(&rdev->fence_drv[ring].last_seq)) + 1ULL;
 if (seq[ring] >= rdev->fence_drv[ring].sync_seq[ring]) {
  return -2;
 }
 r = radeon_fence_wait_seq_timeout(rdev, seq, 0, (0x7fffffff));
 if (r < 0)
  return r;
 return 0;
}
int radeon_fence_wait_empty(struct radeon_device *rdev, int ring)
{
 ...
 if (r < 0) {
  if (r == -11)
   return -11;
  printf("drm:pid%d:%s *ERROR* " "error waiting for ring[%d] to become idle (%ld)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , ring, r);
 }
 return 0;
}
struct radeon_fence *radeon_fence_ref(struct radeon_fence *fence)
...
int radeon_fence_driver_start_ring(struct radeon_device *rdev, int ring)
{
 ...
 if (rdev->wb.use_event || !radeon_ring_supports_scratch_reg(rdev, &rdev->ring[ring])) {
  ...
  if (ring != 5) {
   ...
   index = (((rdev->uvd_fw->size)+((8)-1))&(~((8)-1)));
...
static const char *radeon_fence_get_timeline_name(struct fence *f)
{
 struct radeon_fence *fence = to_radeon_fence(f);
 switch (fence->ring) {
 case 0: return "radeon.gfx";
 case 1: return "radeon.cp1";
 case 2: return "radeon.cp2";
 case 3: return "radeon.dma";
 case 4: return "radeon.dma1";
 case 5: return "radeon.uvd";
 case 6: return "radeon.vce1";
 case 7: return "radeon.vce2";
 default: ({ static int __warned; int __ret = !!(1); if (__ret && !__warned) { printf("WARNING %s failed at %s:%d\n", "1", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_fence.c", 1019); __warned = 1; } __builtin_expect(!!(__ret), 0); }); return "radeon.unk";
 }
}
static inline _Bool radeon_test_signaled(struct radeon_fence *fence)
{
...
radeon_fence_wait_cb(struct fence *fence, struct fence_cb *cb)
{
 struct radeon_wait_cb *wait =
  ({ const __typeof( ((struct radeon_wait_cb *)0)->base ) *__mptr = (cb); (struct radeon_wait_cb *)( (char *)__mptr - __builtin_offsetof(struct radeon_wait_cb, base) );});
 do { __mtx_enter(&sch_mtx ); wakeup(wait->task); __mtx_leave(&sch_mtx ); } while (0);
}
static signed long radeon_fence_default_wait(struct fence *f, _Bool intr,
          signed long t)
{
 struct radeon_fence *fence = to_radeon_fence(f);
 struct radeon_device *rdev = fence->rdev;
 struct radeon_wait_cb cb;
 cb.task = (__curcpu->ci_self)->ci_curproc;
 if (fence_add_callback(f, &cb.base, radeon_fence_wait_cb))
  return t;
 while (t > 0) {
  if (intr)
   set_current_state(0x100);
  else
   set_current_state(0);
  ...
  ((sch_ident != ((void *)0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_fence.c", 1071, "sch_ident != NULL"));
  t = schedule_timeout(t);
  if (t > 0 && intr && ((((__curcpu->ci_self)->ci_curproc)->p_siglist == 0 || ((((__curcpu->ci_self)->ci_curproc)->p_p->ps_flags & 0x00000200) == 0 && (((__curcpu->ci_self)->ci_curproc)->p_siglist & ~((__curcpu->ci_self)->ci_curproc)->p_sigmask) == 0)) ? 0 : issignal((__curcpu->ci_self)->ci_curproc)))
   t = -4;
 }
