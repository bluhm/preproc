...
sr_crypto_discipline_init(struct sr_discipline *sd)
{
 ...
 sd->sd_max_wu = 16;
 for (i = 0; i < 32; i++)
  sd->sd_dis_specific.mdd_crypto.scr_sid[i] = (u_int64_t)-1;
    ...
    int no_chunk, int64_t coerced_size)
{
 ...
 if (coerced_size > ((1ULL << 30) * 32)) {
  sr_error(sd->sd_sc, "%s exceeds maximum size (%lli > %llu)",
      sd->sd_name, coerced_size, ((1ULL << 30) * 32));
  goto done;
 }
 ...
 sd->sd_dis_specific.mdd_crypto.scr_meta = (struct sr_meta_crypto *)omi->omi_som;
 sd->sd_meta->_sdd_invariant.ssd_opt_no++;
 sd->sd_dis_specific.mdd_crypto.key_disk = ((void *)0);
 if (bc->bc_key_disk != (dev_t)(-1)) {
  if (sr_crypto_get_kdf(bc, sd))
   goto done;
  sd->sd_dis_specific.mdd_crypto.key_disk =
      sr_crypto_create_key_disk(sd, bc->bc_key_disk);
  if (sd->sd_dis_specific.mdd_crypto.key_disk == ((void *)0))
   goto done;
  sd->sd_capabilities |= 0x00000002;
 } else if (bc->bc_opaque_flags & 0x02) {
  bc->bc_opaque_status = 0x00;
  rv = 35;
  goto done;
 } else if (sr_crypto_get_kdf(bc, sd))
  goto done;
 if (!(bc->bc_flags & 0x04) && bc->bc_key_disk == (dev_t)(-1))
  goto done;
    ...
    int no_chunk, void *data)
{
 int rv = 22;
 sd->sd_dis_specific.mdd_crypto.key_disk = ((void *)0);
 if (sd->sd_dis_specific.mdd_crypto.scr_meta == ((void *)0))
  goto done;
 if (data != ((void *)0)) {
  __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_maskkey), (data), (sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey)));
 } else if (bc->bc_key_disk != (dev_t)(-1)) {
  sd->sd_dis_specific.mdd_crypto.key_disk =
      sr_crypto_read_key_disk(sd, bc->bc_key_disk);
  if (sd->sd_dis_specific.mdd_crypto.key_disk == ((void *)0))
   goto done;
 } else if (bc->bc_opaque_flags & 0x02) {
  if (bc->bc_opaque == ((void *)0))
   goto done;
  if (sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint) <
      bc->bc_opaque_size)
   goto done;
  if (copyout(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint,
      bc->bc_opaque, bc->bc_opaque_size))
   goto done;
  bc->bc_opaque_status = 0x01;
  rv = 35;
  goto done;
 } else if (bc->bc_opaque_flags & 0x01) {
  if (sr_crypto_get_kdf(bc, sd))
   goto done;
 } else
  goto done;
...
sr_crypto_prepare(struct sr_workunit *wu, int encrypt)
{
 ...
 if (xs->flags & 0x01000) {
  ...
  __builtin_memcpy((crwu->cr_uio.uio_iov->iov_base), (xs->data), (xs->datalen));
 } else
  crwu->cr_uio.uio_iov->iov_base = xs->data;
 blkno = wu->swu_blk_start;
 n = xs->datalen >> 9;
 ((crwu->cr_crp->crp_ndescalloc >= n) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/softraid_crypto.c", 281, "crwu->cr_crp->crp_ndescalloc >= n"));
 crwu->cr_crp->crp_ndesc = n;
 flags = (encrypt ? 0x01 : 0) |
     0x02 | 0x04;
 keyndx = blkno >> 30;
 crwu->cr_crp->crp_sid = sd->sd_dis_specific.mdd_crypto.scr_sid[keyndx];
 crwu->cr_crp->crp_opaque = crwu;
 crwu->cr_crp->crp_ilen = xs->datalen;
 crwu->cr_crp->crp_alloctype = 2;
 crwu->cr_crp->crp_flags = 0x0002 | 0x0008;
 ...
 for (i = 0; i < crwu->cr_crp->crp_ndesc; i++, blkno++) {
  ...
  crd->crd_inject = 0;
  crd->crd_flags = flags;
  crd->CRD_INI.cri_alg = sd->sd_dis_specific.mdd_crypto.scr_alg;
  crd->CRD_INI.cri_klen = sd->sd_dis_specific.mdd_crypto.scr_klen;
  crd->CRD_INI.cri_key = sd->sd_dis_specific.mdd_crypto.scr_key[0];
  __builtin_memcpy((crd->CRD_INI.u.iv), (&blkno), (sizeof(blkno)));
 }
 return (crwu);
}
int
sr_crypto_get_kdf(struct bioc_createraid *bc, struct sr_discipline *sd)
{
 ...
 if (!(bc->bc_opaque_flags & 0x01))
  return (rv);
 if (bc->bc_opaque == ((void *)0))
  return (rv);
 if (bc->bc_opaque_size != sizeof(*kdfinfo))
  return (rv);
 kdfinfo = malloc(bc->bc_opaque_size, 2, 0x0001 | 0x0008);
 if (copyin(bc->bc_opaque, kdfinfo, bc->bc_opaque_size))
  goto out;
 if (kdfinfo->len != bc->bc_opaque_size)
  goto out;
 if (kdfinfo->flags & (1<<1)) {
  if (sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint) <
      kdfinfo->_kdfhint.generic.len)
   goto out;
  __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint), (&kdfinfo->_kdfhint.generic), (kdfinfo->_kdfhint.generic.len));
 }
 if ((kdfinfo->flags & (1<<0))) {
  if (sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey) <
      sizeof(kdfinfo->maskkey))
   goto out;
  __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_maskkey), (&kdfinfo->maskkey), (sizeof(kdfinfo->maskkey)));
 }
 bc->bc_opaque_status = 0x01;
 rv = 0;
out:
 explicit_bzero(kdfinfo, bc->bc_opaque_size);
 free(kdfinfo, 2, bc->bc_opaque_size);
 return (rv);
}
int
    ...
    u_int8_t *key, int key_size, u_char *check_digest)
{
 ...
 SHA1_CTX shactx;
 __builtin_bzero((check_key), (sizeof(check_key)));
 __builtin_bzero((&hmacctx), (sizeof(hmacctx)));
 __builtin_bzero((&shactx), (sizeof(shactx)));
 SHA1Init(&shactx);
...
{
 u_char check_digest[20];
 int rv = 1;
 ;
 if (sd->sd_dis_specific.mdd_crypto.scr_meta->scm_check_alg != 1)
  goto out;
 if (sr_crypto_decrypt((u_char *)sd->sd_dis_specific.mdd_crypto.scr_meta->scm_key,
     (u_char *)sd->sd_dis_specific.mdd_crypto.scr_key,
     sd->sd_dis_specific.mdd_crypto.scr_maskkey, sizeof(sd->sd_dis_specific.mdd_crypto.scr_key),
     sd->sd_dis_specific.mdd_crypto.scr_meta->scm_mask_alg) == -1)
  goto out;
 sr_crypto_calculate_check_hmac_sha1(sd->sd_dis_specific.mdd_crypto.scr_maskkey,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey),
     (u_int8_t *)sd->sd_dis_specific.mdd_crypto.scr_key,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_key),
     check_digest);
 if (__builtin_memcmp((sd->sd_dis_specific.mdd_crypto.scr_meta->_scm_chk.chk_hmac_sha1.sch_mac), (check_digest), (sizeof(check_digest))) != 0) {
  explicit_bzero(sd->sd_dis_specific.mdd_crypto.scr_key,
      sizeof(sd->sd_dis_specific.mdd_crypto.scr_key));
  goto out;
 }
 rv = 0;
out:
 explicit_bzero(&sd->sd_dis_specific.mdd_crypto.scr_maskkey,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey));
...
{
 ;
 if (((256)/8) < sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey))
  return (1);
 sd->sd_dis_specific.mdd_crypto.scr_meta->scm_alg = 2;
 arc4random_buf(sd->sd_dis_specific.mdd_crypto.scr_key,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_key));
 sd->sd_dis_specific.mdd_crypto.scr_meta->scm_mask_alg = 1;
 sr_crypto_encrypt((u_char *)sd->sd_dis_specific.mdd_crypto.scr_key,
     (u_char *)sd->sd_dis_specific.mdd_crypto.scr_meta->scm_key,
     sd->sd_dis_specific.mdd_crypto.scr_maskkey, sizeof(sd->sd_dis_specific.mdd_crypto.scr_key),
     sd->sd_dis_specific.mdd_crypto.scr_meta->scm_mask_alg);
 sd->sd_dis_specific.mdd_crypto.scr_meta->scm_check_alg = 1;
 sr_crypto_calculate_check_hmac_sha1(sd->sd_dis_specific.mdd_crypto.scr_maskkey,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey),
     (u_int8_t *)sd->sd_dis_specific.mdd_crypto.scr_key,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_key),
     sd->sd_dis_specific.mdd_crypto.scr_meta->_scm_chk.chk_hmac_sha1.sch_mac);
 explicit_bzero(sd->sd_dis_specific.mdd_crypto.scr_key,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_key));
 sd->sd_dis_specific.mdd_crypto.scr_meta->scm_flags = (1<<0) |
     (1<<1);
 return (0);
}
int
  ...
  struct sr_crypto_kdfinfo *kdfinfo1, struct sr_crypto_kdfinfo *kdfinfo2)
{
 ...
 if (sd->sd_dis_specific.mdd_crypto.scr_meta->scm_check_alg != 1)
  goto out;
 c = (u_char *)sd->sd_dis_specific.mdd_crypto.scr_meta->scm_key;
 ksz = sizeof(sd->sd_dis_specific.mdd_crypto.scr_key);
 p = malloc(ksz, 2, 0x0001 | 0x0004 | 0x0008);
 if (p == ((void *)0))
  goto out;
 ...
 if (kdfinfo2->flags & (1<<1)) {
  if (kdfinfo2->_kdfhint.generic.len >
      sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint))
   goto out;
  explicit_bzero(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint,
      sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint));
  __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint), (&kdfinfo2->_kdfhint.generic), (kdfinfo2->_kdfhint.generic.len));
 }
 c = (u_char *)sd->sd_dis_specific.mdd_crypto.scr_meta->scm_key;
 ...
 sr_crypto_calculate_check_hmac_sha1(kdfinfo2->maskkey,
     sizeof(kdfinfo2->maskkey), (u_int8_t *)sd->sd_dis_specific.mdd_crypto.scr_key,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_key), check_digest);
 __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_meta->_scm_chk.chk_hmac_sha1.sch_mac), (check_digest), (sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->_scm_chk.chk_hmac_sha1.sch_mac)));
 rv = 0;
out:
 if (p) {
...
sr_crypto_create_key_disk(struct sr_discipline *sd, dev_t dev)
{
 ...
 struct sr_discipline *fakesd = ((void *)0);
 struct sr_metadata *sm = ((void *)0);
 struct sr_meta_chunk *km;
 struct sr_meta_opt_item *omi = ((void *)0);
 struct sr_meta_keydisk *skm;
 struct sr_chunk *key_disk = ((void *)0);
 ...
 if (VOP_IOCTL(vn, ((unsigned long)0x40000000 | ((sizeof(struct disklabel) & 0x1fff) << 16) | ((('d')) << 8) | ((101))), (caddr_t)&label,
     0x0001, ((struct ucred *)-1), (__curcpu->ci_self)->ci_curproc)) {
  ;
  goto done;
 }
 ...
 key_disk = malloc(sizeof(struct sr_chunk), 2, 0x0001 | 0x0008);
 km = &key_disk->src_meta;
 key_disk->src_dev_mm = dev;
 key_disk->src_vn = vn;
 strlcpy(key_disk->src_devname, devname, sizeof(km->_scm_invariant.scm_devname));
 key_disk->src_size = 0;
 km->_scm_invariant.scm_volid = sd->sd_meta->_sdd_invariant.ssd_level;
 km->_scm_invariant.scm_chunk_id = 0;
 km->_scm_invariant.scm_size = 0;
 km->_scm_invariant.scm_coerced_size = 0;
 strlcpy(km->_scm_invariant.scm_devname, devname, sizeof(km->_scm_invariant.scm_devname));
 __builtin_memcpy((&km->_scm_invariant.scm_uuid), (&sd->sd_meta->_sdd_invariant.ssd_uuid), (sizeof(struct sr_uuid)));
 ...
 __builtin_memcpy((&sm->_sdd_invariant.ssd_uuid), (&sd->sd_meta->_sdd_invariant.ssd_uuid), (sizeof(struct sr_uuid)));
 sm->_sdd_invariant.ssd_chunk_no = 1;
 sm->_sdd_invariant.ssd_volid = 0xfffffffe;
 sm->_sdd_invariant.ssd_level = 0xfffffffe;
 sm->_sdd_invariant.ssd_size = 0;
 ...
 snprintf(sm->_sdd_invariant.ssd_revision, sizeof(sm->_sdd_invariant.ssd_revision),
     "%03d", 6);
 fakesd = malloc(sizeof(struct sr_discipline), 2,
     0x0001 | 0x0008);
 fakesd->sd_sc = sd->sd_sc;
 fakesd->sd_meta = sm;
 fakesd->sd_meta_type = 0;
 fakesd->sd_vol_status = 0x00;
 strlcpy(fakesd->sd_name, "KEYDISK", sizeof(fakesd->sd_name));
 { ((&fakesd->sd_meta_opt)->slh_first) = ((void *)0); };
 fakesd->sd_vol.sv_chunks = malloc(sizeof(struct sr_chunk *), 2,
     0x0001 | 0x0008);
 fakesd->sd_vol.sv_chunks[0] = key_disk;
 { ((&fakesd->sd_vol.sv_chunk_list)->slh_first) = ((void *)0); };
 do { (key_disk)->src_link.sle_next = (&fakesd->sd_vol.sv_chunk_list)->slh_first; (&fakesd->sd_vol.sv_chunk_list)->slh_first = (key_disk); } while (0);
 arc4random_buf(sd->sd_dis_specific.mdd_crypto.scr_maskkey,
     sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey));
 omi = malloc(sizeof(struct sr_meta_opt_item), 2,
     0x0001 | 0x0008);
 omi->omi_som = malloc(sizeof(struct sr_meta_keydisk), 2,
     0x0001 | 0x0008);
 omi->omi_som->som_type = 0x03;
 omi->omi_som->som_length = sizeof(struct sr_meta_keydisk);
 skm = (struct sr_meta_keydisk *)omi->omi_som;
 __builtin_memcpy((&skm->skm_maskkey), (sd->sd_dis_specific.mdd_crypto.scr_maskkey), (sizeof(skm->skm_maskkey)));
 do { (omi)->omi_link.sle_next = (&fakesd->sd_meta_opt)->slh_first; (&fakesd->sd_meta_opt)->slh_first = (omi); } while (0);
 fakesd->sd_meta->_sdd_invariant.ssd_opt_no++;
 if (sr_meta_save(fakesd, 0x1)) {
  sr_error(sc, "could not save metadata to %s", devname);
  goto fail;
 }
 goto done;
fail:
 free(key_disk, 2, sizeof(struct sr_chunk));
 key_disk = ((void *)0);
done:
 free(omi, 2, sizeof(struct sr_meta_opt_item));
 if (fakesd && fakesd->sd_vol.sv_chunks)
  free(fakesd->sd_vol.sv_chunks, 2,
      sizeof(struct sr_chunk *));
 free(fakesd, 2, sizeof(struct sr_discipline));
 free(sm, 2, sizeof(struct sr_metadata));
 if (open) {
...
sr_crypto_read_key_disk(struct sr_discipline *sd, dev_t dev)
{
 ...
 struct sr_chunk *key_disk = ((void *)0);
 struct disklabel label;
 struct vnode *vn = ((void *)0);
 char devname[32];
 int c, part, open = 0;
 { ((&som)->slh_first) = ((void *)0); };
 sr_meta_getdevname(sc, dev, devname, sizeof(devname));
 c = sr_chunk_in_use(sc, dev);
 if (c != 0xff && c != 0x01) {
  sr_error(sc, "%s is already in use", devname);
 ...
 if (VOP_OPEN(vn, 0x0001, ((struct ucred *)-1), (__curcpu->ci_self)->ci_curproc)) {
  ;
  vput(vn);
  goto done;
 }
 ...
 if (sr_meta_native_read(sd, dev, sm, ((void *)0))) {
  sr_error(sc, "native bootprobe could not read native metadata");
  goto done;
 }
 if (sr_meta_validate(sd, dev, sm, ((void *)0))) {
  ;
  goto done;
 }
 if (sm->_sdd_invariant.ssd_level != 0xfffffffe) {
 ...
 key_disk->src_vn = vn;
 key_disk->src_size = 0;
 __builtin_memcpy((&key_disk->src_meta), ((struct sr_meta_chunk *)(sm + 1)), (sizeof(key_disk->src_meta)));
 sr_meta_opt_load(sc, sm, &som);
 for((omi) = ((&som)->slh_first); (omi) != ((void *)0); (omi) = ((omi)->omi_link.sle_next)) {
  omh = omi->omi_som;
  if (omh->som_type == 0x03) {
   skm = (struct sr_meta_keydisk *)omh;
   __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_maskkey), (&skm->skm_maskkey), (sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey)));
  } else if (omh->som_type == 0x01) {
   __builtin_memcpy((sd->sd_dis_specific.mdd_crypto.scr_maskkey), (omh + sizeof(struct sr_meta_opt_hdr)), (sizeof(sd->sd_dis_specific.mdd_crypto.scr_maskkey)));
  }
 }
 open = 0;
done:
...
sr_crypto_free_sessions(struct sr_discipline *sd)
{
 u_int i;
 for (i = 0; i < 32; i++) {
  if (sd->sd_dis_specific.mdd_crypto.scr_sid[i] != (u_int64_t)-1) {
   crypto_freesession(sd->sd_dis_specific.mdd_crypto.scr_sid[i]);
   sd->sd_dis_specific.mdd_crypto.scr_sid[i] = (u_int64_t)-1;
  }
 }
}
...
sr_crypto_alloc_resources(struct sr_discipline *sd)
{
 ...
 u_int num_keys, i;
 ;
 sd->sd_dis_specific.mdd_crypto.scr_alg = 15;
 switch (sd->sd_dis_specific.mdd_crypto.scr_meta->scm_alg) {
 case 1:
  sd->sd_dis_specific.mdd_crypto.scr_klen = 256;
  break;
 case 2:
  sd->sd_dis_specific.mdd_crypto.scr_klen = 512;
  break;
 default:
 ...
 for((wu) = ((&sd->sd_wu)->tqh_first); (wu) != ((void *)0); (wu) = ((wu)->swu_next.tqe_next)) {
  crwu = (struct sr_crypto_wu *)wu;
  crwu->cr_uio.uio_iov = &crwu->cr_iov;
  crwu->cr_dmabuf = dma_alloc((64 * 1024), 0x0001);
  crwu->cr_crp = crypto_getreq((64 * 1024) >> 9);
  if (crwu->cr_crp == ((void *)0))
   return (12);
 }
 __builtin_memset((&cri), (0), (sizeof(cri)));
 cri.cri_alg = sd->sd_dis_specific.mdd_crypto.scr_alg;
 cri.cri_klen = sd->sd_dis_specific.mdd_crypto.scr_klen;
 num_keys = ((sd->sd_meta->_sdd_invariant.ssd_size - 1) >>
     30) + 1;
 if (num_keys > 32)
  return (27);
 for (i = 0; i < num_keys; i++) {
  cri.cri_key = sd->sd_dis_specific.mdd_crypto.scr_key[i];
  if (crypto_newsession(&sd->sd_dis_specific.mdd_crypto.scr_sid[i],
      &cri, 0) != 0) {
...
sr_crypto_free_resources(struct sr_discipline *sd)
{
 ...
 ;
 if (sd->sd_dis_specific.mdd_crypto.key_disk != ((void *)0)) {
  explicit_bzero(sd->sd_dis_specific.mdd_crypto.key_disk,
      sizeof(*sd->sd_dis_specific.mdd_crypto.key_disk));
  free(sd->sd_dis_specific.mdd_crypto.key_disk, 2,
      sizeof(*sd->sd_dis_specific.mdd_crypto.key_disk));
 }
 ...
 for((wu) = ((&sd->sd_wu)->tqh_first); (wu) != ((void *)0); (wu) = ((wu)->swu_next.tqe_next)) {
  crwu = (struct sr_crypto_wu *)wu;
  if (crwu->cr_dmabuf)
   dma_free(crwu->cr_dmabuf, (64 * 1024));
  if (crwu->cr_crp)
...
sr_crypto_ioctl(struct sr_discipline *sd, struct bioc_discipline *bd)
{
 ...
 switch (bd->bd_cmd) {
 case 0x01:
  size = sizeof(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint);
  if (bd->bd_data == ((void *)0) || bd->bd_size > size)
   goto bad;
  if (copyout(sd->sd_dis_specific.mdd_crypto.scr_meta->scm_kdfhint,
 ...
 case 0x02:
  ...
  if (kdfpair.kdfinfo1 == ((void *)0) || kdfpair.kdfsize1 > size)
   goto bad;
  if (copyin(kdfpair.kdfinfo1, &kdfinfo1, size))
   goto bad;
  size = sizeof(kdfinfo2);
  if (kdfpair.kdfinfo2 == ((void *)0) || kdfpair.kdfsize2 > size)
   goto bad;
...
sr_crypto_meta_opt_handler(struct sr_discipline *sd, struct sr_meta_opt_hdr *om)
{
 int rv = 22;
 if (om->som_type == 0x01) {
  sd->sd_dis_specific.mdd_crypto.scr_meta = (struct sr_meta_crypto *)om;
  rv = 0;
 }
 return (rv);
}
...
sr_crypto_done(struct sr_workunit *wu)
{
 ...
 struct sr_crypto_wu *crwu;
 int s;
 if (((xs->flags) & (0x00800)) && xs->error == 0) {
