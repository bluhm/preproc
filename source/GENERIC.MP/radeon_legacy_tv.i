# 1 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c"
...
# 4 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c" 2
...
# 5 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c" 2
...
# 6 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c" 2
# 70 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c"
...
static const uint16_t hor_timing_NTSC[32] = {
 0x0007,
 0x003f,
 0x0263,
 0x0a24,
...
static const uint16_t vert_timing_NTSC[32] = {
 0x2001,
 0x200d,
 0x1006,
 0x0c06,
...
static const uint16_t hor_timing_PAL[32] = {
 0x0007,
 0x0058,
 0x027c,
 0x0a31,
...
static const uint16_t vert_timing_PAL[32] = {
 0x2001,
 0x200c,
 0x1005,
 0x0c05,
...
# 171 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c"
             ...
             uint16_t *pll_ref_freq)
{
 ...
 radeon_crtc = ({ __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (radeon_encoder->base.crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
     ...
     unsigned n_wait_loops, unsigned cnt_threshold)
{
 ...
 r100_mm_wreg(rdev, (0x0124), ((r100_mm_rreg(rdev, (0x0124), 0) & 0xffff60ff) | 0x100), 0);
 save_pll_test = rdev->pll_rreg(rdev, (0x0013));
 rdev->pll_wreg(rdev, (0x0013), (save_pll_test & ~(1 << 9)));

 bus_space_write_1(rdev->memt, rdev->rmmio, (0x0008), (0x0013));
 for (i = 0; i < n_tests; i++) {
  bus_space_write_1(rdev->memt, rdev->rmmio, (0x000c + 3), (0));
  for (j = 0; j < n_wait_loops; j++)
   if (bus_space_read_1(rdev->memt, rdev->rmmio, (0x000c + 3)) >= cnt_threshold)
    break;
 }
 rdev->pll_wreg(rdev, (0x0013), (save_pll_test));
 r100_mm_wreg(rdev, (0x0124), (r100_mm_rreg(rdev, (0x0124), 0) & 0xffffe0ff), 0);
}
     ...
     uint16_t addr, uint32_t value)
{
 ...
 r100_mm_wreg(rdev, (0x0844), (value), 0);

 r100_mm_wreg(rdev, (0x0848), (addr), 0);
 r100_mm_wreg(rdev, (0x0848), (addr | (1 << 14)), 0);

 do {
  tmp = r100_mm_rreg(rdev, (0x0848), 0);
  if ((tmp & (1 << 15)) == 0)
   break;
  i++;
 } while (i < 10000);
 r100_mm_wreg(rdev, (0x0848), (0), 0);
}
# 344 "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_legacy_tv.c"
static uint16_t radeon_get_htiming_tables_addr(uint32_t tv_uv_adr)
{
 uint16_t h_table;

 switch ((tv_uv_adr & 0x06000000) >> 25) {
 case 0:
  h_table = 0x1ff;
  break;
 case 1:
  h_table = ((tv_uv_adr & 0x0000ff00) >> 8) * 2;
  break;
 case 2:
  h_table = ((tv_uv_adr & 0x00ff0000) >> 16) * 2;
  break;
 default:
  h_table = 0;
  break;
...
static uint16_t radeon_get_vtiming_tables_addr(uint32_t tv_uv_adr)
{
 ...
 switch ((tv_uv_adr & 0x18000000) >> 27) {
 case 0:
  v_table = ((tv_uv_adr & 0x000000ff) >> 0) * 2 + 1;
  break;
 case 1:
  v_table = ((tv_uv_adr & 0x0000ff00) >> 8) * 2 + 1;
  break;
 case 2:
  v_table = ((tv_uv_adr & 0x00ff0000) >> 16) * 2 + 1;
  break;
 default:
  v_table = 0;
  break;
...
static void radeon_legacy_write_tv_restarts(struct radeon_encoder *radeon_encoder)
{
 ...
 struct radeon_device *rdev = dev->dev_private;
 struct radeon_encoder_tv_dac *tv_dac = radeon_encoder->enc_priv;
 r100_mm_wreg(rdev, (0x0834), (tv_dac->tv.frestart), 0);
 r100_mm_wreg(rdev, (0x0838), (tv_dac->tv.hrestart), 0);
 r100_mm_wreg(rdev, (0x083c), (tv_dac->tv.vrestart), 0);
}

static _Bool radeon_legacy_tv_init_restarts(struct drm_encoder *encoder)
{
 struct drm_device *dev = encoder->dev;
 struct radeon_device *rdev = dev->dev_private;
 ...
 h_offset = tv_dac->h_pos * 10;

 if (tv_dac->tv_std == TV_STD_NTSC ||
     tv_dac->tv_std == TV_STD_NTSC_J ||
     tv_dac->tv_std == TV_STD_PAL_M) {
  h_offset -= 50;
  p1 = hor_timing_NTSC[6];
  p2 = hor_timing_NTSC[8];
 } else {
  p1 = hor_timing_PAL[6];
  p2 = hor_timing_PAL[8];
 }
 ...
 h_changed = (p1 != tv_dac->tv.h_code_timing[6] ||
       p2 != tv_dac->tv.h_code_timing[8]);

 tv_dac->tv.h_code_timing[6] = p1;
 tv_dac->tv.h_code_timing[8] = p2;
 ...
 if (tv_dac->tv_std == TV_STD_NTSC ||
     ...
     tv_dac->tv_std == TV_STD_PAL_M ||
     tv_dac->tv_std == TV_STD_PAL_60)
  v_offset = ((int)(v_total * h_total) * 2 * tv_dac->v_pos) / (int)(525);
 else
  v_offset = ((int)(v_total * h_total) * 2 * tv_dac->v_pos) / (int)(625);
 ...
 if (tv_dac->tv_std == TV_STD_NTSC ||
     tv_dac->tv_std == TV_STD_NTSC_J ||
     tv_dac->tv_std == TV_STD_PAL_M)
  h_inc = (u16)((int)(const_ptr->hor_resolution * 4096 * 233) /
         (tv_dac->h_size * (int)(9478) + (int)(479166)));
 else
  h_inc = (u16)((int)(const_ptr->hor_resolution * 4096 * 188) /
         (tv_dac->h_size * (int)(9360) + (int)(473200)));

 tv_dac->tv.timing_cntl = (tv_dac->tv.timing_cntl & ~0xfff) |
  ((u32)h_inc << 0);
          ...
          struct drm_display_mode *adjusted_mode)
{
 ...
 radeon_crtc = ({ __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (encoder->crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
 ...
 if (tv_dac->tv_std == TV_STD_NTSC ||
     tv_dac->tv_std == TV_STD_NTSC_J) {
  tv_modulator_cntl1 |= (0x46 << 16) |
   (0x3b << 8);
  tv_modulator_cntl2 = (-111 & 0x1ff) |
   ((0 & 0x1ff) << 16);
 } else if (tv_dac->tv_std == TV_STD_SCART_PAL) {
  tv_modulator_cntl1 |= (1 << 6);
  tv_modulator_cntl2 = (0 & 0x1ff) |
   ((0 & 0x1ff) << 16);
 } else {
  tv_modulator_cntl1 |= (1 << 6) |
   (0x3b << 16) |
   (0x3b << 8);
  tv_modulator_cntl2 = (-78 & 0x1ff) |
   ((62 & 0x1ff) << 16);
 }
 ...
 if (tv_dac->tv_std == TV_STD_NTSC ||
     ...
     tv_dac->tv_std == TV_STD_PAL_M ||
     tv_dac->tv_std == TV_STD_PAL_60)
  vert_space = const_ptr->ver_total * 2 * 10000 / 525;
 else
  vert_space = const_ptr->ver_total * 2 * 10000 / 625;

 tmp = r100_mm_rreg(rdev, (0x084c), 0);
 tmp &= 0xe3ff0000;
 tmp |= (vert_space * (1 << 0xe) / 10000);
 tv_vscaler_cntl1 = tmp;

 if (pll_ref_freq == 2700)
  tv_vscaler_cntl1 |= (1 << 29);

 if (const_ptr->hor_resolution == 1024)
  tv_vscaler_cntl1 |= (4 << 26);
 else
  tv_vscaler_cntl1 |= (2 << 26);
 ...
 if (flicker_removal < 3)
  flicker_removal = 3;
 for (i = 0; i < (sizeof((SLOPE_limit)) / sizeof((SLOPE_limit)[0])); ++i) {
  if (flicker_removal == SLOPE_limit[i])
   break;
 }

 tv_y_saw_tooth_cntl = (vert_space * SLOPE_value[i] * (1 << (0xe - 1)) +
    5001) / 10000 / 8 | ((SLOPE_value[i] *
    (1 << (0xe - 1)) / 8) << 16);
 tv_y_fall_cntl =
  (YCOEF_EN_value[i] << 17) | ((YCOEF_value[i] * (1 << 8) / 8) << 24) |
  (1 << 16) | (272 * SLOPE_value[i] / 8) * (1 << (0xe - 1)) /
  1024;
 tv_y_rise_cntl = (1 << 16)|
  (flicker_removal * 1024 - 272) * SLOPE_value[i] / 8 * (1 << (0xe - 1)) / 1024;

 tv_vscaler_cntl2 = r100_mm_rreg(rdev, (0x0854), 0) & 0x00fffff0;
 ...
 tmp = (tv_vscaler_cntl1 >> 0) & 0xffff;
 tmp = ((16384 * 256 * 10) / tmp + 5) / 10;
 tmp = (tmp << 24) | 0x000b0000;
 ...
 r100_mm_wreg(rdev, (0x0800), ((tv_master_cntl | (1 << 0) | (1 << 1) | (1 << 4))), 0);
 ...
 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(1 << 30)); tmp_ |= ((0) & ~(~(1 << 30))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);
 rdev->pll_wreg(rdev, (0x0021), (tv_pll_cntl));
 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(1 << 1)); tmp_ |= (((1 << 1)) & ~(~(1 << 1))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);

 radeon_wait_pll_lock(encoder, 200, 800, 135);

 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(1 << 1)); tmp_ |= ((0) & ~(~(1 << 1))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);

 radeon_wait_pll_lock(encoder, 300, 160, 27);
 radeon_wait_pll_lock(encoder, 200, 800, 135);

 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~0xf); tmp_ |= ((0) & ~(~0xf)); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);
 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(1 << 30)); tmp_ |= (((1 << 30)) & ~(~(1 << 30))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);

 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(3 << 14)); tmp_ |= (((1 << 14)) & ~(~(3 << 14))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);
 do { uint32_t tmp_ = rdev->pll_rreg(rdev, (0x0022)); tmp_ &= (~(1 << 3)); tmp_ |= ((0) & ~(~(1 << 3))); rdev->pll_wreg(rdev, (0x0022), (tmp_)); } while (0);


 r100_mm_wreg(rdev, (0x0804), (tv_rgb_cntl), 0);
 r100_mm_wreg(rdev, (0x080c), (const_ptr->hor_total - 1), 0);
 r100_mm_wreg(rdev, (0x0810), (const_ptr->hor_resolution - 1), 0);
 r100_mm_wreg(rdev, (0x0818), (const_ptr->hor_start), 0);

 r100_mm_wreg(rdev, (0x0820), (const_ptr->ver_total - 1), 0);
 r100_mm_wreg(rdev, (0x0824), (const_ptr->ver_resolution - 1), 0);
 r100_mm_wreg(rdev, (0x082c), (tv_ftotal), 0);
 r100_mm_wreg(rdev, (0x084c), (tv_vscaler_cntl1), 0);
 r100_mm_wreg(rdev, (0x0854), (tv_vscaler_cntl2), 0);

 r100_mm_wreg(rdev, (0x0858), (tv_y_fall_cntl), 0);
 r100_mm_wreg(rdev, (0x085c), (tv_y_rise_cntl), 0);
 r100_mm_wreg(rdev, (0x0860), (tv_y_saw_tooth_cntl), 0);

 r100_mm_wreg(rdev, (0x0800), ((tv_master_cntl | (1 << 0) | (1 << 1))), 0);
 ...
 r100_mm_wreg(rdev, (0x0800), ((tv_master_cntl | (1 << 0))), 0);


 r100_mm_wreg(rdev, (0x0808), (((1 << 3) | (1 << 5))), 0);
 r100_mm_wreg(rdev, (0x0850), (tv_dac->tv.timing_cntl), 0);
 r100_mm_wreg(rdev, (0x0870), (tv_modulator_cntl1), 0);
 r100_mm_wreg(rdev, (0x0874), (tv_modulator_cntl2), 0);
 r100_mm_wreg(rdev, (0x0888), (((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3))), 0);




 r100_mm_wreg(rdev, (0x0890), (0), 0);

 r100_mm_wreg(rdev, (0x0800), (tv_master_cntl), 0);

 r100_mm_wreg(rdev, (0x0868), (((0x17f << 16) | (0x5ff << 0))), 0);

 r100_mm_wreg(rdev, (0x086c), (((0x100 << 16) | (0x100 << 0))), 0);
 ...
 *h_total_disp = (((const_ptr->hor_resolution / 8) - 1) << 16) |
  (((const_ptr->hor_total / 8) - 1) << 0);

 tmp = *h_sync_strt_wid;
 tmp &= ~((0x07 << 0) | (0x3ff << 3));
 tmp |= (((const_ptr->hor_syncstart / 8) - 1) << 3) |
  (const_ptr->hor_syncstart & 7);
 *h_sync_strt_wid = tmp;

 *v_total_disp = ((const_ptr->ver_resolution - 1) << 16) |
  ((const_ptr->ver_total - 1) << 0);

 tmp = *v_sync_strt_wid;
 tmp &= ~(0x7ff << 0);
 tmp |= ((const_ptr->ver_syncstart - 1) << 0);
 *v_sync_strt_wid = tmp;
}
