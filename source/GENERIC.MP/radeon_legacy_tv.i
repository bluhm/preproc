             ...
             uint16_t *pll_ref_freq)
{
 ...
 radeon_crtc = ({ const __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (radeon_encoder->base.crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
     ...
     unsigned n_wait_loops, unsigned cnt_threshold)
{
 ...
 bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x0008), (0x0013));
 for (i = 0; i < n_tests; i++) {
  bus_space_write_1(rdev->memt, rdev->rmmio_bsh, (0x000c + 3), (0));
  for (j = 0; j < n_wait_loops; j++)
   if (bus_space_read_1(rdev->memt, rdev->rmmio_bsh, (0x000c + 3)) >= cnt_threshold)
    break;
 }
          ...
          struct drm_display_mode *adjusted_mode)
{
 ...
 if (!const_ptr)
  return;
 radeon_crtc = ({ const __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (encoder->crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
 tv_master_cntl = ((1 << 5) |
     (1 << 9) |
     (1 << 10) |
     (1 << 31));
