...
uvm_pmr_addr_cmp(const struct vm_page *lhs, const struct vm_page *rhs)
{
 ...
 lhs_addr = ((lhs)->phys_addr);
 rhs_addr = ((rhs)->phys_addr);
...
uvm_pmr_nfindsz(struct uvm_pmemrange *pmr, psize_t sz, int mti)
{
 ...
 ((sz >= 1) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 243, "sz >= 1"));
 if (sz == 1 && !(((&pmr->single[mti])->tqh_first) == ((void *)0)))
  return ((&pmr->single[mti])->tqh_first);
 node = uvm_pmr_size_RBT_ROOT(&pmr->size[mti]);
 best = ((void *)0);
 while (node != ((void *)0)) {
  if ((node - 1)->fpgsz >= sz) {
   best = (node - 1);
   node = uvm_objtree_RBT_LEFT(node);
  } else
   node = uvm_objtree_RBT_RIGHT(node);
 }
 return best;
}
struct vm_page *
...
{
 struct vm_page *npg;
 ((pmr != ((void *)0) && pg != ((void *)0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 269, "pmr != NULL && pg != NULL"));
 if (pg->fpgsz == 1) {
  if (((pg)->pageq.tqe_next) != ((void *)0))
   return ((pg)->pageq.tqe_next);
  else
   npg = uvm_pmr_size_RBT_MIN(&pmr->size[mt]);
 } else
  npg = uvm_pmr_size_RBT_NEXT(pg + 1);
 return npg == ((void *)0) ? ((void *)0) : npg - 1;
}
void
uvm_pmr_pnaddr(struct uvm_pmemrange *pmr, struct vm_page *pg,
    struct vm_page **pg_prev, struct vm_page **pg_next)
{
 ((pg_prev != ((void *)0) && pg_next != ((void *)0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 293, "pg_prev != NULL && pg_next != NULL"));
 *pg_next = uvm_pmr_addr_RBT_NFIND(&pmr->addr, pg);
 if (*pg_next == ((void *)0))
  *pg_prev = uvm_pmr_addr_RBT_MAX(&pmr->addr);
 else
  *pg_prev = uvm_pmr_addr_RBT_PREV(*pg_next);
 ((void)0);
 ((void)0);
 if (*pg_prev != ((void *)0) &&
     (((((*pg_prev)->phys_addr)) >> 13) + (*pg_prev)->fpgsz
     != ((((pg)->phys_addr)) >> 13) ||
     *pg_prev + (*pg_prev)->fpgsz != pg ||
     uvm_pmr_pg_to_memtype(*pg_prev) != uvm_pmr_pg_to_memtype(pg)))
  *pg_prev = ((void *)0);
 if (*pg_next != ((void *)0) &&
     (((((pg)->phys_addr)) >> 13) + pg->fpgsz
     != ((((*pg_next)->phys_addr)) >> 13) ||
     pg + pg->fpgsz != *pg_next ||
...
uvm_pmr_remove_addr(struct uvm_pmemrange *pmr, struct vm_page *pg)
{
 ((void)0);
 ((void)0);
 uvm_pmr_addr_RBT_REMOVE(&pmr->addr, pg);
 pmr->nsegs--;
}
void
uvm_pmr_remove_size(struct uvm_pmemrange *pmr, struct vm_page *pg)
{
 ...
 if (pg->fpgsz == 1) {
  do { if (((pg)->pageq.tqe_next) != ((void *)0)) (pg)->pageq.tqe_next->pageq.tqe_prev = (pg)->pageq.tqe_prev; else (&pmr->single[memtype])->tqh_last = (pg)->pageq.tqe_prev; *(pg)->pageq.tqe_prev = (pg)->pageq.tqe_next; ((pg)->pageq.tqe_prev) = ((void *)-1); ((pg)->pageq.tqe_next) = ((void *)-1); } while (0);
 } else {
  ((void)0);
  uvm_pmr_size_RBT_REMOVE(&pmr->size[memtype], pg + 1);
 }
}
void
...
uvm_pmr_insert_addr(struct uvm_pmemrange *pmr, struct vm_page *pg, int no_join)
{
 ...
 uvm_pmr_addr_RBT_INSERT(&pmr->addr, pg);
 pmr->nsegs++;
 return pg;
}
void
uvm_pmr_insert_size(struct uvm_pmemrange *pmr, struct vm_page *pg)
{
 ...
 memtype = uvm_pmr_pg_to_memtype(pg);
 if (pg->fpgsz == 1)
  do { (pg)->pageq.tqe_next = ((void *)0); (pg)->pageq.tqe_prev = (&pmr->single[memtype])->tqh_last; *(&pmr->single[memtype])->tqh_last = (pg); (&pmr->single[memtype])->tqh_last = &(pg)->pageq.tqe_next; } while (0);
 else
  uvm_pmr_size_RBT_INSERT(&pmr->size[memtype], pg + 1);
}
struct vm_page *
    ...
    struct vm_page *pg, paddr_t boundary)
{
 ...
 if (boundary != 0) {
  first_boundary =
      (((((((pg)->phys_addr)) >> 13) + 1) + ((boundary) - 1)) & ~((boundary) - 1));
 } else
  first_boundary = 0;
 prev = pg;
 next = ((prev)->pageq.tqe_next);
 while (next != ((void *)0) &&
     (boundary == 0 || ((((next)->phys_addr)) >> 13) < first_boundary) &&
     ((((prev)->phys_addr)) >> 13) + 1 == ((((next)->phys_addr)) >> 13) &&
     pmr->low <= ((((next)->phys_addr)) >> 13) &&
     pmr->high > ((((next)->phys_addr)) >> 13) &&
    ...
    struct vm_page **work, int is_desperate)
{
 ...
 ((!(((pgl)->tqh_first) == ((void *)0))) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 560, "!TAILQ_EMPTY(pgl)"));
 start = ((pgl)->tqh_first);
 pmr = uvm_pmemrange_find(((((start)->phys_addr)) >> 13));
 end = uvm_pmr_findnextsegment(pmr, start, boundary);
 if (is_desperate) {
  pmr_iter = pmr;
  for (iter = ((end)->pageq.tqe_next);
      iter != ((void *)0) && start != end;
      iter = ((iter_end)->pageq.tqe_next)) {
   if (pmr->low > ((((iter)->phys_addr)) >> 13) ||
       pmr->high <= ((((iter)->phys_addr)) >> 13)) {
    pmr_iter = uvm_pmemrange_find(((((iter)->phys_addr)) >> 13));
   }
   iter_end = uvm_pmr_findnextsegment(pmr_iter, iter,
       boundary);
   if (((iter_end)->phys_addr) - ((iter)->phys_addr) <
       ((end)->phys_addr) - ((start)->phys_addr)) {
    start = iter;
    end = iter_end;
    pmr = pmr_iter;
   }
 ...
 count = ((((end)->phys_addr) - ((start)->phys_addr)) >> 13) + 1;
 end = ((end)->pageq.tqe_next);
 for (iter = start; iter != end; iter = iter_end) {
  iter_end = ((iter)->pageq.tqe_next);
  do { if (((iter)->pageq.tqe_next) != ((void *)0)) (iter)->pageq.tqe_next->pageq.tqe_prev = (iter)->pageq.tqe_prev; else (pgl)->tqh_last = (iter)->pageq.tqe_prev; *(iter)->pageq.tqe_prev = (iter)->pageq.tqe_next; ((iter)->pageq.tqe_prev) = ((void *)-1); ((iter)->pageq.tqe_next) = ((void *)-1); } while (0);
 }
 start->fpgsz = count;
 inserted = uvm_pmr_insert(pmr, start, 0);
 if (work != ((void *)0) && *work != ((void *)0) &&
     ((((inserted)->phys_addr)) >> 13) <= ((((*work)->phys_addr)) >> 13) &&
     ((((inserted)->phys_addr)) >> 13) + inserted->fpgsz >
     ((((*work)->phys_addr)) >> 13))
  *work = inserted;
 return count;
}
    ...
    paddr_t start, paddr_t end, struct pglist *result)
{
 ...
 ((void)0);
 ((void)0);
 before_sz = start - ((((pg)->phys_addr)) >> 13);
 after_sz = ((((pg)->phys_addr)) >> 13) + pg->fpgsz - end;
 ((void)0);
 do {} while (0);
 uvm_pmr_remove_size(pmr, pg);
 if (before_sz == 0)
 ...
 for (pg_i = pg + before_sz; pg_i != after; pg_i++) {
  ((pg_i->pg_flags & 0x00010000) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 676, "pg_i->pg_flags & PQ_FREE"));
  pg_i->fpgsz = 0;
  do { (pg_i)->pageq.tqe_next = ((void *)0); (pg_i)->pageq.tqe_prev = (result)->tqh_last; *(result)->tqh_last = (pg_i); (result)->tqh_last = &(pg_i)->pageq.tqe_next; } while (0);
 }
 if (before_sz > 0) {
  pg->fpgsz = before_sz;
  uvm_pmr_insert_size(pmr, pg);
 ...
 do {} while (0);
 return (after_sz > 0 ? after : ((void *)0));
}
int
    ...
    paddr_t boundary, int maxseg, int flags, struct pglist *result)
{
 ...
 ((count > 0) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 738, "count > 0"));
 ((start == 0 || end == 0 || start < end) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 739, "start == 0 || end == 0 || start < end"));
 ((align >= 1) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 740, "align >= 1"));
 ((((((align)-1)&(align))==0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 741, "powerof2(align)"));
 ((maxseg > 0) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 742, "maxseg > 0"));
 ((boundary == 0 || ((((boundary)-1)&(boundary))==0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 743, "boundary == 0 || powerof2(boundary)"));
 ((boundary == 0 || maxseg * boundary >= count) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 744, "boundary == 0 || maxseg * boundary >= count"));
 (((((result)->tqh_first) == ((void *)0))) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 745, "TAILQ_EMPTY(result)"));
 if (maxseg == 1 || count == 1)
  flags &= ~0x0008;
 if (maxseg == 1 || count == 1) {
  start_try = 2;
  search[2] = count;
 } else if (maxseg >= count && (flags & 0x0008) == 0) {
  start_try = 2;
  search[2] = 1;
 } else {
  start_try = 0;
...
retry_desperate:
 ...
 for((pmr) = ((&uvm.pmr_control.use)->tqh_first); (pmr) != ((void *)0); (pmr) = ((pmr)->pmr_use.tqe_next)) {
  if (pmr->nsegs == 0)
   continue;
  if (!(((start) == 0 || (start) < (pmr->high)) && ((end) == 0 || (pmr->low) < (end))))
   continue;
...
rescan:
  for (found = uvm_pmr_nfindsz(pmr, search[try], memtype);
   ...
   fstart = ((((found)->phys_addr)) >> 13);
   if (start != 0)
    fstart = (((start)>(fstart))?(start):(fstart));
drain_found:
   if (fnsegs == maxseg) {
    fnsegs--;
    fcount -=
   ...
   fstart = (((fstart) + ((align) - 1)) & ~((align) - 1));
   fend = ((((found)->phys_addr)) >> 13) + found->fpgsz;
   if (end != 0)
    fend = (((end)<(fend))?(end):(fend));
   if (boundary != 0) {
    fend =
        (((fend)<((((fstart + 1) + ((boundary) - 1)) & ~((boundary) - 1))))?(fend):((((fstart + 1) + ((boundary) - 1)) & ~((boundary) - 1))));
   }
   if (fstart >= fend)
    continue;
  ...
  if (++try < (sizeof((search)) / sizeof((search)[0])))
   goto rescan;
  memtype += 1;
  if (memtype == 2)
   memtype = 0;
  if (memtype != memtype_init)
   goto rescan_memtype;
  if (!desperate && ((pmr)->pmr_use.tqe_next) != ((void *)0) &&
      ((pmr)->pmr_use.tqe_next)->use != pmr->use)
   break;
 }
 if (!desperate) {
  desperate = 1;
  start_try = (sizeof((search)) / sizeof((search)[0])) - 1;
  flags &= ~0x0008;
  while (!(((result)->tqh_first) == ((void *)0)))
   uvm_pmr_remove_1strange(result, 0, ((void *)0), 0);
  fnsegs = 0;
  fcount = 0;
...
fail:
 while (!(((result)->tqh_first) == ((void *)0)))
  uvm_pmr_remove_1strange(result, 0, ((void *)0), 0);
 if (flags & 0x0001) {
  if (uvm_wait_pla(((paddr_t)(start) << 13), ((paddr_t)(end) << 13) - 1, ((paddr_t)(count) << 13),
      flags & 0x0010) == 0)
   goto retry;
...
out:
 ...
 for((found) = ((result)->tqh_first); (found) != ((void *)0); (found) = ((found)->pageq.tqe_next)) {
  ...
  atomic_clearbits_int(&found->pg_flags, 0x00000100|0x00010000);
  found->uobject = ((void *)0);
  found->uanon = ((void *)0);
  found->pg_version++;
  ((void)0);
  ((void)0);
  if (diag_prev == ((void *)0) ||
      ((((diag_prev)->phys_addr)) >> 13) + 1 !=
      ((((found)->phys_addr)) >> 13) ||
      (((((diag_prev)->phys_addr)) >> 13) & ~(boundary - 1)) !=
      (((((found)->phys_addr)) >> 13) & ~(boundary - 1)))
   fnsegs++;
  fcount++;
  diag_prev = found;
 }
...
uvm_pmr_freepages(struct vm_page *pg, psize_t count)
{
 ...
 for (i = 0; i < count; i++) {
  ((((((&pg[i])->phys_addr)) >> 13) == ((((pg)->phys_addr)) >> 13) + i) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1111, "atop(VM_PAGE_TO_PHYS(&pg[i])) == atop(VM_PAGE_TO_PHYS(pg)) + i"));
  if (!((pg[i].pg_flags & 0x00010000) == 0 &&
      (((pg[i].pg_flags) & ~(0x00010000|0x00000100|0x3f000000)) == 0x0))) {
   printf("Flags: 0x%x, will panic now.\n",
       pg[i].pg_flags);
  }
  (((pg[i].pg_flags & 0x00010000) == 0 && (((pg[i].pg_flags) & ~(0x00010000|0x00000100|0x3f000000)) == 0x0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1119, "(pg[i].pg_flags & PQ_FREE) == 0 && VALID_FLAGS(pg[i].pg_flags)"));
 ...
 for (i = count; i > 0; i -= pmr_count) {
  ...
  pmr_count = (((i)<(pmr->high - ((((pg)->phys_addr)) >> 13)))?(i):(pmr->high - ((((pg)->phys_addr)) >> 13)));
  pg->fpgsz = pmr_count;
...
uvm_pmr_freepageq(struct pglist *pgl)
{
 ...
 for((pg) = ((pgl)->tqh_first); (pg) != ((void *)0); (pg) = ((pg)->pageq.tqe_next)) {
  if (!((pg->pg_flags & 0x00010000) == 0 &&
      (((pg->pg_flags) & ~(0x00010000|0x00000100|0x3f000000)) == 0x0))) {
   printf("Flags: 0x%x, will panic now.\n",
       pg->pg_flags);
  }
  (((pg->pg_flags & 0x00010000) == 0 && (((pg->pg_flags) & ~(0x00010000|0x00000100|0x3f000000)) == 0x0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1163, "(pg->pg_flags & PQ_FREE) == 0 && VALID_FLAGS(pg->pg_flags)"));
 ...
 while (!(((pgl)->tqh_first) == ((void *)0))) {
  ...
  uvmexp.free += plen;
  uvm_wakeup_pla(pstart, ((paddr_t)(plen) << 13));
 }
 wakeup(&uvmexp.free);
 if (uvmexp.zeropages < (uvmexp.free / 8))
  wakeup(&uvmexp.zeropages);
    ...
    struct uvm_pmemrange *pmr)
{
 ...
 int cmp = 1;
 for((iter) = ((useq)->tqh_first); (iter) != ((void *)0); (iter) = ((iter)->pmr_use.tqe_next)) {
 ...
 if (iter == ((void *)0))
  do { (pmr)->pmr_use.tqe_next = ((void *)0); (pmr)->pmr_use.tqe_prev = (useq)->tqh_last; *(useq)->tqh_last = (pmr); (useq)->tqh_last = &(pmr)->pmr_use.tqe_next; } while (0);
 else
  do { (pmr)->pmr_use.tqe_prev = (iter)->pmr_use.tqe_prev; (pmr)->pmr_use.tqe_next = (iter); *(iter)->pmr_use.tqe_prev = (pmr); (iter)->pmr_use.tqe_prev = &(pmr)->pmr_use.tqe_next; } while (0);
 return ((void *)0);
}
void
uvm_pmr_split(paddr_t pageno)
{
 ...
 do {} while (0);
 do {} while (0);
 ((drain->nsegs == 0) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1365, "drain->nsegs == 0"));
 for ((rebuild) = uvm_pmr_addr_RBT_MIN((&pmr->addr)); (rebuild) != ((void *)0); (rebuild) = uvm_pmr_addr_RBT_NEXT((rebuild))) {
  if (((((rebuild)->phys_addr)) >> 13) >= pageno)
   break;
 }
 if (rebuild == ((void *)0))
  prev = uvm_pmr_addr_RBT_MAX(&pmr->addr);
 else
  prev = uvm_pmr_addr_RBT_PREV(rebuild);
 ((prev == ((void *)0) || ((((prev)->phys_addr)) >> 13) < pageno) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1375, "prev == NULL || atop(VM_PAGE_TO_PHYS(prev)) < pageno"));
 if (prev != ((void *)0) &&
     ((((prev)->phys_addr)) >> 13) + prev->fpgsz > pageno) {
  psize_t before, after;
  ((((((prev)->phys_addr)) >> 13) < pageno) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1384, "atop(VM_PAGE_TO_PHYS(prev)) < pageno"));
  uvm_pmr_remove(pmr, prev);
  prev_sz = prev->fpgsz;
  before = pageno - ((((prev)->phys_addr)) >> 13);
  after = ((((prev)->phys_addr)) >> 13) + prev_sz - pageno;
  ((before > 0) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1391, "before > 0"));
  ((after > 0) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1392, "after > 0"));
  prev->fpgsz = before;
  uvm_pmr_insert(pmr, prev, 1);
  (prev + before)->fpgsz = after;
  uvm_pmr_insert(drain, prev + before, 1);
 }
 for (; rebuild != ((void *)0); rebuild = next) {
  next = uvm_pmr_addr_RBT_NEXT(rebuild);
...
uvm_pmr_allocpmr(void)
{
 ...
 ((nw != ((void *)0)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1477, "nw != NULL"));
 __builtin_memset((nw), (0), (sizeof(struct uvm_pmemrange)));
 uvm_pmr_addr_RBT_INIT(&nw->addr);
 for (i = 0; i < 2; i++) {
  uvm_pmr_size_RBT_INIT(&nw->size[i]);
  do { (&nw->single[i])->tqh_first = ((void *)0); (&nw->single[i])->tqh_last = &(&nw->single[i])->tqh_first; } while (0);
 }
 return nw;
}
void
uvm_pmr_init(void)
{
 ...
 int i;
 do { (&uvm.pmr_control.use)->tqh_first = ((void *)0); (&uvm.pmr_control.use)->tqh_last = &(&uvm.pmr_control.use)->tqh_first; } while (0);
 uvm_pmemrange_addr_RBT_INIT(&uvm.pmr_control.addr);
 do { (&uvm.pmr_control.allocs)->tqh_first = ((void *)0); (&uvm.pmr_control.allocs)->tqh_last = &(&uvm.pmr_control.allocs)->tqh_first; } while (0);
 new_pmr = uvm_pmr_allocpmr();
 new_pmr->low = 0;
 new_pmr->high = (((paddr_t)-1) >> 13) + 1;
 uvm_pmemrange_addr_RBT_INSERT(&uvm.pmr_control.addr, new_pmr);
 uvm_pmemrange_use_insert(&uvm.pmr_control.use, new_pmr);
 for (i = 0; uvm_md_constraints[i] != ((void *)0); i++) {
...
{
 struct uvm_pmemrange *pmr;
 pmr = uvm_pmemrange_addr_RBT_ROOT(&uvm.pmr_control.addr);
 while (pmr != ((void *)0)) {
  if (pmr->low > pageno)
   pmr = uvm_pmemrange_addr_RBT_LEFT(pmr);
  else if (pmr->high <= pageno)
   pmr = uvm_pmemrange_addr_RBT_RIGHT(pmr);
  else
   break;
 }
 return pmr;
...
uvm_pmr_isfree(struct vm_page *pg)
{
 ...
 if (pmr == ((void *)0))
  return 0;
 r = uvm_pmr_addr_RBT_NFIND(&pmr->addr, pg);
 if (r == ((void *)0))
  r = uvm_pmr_addr_RBT_MAX(&pmr->addr);
 else if (r != pg)
  r = uvm_pmr_addr_RBT_PREV(r);
 if (r == ((void *)0))
  return 0;
 ((void)0);
 return ((((r)->phys_addr)) >> 13) + r->fpgsz >
     ((((pg)->phys_addr)) >> 13);
}
struct vm_page*
    ...
    paddr_t start, paddr_t end, int memtype)
{
 ...
 ((void)0);
 root = init_root;
 if (start != 0 && ((((root)->phys_addr)) >> 13) + root->fpgsz <= start)
  direction = 1;
 else if (end != 0 && ((((root)->phys_addr)) >> 13) >= end)
  direction = -1;
 else
  return root;
 while (root && !(((start) == 0 || (start) < (((((root)->phys_addr)) >> 13) + root->fpgsz)) && ((end) == 0 || (((((root)->phys_addr)) >> 13)) < (end)))) {
  if (direction == 1)
   root = uvm_objtree_RBT_RIGHT(root);
  else
   root = uvm_objtree_RBT_LEFT(root);
 }
 if (root == ((void *)0) || uvm_pmr_pg_to_memtype(root) == memtype)
  return root;
 high = root;
 high_next = uvm_objtree_RBT_RIGHT(high);
 while (high_next != ((void *)0) && (((start) == 0 || (start) < (((((high_next)->phys_addr)) >> 13) + high_next->fpgsz)) && ((end) == 0 || (((((high_next)->phys_addr)) >> 13)) < (end)))) {
  high = high_next;
 ...
 low = root;
 low_next = uvm_objtree_RBT_LEFT(low);
 while (low_next != ((void *)0) && (((start) == 0 || (start) < (((((low_next)->phys_addr)) >> 13) + low_next->fpgsz)) && ((end) == 0 || (((((low_next)->phys_addr)) >> 13)) < (end)))) {
  low = low_next;
 ...
 if (low == high)
  return ((void *)0);
 for (low = uvm_pmr_addr_RBT_NEXT(low);
     low != high;
     low = uvm_pmr_addr_RBT_NEXT(low)) {
  ((void)0);
  if (uvm_pmr_pg_to_memtype(low) == memtype)
   return low;
 }
    ...
    paddr_t start, paddr_t end, int memtype_only)
{
 ...
 for((pmr) = ((&uvm.pmr_control.use)->tqh_first); (pmr) != ((void *)0); (pmr) = ((pmr)->pmr_use.tqe_next)) {
  if (fcount == count)
   break;
  if (!(start == 0 && end == 0) &&
      !(((start) == 0 || (start) < (pmr->high)) && ((end) == 0 || (pmr->low) < (end))))
   continue;
  if (pmr->nsegs == 0)
   continue;
  memtype = memtype_init;
  while (fcount != count) {
   found = ((&pmr->single[memtype])->tqh_first);
   while (found && !(((start) == 0 || (start) < (((((found)->phys_addr)) >> 13) + 1)) && ((end) == 0 || (((((found)->phys_addr)) >> 13)) < (end))))
    found = ((found)->pageq.tqe_next);
   if (found == ((void *)0)) {
    found = uvm_pmr_size_RBT_MIN(&pmr->size[memtype]);
    if (found != ((void *)0)) {
     found--;
     if (!(((start) == 0 || (start) < (((((found)->phys_addr)) >> 13) + found->fpgsz)) && ((end) == 0 || (((((found)->phys_addr)) >> 13)) < (end))))
      found = ((void *)0);
    }
   }
   if (found == ((void *)0)) {
    found = uvm_pmr_addr_RBT_ROOT(&pmr->addr);
    if (found != ((void *)0)) {
   ...
   if (found != ((void *)0)) {
    ...
    if (end != 0 && ((((found)->phys_addr)) >> 13) +
     ...
     ((start == 0 || ((((found)->phys_addr)) >> 13) + found->fpgsz > start) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../uvm/uvm_pmemrange.c", 1778, "start == 0 || atop(VM_PAGE_TO_PHYS(found)) + found->fpgsz > start"));
    ...
    while (found->fpgsz > 0 && fcount < count &&
     ...
     found->fpgsz--;
     fcount++;
     do { if (((&found[found->fpgsz])->pageq.tqe_next = (result)->tqh_first) != ((void *)0)) (result)->tqh_first->pageq.tqe_prev = &(&found[found->fpgsz])->pageq.tqe_next; else (result)->tqh_last = &(&found[found->fpgsz])->pageq.tqe_next; (result)->tqh_first = (&found[found->fpgsz]); (&found[found->fpgsz])->pageq.tqe_prev = &(result)->tqh_first; } while (0);
    }
    if (found->fpgsz > 0) {
...
uvm_pmr_print(void)
{
 ...
 for((pmr) = ((&uvm.pmr_control.use)->tqh_first); (pmr) != ((void *)0); (pmr) = ((pmr)->pmr_use.tqe_next)) {
  ...
  free = 0;
  for (mt = 0; mt < 2; mt++) {
   pg = uvm_pmr_size_RBT_MAX(&pmr->size[mt]);
   if (pg != ((void *)0))
    pg--;
   else
    pg = ((&pmr->single[mt])->tqh_first);
   size[mt] = (pg == ((void *)0) ? 0 : pg->fpgsz);
   for ((pg) = uvm_pmr_addr_RBT_MIN((&pmr->addr)); (pg) != ((void *)0); (pg) = uvm_pmr_addr_RBT_NEXT((pg)))
    free += pg->fpgsz;
  }
...
uvm_wait_pla(paddr_t low, paddr_t high, paddr_t size, int failok)
{
 ...
 for (;;) {
  ...
  pma.pm_size = size;
  pma.pm_flags = 0x01;
  do { (&pma)->pmq.tqe_next = ((void *)0); (&pma)->pmq.tqe_prev = (&uvm.pmr_control.allocs)->tqh_last; *(&uvm.pmr_control.allocs)->tqh_last = (&pma); (&uvm.pmr_control.allocs)->tqh_last = &(&pma)->pmq.tqe_next; } while (0);
  wakeup(&uvm.pagedaemon);
  while (pma.pm_flags & (0x01 | 0x02))
   msleep(&pma, &uvm.fpageqlock, 4, wmsg, 0);
  if (!(pma.pm_flags & 0x20) &&
      pma.pm_flags & 0x10) {
   if (failok)
    return 12;
...
uvm_wakeup_pla(paddr_t low, psize_t len)
{
 ...
 paddr_t high;
 high = low + len;
 for (pma = ((&uvm.pmr_control.allocs)->tqh_first); pma != ((void *)0);
     pma = pma_next) {
  pma_next = ((pma)->pmq.tqe_next);
  if (low < pma->pm_constraint.ucr_high &&
      high > pma->pm_constraint.ucr_low) {
   pma->pm_flags |= 0x20;
   if (!(pma->pm_flags & 0x02)) {
    pma->pm_flags &= ~0x01;
    do { if (((pma)->pmq.tqe_next) != ((void *)0)) (pma)->pmq.tqe_next->pmq.tqe_prev = (pma)->pmq.tqe_prev; else (&uvm.pmr_control.allocs)->tqh_last = (pma)->pmq.tqe_prev; *(pma)->pmq.tqe_prev = (pma)->pmq.tqe_next; ((pma)->pmq.tqe_prev) = ((void *)-1); ((pma)->pmq.tqe_next) = ((void *)-1); } while (0);
    wakeup(pma);
   }
  }
 }
...
uvm_pagezero_thread(void *arg)
{
 ...
 struct vm_page *pg;
 int count;
 (__curcpu->ci_self)->ci_curproc->p_p->ps_nice = 20 + 20;
 _kernel_unlock();
