...
struct leinit {
 u_int32_t init_mode;
 u_int32_t init_padr[2];
 u_int16_t init_ladrf[4];
 u_int32_t init_rdra;
 u_int32_t init_tdra;
 int32_t pad;
};
...
struct pcn_control_data {
 struct letmd pcd_txdescs[512];
 struct lermd pcd_rxdescs[128];
...
struct pcn_softc {
 ...
 struct timeout sc_tick_timeout;
 bus_dmamap_t sc_cddmamap;
 struct pcn_txsoft sc_txsoft[128];
 struct pcn_rxsoft sc_rxsoft[128];
 struct pcn_control_data *sc_control_data;
 const char * const *sc_rcvfw_desc;
 int sc_rcvfw;
 const char * const *sc_xmtsp_desc;
 int sc_xmtsp;
 const char * const *sc_xmtfw_desc;
 int sc_xmtfw;
 int sc_flags;
 int sc_swstyle;
 int sc_txfree;
...
pcn_csr_read(struct pcn_softc *sc, int reg64)
{
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x14, reg64);
 return (bus_space_read_4(sc->sc_st, sc->sc_sh, 0x10));
}
static __inline void
pcn_csr_write(struct pcn_softc *sc, int reg64, uint32_t val)
{
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x14, reg64);
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x10, val);
}
static __inline uint32_t
pcn_bcr_read(struct pcn_softc *sc, int reg64)
{
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x14, reg64);
 return (bus_space_read_4(sc->sc_st, sc->sc_sh, 0x1c));
}
static __inline void
pcn_bcr_write(struct pcn_softc *sc, int reg64, uint32_t val)
{
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x14, reg64);
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x1c, val);
}
static const struct pcn_variant *
...
pcn_match(struct device *parent, void *match, void *aux)
{
 ...
 if ((((pa->pa_id) >> 0) & 0xffff) == 0x1023 &&
     (((pa->pa_id) >> 16) & 0xffff) == 0x2000 &&
     (((pa->pa_class) >> 24) & 0xff) == 0x02)
  return(1);
...
pcn_attach(struct device *parent, struct device *self, void *aux)
{
 ...
 int i, rseg, error;
 uint32_t chipid, reg64;
 uint8_t enaddr[6];
 timeout_set(&sc->sc_tick_timeout, pcn_tick, sc);
 ioh_valid = (pci_mapreg_map(pa, (0x10 + 0x00), 0x00000001, 0,
     &iot, &ioh, ((void *)0), ((void *)0), 0) == 0);
 memh_valid = (pci_mapreg_map(pa, (0x10 + 0x04),
 ...
 pci_set_powerstate(pc, pa->pa_tag, 0x0000);
 pcn_reset(sc);
 for (i = 0; i < 6; i++)
  enaddr[i] = bus_space_read_1(sc->sc_st, sc->sc_sh,
      0x00 + i);
 chipid = pcn_csr_read(sc, 0x0058);
 sc->sc_variant = pcn_lookup_variant((((chipid) >> 12) & 0xffff));
 if (pci_intr_map(pa, &ih)) {
  printf(": unable to map interrupt\n");
  return;
 }
 intrstr = pci_intr_string(pc, ih);
 sc->sc_ih = pci_intr_establish(pc, ih, 6, pcn_intr, sc,
     self->dv_xname);
 if (sc->sc_ih == ((void *)0)) {
 ...
 if ((error = bus_dmamem_alloc(sc->sc_dmat,
      sizeof(struct pcn_control_data), (1 << 13), 0, &seg, 1, &rseg,
      0)) != 0) {
 ...
 if ((error = bus_dmamap_load(sc->sc_dmat, sc->sc_cddmamap,
      sc->sc_control_data, sizeof(struct pcn_control_data), ((void *)0),
      0)) != 0) {
 ...
 for (i = 0; i < 128; i++) {
  if ((error = bus_dmamap_create(sc->sc_dmat, (1 << 11), 1,
       (1 << 11), 0, 0, &sc->sc_rxsoft[i].rxs_dmamap)) != 0) {
 ...
 printf(", %s, rev %d: %s, address %s\n", sc->sc_variant->pcv_desc,
     (((chipid) >> 28) & 0x7), intrstr, ether_sprintf(enaddr));
 ...
 default:
  ...
  reg64 = pcn_bcr_read(sc, 0x0019) & 0x00ff;
  if (reg64 != 0)
...
pcn_start(struct ifnet *ifp)
{
 ...
 for (;;) {
  ...
  if (bus_dmamap_load_mbuf(sc->sc_dmat, dmamap, m0,
   ...
   m_copydata(m0, 0, m0->M_dat.MH.MH_pkthdr.len, ((caddr_t)((m)->m_hdr.mh_data)));
   m->M_dat.MH.MH_pkthdr.len = m->m_hdr.mh_len = m0->M_dat.MH.MH_pkthdr.len;
  ...
  if (sc->sc_swstyle == 3) {
   for (nexttx = sc->sc_txnext, seg = 0;
    ...
    sc->sc_control_data->pcd_txdescs[nexttx].tmd2 =
        (__uint32_t)(__builtin_constant_p(dmamap->dm_segs[seg].ds_addr) ? (__uint32_t)(((__uint32_t)(dmamap->dm_segs[seg].ds_addr) & 0xff) << 24 | ((__uint32_t)(dmamap->dm_segs[seg].ds_addr) & 0xff00) << 8 | ((__uint32_t)(dmamap->dm_segs[seg].ds_addr) & 0xff0000) >> 8 | ((__uint32_t)(dmamap->dm_segs[seg].ds_addr) & 0xff000000) >> 24) : __swap32md(dmamap->dm_segs[seg].ds_addr));
    sc->sc_control_data->pcd_txdescs[nexttx].tmd1 =
        (__uint32_t)(__builtin_constant_p((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))) ? (__uint32_t)(((__uint32_t)((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))) & 0xff) << 24 | ((__uint32_t)((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))) & 0xff00) << 8 | ((__uint32_t)((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))) & 0xff0000) >> 8 | ((__uint32_t)((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))) & 0xff000000) >> 24) : __swap32md((0xf<<12) | (nexttx == sc->sc_txnext ? 0 : (1<<31)) | ((~(dmamap->dm_segs[seg].ds_len) + 1) & (0xfff))));
    lasttx = nexttx;
   }
  } else {
   for (nexttx = sc->sc_txnext, seg = 0;
  ...
  if ((sc->sc_txsnext & 7) == 0)
   sc->sc_control_data->pcd_txdescs[lasttx].tmd1 |= (__uint32_t)(__builtin_constant_p((1<<28)) ? (__uint32_t)(((__uint32_t)((1<<28)) & 0xff) << 24 | ((__uint32_t)((1<<28)) & 0xff00) << 8 | ((__uint32_t)((1<<28)) & 0xff0000) >> 8 | ((__uint32_t)((1<<28)) & 0xff000000) >> 24) : __swap32md((1<<28)));
  sc->sc_control_data->pcd_txdescs[lasttx].tmd1 |= (__uint32_t)(__builtin_constant_p((1<<24)) ? (__uint32_t)(((__uint32_t)((1<<24)) & 0xff) << 24 | ((__uint32_t)((1<<24)) & 0xff00) << 8 | ((__uint32_t)((1<<24)) & 0xff0000) >> 8 | ((__uint32_t)((1<<24)) & 0xff000000) >> 24) : __swap32md((1<<24)));
  sc->sc_control_data->pcd_txdescs[sc->sc_txnext].tmd1 |=
      (__uint32_t)(__builtin_constant_p((1<<31)|(1<<25)) ? (__uint32_t)(((__uint32_t)((1<<31)|(1<<25)) & 0xff) << 24 | ((__uint32_t)((1<<31)|(1<<25)) & 0xff00) << 8 | ((__uint32_t)((1<<31)|(1<<25)) & 0xff0000) >> 8 | ((__uint32_t)((1<<31)|(1<<25)) & 0xff000000) >> 24) : __swap32md((1<<31)|(1<<25)));
  ...
  sc->sc_txnext = nexttx;
  sc->sc_txsfree--;
  sc->sc_txsnext = (((sc->sc_txsnext) + 1) & (128 - 1));
  if (ifp->if_bpf)
...
pcn_intr(void *arg)
{
 ...
 for (wantinit = 0; wantinit == 0;) {
  csr0 = pcn_csr_read(sc, 0x0000);
  if ((csr0 & 0x0080) == 0)
   break;
  pcn_csr_write(sc, 0x0000, csr0 &
      (0x0040|0x4000|0x1000|0x0800|0x0400|
       0x0200|0x0100));
  handled = 1;
  if (csr0 & 0x0400)
...
{
 int i;
 pcn_csr_write(sc, 0x0005, sc->sc_csr5 | 0x0001);
 for (i = 0; i < 10000; i++) {
  if (pcn_csr_read(sc, 0x0005) & 0x0001)
   return;
  delay(5);
 }
...
pcn_txintr(struct pcn_softc *sc)
{
 ...
 for (i = sc->sc_txsdirty; sc->sc_txsfree != 128;
  ...
  tmd1 = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_txdescs[txs->txs_lastdesc].tmd1));
  if (tmd1 & (1<<31))
   break;
  for (j = txs->txs_firstdesc;; j = (((j) + 1) & (512 - 1))) {
   tmd = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_txdescs[j].tmd1) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd1) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd1) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd1) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd1) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_txdescs[j].tmd1));
   if (tmd & (1<<30)) {
    ifp->if_data.ifi_oerrors++;
    if (sc->sc_swstyle == 3)
     tmd2 = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_txdescs[j].tmd0) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd0) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd0) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd0) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd0) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_txdescs[j].tmd0));
    else
     tmd2 = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_txdescs[j].tmd2) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd2) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd2) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd2) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_txdescs[j].tmd2) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_txdescs[j].tmd2));
    if (tmd2 & (1<<30)) {
     if (sc->sc_xmtsp < 3) {
      sc->sc_xmtsp++;
      printf("%s: transmit "
          ...
          sc->sc_xmtsp]);
      pcn_spnd(sc);
      pcn_csr_write(sc, 0x0050,
          ((sc->sc_rcvfw) << 12) |
          ((sc->sc_xmtsp) << 10) |
          ((sc->sc_xmtfw) << 8));
      pcn_csr_write(sc, 0x0005,
          sc->sc_csr5);
     } else {
      printf("%s: transmit "
          "underrun\n",
  ...
  if (tmd1 & (1<<27))
   ...
   ifp->if_data.ifi_collisions += 2;
  }
 next_packet:
  sc->sc_txfree += txs->txs_dmamap->dm_nsegs;
  bus_dmamap_sync(sc->sc_dmat, txs->txs_dmamap,
      0, txs->txs_dmamap->dm_mapsize, 0x08);
  bus_dmamap_unload(sc->sc_dmat, txs->txs_dmamap);
  m_freem(txs->txs_mbuf);
  txs->txs_mbuf = ((void *)0);
 }
 sc->sc_txsdirty = i;
 if (sc->sc_txsfree == 128)
  ifp->if_timer = 0;
}
int
pcn_rxintr(struct pcn_softc *sc)
{
 ...
 for (i = sc->sc_rxptr;; i = (((i) + 1) & (128 - 1))) {
  ...
  rmd1 = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_rxdescs[i].rmd1) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd1) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd1) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd1) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd1) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_rxdescs[i].rmd1));
  if (rmd1 & (1<<31))
   break;
  if (__builtin_expect((((rmd1 & ((1<<25)|(1<<24)|(1<<30))) != ((1<<25)|(1<<24))) != 0), 0)) {
   ...
   if (rmd1 & (1<<30)) {
    ...
    if (rmd1 & (1<<28))
     ...
     if (rmd1 & ((1<<29))) printf("%s: %s\n", sc->sc_dev.dv_xname, "framing error");;
     if (rmd1 & ((1<<27))) printf("%s: %s\n", sc->sc_dev.dv_xname, "CRC error");;
     if (rmd1 & ((1<<26))) printf("%s: %s\n", sc->sc_dev.dv_xname, "buffer error");;
    }
    do { struct pcn_rxsoft *__rxs = &(sc)->sc_rxsoft[(i)]; struct lermd *__rmd = &(sc)->sc_control_data->pcd_rxdescs[(i)]; struct mbuf *__m = __rxs->rxs_mbuf; __m->m_hdr.mh_data = __m->M_dat.MH.MH_dat.MH_ext.ext_buf + 2; if ((sc)->sc_swstyle == 3) { __rmd->rmd2 = (__uint32_t)(__builtin_constant_p(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) ? (__uint32_t)(((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff) << 24 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff00) << 8 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff0000) >> 8 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff000000) >> 24) : __swap32md(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2)); __rmd->rmd0 = 0; } else { __rmd->rmd2 = 0; __rmd->rmd0 = (__uint32_t)(__builtin_constant_p(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) ? (__uint32_t)(((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff) << 24 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff00) << 8 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff0000) >> 8 | ((__uint32_t)(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2) & 0xff000000) >> 24) : __swap32md(__rxs->rxs_dmamap->dm_segs[0].ds_addr + 2)); } __rmd->rmd1 = (__uint32_t)(__builtin_constant_p((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff))) ? (__uint32_t)(((__uint32_t)((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff))) & 0xff) << 24 | ((__uint32_t)((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff))) & 0xff00) << 8 | ((__uint32_t)((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff))) & 0xff0000) >> 8 | ((__uint32_t)((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff))) & 0xff000000) >> 24) : __swap32md((1<<31)|(0xf<<12)| ((~((1 << 11) - 2) + 1) & (0xfff)))); bus_dmamap_sync(((sc))->sc_dmat, ((sc))->sc_cddmamap, __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[(((i)))]), sizeof(struct lermd), (0x01|0x04));} while( 0);
    continue;
   }
  }
  ...
  if (sc->sc_swstyle == 3)
   len = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_rxdescs[i].rmd0) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd0) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd0) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd0) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd0) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_rxdescs[i].rmd0)) & (0xfff);
  else
   len = (__uint32_t)(__builtin_constant_p(sc->sc_control_data->pcd_rxdescs[i].rmd2) ? (__uint32_t)(((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd2) & 0xff) << 24 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd2) & 0xff00) << 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd2) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_control_data->pcd_rxdescs[i].rmd2) & 0xff000000) >> 24) : __swap32md(sc->sc_control_data->pcd_rxdescs[i].rmd2)) & (0xfff);
  len -= 4;
...
{
 (void) bus_space_read_2(sc->sc_st, sc->sc_sh, 0x14);
 (void) bus_space_read_4(sc->sc_st, sc->sc_sh, 0x18);
 delay(1000);
 bus_space_write_4(sc->sc_st, sc->sc_sh, 0x10, 0);
}
int
pcn_init(struct ifnet *ifp)
{
 struct pcn_softc *sc = ifp->if_softc;
 struct pcn_rxsoft *rxs;
 uint8_t *enaddr = ((caddr_t)((ifp->if_sadl)->sdl_data + (ifp->if_sadl)->sdl_nlen));
 int i, error = 0;
 uint32_t reg64;
 pcn_stop(ifp, 0);
 pcn_reset(sc);
 if (sc->sc_variant->pcv_chipid == 0x2430)
  sc->sc_swstyle = 2;
 else
  sc->sc_swstyle = 3;
 pcn_bcr_write(sc, 0x0014, sc->sc_swstyle);
 __builtin_memset((sc->sc_control_data->pcd_txdescs), (0), (sizeof(sc->sc_control_data->pcd_txdescs)));
 do { int __x, __n; __x = (0); __n = (512); if ((__x + __n) > 512) { bus_dmamap_sync((sc)->sc_dmat, (sc)->sc_cddmamap, __builtin_offsetof(struct pcn_control_data, pcd_txdescs[(__x)]), sizeof(struct letmd) * (512 - __x), (0x01|0x04)); __n -= (512 - __x); __x = 0; } bus_dmamap_sync((sc)->sc_dmat, (sc)->sc_cddmamap, __builtin_offsetof(struct pcn_control_data, pcd_txdescs[(__x)]), sizeof(struct letmd) * __n, (0x01|0x04)); } while ( 0);
 sc->sc_txfree = 512;
 sc->sc_txnext = 0;
 for (i = 0; i < 128; i++)
  sc->sc_txsoft[i].txs_mbuf = ((void *)0);
 sc->sc_txsfree = 128;
 sc->sc_txsnext = 0;
 sc->sc_txsdirty = 0;
 for (i = 0; i < 128; i++) {
 ...
 if (sc->sc_flags & 0x0001) {
  pcn_bcr_write(sc, 0x0002,
      pcn_bcr_read(sc, 0x0002) & ~0x0002);
  sc->sc_mode |= ((3) << 7);
  pcn_bcr_write(sc, 0x0020,
      pcn_bcr_read(sc, 0x0020) | 0x0080);
 }
 pcn_set_filter(sc);
 sc->sc_control_data->pcd_initblock.init_rdra = (__uint32_t)(__builtin_constant_p(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))) ? (__uint32_t)(((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))) & 0xff) << 24 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))) & 0xff00) << 8 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))) & 0xff0000) >> 8 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))) & 0xff000000) >> 24) : __swap32md(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_rxdescs[((0))]))));
 sc->sc_control_data->pcd_initblock.init_tdra = (__uint32_t)(__builtin_constant_p(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))) ? (__uint32_t)(((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))) & 0xff) << 24 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))) & 0xff00) << 8 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))) & 0xff0000) >> 8 | ((__uint32_t)(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))) & 0xff000000) >> 24) : __swap32md(((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_txdescs[((0))]))));
 sc->sc_control_data->pcd_initblock.init_mode = (__uint32_t)(__builtin_constant_p(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)) ? (__uint32_t)(((__uint32_t)(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)) & 0xff) << 24 | ((__uint32_t)(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)) & 0xff00) << 8 | ((__uint32_t)(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)) & 0xff0000) >> 8 | ((__uint32_t)(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)) & 0xff000000) >> 24) : __swap32md(sc->sc_mode | ((ffs(512) - 1) << 28) | ((ffs(128) - 1) << 20)));
 sc->sc_control_data->pcd_initblock.init_padr[0] = (__uint32_t)(__builtin_constant_p(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)) ? (__uint32_t)(((__uint32_t)(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)) & 0xff) << 24 | ((__uint32_t)(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)) & 0xff00) << 8 | ((__uint32_t)(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)) & 0xff0000) >> 8 | ((__uint32_t)(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)) & 0xff000000) >> 24) : __swap32md(enaddr[0] | (enaddr[1] << 8) | (enaddr[2] << 16) | (enaddr[3] << 24)));
 sc->sc_control_data->pcd_initblock.init_padr[1] = (__uint32_t)(__builtin_constant_p(enaddr[4] | (enaddr[5] << 8)) ? (__uint32_t)(((__uint32_t)(enaddr[4] | (enaddr[5] << 8)) & 0xff) << 24 | ((__uint32_t)(enaddr[4] | (enaddr[5] << 8)) & 0xff00) << 8 | ((__uint32_t)(enaddr[4] | (enaddr[5] << 8)) & 0xff0000) >> 8 | ((__uint32_t)(enaddr[4] | (enaddr[5] << 8)) & 0xff000000) >> 24) : __swap32md(enaddr[4] | (enaddr[5] << 8)));
 pcn_csr_write(sc, 0x0003, 0x1000|0x0100|0x0040);
 pcn_csr_write(sc, 0x0004, 0x4000|0x0800|
     0x0100|0x0010|0x0004);
 sc->sc_csr5 = 0x4000|0x0400;
 pcn_csr_write(sc, 0x0005, sc->sc_csr5);
 switch (sc->sc_variant->pcv_chipid) {
 case 0x2430:
 case 0x2621:
  break;
 default:
  pcn_csr_write(sc, 0x0007, 0x8000);
  break;
 }
 reg64 = pcn_bcr_read(sc, 0x0012);
 switch (sc->sc_variant->pcv_chipid) {
 case 0x2430:
  break;
 case 0x2621:
  ...
  break;
 default:
  reg64 |= 0x0040|0x0020|0x0800;
  break;
 }
 pcn_bcr_write(sc, 0x0012, reg64);
 pcn_csr_write(sc, 0x0050, ((sc->sc_rcvfw) << 12) |
     ((sc->sc_xmtsp) << 10) | ((sc->sc_xmtfw) << 8));
 bus_dmamap_sync((sc)->sc_dmat, (sc)->sc_cddmamap, __builtin_offsetof(struct pcn_control_data, pcd_initblock), sizeof(struct leinit), (0x04));
 pcn_csr_write(sc, 0x0001, ((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_initblock)) & 0xffff);
 pcn_csr_write(sc, 0x0002, (((sc)->sc_cddmamap->dm_segs[0].ds_addr + __builtin_offsetof(struct pcn_control_data, pcd_initblock)) >> 16) & 0xffff);
 pcn_csr_write(sc, 0x0000, 0x0001);
 delay(100);
 for (i = 0; i < 10000; i++) {
  if (pcn_csr_read(sc, 0x0000) & 0x0100)
   break;
  delay(10);
 }
 bus_dmamap_sync((sc)->sc_dmat, (sc)->sc_cddmamap, __builtin_offsetof(struct pcn_control_data, pcd_initblock), sizeof(struct leinit), (0x08));
 if (i == 10000) {
 ...
 pcn_csr_write(sc, 0x0000, 0x0040|0x0002|0x0100);
 if (sc->sc_flags & 0x0001) {
  timeout_add_sec(&sc->sc_tick_timeout, 1);
 }
 ifp->if_flags |= 0x40;
...
pcn_stop(struct ifnet *ifp, int disable)
{
 ...
 ifq_clr_oactive(&ifp->if_snd);
 ifp->if_timer = 0;
 pcn_csr_write(sc, 0x0000, 0x0004);
 for (i = 0; i < 128; i++) {
  txs = &sc->sc_txsoft[i];
...
pcn_set_filter(struct pcn_softc *sc)
{
 ...
 if (ifp->if_flags & 0x100 || ac->ac_multirangecnt > 0) {
  ...
  if (ifp->if_flags & 0x100)
   sc->sc_mode |= 0x8000;
  sc->sc_control_data->pcd_initblock.init_ladrf[0] =
      sc->sc_control_data->pcd_initblock.init_ladrf[1] =
      sc->sc_control_data->pcd_initblock.init_ladrf[2] =
      sc->sc_control_data->pcd_initblock.init_ladrf[3] = 0xffff;
 } else {
  sc->sc_control_data->pcd_initblock.init_ladrf[0] =
      sc->sc_control_data->pcd_initblock.init_ladrf[1] =
      sc->sc_control_data->pcd_initblock.init_ladrf[2] =
      sc->sc_control_data->pcd_initblock.init_ladrf[3] = 0;
  ...
  while (enm != ((void *)0)) {
   ...
   sc->sc_control_data->pcd_initblock.init_ladrf[crc >> 4] |=
       (__uint16_t)(__builtin_constant_p(1 << (crc & 0xf)) ? (__uint16_t)(((__uint16_t)(1 << (crc & 0xf)) & 0xffU) << 8 | ((__uint16_t)(1 << (crc & 0xf)) & 0xff00U) >> 8) : __swap16md(1 << (crc & 0xf)));
...
{
 ifmedia_init(&sc->sc_mii.mii_media, 0xff00000000000000ULL, pcn_79c970_mediachange,
     pcn_79c970_mediastatus);
 ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|5,
     0, ((void *)0));
 if (sc->sc_variant->pcv_chipid == 0x2621)
  ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|5|0x0000010000000000ULL,
      0, ((void *)0));
 ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|3,
     1, ((void *)0));
 if (sc->sc_variant->pcv_chipid == 0x2621)
  ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|3|0x0000010000000000ULL,
      1, ((void *)0));
 ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|0ULL,
     0, ((void *)0));
 if (sc->sc_variant->pcv_chipid == 0x2621)
  ifmedia_add(&sc->sc_mii.mii_media, 0x0000000000000100ULL|0ULL|0x0000010000000000ULL,
      0, ((void *)0));
...
pcn_79c970_mediachange(struct ifnet *ifp)
{
 struct pcn_softc *sc = ifp->if_softc;
 uint32_t reg64;
 if (((sc->sc_mii.mii_media.ifm_media) & 0x00000000000000ffULL) == 0ULL) {
  reg64 = pcn_bcr_read(sc, 0x0002);
  reg64 |= 0x0002;
  pcn_bcr_write(sc, 0x0002, reg64);
 } else {
  reg64 = pcn_bcr_read(sc, 0x0002);
  reg64 &= ~0x0002;
  pcn_bcr_write(sc, 0x0002, reg64);
  reg64 = pcn_csr_read(sc, 0x000f);
  reg64 = (reg64 & ~((3) << 7)) |
      ((sc->sc_mii.mii_media.ifm_cur->ifm_data) << 7);
  pcn_csr_write(sc, 0x000f, reg64);
 }
 if ((sc->sc_mii.mii_media.ifm_media & 0x0000010000000000ULL) != 0) {
  reg64 = 0x0001;
  if (((sc->sc_mii.mii_media.ifm_media) & 0x00000000000000ffULL) == 5)
   reg64 |= 0x0002;
  pcn_bcr_write(sc, 0x0009, reg64);
 } else
  pcn_bcr_write(sc, 0x0009, 0);
 return (0);
}
void
...
int
pcn_mii_readreg(struct device *self, int phy, int reg64)
{
 struct pcn_softc *sc = (void *) self;
 uint32_t rv;
 pcn_bcr_write(sc, 0x0021, reg64 | (phy << 5));
 rv = pcn_bcr_read(sc, 0x0022) & 0xffff;
 if (rv == 0xffff)
  return (0);
 return (rv);
}
void
pcn_mii_writereg(struct device *self, int phy, int reg64, int val)
{
 struct pcn_softc *sc = (void *) self;
 pcn_bcr_write(sc, 0x0021, reg64 | (phy << 5));
 pcn_bcr_write(sc, 0x0022, val);
}
void
pcn_mii_statchg(struct device *self)
{
 struct pcn_softc *sc = (void *) self;
 if ((sc->sc_mii.mii_media_active & 0x0000010000000000ULL) != 0)
  pcn_bcr_write(sc, 0x0009, 0x0001);
 else
  pcn_bcr_write(sc, 0x0009, 0);
}
