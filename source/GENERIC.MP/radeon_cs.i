...
{
 list_add_tail(item, &b->bucket[min(priority, 32u)]);
}
...
static int radeon_cs_parser_relocs(struct radeon_cs_parser *p)
{
 ...
 for (i = 0; i < p->nrelocs; i++) {
  ...
  p->relocs[i].robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
  priority = (r->flags & (0xf << 0)) * 2
      + !!r->write_domain;
  if (p->ring == 5 &&
   ...
   if (domain & 0x1) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "RADEON_GEM_DOMAIN_CPU is not valid " "for command submission\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
    return -22;
   }
  ...
  if (radeon_ttm_tt_has_userptr(p->relocs[i].robj->tbo.ttm)) {
   ...
   if (!(domain & 0x2)) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Only RADEON_GEM_DOMAIN_GTT is " "allowed for userptr BOs\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
    return -22;
   }
   need_mmap_lock = 1;
   domain = 0x2;
...
int radeon_cs_parser_init(struct radeon_cs_parser *p, void *data)
{
 ...
 p->chunk_ib = ((void *)0);
 p->chunk_relocs = ((void *)0);
 p->chunk_flags = ((void *)0);
 p->chunk_const_ib = ((void *)0);
 ...
 for (i = 0; i < p->nchunks; i++) {
  ...
  if (user_chunk.chunk_id == 0x01) {
   p->chunk_relocs = &p->chunks[i];
  }
  if (user_chunk.chunk_id == 0x02) {
   p->chunk_ib = &p->chunks[i];
   if (p->chunks[i].length_dw == 0)
    return -22;
  }
  if (user_chunk.chunk_id == 0x04) {
   p->chunk_const_ib = &p->chunks[i];
   if (p->chunks[i].length_dw == 0)
    return -22;
  }
  if (user_chunk.chunk_id == 0x03) {
   p->chunk_flags = &p->chunks[i];
   if (p->chunks[i].length_dw == 0)
    return -22;
  }
  size = p->chunks[i].length_dw;
  cdata = (void *)(unsigned long)user_chunk.chunk_data;
 ...
 if (p->rdev) {
  ...
  if (radeon_cs_get_ring(p, ring, priority))
   return -22;
  if ((p->cs_flags & 0x02) == 0) {
   if (p->rdev->asic->ring[p->ring]->cs_parse == ((void *)0)) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Ring %d requires VM!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , p->ring);
    return -22;
   }
  } else {
   if (p->rdev->asic->ring[p->ring]->ib_parse == ((void *)0)) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "VM not supported on ring %d!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , p->ring);
    return -22;
   }
  }
 }
...
static int cmp_size_smaller_first(void *priv, struct list_head *a,
      struct list_head *b)
{
 struct radeon_bo_list *la = ({ const __typeof( ((struct radeon_bo_list *)0)->tv.head ) *__mptr = (a); (struct radeon_bo_list *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo_list, tv.head) );});
 struct radeon_bo_list *lb = ({ const __typeof( ((struct radeon_bo_list *)0)->tv.head ) *__mptr = (b); (struct radeon_bo_list *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo_list, tv.head) );});
 return (int)la->robj->tbo.num_pages - (int)lb->robj->tbo.num_pages;
}
static void radeon_cs_parser_fini(struct radeon_cs_parser *parser, int error, _Bool backoff)
{
 unsigned i;
 if (!error) {
         ...
         struct radeon_cs_parser *parser)
{
 ...
 if (r) {
  if (r != -4)
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to sync rings: %i\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
  return r;
 }
 if (parser->ring == 5)
  radeon_uvd_note_usage(rdev);
 else if ((parser->ring == 6) ||
   (parser->ring == 7))
       ...
       struct radeon_vm *vm)
{
 ...
 if (vm->ib_bo_va == ((void *)0)) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Tmp BO not in VM!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return -22;
 }
...
static int radeon_cs_ib_fill(struct radeon_device *rdev, struct radeon_cs_parser *parser)
{
 ...
 if (ib_chunk->kdata)
  __builtin_memcpy((parser->ib.ptr), (ib_chunk->kdata), (ib_chunk->length_dw * 4));
