...
static int radeon_cs_parser_relocs(struct radeon_cs_parser *p)
{
 ...
 for (i = 0; i < p->nrelocs; i++) {
  ...
  if (!duplicate) {
   ...
   p->relocs[i].robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((p->relocs[i].gobj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
       ...
       struct radeon_vm *vm)
{
 ...
 for (lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = ((&parser->validated)->next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );}); &lobj->tv.head != (&parser->validated); lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = (lobj->tv.head.next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );})) {
  bo = lobj->bo;
