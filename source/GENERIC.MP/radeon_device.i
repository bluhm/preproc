...
static struct radeon_px_quirk radeon_px_quirk_list[] = {
 { 0x1002, 0x6760, 0x1025, 0x0672, (1 << 0) },
 { 0x1002, 0x6741, 0x1043, 0x108c, (1 << 0) },
 { 0x1002, 0x6840, 0x1043, 0x2122, (1 << 0) },
 { 0x1002, 0x6741, 0x1043, 0x2122, (1 << 0) },
 { 0x1002, 0x6741, 0x106b, 0x00e2, (1 << 1) },
 { 0, 0, 0, 0, 0 },
};
_Bool radeon_is_px(struct drm_device *dev)
{
...
void radeon_program_register_sequence(struct radeon_device *rdev,
          ...
          const u32 array_size)
{
 u32 tmp, reg64, and_mask, or_mask;
 int i;
 if (array_size % 3)
  return;
 for (i = 0; i < array_size; i +=3) {
  ...
  if (and_mask == 0xffffffff) {
   ...
   tmp = r100_mm_rreg(rdev, (reg64), 0);
   tmp &= ~and_mask;
   tmp |= or_mask;
  }
  r100_mm_wreg(rdev, (reg64), (tmp), 0);
 }
}
void radeon_pci_config_reset(struct radeon_device *rdev)
{
 pci_write_config_dword(rdev->pdev, 0x7c, 0x39d5e86b);
}
...
{
 rdev->doorbell.num_doorbells = ({ u32 __min_a = (rdev->doorbell.size / sizeof(u32)); u32 __min_b = (1024); __min_a < __min_b ? __min_a : __min_b; });
 if (rdev->doorbell.num_doorbells == 0)
  return -22;
 do { } while( 0);
 do { } while( 0);
 __builtin_memset((&rdev->doorbell.used), (0), (sizeof(rdev->doorbell.used)));
 return 0;
}
...
static _Bool radeon_device_is_virtual(void)
{
 return 0;
}
...
int radeon_dummy_page_init(struct radeon_device *rdev)
{
 ...
 rdev->dummy_page.entry = (rdev)->asic->gart.get_page_entry((rdev->dummy_page.addr), (0));
 return 0;
}
void radeon_dummy_page_fini(struct radeon_device *rdev)
{
...
int radeon_atombios_init(struct radeon_device *rdev)
{
 ...
 _rw_init_flags(&rdev->mode_info.atom_context->scratch_mutex, "atomscr", 0, ((void *)0));
...
static void radeon_check_arguments(struct radeon_device *rdev)
{
 ...
 if (!radeon_check_pot_argument(radeon_vm_size)) {
  printf("drm:pid%d:%s *WARNING* " "VM size (%d) must be a power of 2\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , radeon_vm_size);
  radeon_vm_size = 4;
 }
 if (radeon_vm_size < 1) {
  printf("drm:pid%d:%s *WARNING* " "VM size (%d) to small, min is 1GB\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , radeon_vm_size);
  radeon_vm_size = 4;
 }
 if (radeon_vm_size > 1024) {
  printf("drm:pid%d:%s *WARNING* " "VM size (%d) too large, max is 1TB\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , radeon_vm_size);
  radeon_vm_size = 4;
 }
 if (radeon_vm_block_size == -1) {
  unsigned bits = ((sizeof(radeon_vm_size) <= 4) ? (fls(radeon_vm_size) - 1) : (flsl(radeon_vm_size) - 1)) + 18;
  if (radeon_vm_size <= 8)
   radeon_vm_block_size = bits - 9;
  else
   radeon_vm_block_size = (bits + 3) / 2;
 } else if (radeon_vm_block_size < 9) {
  printf("drm:pid%d:%s *WARNING* " "VM page table size (%d) too small\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , radeon_vm_block_size);
  radeon_vm_block_size = 9;
 }
 if (radeon_vm_block_size > 24 ||
     (radeon_vm_size * 1024) < (1ull << radeon_vm_block_size)) {
  printf("drm:pid%d:%s *WARNING* " "VM page table size (%d) too large\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , radeon_vm_block_size);
  radeon_vm_block_size = 9;
 }
}
int radeon_device_init(struct radeon_device *rdev,
         ...
         uint32_t flags)
{
 ...
 rdev->fence_context = fence_context_alloc(8);
 ...
 _rw_init_flags(&rdev->srbm_mutex, "srbm", 0, ((void *)0));
 _rw_init_flags(&rdev->grbm_idx_mutex, "grbm", 0, ((void *)0));
 _rw_init_flags(&rdev->pm.mclk_lock, "mclk", 0, ((void *)0));
 _rw_init_flags(&rdev->exclusive_lock, "rdnexc", 0, ((void *)0));
 init_waitqueue_head(&rdev->irq.vblank_queue);
 _rw_init_flags(&rdev->mn_lock, "mnlk", 0, ((void *)0));
 __hash_init(rdev->mn_hash, (sizeof((rdev->mn_hash)) / sizeof((rdev->mn_hash)[0])));
 ...
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->mmio_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->smc_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->pll_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->mc_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->pcie_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->pciep_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->pif_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->cg_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->uvd_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->rcu_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->didt_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { (void)(((void *)0)); (void)(0); __mtx_init((&rdev->end_idx_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 do { } while( 0);
 do { } while( 0);
 if (rdev->family >= CHIP_BONAIRE)
 ...
 if (r) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "registering gem debugfs failed (%d).\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
 }
 r = radeon_mst_debugfs_init(rdev);
 if (r) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "registering mst debugfs failed (%d).\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
 }
 ...
 if (rdev->pm.dpm_enabled &&
  ...
  rdev->asic->dpm.disable((rdev));
  rdev->asic->dpm.enable((rdev));
...
int radeon_suspend_kms(struct drm_device *dev, _Bool suspend, _Bool fbcon)
{
 ...
 for (crtc = ({ const __typeof( ((__typeof(*crtc) *)0)->head ) *__mptr = ((&dev->mode_config.crtc_list)->next); (__typeof(*crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*crtc), head) );}); &crtc->head != (&dev->mode_config.crtc_list); crtc = ({ const __typeof( ((__typeof(*crtc) *)0)->head ) *__mptr = (crtc->head.next); (__typeof(*crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*crtc), head) );})) {
  ...
  struct radeon_framebuffer *rfb = ({ const __typeof( ((struct radeon_framebuffer *)0)->base ) *__mptr = (crtc->primary->fb); (struct radeon_framebuffer *)( (char *)__mptr - __builtin_offsetof(struct radeon_framebuffer, base) );});
  struct radeon_bo *robj;
  if (radeon_crtc->cursor_bo) {
   struct radeon_bo *robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((radeon_crtc->cursor_bo)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
  ...
  robj = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((rfb->obj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
...
int radeon_resume_kms(struct drm_device *dev, _Bool resume, _Bool fbcon)
{
 ...
 if ((rdev->pm.pm_method == PM_METHOD_DPM) && rdev->pm.dpm_enabled) {
  ...
  if (r) {
   ...
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "radeon_pm_late_init failed, disabling dpm\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  }
 } else {
 ...
 for (crtc = ({ const __typeof( ((__typeof(*crtc) *)0)->head ) *__mptr = ((&dev->mode_config.crtc_list)->next); (__typeof(*crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*crtc), head) );}); &crtc->head != (&dev->mode_config.crtc_list); crtc = ({ const __typeof( ((__typeof(*crtc) *)0)->head ) *__mptr = (crtc->head.next); (__typeof(*crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*crtc), head) );})) {
  ...
  if (radeon_crtc->cursor_bo) {
   ...
   if (r == 0) {
    ...
    if (r != 0)
     printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to pin cursor BO (%d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , r);
    radeon_bo_unreserve(robj);
   }
  }
 }
...
{
 unsigned ring_sizes[8];
 uint32_t *ring_data[8];
 _Bool saved = 0;
 int i, r;
 int resched;
 ...
 __sync_fetch_and_add(&rdev->gpu_reset_counter, 1);
 radeon_save_bios_scratch_regs(rdev);
        ...
        unsigned nfiles)
{
 ...
 if (i > 32) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Reached maximum number of debugfs components.\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Report so we increase " "RADEON_DEBUGFS_MAX_COMPONENTS.\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return -22;
 }
