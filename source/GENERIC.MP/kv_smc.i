...
struct SMU7_Fusion_DpmTable
{
    ...
    uint16_t FpsHighT;
    SMU7_Fusion_GraphicsLevel GraphicsLevel [8];
    SMU7_Fusion_ACPILevel ACPILevel;
    SMU7_Fusion_UvdLevel UvdLevel [8];
    SMU7_Fusion_ExtClkLevel VceLevel [8];
    SMU7_Fusion_ExtClkLevel AcpLevel [8];
    SMU7_Fusion_ExtClkLevel SamuLevel [8];
...
{
    SMU7_Fusion_GIOLevel GIOLevel [8];
    SMU7_PIDController GioPIDController;
...
struct kv_ps {
 struct kv_pl levels[5];
 u32 num_levels;
 _Bool need_dfs_bypass;
 u8 dpm0_pg_nb_ps_lo;
 u8 dpm0_pg_nb_ps_hi;
 u8 dpmx_nb_ps_lo;
 u8 dpmx_nb_ps_hi;
...
struct kv_power_info {
 ...
 _Bool enable_nb_ps_policy;
 _Bool disable_nb_ps3_in_battery;
 _Bool video_start;
 _Bool battery_state;
 u32 lowest_valid;
 u32 highest_valid;
 ...
 u8 samu_level_count;
 u16 fps_high_t;
 SMU7_Fusion_GraphicsLevel graphics_level[8];
 SMU7_Fusion_ACPILevel acpi_level;
 SMU7_Fusion_UvdLevel uvd_level[8];
 SMU7_Fusion_ExtClkLevel vce_level[8];
 SMU7_Fusion_ExtClkLevel acp_level[8];
 SMU7_Fusion_ExtClkLevel samu_level[8];
 u8 uvd_boot_level;
 u8 vce_boot_level;
 u8 acp_boot_level;
 u8 samu_boot_level;
 ...
 _Bool vce_power_gated;
 _Bool acp_power_gated;
 _Bool samu_power_gated;
 _Bool nb_dpm_enabled;
 _Bool enable_didt;
 _Bool enable_dpm;
 _Bool enable_auto_thermal_throttling;
 _Bool enable_nb_dpm;
 ...
 _Bool caps_samu_pg;
 _Bool caps_acp_pg;
 _Bool caps_stable_p_state;
 _Bool caps_enable_dfs_bypass;
 _Bool caps_sclk_ds;
 struct radeon_ps current_rps;
...
int kv_read_smc_sram_dword(struct radeon_device *rdev, u32 smc_address,
      u32 *value, u32 limit);
int kv_smc_dpm_enable(struct radeon_device *rdev, _Bool enable);
int kv_smc_bapm_enable(struct radeon_device *rdev, _Bool enable);
...
int kv_notify_message_to_smu(struct radeon_device *rdev, u32 id)
{
 u32 i;
 u32 tmp = 0;
 r100_mm_wreg(rdev, (0x250), (id & 0xffff), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  if ((r100_mm_rreg(rdev, (0x254), 0) & 0xffff) != 0)
   break;
  udelay(1);
 }
 tmp = r100_mm_rreg(rdev, (0x254), 0) & 0xffff;
 if (tmp != 1) {
  if (tmp == 0xFF)
   return -22;
  else if (tmp == 0xFE)
...
{
 int ret;
 ret = kv_notify_message_to_smu(rdev, ((uint16_t) 0x162));
 if (ret == 0)
  *enable_mask = tn_smc_rreg(rdev, (0x80000068));
 return ret;
}
int kv_send_msg_to_smc_with_parameter(struct radeon_device *rdev,
          PPSMC_Msg msg, u32 parameter)
...
{
 if (enable)
  return kv_notify_message_to_smu(rdev, ((uint16_t) 0x14e));
 else
  return kv_notify_message_to_smu(rdev, ((uint16_t) 0x14f));
}
int kv_smc_bapm_enable(struct radeon_device *rdev, _Bool enable)
{
 if (enable)
  return kv_notify_message_to_smu(rdev, ((uint32_t) 0x120));
 else
  return kv_notify_message_to_smu(rdev, ((uint32_t) 0x121));
}
int kv_copy_bytes_to_smc(struct radeon_device *rdev,
    u32 smc_start_address,
    const u8 *src, u32 byte_count, u32 limit)
