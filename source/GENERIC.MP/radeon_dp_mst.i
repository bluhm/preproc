...
{
 static const int offsets[] = { (0x6df0 - 0x6df0),
           (0x79f0 - 0x6df0),
           (0x105f0 - 0x6df0),
           (0x111f0 - 0x6df0),
           (0x11df0 - 0x6df0),
...
static int radeon_dp_mst_set_be_cntl(struct radeon_encoder *primary,
         ...
         enum radeon_hpd_id hpd, _Bool enable)
{
 ...
 int retries = 0;
 uint32_t temp;
 reg64 = r100_mm_rreg(rdev, (0x7140 + primary->offset), 0);
 reg64 &= ~(((7) & 0x7) << 16);
 reg64 |= (((5) & 0x7) << 16);
 if (enable)
  reg64 |= (((1 << mst_enc->fe) & 0x7f) << 8);
 else
  reg64 &= ~(((1 << mst_enc->fe) & 0x7f) << 8);
 reg64 |= (((hpd) & 0x7) << 28);
 do { } while( 0);
 r100_mm_wreg(rdev, (0x7140 + primary->offset), (reg64), 0);
 if (enable) {
  uint32_t offset = radeon_atom_set_enc_offset(mst_enc->fe);
  do {
   temp = r100_mm_rreg(rdev, (0x7000 + offset), 0);
  } while ((temp & (0x1 << 24)) && retries++ < 10000);
  if (retries == 10000)
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "timed out waiting for FE %d %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , primary->offset, mst_enc->fe);
 }
 return 0;
}
        ...
        int slots)
{
 ...
 temp = r100_mm_rreg(rdev, (0x7390 + satreg + primary->offset), 0);
 val = (((slots) & 0x3f) << 8) | (((fe) & 0x7) << 0);
 val <<= (16 * satidx);
 temp &= ~(0xffff << (16 * satidx));
 temp |= val;
 do { } while( 0);
 r100_mm_wreg(rdev, (0x7390 + satreg + primary->offset), (temp), 0);
 r100_mm_wreg(rdev, (0x739c + primary->offset), (1), 0);
 do {
  temp = r100_mm_rreg(rdev, (0x739c + primary->offset), 0);
 } while ((temp & 0x1) && retries++ < 10000);
 if (retries == 10000)
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "timed out waitin for SAT update %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , primary->offset);
 return 0;
}
            ...
            struct radeon_encoder *primary)
{
 ...
 struct drm_connector *connector;
 __builtin_memset((new_attribs), (0), (sizeof(new_attribs)));
...
static int radeon_dp_mst_set_vcp_size(struct radeon_encoder *mst, uint32_t x, uint32_t y)
{
 ...
 uint32_t offset = radeon_atom_set_enc_offset(mst_enc->fe);
 int retries = 0;
 val = (((x) & 0x3f) << 26) | (((y) & 0x3ffffff) << 0);
 r100_mm_wreg(rdev, (0x7384 + offset), (val), 0);
 do {
  temp = r100_mm_rreg(rdev, (0x738c + offset), 0);
 } while ((temp & 0x1) && (retries++ < 10000));
 if (retries >= 10000)
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "timed out wait for rate cntl %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , mst_enc->fe);
 return 0;
}
static int radeon_dp_mst_get_ddc_modes(struct drm_connector *connector)
{
 ...
 drm_mode_connector_update_edid_property(&radeon_connector->base, ((void *)0));
 return ret;
}
...
radeon_dp_mst_detect(struct drm_connector *connector, _Bool force)
{
 struct radeon_connector *radeon_connector = ({ const __typeof( ((struct radeon_connector *)0)->base ) *__mptr = (connector); (struct radeon_connector *)( (char *)__mptr - __builtin_offsetof(struct radeon_connector, base) );});
 struct radeon_connector *master = radeon_connector->mst_port;
...
static struct drm_connector *radeon_dp_add_mst_connector(struct drm_dp_mst_topology_mgr *mgr,
        ...
        const char *pathprop)
{
 struct radeon_connector *master = ({ const __typeof( ((struct radeon_connector *)0)->mst_mgr ) *__mptr = (mgr); (struct radeon_connector *)( (char *)__mptr - __builtin_offsetof(struct radeon_connector, mst_mgr) );});
 struct drm_device *dev = master->base.dev;
 struct radeon_connector *radeon_connector;
 struct drm_connector *connector;
 radeon_connector = kzalloc(sizeof(*radeon_connector), (0x0001 | 0x0004));
 if (!radeon_connector)
  return ((void *)0);
 radeon_connector->is_mst_connector = 1;
 connector = &radeon_connector->base;
 radeon_connector->port = port;
 radeon_connector->mst_port = master;
 do { } while( 0);
 drm_connector_init(dev, connector, &radeon_dp_mst_connector_funcs, 10);
...
void radeon_dp_mst_prepare_pll(struct drm_crtc *crtc, struct drm_display_mode *mode)
{
 ...
 struct radeon_device *rdev = dev->dev_private;
 struct radeon_encoder *radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (radeon_crtc->encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
...
radeon_mst_encoder_dpms(struct drm_encoder *encoder, int mode)
{
 ...
 if (!((rdev->family >= CHIP_BARTS))) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "got mst dpms on non-DCE5\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return;
 }
 ...
 case 0:
  ...
  if (dig_enc->active_mst_links == 1) {
   ...
   mst_enc->fe_from_be = 1;
   atombios_set_mst_encoder_crtc_source(encoder, mst_enc->fe);
   atombios_dig_encoder_setup(&primary->base, 0x0f, 0);
   atombios_dig_transmitter_setup2(&primary->base, 1,
   ...
   if (mst_enc->fe == -1)
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "failed to get frontend for dig encoder\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   mst_enc->fe_from_be = 0;
   atombios_set_mst_encoder_crtc_source(encoder, mst_enc->fe);
  }
  do { } while( 0);
  ...
  radeon_dp_mst_set_be_cntl(primary, mst_enc,
       radeon_connector->mst_port->hpd.hpd, 1);
  mst_enc->enc_active = 1;
  radeon_dp_mst_update_stream_attribs(radeon_connector->mst_port, primary);
  radeon_dp_mst_set_vcp_size(radeon_encoder, slots, 0);
  atombios_dig_encoder_setup2(&primary->base, 0x0d, 0,
         mst_enc->fe);
 ...
 case 2:
 case 3:
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "DPMS OFF %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , dig_enc->active_mst_links);
  if (!mst_enc->enc_active)
   return;
  ...
  mst_enc->enc_active = 0;
  radeon_dp_mst_update_stream_attribs(radeon_connector->mst_port, primary);
  radeon_dp_mst_set_be_cntl(primary, mst_enc,
       radeon_connector->mst_port->hpd.hpd, 0);
  atombios_dig_encoder_setup2(&primary->base, 0x0c, 0,
         mst_enc->fe);
...
static _Bool radeon_mst_mode_fixup(struct drm_encoder *encoder,
       const struct drm_display_mode *mode,
       struct drm_display_mode *adjusted_mode)
{
 struct radeon_encoder_mst *mst_enc;
...
static void radeon_mst_encoder_prepare(struct drm_encoder *encoder)
{
 ...
 radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
 radeon_mst_encoder_dpms(encoder, 3);
 mst_enc = radeon_encoder->enc_priv;
 primary = mst_enc->primary;
 dig_enc = primary->enc_priv;
...
{
 radeon_mst_encoder_dpms(encoder, 0);
 do { } while( 0);
}
...
radeon_dp_create_fake_mst_encoder(struct radeon_connector *connector)
{
 ...
 do { } while( 0);
 radeon_encoder = kzalloc(sizeof(*radeon_encoder), (0x0001 | 0x0004));
 if (!radeon_encoder)
  return ((void *)0);
 radeon_encoder->enc_priv = kzalloc(sizeof(*mst_enc), (0x0001 | 0x0004));
 ...
 mst_enc->primary = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (enc_master); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
...
radeon_dp_mst_probe(struct radeon_connector *radeon_connector)
{
 ...
 if (!((rdev->family >= CHIP_BARTS)))
  return 0;
 if (dig_connector->dpcd[0x000] < 0x12)
  return 0;
 ret = drm_dp_dpcd_read(&radeon_connector->ddc_bus->aux, 0x021, msg,
          1);
 if (ret) {
  if (msg[0] & (1 << 0)) {
   do { } while( 0);
