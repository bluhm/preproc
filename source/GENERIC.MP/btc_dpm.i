          ...
          u32 *sclk, u32 *mclk)
{
 ...
 if ((sclk == ((void *)0)) || (mclk == ((void *)0)))
  return;
 num_blacklist_clocks = (sizeof((btc_blacklist_clocks)) / sizeof((btc_blacklist_clocks)[0]));
...
static int btc_disable_ulv(struct radeon_device *rdev)
{
 ...
 if (eg_pi->ulv.supported) {
  if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x63)) != ((uint8_t)0x01))
   return -22;
 }
 return 0;
}
      ...
      RV770_SMC_STATETABLE *table)
{
 ...
 if (ulv_pl->vddc) {
  ret = cypress_convert_power_level_to_smc(rdev,
        ...
        0);
  if (ret == 0) {
   table->ULVState.levels[0].arbValue = 0x0a;
   ...
   table->ULVState.flags |= 0x01;
   r100_mm_wreg(rdev, (0x8c8), (0x00001450), 0);
   r100_mm_wreg(rdev, (0x8cc), (0x00040035), 0);
  }
 }
 return ret;
}
      ...
      const u32 *sequence, u32 count)
{
 ...
 for (i = 0; i < length; i+=3) {
  tmp = r100_mm_rreg(rdev, (sequence[i]), 0);
  tmp &= ~sequence[i+2];
  tmp |= sequence[i+1] & sequence[i+2];
  r100_mm_wreg(rdev, (sequence[i]), (tmp), 0);
 }
}
static void btc_cg_clock_gating_default(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_BARTS) {
  ...
  count = sizeof(barts_cgcg_cgls_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_TURKS) {
  p = (const u32 *)&turks_cgcg_cgls_default;
  count = sizeof(turks_cgcg_cgls_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_CAICOS) {
  p = (const u32 *)&caicos_cgcg_cgls_default;
  count = sizeof(caicos_cgcg_cgls_default) / (3 * sizeof(u32));
 } else
  return;
           ...
           _Bool enable)
{
 ...
 if (enable) {
  if (rdev->family == CHIP_BARTS) {
   ...
   count = sizeof(barts_cgcg_cgls_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_cgcg_cgls_enable;
   count = sizeof(turks_cgcg_cgls_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_cgcg_cgls_enable;
   count = sizeof(caicos_cgcg_cgls_enable) / (3 * sizeof(u32));
  } else
   return;
 } else {
  if (rdev->family == CHIP_BARTS) {
   p = (const u32 *)&barts_cgcg_cgls_disable;
   count = sizeof(barts_cgcg_cgls_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_cgcg_cgls_disable;
   count = sizeof(turks_cgcg_cgls_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_cgcg_cgls_disable;
   count = sizeof(caicos_cgcg_cgls_disable) / (3 * sizeof(u32));
  } else
   return;
 }
 btc_program_mgcg_hw_sequence(rdev, p, count);
...
static void btc_mg_clock_gating_default(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_BARTS) {
  ...
  count = sizeof(barts_mgcg_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_TURKS) {
  p = (const u32 *)&turks_mgcg_default;
  count = sizeof(turks_mgcg_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_CAICOS) {
  p = (const u32 *)&caicos_mgcg_default;
  count = sizeof(caicos_mgcg_default) / (3 * sizeof(u32));
 } else
  return;
 btc_program_mgcg_hw_sequence(rdev, p, count);
}
           ...
           _Bool enable)
{
 ...
 if (enable) {
  if (rdev->family == CHIP_BARTS) {
   ...
   count = sizeof(barts_mgcg_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_mgcg_enable;
   count = sizeof(turks_mgcg_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_mgcg_enable;
   count = sizeof(caicos_mgcg_enable) / (3 * sizeof(u32));
  } else
   return;
 } else {
  if (rdev->family == CHIP_BARTS) {
   p = (const u32 *)&barts_mgcg_disable[0];
   count = sizeof(barts_mgcg_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_mgcg_disable[0];
   count = sizeof(turks_mgcg_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_mgcg_disable[0];
   count = sizeof(caicos_mgcg_disable) / (3 * sizeof(u32));
  } else
   return;
 }
 btc_program_mgcg_hw_sequence(rdev, p, count);
...
static void btc_ls_clock_gating_default(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_BARTS) {
  ...
  count = sizeof(barts_sysls_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_TURKS) {
  p = (const u32 *)&turks_sysls_default;
  count = sizeof(turks_sysls_default) / (3 * sizeof(u32));
 } else if (rdev->family == CHIP_CAICOS) {
  p = (const u32 *)&caicos_sysls_default;
  count = sizeof(caicos_sysls_default) / (3 * sizeof(u32));
 } else
  return;
 btc_program_mgcg_hw_sequence(rdev, p, count);
}
           ...
           _Bool enable)
{
 ...
 if (enable) {
  if (rdev->family == CHIP_BARTS) {
   ...
   count = sizeof(barts_sysls_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_sysls_enable;
   count = sizeof(turks_sysls_enable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_sysls_enable;
   count = sizeof(caicos_sysls_enable) / (3 * sizeof(u32));
  } else
   return;
 } else {
  if (rdev->family == CHIP_BARTS) {
   p = (const u32 *)&barts_sysls_disable;
   count = sizeof(barts_sysls_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_TURKS) {
   p = (const u32 *)&turks_sysls_disable;
   count = sizeof(turks_sysls_disable) / (3 * sizeof(u32));
  } else if (rdev->family == CHIP_CAICOS) {
   p = (const u32 *)&caicos_sysls_disable;
   count = sizeof(caicos_sysls_disable) / (3 * sizeof(u32));
  } else
   return;
 }
 btc_program_mgcg_hw_sequence(rdev, p, count);
         ...
         struct radeon_ps *radeon_boot_state)
{
 ...
 if (eg_pi->sclk_deep_sleep)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x8c0), 0); tmp_ &= (~(0xff << 16)); tmp_ |= ((((32) << 16)) & ~(~(0xff << 16))); r100_mm_wreg(rdev, (0x8c0), (tmp_), 0); } while (0);
...
int btc_reset_to_default(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x84)) != ((uint8_t)0x01))
  return -22;
 return 0;
}
static void btc_stop_smc(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (((r100_mm_rreg(rdev, (0x6b28), 0) & (3 << 0)) >> 0) != 1)
   break;
  udelay(1);
 }
 udelay(100);
...
void btc_read_arb_registers(struct radeon_device *rdev)
{
 ...
 arb_registers->mc_arb_dram_timing = r100_mm_rreg(rdev, (0x2774), 0);
 arb_registers->mc_arb_dram_timing2 = r100_mm_rreg(rdev, (0x2778), 0);
 arb_registers->mc_arb_rfsh_rate = r100_mm_rreg(rdev, (0x27b0), 0);
 arb_registers->mc_arb_burst_time = r100_mm_rreg(rdev, (0x2808), 0);
}
static void btc_set_arb0_registers(struct radeon_device *rdev,
       struct evergreen_arb_registers *arb_registers)
{
 u32 val;
 r100_mm_wreg(rdev, (0x2774), (arb_registers->mc_arb_dram_timing), 0);
 r100_mm_wreg(rdev, (0x2778), (arb_registers->mc_arb_dram_timing2), 0);
 val = (arb_registers->mc_arb_rfsh_rate & (0xff << 0)) >>
  0;
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x27b0), 0); tmp_ &= (~(0xff << 0)); tmp_ |= ((((val) << 0)) & ~(~(0xff << 0))); r100_mm_wreg(rdev, (0x27b0), (tmp_), 0); } while (0);
 val = (arb_registers->mc_arb_burst_time & (0x1f << 0)) >>
  0;
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x2808), 0); tmp_ &= (~(0x1f << 0)); tmp_ |= ((((val) << 0)) & ~(~(0x1f << 0))); r100_mm_wreg(rdev, (0x2808), (tmp_), 0); } while (0);
}
...
static _Bool btc_is_state_ulv_compatible(struct radeon_device *rdev,
     struct radeon_ps *radeon_state)
{
...
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x62)) != ((uint8_t)0x01))
  return -22;
 return 0;
}
...
static _Bool btc_check_s0_mc_reg_index(u16 in_reg, u16 *out_reg)
{
 _Bool result = 1;
 switch (in_reg) {
 case 0x28a0 >> 2:
     ...
     struct evergreen_mc_reg_table *table)
{
 ...
 for (i = 0, j = table->last; i < table->last; i++) {
  ...
  case 0x2a04 >> 2:
   tmp = r100_mm_rreg(rdev, (0x2a0c), 0);
   ...
   if (j >= 16)
    return -22;
   tmp = r100_mm_rreg(rdev, (0x2aac), 0);
   ...
   break;
  case 0x2a08 >> 2:
   tmp = r100_mm_rreg(rdev, (0x2b44), 0);
...
static void btc_init_stutter_mode(struct radeon_device *rdev)
{
 ...
 if (pi->mclk_stutter_mode_threshold) {
  if (pi->mem_gddr5) {
   tmp = r100_mm_rreg(rdev, (0x28d4), 0);
   if ((0x200 & tmp) == 0) {
    tmp = (tmp & 0xfffffc0b) | 0x204;
    r100_mm_wreg(rdev, (0x28d4), (tmp), 0);
   }
  }
 }
}
...
int btc_dpm_set_power_state(struct radeon_device *rdev)
{
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "btc_set_power_state_conditionally_enable_ulv failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 return 0;
}
