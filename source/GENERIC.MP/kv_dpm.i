...
void kv_dpm_powergate_uvd(struct radeon_device *rdev, _Bool gate);
static void kv_dpm_powergate_vce(struct radeon_device *rdev, _Bool gate);
static void kv_dpm_powergate_samu(struct radeon_device *rdev, _Bool gate);
static void kv_dpm_powergate_acp(struct radeon_device *rdev, _Bool gate);
...
static void kv_do_enable_didt(struct radeon_device *rdev, _Bool enable)
{
...
static int kv_enable_didt(struct radeon_device *rdev, _Bool enable)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 int ret;
 if (pi->caps_sq_ramping ||
...
static int kv_enable_smc_cac(struct radeon_device *rdev, _Bool enable)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 int ret = 0;
 if (pi->caps_cac) {
  if (enable) {
   ret = kv_notify_message_to_smu(rdev, ((uint8_t)0x53));
   if (ret)
    pi->cac_enabled = 0;
   else
    pi->cac_enabled = 1;
  } else if (pi->cac_enabled) {
   kv_notify_message_to_smu(rdev, ((uint8_t)0x54));
   pi->cac_enabled = 0;
  }
 }
 return ret;
...
static int kv_process_firmware_header(struct radeon_device *rdev)
{
 ...
 u32 tmp;
 int ret;
 ret = kv_read_smc_sram_dword(rdev, 0x20000 +
...
static int kv_enable_dpm_voltage_scaling(struct radeon_device *rdev)
{
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsVoltageChangeEnable),
...
static int kv_set_dpm_interval(struct radeon_device *rdev)
{
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsInterval),
...
static int kv_set_dpm_boot_state(struct radeon_device *rdev)
{
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsBootLevel),
...
{
 tn_smc_wreg(rdev, (0xC02001A8), (0x3FFFC100));
}
    ...
    u32 index, u32 sclk)
{
 ...
 pi->graphics_level[index].SclkFrequency = ((__uint32_t)(sclk));
 return 0;
}
...
static int kv_set_vid(struct radeon_device *rdev, u32 index, u32 vid)
{
 ...
 pi->graphics_level[index].MinVddNb =
  ((__uint32_t)(kv_convert_2bit_index_to_voltage(rdev, vid)));
 return 0;
}
static int kv_set_at(struct radeon_device *rdev, u32 index, u32 at)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 pi->graphics_level[index].AT = ((__uint16_t)((u16)at));
 return 0;
}
...
{
 u32 sclk_pwrmgt_cntl = tn_smc_rreg(rdev, (0xC0200008));
 sclk_pwrmgt_cntl &= ~((1 << 5) | (1 << 4));
 sclk_pwrmgt_cntl |= (1 << 21);
 tn_smc_wreg(rdev, (0xC0200008), (sclk_pwrmgt_cntl));
}
...
static int kv_freeze_sclk_dpm(struct radeon_device *rdev, _Bool freeze)
{
 return kv_notify_message_to_smu(rdev, freeze ?
     ((uint16_t) 0x189) : ((uint16_t) 0x18A));
}
...
static int kv_unforce_levels(struct radeon_device *rdev)
{
 if (rdev->family == CHIP_KABINI || rdev->family == CHIP_MULLINS)
  return kv_notify_message_to_smu(rdev, ((uint8_t)0x41));
 else
...
static int kv_update_sclk_t(struct radeon_device *rdev)
{
 ...
 if (pi->caps_sclk_throttle_low_notification) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, LowSclkInterruptT),
...
static int kv_enable_auto_thermal_throttling(struct radeon_device *rdev)
{
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsThermThrottleEnable),
...
static int kv_upload_dpm_settings(struct radeon_device *rdev)
{
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsLevel),
       (u8 *)&pi->graphics_level,
       sizeof(SMU7_Fusion_GraphicsLevel) * 8,
       pi->sram_end);
 if (ret)
  return ret;
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, GraphicsDpmLevelCount),
...
static int kv_populate_uvd_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < table->count; i++) {
  if (pi->high_voltage_t &&
   ...
   break;
  pi->uvd_level[i].VclkFrequency = ((__uint32_t)(table->entries[i].vclk));
  pi->uvd_level[i].DclkFrequency = ((__uint32_t)(table->entries[i].dclk));
  pi->uvd_level[i].MinVddNb = ((__uint16_t)(table->entries[i].v));
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, UvdLevelCount),
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, UVDInterval),
       &pi->uvd_interval,
       sizeof(u8), pi->sram_end);
 if (ret)
  return ret;
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, UvdLevel),
       (u8 *)&pi->uvd_level,
       sizeof(SMU7_Fusion_UvdLevel) * 8,
       pi->sram_end);
 return ret;
}
static int kv_populate_vce_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < table->count; i++) {
  if (pi->high_voltage_t &&
      pi->high_voltage_t < table->entries[i].v)
   break;
  pi->vce_level[i].Frequency = ((__uint32_t)(table->entries[i].evclk));
  pi->vce_level[i].MinVoltage = ((__uint16_t)(table->entries[i].v));
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, VceLevelCount),
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, VCEInterval),
       (u8 *)&pi->vce_interval,
       sizeof(u8),
       pi->sram_end);
 if (ret)
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, VceLevel),
       (u8 *)&pi->vce_level,
       sizeof(SMU7_Fusion_ExtClkLevel) * 8,
       pi->sram_end);
 return ret;
}
static int kv_populate_samu_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < table->count; i++) {
  if (pi->high_voltage_t &&
      pi->high_voltage_t < table->entries[i].v)
   break;
  pi->samu_level[i].Frequency = ((__uint32_t)(table->entries[i].clk));
  pi->samu_level[i].MinVoltage = ((__uint16_t)(table->entries[i].v));
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, SamuLevelCount),
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, SAMUInterval),
       (u8 *)&pi->samu_interval,
       sizeof(u8),
       pi->sram_end);
 if (ret)
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, SamuLevel),
       (u8 *)&pi->samu_level,
       sizeof(SMU7_Fusion_ExtClkLevel) * 8,
       pi->sram_end);
 if (ret)
...
static int kv_populate_acp_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < table->count; i++) {
  pi->acp_level[i].Frequency = ((__uint32_t)(table->entries[i].clk));
  pi->acp_level[i].MinVoltage = ((__uint16_t)(table->entries[i].v));
  ret = radeon_atom_get_clock_dividers(rdev, 2,
           table->entries[i].clk, 0, &dividers);
  if (ret)
   return ret;
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, AcpLevelCount),
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, ACPInterval),
       (u8 *)&pi->acp_interval,
       sizeof(u8),
       pi->sram_end);
 if (ret)
 ...
 ret = kv_copy_bytes_to_smc(rdev,
       pi->dpm_table_start +
       __builtin_offsetof(SMU7_Fusion_DpmTable, AcpLevel),
       (u8 *)&pi->acp_level,
       sizeof(SMU7_Fusion_ExtClkLevel) * 8,
       pi->sram_end);
 if (ret)
...
static int kv_enable_ulv(struct radeon_device *rdev, _Bool enable)
{
 return kv_notify_message_to_smu(rdev, enable ?
     ((uint8_t)0x62) : ((uint8_t)0x63));
}
...
void kv_dpm_enable_bapm(struct radeon_device *rdev, _Bool enable)
{
 ...
 if (pi->bapm_enable) {
  ...
  if (ret)
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_smc_bapm_enable failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 }
}
static void kv_enable_thermal_int(struct radeon_device *rdev, _Bool enable)
{
 u32 thermal_int;
 thermal_int = tn_smc_rreg(rdev, (0xC2100028));
 if (enable)
  thermal_int |= (1 << 24) | (1 << 25);
 else
  thermal_int &= ~((1 << 24) | (1 << 25));
 tn_smc_wreg(rdev, (0xC2100028), (thermal_int));
}
int kv_dpm_enable(struct radeon_device *rdev)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_process_firmware_header failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_program_bootup_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 kv_calculate_dfs_bypass_settings(rdev);
 ret = kv_upload_dpm_settings(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_upload_dpm_settings failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_populate_uvd_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_populate_uvd_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_populate_vce_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_populate_vce_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_populate_samu_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_populate_samu_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_populate_acp_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_populate_acp_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 kv_program_vc(rdev);
 kv_start_am(rdev);
 if (pi->enable_auto_thermal_throttling) {
  ...
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_enable_auto_thermal_throttling failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 ret = kv_enable_dpm_voltage_scaling(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_enable_dpm_voltage_scaling failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_set_dpm_interval(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_set_dpm_interval failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_set_dpm_boot_state(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_set_dpm_boot_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_enable_ulv(rdev, 1);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_enable_ulv failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 kv_start_dpm(rdev);
 ret = kv_enable_didt(rdev, 1);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_enable_didt failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = kv_enable_smc_cac(rdev, 1);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_enable_smc_cac failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
...
int kv_dpm_late_enable(struct radeon_device *rdev)
{
 ...
 if (rdev->irq.installed &&
  ...
  ret = kv_set_thermal_temperature_range(rdev, (90 * 1000), (120 * 1000));
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_set_thermal_temperature_range failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
...
static int kv_init_fps_limits(struct radeon_device *rdev)
{
 ...
 if (pi->caps_fps) {
  u16 tmp;
  tmp = 45;
  pi->fps_high_t = ((__uint16_t)(tmp));
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, FpsHighT),
        (u8 *)&pi->fps_high_t,
        sizeof(u16), pi->sram_end);
  tmp = 30;
  pi->fps_low_t = ((__uint16_t)(tmp));
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, FpsLowT),
        (u8 *)&pi->fps_low_t,
        sizeof(u16), pi->sram_end);
 }
 return ret;
...
static int kv_enable_uvd_dpm(struct radeon_device *rdev, _Bool enable)
{
 return kv_notify_message_to_smu(rdev, enable ?
     ((uint16_t) 0x154) : ((uint16_t) 0x155));
}
static int kv_enable_vce_dpm(struct radeon_device *rdev, _Bool enable)
{
 return kv_notify_message_to_smu(rdev, enable ?
     ((uint16_t) 0x15a) : ((uint16_t) 0x15b));
}
static int kv_enable_samu_dpm(struct radeon_device *rdev, _Bool enable)
{
 return kv_notify_message_to_smu(rdev, enable ?
     ((uint16_t) 0x156) : ((uint16_t) 0x157));
}
static int kv_enable_acp_dpm(struct radeon_device *rdev, _Bool enable)
{
 return kv_notify_message_to_smu(rdev, enable ?
     ((uint16_t) 0x158) : ((uint16_t) 0x159));
}
static int kv_update_uvd_dpm(struct radeon_device *rdev, _Bool gate)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 struct radeon_uvd_clock_voltage_dependency_table *table =
  &rdev->pm.dpm.dyn_state.uvd_clock_voltage_dependency_table;
 ...
 if (!gate) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, UvdBootLevel),
        ...
        struct radeon_ps *radeon_current_state)
{
 ...
 if (radeon_new_state->evclk > 0 && radeon_current_state->evclk == 0) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, VceBootLevel),
...
static int kv_update_samu_dpm(struct radeon_device *rdev, _Bool gate)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 struct radeon_clock_voltage_dependency_table *table =
  &rdev->pm.dpm.dyn_state.samu_clock_voltage_dependency_table;
 ...
 if (!gate) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, SamuBootLevel),
...
static int kv_update_acp_dpm(struct radeon_device *rdev, _Bool gate)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 struct radeon_clock_voltage_dependency_table *table =
  &rdev->pm.dpm.dyn_state.acp_clock_voltage_dependency_table;
 ...
 if (!gate) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        pi->dpm_table_start +
        __builtin_offsetof(SMU7_Fusion_DpmTable, AcpBootLevel),
...
void kv_dpm_powergate_uvd(struct radeon_device *rdev, _Bool gate)
{
 ...
 if (gate) {
  if (pi->caps_uvd_pg) {
   uvd_v1_0_stop(rdev);
   cik_update_cg(rdev, (1 << 3), 0);
  }
  kv_update_uvd_dpm(rdev, gate);
  if (pi->caps_uvd_pg)
   kv_notify_message_to_smu(rdev, ((uint8_t)0x60));
 } else {
  if (pi->caps_uvd_pg) {
   kv_notify_message_to_smu(rdev, ((uint8_t)0x61));
   uvd_v4_2_resume(rdev);
   uvd_v1_0_start(rdev);
   cik_update_cg(rdev, (1 << 3), 1);
  }
  kv_update_uvd_dpm(rdev, gate);
 }
}
static void kv_dpm_powergate_vce(struct radeon_device *rdev, _Bool gate)
{
 ...
 if (gate) {
  if (pi->caps_vce_pg) {
   kv_notify_message_to_smu(rdev, ((uint32_t) 0x10e));
  }
 } else {
  if (pi->caps_vce_pg) {
   kv_notify_message_to_smu(rdev, ((uint32_t) 0x10f));
...
static void kv_dpm_powergate_samu(struct radeon_device *rdev, _Bool gate)
{
 ...
 if (gate) {
  ...
  if (pi->caps_samu_pg)
   kv_notify_message_to_smu(rdev, ((uint16_t) 0x139));
 } else {
  if (pi->caps_samu_pg)
   kv_notify_message_to_smu(rdev, ((uint16_t) 0x13a));
...
static void kv_dpm_powergate_acp(struct radeon_device *rdev, _Bool gate)
{
 ...
 if (gate) {
  ...
  if (pi->caps_acp_pg)
   kv_notify_message_to_smu(rdev, ((uint16_t) 0x137));
 } else {
  if (pi->caps_acp_pg)
   kv_notify_message_to_smu(rdev, ((uint16_t) 0x138));
 ...
 if (pi->caps_enable_dfs_bypass) {
  ...
  ret = kv_copy_bytes_to_smc(rdev,
        (pi->dpm_table_start +
         ...
         __builtin_offsetof(SMU7_Fusion_GraphicsLevel, ClkBypassCntl)),
        &clk_bypass_cntl,
        sizeof(u8), pi->sram_end);
 }
 return ret;
       ...
       _Bool enable)
{
 ...
 if (enable) {
  if (pi->enable_nb_dpm && !pi->nb_dpm_enabled) {
   ret = kv_notify_message_to_smu(rdev, ((uint16_t) 0x140));
   if (ret == 0)
    pi->nb_dpm_enabled = 1;
  }
 } else {
  if (pi->enable_nb_dpm && pi->nb_dpm_enabled) {
   ret = kv_notify_message_to_smu(rdev, ((uint16_t) 0x141));
   if (ret == 0)
    pi->nb_dpm_enabled = 0;
  }
 }
...
int kv_dpm_set_power_state(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_KABINI || rdev->family == CHIP_MULLINS) {
  if (pi->enable_dpm) {
   ...
   if (ret) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_calculate_ds_divider failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
    return ret;
   }
   ...
   if (ret) {
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "kv_update_vce_dpm failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
    return ret;
   }
...
static int kv_force_dpm_highest(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_KABINI || rdev->family == CHIP_MULLINS)
  return kv_send_msg_to_smc_with_parameter(rdev, ((uint32_t) 0x104), i);
 else
 ...
 for (i = pi->lowest_valid; i <= pi->highest_valid; i++) {
  pi->graphics_level[i].DeepSleepDivId =
   kv_get_sleep_divider_id_from_clock(rdev,
          ((__uint32_t)(pi->graphics_level[i].SclkFrequency)),
          sclk_in_sr);
 }
 return 0;
}
        ...
        struct radeon_ps *new_rps)
{
 ...
 if (rdev->family == CHIP_KABINI || rdev->family == CHIP_MULLINS)
  return;
 if (pi->sys_info.nb_dpm_enable) {
  nbdpmconfig1 = tn_smc_rreg(rdev, (0x3F9E8));
  nbdpmconfig1 &= ~(0x000000ff | 0x0000ff00 |
      0x00ff0000 | 0xff000000);
  nbdpmconfig1 |= (((new_ps->dpm0_pg_nb_ps_lo) << 0) |
     ((new_ps->dpm0_pg_nb_ps_hi) << 8) |
     ((new_ps->dpmx_nb_ps_lo) << 16) |
     ((new_ps->dpmx_nb_ps_hi) << 24));
  tn_smc_wreg(rdev, (0x3F9E8), (nbdpmconfig1));
 }
}
static int kv_set_thermal_temperature_range(struct radeon_device *rdev,
         int min_temp, int max_temp)
{
 ...
 tmp = tn_smc_rreg(rdev, (0xC2100028));
 tmp &= ~(0x000000FF | 0x0000FF00);
 tmp |= (((49 + (high_temp / 1000)) << 0) |
  ((49 + (low_temp / 1000)) << 8));
 tn_smc_wreg(rdev, (0xC2100028), (tmp));
...
static int kv_parse_sys_info_table(struct radeon_device *rdev)
{
 ...
 if (atom_parse_data_header(mode_info->atom_context, index, ((void *)0),
  ...
  pi->sys_info.bootup_sclk = (__uint32_t)(__builtin_constant_p(igp_info->info_8.ulBootUpEngineClock) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulBootUpEngineClock) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulBootUpEngineClock) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulBootUpEngineClock) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulBootUpEngineClock) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulBootUpEngineClock));
  pi->sys_info.bootup_uma_clk = (__uint32_t)(__builtin_constant_p(igp_info->info_8.ulBootUpUMAClock) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulBootUpUMAClock) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulBootUpUMAClock) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulBootUpUMAClock) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulBootUpUMAClock) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulBootUpUMAClock));
  pi->sys_info.bootup_nb_voltage_index =
   (__uint16_t)(__builtin_constant_p(igp_info->info_8.usBootUpNBVoltage) ? (__uint16_t)(((__uint16_t)(igp_info->info_8.usBootUpNBVoltage) & 0xffU) << 8 | ((__uint16_t)(igp_info->info_8.usBootUpNBVoltage) & 0xff00U) >> 8) : __swap16md(igp_info->info_8.usBootUpNBVoltage));
  ...
  if ((__uint32_t)(__builtin_constant_p(igp_info->info_8.ulSystemConfig) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulSystemConfig) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulSystemConfig) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulSystemConfig) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulSystemConfig) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulSystemConfig)) & (1 << 3))
   pi->sys_info.nb_dpm_enable = 1;
  else
   pi->sys_info.nb_dpm_enable = 0;
  for (i = 0; i < 4; i++) {
   pi->sys_info.nbp_memory_clock[i] =
    (__uint32_t)(__builtin_constant_p(igp_info->info_8.ulNbpStateMemclkFreq[i]) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulNbpStateMemclkFreq[i]) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulNbpStateMemclkFreq[i]) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulNbpStateMemclkFreq[i]) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulNbpStateMemclkFreq[i]) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulNbpStateMemclkFreq[i]));
   pi->sys_info.nbp_n_clock[i] =
    (__uint32_t)(__builtin_constant_p(igp_info->info_8.ulNbpStateNClkFreq[i]) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulNbpStateNClkFreq[i]) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulNbpStateNClkFreq[i]) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulNbpStateNClkFreq[i]) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulNbpStateNClkFreq[i]) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulNbpStateNClkFreq[i]));
  }
  if ((__uint32_t)(__builtin_constant_p(igp_info->info_8.ulGPUCapInfo) ? (__uint32_t)(((__uint32_t)(igp_info->info_8.ulGPUCapInfo) & 0xff) << 24 | ((__uint32_t)(igp_info->info_8.ulGPUCapInfo) & 0xff00) << 8 | ((__uint32_t)(igp_info->info_8.ulGPUCapInfo) & 0xff0000) >> 8 | ((__uint32_t)(igp_info->info_8.ulGPUCapInfo) & 0xff000000) >> 24) : __swap32md(igp_info->info_8.ulGPUCapInfo)) &
      0x10)
...
static int kv_parse_power_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < state_array->ucNumEntries; i++) {
  ...
  if (!rdev->pm.power_state[i].clock_info)
   return -22;
  ps = kzalloc(sizeof(struct kv_ps), (0x0001 | 0x0004));
  if (ps == ((void *)0)) {
...
int kv_dpm_init(struct radeon_device *rdev)
{
 ...
 int ret, i;
 pi = kzalloc(sizeof(struct kv_power_info), (0x0001 | 0x0004));
 if (pi == ((void *)0))
  return -12;
 rdev->pm.dpm.priv = pi;
          ...
          struct seq_file *m)
{
 struct kv_power_info *pi = kv_get_pi(rdev);
 u32 current_index =
  (tn_smc_rreg(rdev, (0xC0200014)) & (0x1f << 16)) >>
  16;
 u32 sclk, tmp;
 u16 vddc;
 if (current_index >= 8) {
  ...
  sclk = ((__uint32_t)(pi->graphics_level[current_index].SclkFrequency));
  tmp = (tn_smc_rreg(rdev, (0xC0200094)) & (0xff << 1)) >>
   1;
