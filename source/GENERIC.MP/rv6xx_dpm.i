...
static void rv6xx_force_pcie_gen1(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (!(rdev->pciep_rreg(rdev, (0xa4)) & (1 << 11)))
   break;
  udelay(1);
 }
 tmp = rdev->pciep_rreg(rdev, (0xa4));
...
{
 u32 tmp;
 tmp = rdev->pciep_rreg(rdev, (0xa4)) & ~(3 << 12);
 if (enable)
  tmp |= ((1) << 12);
 else
  tmp |= ((0) << 12);
...
static struct rv6xx_sclk_stepping rv6xx_next_vco_step(struct radeon_device *rdev,
            ...
            _Bool increasing_vco, u32 step_size)
{
...
static _Bool rv6xx_can_step_post_div(struct radeon_device *rdev,
        struct rv6xx_sclk_stepping *cur,
...
static _Bool rv6xx_reached_stepping_target(struct radeon_device *rdev,
       struct rv6xx_sclk_stepping *cur,
       struct rv6xx_sclk_stepping *target,
       _Bool increasing_vco)
{
                                 ...
                                 u32 start_index, u8 *end_index)
{
 ...
 while (1) {
  ...
  if (rv6xx_can_step_post_div(rdev, &cur, &target))
   ...
   next = rv6xx_next_vco_step(rdev, &cur, increasing_vco, 20);
  if (rv6xx_reached_stepping_target(rdev, &next, &target, increasing_vco)) {
   struct rv6xx_sclk_stepping tiny =
    rv6xx_next_vco_step(rdev, &target, !increasing_vco, 5);
...
static void rv6xx_set_engine_spread_spectrum_clk_s(struct radeon_device *rdev,
         u32 index, u32 clk_s)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x820 + (index * 4)), 0); tmp_ &= (~(0xff << 3)); tmp_ |= ((((clk_s) << 3)) & ~(~(0xff << 3))); r100_mm_wreg(rdev, (0x820 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_set_engine_spread_spectrum_clk_v(struct radeon_device *rdev,
         u32 index, u32 clk_v)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x820 + (index * 4)), 0); tmp_ &= (~(0x7ff << 11)); tmp_ |= ((((clk_v) << 11)) & ~(~(0x7ff << 11))); r100_mm_wreg(rdev, (0x820 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_enable_engine_spread_spectrum(struct radeon_device *rdev,
      u32 index, _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x820 + (index * 4)), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x820 + (index * 4)), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x820 + (index * 4)), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x820 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_set_memory_spread_spectrum_clk_s(struct radeon_device *rdev,
         u32 clk_s)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x830), 0); tmp_ &= (~(0xff << 3)); tmp_ |= ((((clk_s) << 3)) & ~(~(0xff << 3))); r100_mm_wreg(rdev, (0x830), (tmp_), 0); } while (0);
}
static void rv6xx_set_memory_spread_spectrum_clk_v(struct radeon_device *rdev,
         u32 clk_v)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x830), 0); tmp_ &= (~(0x7ff << 11)); tmp_ |= ((((clk_v) << 11)) & ~(~(0x7ff << 11))); r100_mm_wreg(rdev, (0x830), (tmp_), 0); } while (0);
}
static void rv6xx_enable_memory_spread_spectrum(struct radeon_device *rdev,
      _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x830), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x830), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x830), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x830), (tmp_), 0); } while (0);
}
static void rv6xx_enable_dynamic_spread_spectrum(struct radeon_device *rdev,
       _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 21)); tmp_ |= (((1 << 21)) & ~(~(1 << 21))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 21)); tmp_ |= ((0) & ~(~(1 << 21))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
static void rv6xx_memory_clock_entry_enable_post_divider(struct radeon_device *rdev,
        u32 index, _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6e8 + (index * 4)), 0); tmp_ &= (~(1 << 28)); tmp_ |= (((1 << 28)) & ~(~(1 << 28))); r100_mm_wreg(rdev, (0x6e8 + (index * 4)), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6e8 + (index * 4)), 0); tmp_ &= (~(1 << 28)); tmp_ |= ((0) & ~(~(1 << 28))); r100_mm_wreg(rdev, (0x6e8 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_memory_clock_entry_set_post_divider(struct radeon_device *rdev,
            u32 index, u32 divider)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6e8 + (index * 4)), 0); tmp_ &= (~(0xff << 0)); tmp_ |= ((((divider) << 0)) & ~(~(0xff << 0))); r100_mm_wreg(rdev, (0x6e8 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_memory_clock_entry_set_feedback_divider(struct radeon_device *rdev,
         u32 index, u32 divider)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6e8 + (index * 4)), 0); tmp_ &= (~(0xfff << 8)); tmp_ |= ((((divider) << 8)) & ~(~(0xfff << 8))); r100_mm_wreg(rdev, (0x6e8 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_memory_clock_entry_set_reference_divider(struct radeon_device *rdev,
          u32 index, u32 divider)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6e8 + (index * 4)), 0); tmp_ &= (~(0x3f << 20)); tmp_ |= ((((divider) << 20)) & ~(~(0x3f << 20))); r100_mm_wreg(rdev, (0x6e8 + (index * 4)), (tmp_), 0); } while (0);
}
static void rv6xx_vid_response_set_brt(struct radeon_device *rdev, u32 rt)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6f8), 0); tmp_ &= (~(0xff << 24)); tmp_ |= ((((rt) << 24)) & ~(~(0xff << 24))); r100_mm_wreg(rdev, (0x6f8), (tmp_), 0); } while (0);
}
static void rv6xx_enable_engine_feedback_and_reference_sync(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x60c), 0); tmp_ &= (~(1 << 5)); tmp_ |= (((1 << 5)) & ~(~(1 << 5))); r100_mm_wreg(rdev, (0x60c), (tmp_), 0); } while (0);
}
       ...
       u32 clock, enum r600_power_level level)
{
 ...
 if (clock && pi->sclk_ss) {
  if (radeon_atom_get_clock_dividers(rdev, 2, clock, 0, &dividers) == 0) {
...
{
 struct atom_clock_dividers dividers;
 if (radeon_atom_get_clock_dividers(rdev, 1, clock, 0, &dividers))
     return -22;
           ...
           u32 *vco_freq)
{
 ...
 u32 vco_freq_temp;
 if (radeon_atom_get_clock_dividers(rdev, 1,
...
static void rv6xx_program_backbias_stepping_parameters_except_lowest_entry(struct radeon_device *rdev)
{
 struct rv6xx_power_info *pi = rv6xx_get_pi(rdev);
 if (pi->hw.backbias[1])
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 14)); tmp_ |= (((1 << 14)) & ~(~(1 << 14))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 14)); tmp_ |= ((0) & ~(~(1 << 14))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
 if (pi->hw.backbias[2])
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 13)); tmp_ |= (((1 << 13)) & ~(~(1 << 13))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 13)); tmp_ |= ((0) & ~(~(1 << 13))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
}
...
static void rv6xx_program_backbias_stepping_parameters_lowest_entry(struct radeon_device *rdev)
{
 struct rv6xx_power_info *pi = rv6xx_get_pi(rdev);
 if (pi->hw.backbias[0])
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 15)); tmp_ |= (((1 << 15)) & ~(~(1 << 15))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x740), 0); tmp_ &= (~(1 << 15)); tmp_ |= ((0) & ~(~(1 << 15))); r100_mm_wreg(rdev, (0x740), (tmp_), 0); } while (0);
}
      ...
      u32 engine_clock)
{
 ...
 u32 tmp;
 tmp = (r100_mm_rreg(rdev, (0x2408), 0) & 0x0000001C) >> 2;
 dram_rows = 1 << (tmp + 10);
 dram_refresh_rate = 1 << ((r100_mm_rreg(rdev, (0x2894), 0) & 0x3) + 3);
...
static void rv6xx_program_memory_timing_parameters(struct radeon_device *rdev)
{
 ...
 sqm_ratio = (((64 * high_clock / pi->hw.sclks[R600_POWER_LEVEL_LOW]) << 0) |
       ((64 * high_clock / pi->hw.sclks[R600_POWER_LEVEL_MEDIUM]) << 8) |
       ((64 * high_clock / pi->hw.sclks[R600_POWER_LEVEL_HIGH]) << 16) |
       ((64 * high_clock / pi->hw.sclks[R600_POWER_LEVEL_HIGH]) << 24));
 r100_mm_wreg(rdev, (0x2424), (sqm_ratio), 0);
 arb_refresh_rate =
  (((calculate_memory_refresh_rate(rdev, pi->hw.sclks[R600_POWER_LEVEL_LOW])) << 0) |
   ((calculate_memory_refresh_rate(rdev, pi->hw.sclks[R600_POWER_LEVEL_MEDIUM])) << 8) |
   ((calculate_memory_refresh_rate(rdev, pi->hw.sclks[R600_POWER_LEVEL_HIGH])) << 16) |
   ((calculate_memory_refresh_rate(rdev, pi->hw.sclks[R600_POWER_LEVEL_HIGH])) << 24));
 r100_mm_wreg(rdev, (0x2464), (arb_refresh_rate), 0);
}
static void rv6xx_program_mpll_timing_parameters(struct radeon_device *rdev)
{
 struct rv6xx_power_info *pi = rv6xx_get_pi(rdev);
 r600_set_mpll_lock_time(rdev, 100 *
    pi->mpll_ref_div);
 r600_set_mpll_reset_time(rdev, 150);
}
static void rv6xx_program_bsp(struct radeon_device *rdev)
{
 struct rv6xx_power_info *pi = rv6xx_get_pi(rdev);
 u32 ref_clk = rdev->clock.spll.reference_freq;
 r600_calculate_u_and_p(10000,
          ref_clk, 16,
          &pi->bsp,
          &pi->bsu);
...
static void rv6xx_enable_display_gap(struct radeon_device *rdev, _Bool enable)
{
 if (enable) {
  u32 tmp = (((R600_PM_DISPLAY_GAP_VBLANK_OR_WM) << 0) |
      ((R600_PM_DISPLAY_GAP_VBLANK_OR_WM) << 2) |
      ((R600_PM_DISPLAY_GAP_IGNORE) << 24) |
      ((R600_PM_DISPLAY_GAP_IGNORE) << 26) |
      ((0x3FFF) << 4) |
      ((7) << 20));
  r100_mm_wreg(rdev, (0x7dc), (tmp), 0);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x624), 0); tmp_ &= (~(1 << 25)); tmp_ |= (((1 << 25)) & ~(~(1 << 25))); r100_mm_wreg(rdev, (0x624), (tmp_), 0); } while (0);
 } else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x624), 0); tmp_ &= (~(1 << 25)); tmp_ |= ((0) & ~(~(1 << 25))); r100_mm_wreg(rdev, (0x624), (tmp_), 0); } while (0);
}
...
static void rv6xx_enable_backbias(struct radeon_device *rdev, _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~((1 << 16) | (1 << 18))); tmp_ |= (((1 << 16) | (1 << 18)) & ~(~((1 << 16) | (1 << 18)))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~((1 << 17) | (1 << 16) | (1 << 18))); tmp_ |= ((0) & ~(~((1 << 17) | (1 << 16) | (1 << 18)))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
static void rv6xx_program_display_gap(struct radeon_device *rdev)
{
 u32 tmp = r100_mm_rreg(rdev, (0x7dc), 0);
 tmp &= ~((3 << 24) | (3 << 26));
      ...
      struct radeon_ps *old_ps)
{
 ...
 rv6xx_program_voltage_stepping_entry(rdev, R600_POWER_LEVEL_CTXSW,
          safe_voltage);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(3 << 6)); tmp_ |= ((((R600_POWER_LEVEL_CTXSW) << 6)) & ~(~(3 << 6))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
        ...
        struct radeon_ps *old_ps)
{
 struct rv6xx_ps *new_state = rv6xx_get_ps(new_ps);
 struct rv6xx_ps *old_state = rv6xx_get_ps(old_ps);
 if ((new_state->low.flags & 4) &&
     (old_state->low.flags & 4))
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 17)); tmp_ |= (((1 << 17)) & ~(~(1 << 17))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 17)); tmp_ |= ((0) & ~(~(1 << 17))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
         ...
         struct radeon_ps *old_ps)
{
 struct rv6xx_ps *new_state = rv6xx_get_ps(new_ps);
 struct rv6xx_ps *old_state = rv6xx_get_ps(old_ps);
 if ((new_state->low.flags & 1) !=
     (old_state->low.flags & 1))
...
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 10)); tmp_ |= (((1 << 10)) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 10)); tmp_ |= ((0) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
static void rv6xx_enable_dynamic_backbias_control(struct radeon_device *rdev,
        _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 18)); tmp_ |= (((1 << 18)) & ~(~(1 << 18))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x618), 0); tmp_ &= (~(1 << 18)); tmp_ |= ((0) & ~(~(1 << 18))); r100_mm_wreg(rdev, (0x618), (tmp_), 0); } while (0);
}
     ...
     u16 target_voltage)
{
 ...
 if ((radeon_atom_get_voltage_step(rdev, 1,
       &voltage_step)) ||
     (radeon_atom_round_to_true_voltage(rdev, 1,
 ...
 while (current_voltage != true_target_voltage) {
  ...
  rv6xx_program_voltage_stepping_entry(rdev, R600_POWER_LEVEL_CTXSW,
           current_voltage);
  mdelay((rdev->pm.dpm.voltage_response_time + 999) / 1000);
 }
 return 0;
}
...
static void rv6xx_set_dpm_event_sources(struct radeon_device *rdev, u32 sources)
{
 ...
 if (want_thermal_protection) {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x7f0), 0); tmp_ &= (~(7 << 0)); tmp_ |= ((((dpm_event_src) << 0)) & ~(~(7 << 0))); r100_mm_wreg(rdev, (0x7f0), (tmp_), 0); } while (0);
...
static void rv6xx_reset_lvtm_data_sync(struct radeon_device *rdev)
{
 if (((rdev->family >= CHIP_RV620)))
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x7f98), 0); tmp_ &= (~(1 << 8)); tmp_ |= (((1 << 8)) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0x7f98), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x7adc), 0); tmp_ &= (~(1 << 8)); tmp_ |= (((1 << 8)) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0x7adc), (tmp_), 0); } while (0);
}
...
static int rv6xx_parse_power_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < power_info->pplib.ucNumStates; i++) {
  ...
  if (power_info->pplib.ucStateEntrySize - 1) {
   u8 *idx;
   ps = kzalloc(sizeof(struct rv6xx_ps), (0x0001 | 0x0004));
   if (ps == ((void *)0)) {
...
int rv6xx_dpm_init(struct radeon_device *rdev)
{
 ...
 int ret;
 pi = kzalloc(sizeof(struct rv6xx_power_info), (0x0001 | 0x0004));
 if (pi == ((void *)0))
  return -12;
 rdev->pm.dpm.priv = pi;
