...
strbuf_reset(struct strbuf_ctl *sb)
{
 ...
 if (bus_space_read_8(sb->sb_bustag, sb->sb_sb,
  ...
  if (pmap_extract((&kernel_pmap_),
   ...
   sb->sb_flush = ((void *)0);
  do { (void)(((void *)0)); (void)(0); __mtx_init((&sb->sb_mtx), ((((15)) > 0 && ((15)) < 12) ? 12 : ((15)))); } while (0);
 }
}
void
...
void iommu_page_tree_SPLAY(struct iommu_page_tree *, struct iommu_page_entry *); void iommu_page_tree_SPLAY_MINMAX(struct iommu_page_tree *, int); struct iommu_page_entry *iommu_page_tree_SPLAY_INSERT(struct iommu_page_tree *, struct iommu_page_entry *); struct iommu_page_entry *iommu_page_tree_SPLAY_REMOVE(struct iommu_page_tree *, struct iommu_page_entry *); static __attribute__((__unused__)) __inline struct iommu_page_entry * iommu_page_tree_SPLAY_FIND(struct iommu_page_tree *head, struct iommu_page_entry *elm) { if (((head)->sph_root == ((void *)0))) return(((void *)0)); iommu_page_tree_SPLAY(head, elm); if ((iomap_compare)(elm, (head)->sph_root) == 0) return (head->sph_root); return (((void *)0)); } static __attribute__((__unused__)) __inline struct iommu_page_entry * iommu_page_tree_SPLAY_NEXT(struct iommu_page_tree *head, struct iommu_page_entry *elm) { iommu_page_tree_SPLAY(head, elm); if ((elm)->ipe_node.spe_right != ((void *)0)) { elm = (elm)->ipe_node.spe_right; while ((elm)->ipe_node.spe_left != ((void *)0)) { elm = (elm)->ipe_node.spe_left; } } else elm = ((void *)0); return (elm); } static __attribute__((__unused__)) __inline struct iommu_page_entry * iommu_page_tree_SPLAY_MIN_MAX(struct iommu_page_tree *head, int val) { iommu_page_tree_SPLAY_MINMAX(head, val); return ((head)->sph_root); };
