...
static struct radeon_device *radeon_get_rdev(struct ttm_bo_device *bdev)
{
 struct radeon_mman *mman;
 struct radeon_device *rdev;
 mman = ({ const __typeof( ((struct radeon_mman *)0)->bdev ) *__mptr = (bdev); (struct radeon_mman *)( (char *)__mptr - __builtin_offsetof(struct radeon_mman, bdev) );});
 rdev = ({ const __typeof( ((struct radeon_device *)0)->mman ) *__mptr = (mman); (struct radeon_device *)( (char *)__mptr - __builtin_offsetof(struct radeon_device, mman) );});
 return rdev;
}
    ...
    struct ttm_placement *placement)
{
 ...
 case 2:
  if (rbo->rdev->ring[(rbo->rdev)->asic->copy.copy_ring_index].ready == 0)
   radeon_ttm_placement_from_domain(rbo, 0x1);
  else if (rbo->rdev->mc.visible_vram_size < rbo->rdev->mc.real_vram_size &&
    bo->mem.start < (rbo->rdev->mc.visible_vram_size >> 13)) {
   unsigned fpfn = rbo->rdev->mc.visible_vram_size >> 13;
   int i;
...
{
 struct radeon_bo *rbo = ({ const __typeof( ((struct radeon_bo *)0)->tbo ) *__mptr = (bo); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, tbo) );});
        ...
        struct ttm_mem_reg *new_mem)
{
 ...
 ((!(old_mem->mm_node != ((void *)0))) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_ttm.c", 242, "!(old_mem->mm_node != ((void *)0))"));
 *old_mem = *new_mem;
   ...
   struct ttm_mem_reg *old_mem)
{
 ...
 old_start = (u64)old_mem->start << 13;
 new_start = (u64)new_mem->start << 13;
 ...
 num_pages = new_mem->num_pages * ((1 << 13) / 4096);
 fence = (rdev)->asic->copy.copy((rdev), (old_start), (new_start), (num_pages), (bo->resv));
 if (IS_ERR(fence))
    ...
    struct ttm_mem_reg *new_mem)
{
 ...
 placements.fpfn = 0;
 placements.lpfn = 0;
 placements.flags = ((1 << 16) | (1 << 17) | (1 << 18)) | (1 << 1);
...
memcpy:
 ...
 __sync_fetch_and_add_8(&rdev->num_bytes_moved, (u64)bo->num_pages << 13);
 return 0;
}
...
static int radeon_ttm_tt_populate(struct ttm_tt *ttm)
{
 ...
 if (ttm->state != tt_unpopulated)
  return 0;
 if (gtt && gtt->userptr) {
  ttm->sg = kzalloc(sizeof(struct sg_table), (0x0001 | 0x0004));
  if (!ttm->sg)
   return -12;
  ttm->page_flags |= (1 << 8);
...
_Bool radeon_ttm_tt_has_userptr(struct ttm_tt *ttm)
{
 struct radeon_ttm_tt *gtt = radeon_ttm_tt_to_gtt(ttm);
 if (gtt == ((void *)0))
  return 0;
...
_Bool radeon_ttm_tt_is_readonly(struct ttm_tt *ttm)
{
 struct radeon_ttm_tt *gtt = radeon_ttm_tt_to_gtt(ttm);
 if (gtt == ((void *)0))
  return 0;
 return !!(gtt->userflags & (1 << 0));
}
