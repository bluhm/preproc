...
{
 for (entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = (entry->head.prev); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );}); &entry->head != (list); entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = (entry->head.prev); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );})) {
...
{
 struct ttm_validate_buffer *entry;
 for (entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = ((list)->next); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );}); &entry->head != (list); entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = (entry->head.next); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );})) {
  struct ttm_buffer_object *bo = entry->bo;
    ...
    struct list_head *list)
{
 ...
 if (list_empty(list))
  return;
 entry = ({ const __typeof( ((struct ttm_validate_buffer *)0)->head ) *__mptr = ((list)->next); (struct ttm_validate_buffer *)( (char *)__mptr - __builtin_offsetof(struct ttm_validate_buffer, head) );});
 glob = entry->bo->glob;
      ...
      struct list_head *dups)
{
 ...
 for (entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = ((list)->next); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );}); &entry->head != (list); entry = ({ const __typeof( ((__typeof(*entry) *)0)->head ) *__mptr = (entry->head.next); (__typeof(*entry) *)( (char *)__mptr - __builtin_offsetof(__typeof(*entry), head) );})) {
  struct ttm_buffer_object *bo = entry->bo;
  ret = __ttm_bo_reserve(bo, intr, (ticket == ((void *)0)), 1,
           ticket);
  if (!ret && __builtin_expect(!!((*(&bo->cpu_writers)) > 0), 0)) {
   __ttm_bo_unreserve(bo);
   ret = -16;
  } else if (ret == -37 && dups) {
   struct ttm_validate_buffer *safe = entry;
   entry = ({ const __typeof( ((typeof(*(entry)) *)0)->head ) *__mptr = (((entry)->head.prev)); (typeof(*(entry)) *)( (char *)__mptr - __builtin_offsetof(typeof(*(entry)), head) );});
   list_del(&safe->head);
     ...
     struct list_head *list, struct fence *fence)
{
 ...
 if (list_empty(list))
  return;
 bo = ({ const __typeof( ((struct ttm_validate_buffer *)0)->head ) *__mptr = ((list)->next); (struct ttm_validate_buffer *)( (char *)__mptr - __builtin_offsetof(struct ttm_validate_buffer, head) );})->bo;
 bdev = bo->bdev;
 driver = bdev->driver;
 glob = bo->glob;
 __mtx_enter(&glob->lru_lock );
