...
static void atombios_scaler_setup(struct drm_crtc *crtc)
{
 ...
 struct radeon_encoder *radeon_encoder =
  ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (radeon_crtc->encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
...
static void atombios_blank_crtc(struct drm_crtc *crtc, int state)
{
 ...
 if (((rdev->family >= CHIP_BONAIRE))) {
  vga_control = r100_mm_rreg(rdev, (vga_control_regs[radeon_crtc->crtc_id]), 0);
  r100_mm_wreg(rdev, (vga_control_regs[radeon_crtc->crtc_id]), (vga_control | 1), 0);
 }
 args.ucCRTC = radeon_crtc->crtc_id;
 args.ucBlanking = state;
 atom_execute_table(rdev->mode_info.atom_context, index, (uint32_t *)&args);
 if (((rdev->family >= CHIP_BONAIRE))) {
  r100_mm_wreg(rdev, (vga_control_regs[radeon_crtc->crtc_id]), (vga_control), 0);
 }
}
          ...
          struct drm_display_mode *mode)
{
 ...
 if (((rdev->family >= CHIP_RS600))) {
  ...
  if (((rdev->family >= CHIP_PALM) && (rdev->flags & RADEON_IS_IGP)) || ((rdev->family >= CHIP_ARUBA) && (rdev->flags & RADEON_IS_IGP)) || ((rdev->family >= CHIP_BONAIRE)))
 ...
 if (((rdev->family >= CHIP_RV620))) {
  ...
  case 1:
   ...
   case 2:
    ...
    break;
   case 3:
    args.v3.sInput.usPixelClock = (__uint16_t)(__builtin_constant_p(clock / 10) ? (__uint16_t)(((__uint16_t)(clock / 10) & 0xffU) << 8 | ((__uint16_t)(clock / 10) & 0xff00U) >> 8) : __swap16md(clock / 10));
        ...
        u32 dispclk)
{
 ...
 case 1:
  ...
  case 6:
   ...
   if (((rdev->family >= CHIP_ARUBA) && (rdev->flags & RADEON_IS_IGP)) || ((rdev->family >= CHIP_BONAIRE)))
    args.v6.ucPpll = 8;
...
static _Bool atombios_crtc_prepare_pll(struct drm_crtc *crtc, struct drm_display_mode *mode)
{
 ...
 } else if ((radeon_encoder->active_device & (((0x1L << 0x00000001 ) | (0x1L << 0x00000005 )) | ((0x1L << 0x00000003 ) | (0x1L << 0x00000007 ) | (0x1L << 0x00000009 ) | (0x1L << 0x0000000A ) | (0x1L << 0x0000000B ) | (0x1L << 0x00000006 )))) ||
     ...
     int x, int y, int atomic)
{
 ...
 if (atomic) {
  radeon_fb = ({ const __typeof( ((struct radeon_framebuffer *)0)->base ) *__mptr = (fb); (struct radeon_framebuffer *)( (char *)__mptr - __builtin_offsetof(struct radeon_framebuffer, base) );});
  target_fb = fb;
 }
 else {
  radeon_fb = ({ const __typeof( ((struct radeon_framebuffer *)0)->base ) *__mptr = (crtc->primary->fb); (struct radeon_framebuffer *)( (char *)__mptr - __builtin_offsetof(struct radeon_framebuffer, base) );});
 ...
 case ((__u32)('B') | ((__u32)('A') << 8) | ((__u32)('3') << 16) | ((__u32)('0') << 24)):
  ...
  break;
 default:
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Unsupported screen format %s\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , drm_get_format_name(target_fb->pixel_format));
  return -22;
 }
 if (tiling_flags & 0x1) {
  ...
  if (rdev->family >= CHIP_TAHITI) {
   ...
   if (rdev->family >= CHIP_BONAIRE) {
    ...
    for (index = 0; tileb > 64; index++)
     tileb >>= 1;
    if (index >= 16) {
     printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Wrong screen bpp (%u) or tile split (%u)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , target_fb->bits_per_pixel, tile_split);
     return -22;
    }
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6808 + radeon_crtc->crtc_offset), 0); tmp_ &= (~(1 << 8)); tmp_ |= (((bypass_lut ? (1 << 8) : 0)) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0x6808 + radeon_crtc->crtc_offset), (tmp_), 0); } while (0);
 if (bypass_lut)
  do { } while( 0);
 ...
 if (rdev->family >= CHIP_BONAIRE)
  r100_mm_wreg(rdev, (0x6b0c + radeon_crtc->crtc_offset), (target_fb->height), 0);
 else
 ...
 r100_mm_wreg(rdev, (0x6ef8 + radeon_crtc->crtc_offset), (3), 0);
 if (!atomic && fb && fb != crtc->primary->fb) {
  radeon_fb = ({ const __typeof( ((struct radeon_framebuffer *)0)->base ) *__mptr = (fb); (struct radeon_framebuffer *)( (char *)__mptr - __builtin_offsetof(struct radeon_framebuffer, base) );});
  rbo = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((radeon_fb->obj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
  r = radeon_bo_reserve(rbo, 0);
  if (__builtin_expect(!!(r != 0), 0))
   return r;
  radeon_bo_unpin(rbo);
      ...
      int x, int y, int atomic)
{
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6108 + radeon_crtc->crtc_offset), 0); tmp_ &= (~(1 << 8)); tmp_ |= (((bypass_lut ? (1 << 8) : 0)) & ~(~(1 << 8))); r100_mm_wreg(rdev, (0x6108 + radeon_crtc->crtc_offset), (tmp_), 0); } while (0);
 if (bypass_lut)
  do { } while( 0);
 ...
 r100_mm_wreg(rdev, (0x60e4 + radeon_crtc->crtc_offset), (3), 0);
 if (!atomic && fb && fb != crtc->primary->fb) {
  radeon_fb = ({ const __typeof( ((struct radeon_framebuffer *)0)->base ) *__mptr = (fb); (struct radeon_framebuffer *)( (char *)__mptr - __builtin_offsetof(struct radeon_framebuffer, base) );});
  rbo = ({ const __typeof( ((struct radeon_bo *)0)->gem_base ) *__mptr = ((radeon_fb->obj)); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, gem_base) );});
  r = radeon_bo_reserve(rbo, 0);
...
static u32 radeon_get_pll_use_mask(struct drm_crtc *crtc)
{
 ...
 u32 pll_in_use = 0;
 for (test_crtc = ({ const __typeof( ((__typeof(*test_crtc) *)0)->head ) *__mptr = ((&dev->mode_config.crtc_list)->next); (__typeof(*test_crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*test_crtc), head) );}); &test_crtc->head != (&dev->mode_config.crtc_list); test_crtc = ({ const __typeof( ((__typeof(*test_crtc) *)0)->head ) *__mptr = (test_crtc->head.next); (__typeof(*test_crtc) *)( (char *)__mptr - __builtin_offsetof(__typeof(*test_crtc), head) );})) {
  if (crtc == test_crtc)
   continue;
  test_radeon_crtc = ({ const __typeof( ((struct radeon_crtc *)0)->base ) *__mptr = (test_crtc); (struct radeon_crtc *)( (char *)__mptr - __builtin_offsetof(struct radeon_crtc, base) );});
...
static void atombios_crtc_disable(struct drm_crtc *crtc)
{
 ...
 if (crtc->primary->fb) {
  ...
  if (__builtin_expect(!!(r), 0))
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "failed to reserve rbo before unpin\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  else {
   radeon_bo_unpin(rbo);
   radeon_bo_unreserve(rbo);
  }
 ...
 if (((rdev->family >= CHIP_CEDAR)))
  r100_mm_wreg(rdev, (0x6800 + radeon_crtc->crtc_offset), (0), 0);
 else if (((rdev->family >= CHIP_RS600)))
  r100_mm_wreg(rdev, (0x6100 + radeon_crtc->crtc_offset), (0), 0);
 if (((rdev->family >= CHIP_ARUBA)))
