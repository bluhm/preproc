...
static void ttm_pool_kobj_release(struct kobject *kobj)
{
 struct ttm_pool_manager *m =
  ({ const __typeof( ((struct ttm_pool_manager *)0)->kobj ) *__mptr = (kobj); (struct ttm_pool_manager *)( (char *)__mptr - __builtin_offsetof(struct ttm_pool_manager, kobj) );});
 kfree(m);
}
...
static int ttm_page_pool_free(struct ttm_page_pool *pool, unsigned nr_free,
         _Bool use_static)
{
 static struct vm_page *static_buf[((1 << 13)/sizeof(struct vm_page *))];
...
restart:
 _spin_lock_irqsave(&pool->lock, irq_flags );
 for ((p) = (*(((struct pglist *)((&pool->list)->tqh_last))->tqh_last)); (p) != ((void *)0) && ((p1) = (*(((struct pglist *)((p)->pageq.tqe_prev))->tqh_last)), 1); (p) = (p1)) {
  ...
  if (freed_pages >= ((1 << 13)/sizeof(struct vm_page *))) {
   ...
   _spin_unlock_irqrestore(&pool->lock, irq_flags );
...
static void ttm_page_pool_init_locked(struct ttm_page_pool *pool, gfp_t flags,
  char *name)
{
 do { (void)(((void *)0)); (void)(0); __mtx_init((&pool->lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 pool->fill_lock = 0;
...
{
 int ret;
 ({ int __ret = !!(_manager); if (__ret) printf("WARNING %s failed at %s:%d\n", "_manager", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/ttm/ttm_page_alloc.c", 855); __builtin_expect(!!(__ret), 0); });
 do { } while(0);
 ...
 ttm_page_pool_init_locked(&_manager->wc_pool_dma32,
      (0x0001 | 0x0004) | 0, "wc dma");
 ttm_page_pool_init_locked(&_manager->uc_pool_dma32,
      (0x0001 | 0x0004) | 0, "uc dma");
...
void ttm_page_alloc_fini(void)
{
 ...
 for (i = 0; i < 4; ++i)
  ttm_page_pool_free(&_manager->pools[i], (~0U), 1);
 kobject_put(&_manager->kobj);
 _manager = ((void *)0);
}
