...
void drm_modeset_drop_locks(struct drm_modeset_acquire_ctx *ctx)
{
 ...
 while (!list_empty(&ctx->locked)) {
  ...
  lock = ({ const __typeof( ((struct drm_modeset_lock *)0)->head ) *__mptr = ((&ctx->locked)->next); (struct drm_modeset_lock *)( (char *)__mptr - __builtin_offsetof(struct drm_modeset_lock, head) );});
