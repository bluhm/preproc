       ...
       u32 smc_address, u32 limit)
{
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x240), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x240), (tmp_), 0); } while (0);
 return 0;
}
int ci_copy_bytes_to_smc(struct radeon_device *rdev,
    u32 smc_start_address,
...
_Bool ci_is_smc_running(struct radeon_device *rdev)
{
 ...
 u32 pc_c = tn_smc_rreg(rdev, (0x80000370));
 if (!(clk & (1 << 0)) && (0x20100 <= pc_c))
  return 1;
 return 0;
}
PPSMC_Result ci_send_msg_to_smc(struct radeon_device *rdev, PPSMC_Msg msg)
{
 ...
 if (!ci_is_smc_running(rdev))
  return ((uint8_t)0xFF);
 r100_mm_wreg(rdev, (0x250), (msg), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  tmp = r100_mm_rreg(rdev, (0x254), 0);
  if (tmp != 0)
   break;
  udelay(1);
 }
...
int ci_load_smc_ucode(struct radeon_device *rdev, u32 limit)
{
 ...
 if (rdev->new_fw) {
  ...
  default:
   ...
   do { panic("BUG at %s:%d", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/ci_smc.c", 239); } while (0);
  }
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x240), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x240), (tmp_), 0); } while (0);
 while (ucode_size >= 4) {
  data = (src[0] << 24) | (src[1] << 16) | (src[2] << 8) | src[3];
  r100_mm_wreg(rdev, (0x204), (data), 0);
  src += 4;
