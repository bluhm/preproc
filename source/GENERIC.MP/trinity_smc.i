...
static int trinity_notify_message_to_smu(struct radeon_device *rdev, u32 id)
{
 int i;
 u32 v = 0;
 r100_mm_wreg(rdev, (0x22c), (id), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (r100_mm_rreg(rdev, (0x230), 0) != 0)
   break;
  udelay(1);
 }
 v = r100_mm_rreg(rdev, (0x230), 0);
 if (v != 1) {
  if (v == 0xFF) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "SMC failed to handle the message!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return -22;
  } else if (v == 0xFE) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Unknown SMC message!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return -22;
  }
 }
 return 0;
...
{
 if (enable)
  return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x120));
 else
  return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x121));
}
int trinity_dpm_config(struct radeon_device *rdev, _Bool enable)
{
 if (enable)
  tn_smc_wreg(rdev, (0xe0003040), (1));
 else
  tn_smc_wreg(rdev, (0xe0003040), (0));
 return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x102));
}
int trinity_dpm_force_state(struct radeon_device *rdev, u32 n)
{
 tn_smc_wreg(rdev, (0xe0003040), (n));
 return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x104));
}
int trinity_dpm_n_levels_disabled(struct radeon_device *rdev, u32 n)
{
 tn_smc_wreg(rdev, (0xe0003040), (n));
 return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x112));
}
int trinity_uvd_dpm_config(struct radeon_device *rdev)
{
 return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x124));
}
int trinity_dpm_no_forced_level(struct radeon_device *rdev)
{
 return trinity_notify_message_to_smu(rdev, ((uint8_t)0x41));
}
int trinity_dce_enable_voltage_adjustment(struct radeon_device *rdev,
       _Bool enable)
{
 if (enable)
  return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x11e));
 else
  return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x11d));
}
int trinity_gfx_dynamic_mgpg_config(struct radeon_device *rdev)
{
 return trinity_notify_message_to_smu(rdev, ((uint32_t) 0x108));
}
void trinity_acquire_mutex(struct radeon_device *rdev)
{
 int i;
 r100_mm_wreg(rdev, (0x220), (1), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  if ((r100_mm_rreg(rdev, (0x220), 0) & 0xffff) == 1)
   break;
  udelay(1);
 }
}
