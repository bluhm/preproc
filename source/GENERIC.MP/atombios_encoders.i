    ...
    struct drm_connector *drm_connector)
{
 ...
 struct radeon_encoder_atom_dig *dig;
 char bl_name[16];
 if ((rdev->pdev->subsystem_vendor == 0x106b) &&
       ...
       struct drm_display_mode *adjusted_mode)
{
 ...
 } else if (radeon_encoder->active_device & (((0x1L << 0x00000002 )))) {
...
atombios_dvo_setup(struct drm_encoder *encoder, int action)
{
 ...
 case 1:
  ...
  case 4:
   ...
   args.dvo_v4.usPixelClock = (__uint16_t)(__builtin_constant_p(radeon_encoder->pixel_clock / 10) ? (__uint16_t)(((__uint16_t)(radeon_encoder->pixel_clock / 10) & 0xffU) << 8 | ((__uint16_t)(radeon_encoder->pixel_clock / 10) & 0xff00U) >> 8) : __swap16md(radeon_encoder->pixel_clock / 10));
 ...
 case 10:
  ...
  if ((dig_connector->dp_sink_type == 0x13) ||
   ...
   if (radeon_audio != 0 &&
       ...
       ((rdev->family >= CHIP_CEDAR)) && !((rdev->family >= CHIP_BARTS)))
    return 5;
   return 0;
    ...
    int action)
{
 ...
 struct radeon_device *rdev = dev->dev_private;
 struct radeon_encoder *radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
 struct radeon_encoder *ext_radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (ext_encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
...
radeon_atom_encoder_dpms_avivo(struct drm_encoder *encoder, int mode)
{
 ...
 case 0x15:
  if (radeon_encoder->active_device & (((0x1L << 0x00000002 ))))
   index = (((char*)(&((ATOM_MASTER_LIST_OF_COMMAND_TABLES*)0)->Gfx_Harvesting)-(char*)0)/sizeof(USHORT));
  else if (radeon_encoder->active_device & ((0x1L << 0x00000008 )))
...
radeon_atom_encoder_dpms_dig(struct drm_encoder *encoder, int mode)
{
 ...
 _Bool travis_quirk = 0;
 if (connector) {
  radeon_connector = ({ const __typeof( ((struct radeon_connector *)0)->base ) *__mptr = (connector); (struct radeon_connector *)( (char *)__mptr - __builtin_offsetof(struct radeon_connector, base) );});
  radeon_dig_connector = radeon_connector->con_priv;
  if ((radeon_connector_encoder_get_dp_bridge_encoder_id(connector) ==
       0x23) &&
      (radeon_encoder->devices & (((0x1L << 0x00000001 ) | (0x1L << 0x00000005 )))) &&
 ...
 case 3:
  ...
  if ((((atombios_get_encoder_mode(encoder)) == 0) || ((atombios_get_encoder_mode(encoder)) == 5)) &&
   ...
   radeon_dp_set_rx_power_state(connector, 0x2);
  if (((rdev->family >= CHIP_CEDAR))) {
   atombios_dig_transmitter_setup(encoder,
             0, 0, 0);
  } else {
...
atombios_set_mst_encoder_crtc_source(struct drm_encoder *encoder, int fe)
{
 ...
 if (!atom_parse_cmd_header(rdev->mode_info.atom_context, index, &frev, &crev))
  return;
 if (frev != 1 && crev != 2)
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Unknown table for MST %d, %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , frev, crev);
 args.v2.ucCRTC = radeon_crtc->crtc_id;
 args.v2.ucEncodeMode = 5;
 switch (fe) {
 case 0:
  args.v2.ucEncoderID = 0x03;
  break;
         ...
         struct drm_display_mode *mode)
{
 ...
 if (((rdev->family >= CHIP_RS600)) &&
  ...
  if (((rdev->family >= CHIP_BONAIRE))) {
   if (mode->flags & (1<<4))
    r100_mm_wreg(rdev, (0x6b00 + radeon_crtc->crtc_offset), ((1 << 3)), 0);
   else
...
int radeon_atom_pick_dig_encoder(struct drm_encoder *encoder, int fe_idx)
{
 ...
 for (test_encoder = ({ const __typeof( ((__typeof(*test_encoder) *)0)->head ) *__mptr = ((&dev->mode_config.encoder_list)->next); (__typeof(*test_encoder) *)( (char *)__mptr - __builtin_offsetof(__typeof(*test_encoder), head) );}); &test_encoder->head != (&dev->mode_config.encoder_list); test_encoder = ({ const __typeof( ((__typeof(*test_encoder) *)0)->head ) *__mptr = (test_encoder->head.next); (__typeof(*test_encoder) *)( (char *)__mptr - __builtin_offsetof(__typeof(*test_encoder), head) );})) {
  ...
  if (!radeon_encoder_is_digital(test_encoder))
   continue;
  radeon_test_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (test_encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
...
assigned:
 if (enc_idx == -1) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Got encoder index incorrect - returning 0\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return 0;
 }
 if (rdev->mode_info.active_encoders & (1 << enc_idx)) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "chosen encoder in use %d\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , enc_idx);
 }
 rdev->mode_info.active_encoders |= (1 << enc_idx);
 return enc_idx;
}
 ...
 if (radeon_encoder_is_digital(encoder)) {
  if (atombios_get_encoder_mode(encoder) == 3) {
   if (rdev->asic->display.hdmi_enable)
    (rdev)->asic->display.hdmi_enable((encoder), (0));
  }
  if (atombios_get_encoder_mode(encoder) != 5) {
   dig = radeon_encoder->enc_priv;
   radeon_atom_release_dig_encoder(rdev, dig->dig_encoder);
   dig->dig_encoder = -1;
