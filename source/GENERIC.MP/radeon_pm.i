...
static _Bool radeon_pm_in_vbl(struct radeon_device *rdev);
static _Bool radeon_pm_debug_check_in_vbl(struct radeon_device *rdev, _Bool finish);
 ...
 if ((rdev->pm.pm_method == PM_METHOD_DPM) && rdev->pm.dpm_enabled) {
  ...
  if (rdev->family == CHIP_ARUBA) {
   if (rdev->asic->dpm.enable_bapm)
    rdev->asic->dpm.enable_bapm((rdev), (rdev->pm.dpm.ac_power));
  }
...
static void radeon_pm_update_profile(struct radeon_device *rdev)
{
 ...
 case PM_PROFILE_DEFAULT:
  rdev->pm.profile_index = 0;
  break;
 case PM_PROFILE_AUTO:
  if (power_supply_is_system_supplied() > 0) {
   if (rdev->pm.active_crtc_count > 1)
    rdev->pm.profile_index = 6;
   else
    rdev->pm.profile_index = 3;
  } else {
   if (rdev->pm.active_crtc_count > 1)
    rdev->pm.profile_index = 5;
   else
    rdev->pm.profile_index = 2;
  }
  break;
 case PM_PROFILE_LOW:
  if (rdev->pm.active_crtc_count > 1)
...
static void radeon_sync_with_vblank(struct radeon_device *rdev)
{
 if (rdev->pm.active_crtcs) {
  ...
  ({ long __ret = (((uint64_t)(200)) * hz / 1000); if (!(rdev->pm.vblank_sync)) __ret = ({ long ret = (((uint64_t)(200)) * hz / 1000); do { int deadline, __error; ((!cold) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_pm.c", 162, "!cold")); __mtx_enter(&sch_mtx ); ((void)_atomic_add_int_nv((&(rdev->irq.vblank_queue).count), 1)); deadline = ticks + ret; __error = msleep(&rdev->irq.vblank_queue, &sch_mtx, 0, "drmweti", ret); ret = deadline - ticks; ((void)_atomic_sub_int_nv((&(rdev->irq.vblank_queue).count), 1)); if (__error == -1 || __error == 4) { ret = -4; __mtx_leave(&sch_mtx ); break; } if ((((uint64_t)(200)) * hz / 1000) && (ret <= 0 || __error == 35)) { __mtx_leave(&sch_mtx ); ret = ((rdev->pm.vblank_sync)) ? 1 : 0; break; } __mtx_leave(&sch_mtx ); } while (ret > 0 && !(rdev->pm.vblank_sync)); ret; }); __ret; });
 }
}
 ...
 if ((rdev)->asic->gui_idle((rdev))) {
  ...
  if ((rdev->pm.pm_method == PM_METHOD_PROFILE) &&
   ...
   mclk = rdev->pm.power_state[rdev->pm.requested_power_state_index].
    clock_info[rdev->pm.profiles[6].dpms_on_cm_idx].mclk;
  else
   mclk = rdev->pm.power_state[rdev->pm.requested_power_state_index].
  ...
  (rdev)->asic->pm.prepare((rdev));
  if (!misc_after)
   (rdev)->asic->pm.misc((rdev));
  if (sclk != rdev->pm.current_sclk) {
   radeon_pm_debug_check_in_vbl(rdev, 0);
   (rdev)->asic->pm.set_engine_clock((rdev), (sclk));
  ...
  if (rdev->asic->pm.set_memory_clock && (mclk != rdev->pm.current_mclk)) {
   radeon_pm_debug_check_in_vbl(rdev, 0);
   (rdev)->asic->pm.set_memory_clock((rdev), (mclk));
   radeon_pm_debug_check_in_vbl(rdev, 1);
...
static void radeon_pm_set_clocks(struct radeon_device *rdev)
{
 ...
 if ((rdev->pm.requested_clock_mode_index == rdev->pm.current_clock_mode_index) &&
     (rdev->pm.requested_power_state_index == rdev->pm.current_power_state_index))
  return;
 _rw_enter_write(&rdev->pm.mclk_lock );
 ...
 for (i = 0; i < 8; i++) {
  ...
  if (r) {
   ...
   _rw_exit_write(&rdev->pm.mclk_lock );
   return;
  }
 }
...
{
 struct radeon_device *rdev =
  ({ const __typeof( ((struct radeon_device *)0)->pm.dpm.thermal.work ) *__mptr = (work); (struct radeon_device *)( (char *)__mptr - __builtin_offsetof(struct radeon_device, pm.dpm.thermal.work) );});
 enum radeon_pm_state_type dpm_state = POWER_STATE_TYPE_INTERNAL_THERMAL;
 if (!rdev->pm.dpm_enabled)
  return;
 if (rdev->asic->pm.get_temperature) {
  int temp = (rdev)->asic->pm.get_temperature((rdev));
 ...
 if (dpm_state == POWER_STATE_TYPE_INTERNAL_THERMAL)
  rdev->pm.dpm.thermal_active = 1;
 else
  rdev->pm.dpm.thermal_active = 0;
 rdev->pm.dpm.state = dpm_state;
 _rw_exit_write(&rdev->pm.mutex );
 radeon_pm_compute_clocks(rdev);
}
static _Bool radeon_dpm_single_display(struct radeon_device *rdev)
{
 _Bool single_display = (rdev->pm.dpm.new_active_crtc_count < 2) ?
  1 : 0;
 if (single_display && rdev->asic->dpm.vblank_too_short) {
  if (rdev->asic->dpm.vblank_too_short((rdev)))
   single_display = 0;
 }
           ...
           enum radeon_pm_state_type dpm_state)
{
 ...
 struct radeon_ps *ps;
 u32 ui_class;
 _Bool single_display = radeon_dpm_single_display(rdev);
...
restart_search:
 for (i = 0; i < rdev->pm.dpm.num_ps; i++) {
  ps = &rdev->pm.dpm.ps[i];
  ui_class = ps->class & 0x0007;
  switch (dpm_state) {
  case POWER_STATE_TYPE_BATTERY:
   if (ui_class == 1) {
    if (ps->caps & 0x00000001) {
     if (single_display)
      return ps;
    } else
     return ps;
...
force:
 if (radeon_dpm == 1) {
  printf("switching from power state:\n");
  rdev->asic->dpm.print_power_state((rdev), (rdev->pm.dpm.current_ps));
  printf("switching to power state:\n");
  rdev->asic->dpm.print_power_state((rdev), (rdev->pm.dpm.requested_ps));
 }
 _rw_enter_write(&rdev->pm.mclk_lock );
 _rw_enter_write(&rdev->ring_lock );
 ps->vce_active = rdev->pm.dpm.vce_active;
 ret = rdev->asic->dpm.pre_set_power_state((rdev));
 if (ret)
  goto done;
 (rdev)->asic->display.bandwidth_update((rdev));
 rdev->asic->dpm.display_configuration_changed((rdev));
 ...
 if (rdev->asic->dpm.force_performance_level) {
  if (rdev->pm.dpm.thermal_active) {
   ...
   rdev->asic->dpm.force_performance_level((rdev), (RADEON_DPM_FORCED_LEVEL_LOW));
   rdev->pm.dpm.forced_level = level;
  } else {
   rdev->asic->dpm.force_performance_level((rdev), (rdev->pm.dpm.forced_level));
  }
 }
done:
 _rw_exit_write(&rdev->ring_lock );
...
void radeon_dpm_enable_uvd(struct radeon_device *rdev, _Bool enable)
{
 ...
 if (rdev->asic->dpm.powergate_uvd) {
  ...
  rdev->asic->dpm.powergate_uvd((rdev), (!enable));
  _rw_exit_write(&rdev->pm.mutex );
 } else {
  if (enable) {
   _rw_enter_write(&rdev->pm.mutex );
...
static void radeon_pm_resume_old(struct radeon_device *rdev)
{
 if ((rdev->family >= CHIP_BARTS) &&
  ...
  if (rdev->pm.default_sclk)
   (rdev)->asic->pm.set_engine_clock((rdev), (rdev->pm.default_sclk));
  if (rdev->pm.default_mclk)
   (rdev)->asic->pm.set_memory_clock((rdev), (rdev->pm.default_mclk));
 }
 _rw_enter_write(&rdev->pm.mutex );
...
static void radeon_pm_resume_dpm(struct radeon_device *rdev)
{
 ...
 _rw_enter_write(&rdev->pm.mutex );
 rdev->pm.dpm.current_ps = rdev->pm.dpm.requested_ps = rdev->pm.dpm.boot_ps;
 rdev->asic->dpm.setup_asic((rdev));
 ret = rdev->asic->dpm.enable((rdev));
 _rw_exit_write(&rdev->pm.mutex );
 if (ret)
 ...
 return;
dpm_resume_fail:
 printf("error: [" "drm" ":pid%d:%s] *ERROR* " "radeon: dpm resume failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 if ((rdev->family >= CHIP_BARTS) &&
     (rdev->family <= CHIP_CAYMAN) &&
     rdev->mc_fw) {
  if (rdev->pm.default_vddc)
...
static int radeon_pm_init_old(struct radeon_device *rdev)
{
 ...
 if (rdev->pm.num_power_states > 1) {
  if (radeon_debugfs_pm_init(rdev)) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to register debugfs file for PM!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  }
  do { } while( 0);
 }
 return 0;
...
static void radeon_dpm_print_power_states(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < rdev->pm.dpm.num_ps; i++) {
  ...
  rdev->asic->dpm.print_power_state((rdev), (&rdev->pm.dpm.ps[i]));
 }
}
static int radeon_pm_init_dpm(struct radeon_device *rdev)
{
 ...
 rdev->pm.dpm_enabled = 1;
 if (radeon_debugfs_pm_init(rdev)) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to register debugfs file for dpm!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 }
 do { } while( 0);
 return 0;
dpm_failed:
 ...
 printf("error: [" "drm" ":pid%d:%s] *ERROR* " "radeon: dpm initialization failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 return ret;
}
...
static struct radeon_dpm_quirk radeon_dpm_quirk_list[] = {
 { 0x1002, 0x6759, 0x1682, 0x3195 },
 { 0x1002, 0x6840, 0x1179, 0xfb81 },
 { 0, 0, 0, 0 },
};
int radeon_pm_init(struct radeon_device *rdev)
{
...
int radeon_pm_late_init(struct radeon_device *rdev)
{
 ...
 if (rdev->pm.pm_method == PM_METHOD_DPM) {
  if (rdev->pm.dpm_enabled) {
   _rw_enter_write(&rdev->pm.mutex );
   ret = rdev->asic->dpm.late_enable((rdev));
   _rw_exit_write(&rdev->pm.mutex );
   if (ret) {
    rdev->pm.dpm_enabled = 0;
...
static _Bool radeon_pm_debug_check_in_vbl(struct radeon_device *rdev, _Bool finish)
{
 _Bool in_vbl = radeon_pm_in_vbl(rdev);
 if (in_vbl == 0)
  do { } while( 0);
 return in_vbl;
}
static void radeon_dynpm_idle_work_handler(struct work_struct *work)
{
 struct radeon_device *rdev;
 int resched;
 rdev = ({ const __typeof( ((struct radeon_device *)0)->pm.dynpm_idle_work.work ) *__mptr = (work); (struct radeon_device *)( (char *)__mptr - __builtin_offsetof(struct radeon_device, pm.dynpm_idle_work.work) );});
