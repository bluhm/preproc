...
static void radeon_do_test_moves(struct radeon_device *rdev, int flag)
{
 ...
 for (i = 0; i < n; i++) {
  ...
  if (ring == 3)
   fence = (rdev)->asic->copy.dma((rdev), (gtt_addr), (vram_addr), (size / 4096), (vram_obj->tbo.resv));
  else
   fence = (rdev)->asic->copy.blit((rdev), (gtt_addr), (vram_addr), (size / 4096), (vram_obj->tbo.resv));
  if (IS_ERR(fence)) {
  ...
  radeon_bo_kunmap(vram_obj);
  if (ring == 3)
   fence = (rdev)->asic->copy.dma((rdev), (vram_addr), (gtt_addr), (size / 4096), (vram_obj->tbo.resv));
  else
   fence = (rdev)->asic->copy.blit((rdev), (vram_addr), (gtt_addr), (size / 4096), (vram_obj->tbo.resv));
  if (IS_ERR(fence)) {
          ...
          struct radeon_fence **fence)
{
 ...
 if (ring->idx == 5) {
  ...
  if (r) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to get dummy create msg\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return r;
  }
  r = radeon_uvd_get_destroy_msg(rdev, ring->idx, handle, fence);
  if (r) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to get dummy destroy msg\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return r;
  }
 } else if (ring->idx == 6 ||
     ring->idx == 7) {
       ...
       struct radeon_ring *ringC)
{
 ...
 if (radeon_fence_signaled(fenceB)) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Fence B signaled without waiting for semaphore.\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  goto out_cleanup;
 }
...
static _Bool radeon_test_sync_possible(struct radeon_ring *ringA,
          struct radeon_ring *ringB)
{
 if (ringA->idx == 7 &&
     ringB->idx == 6)
