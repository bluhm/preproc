...
struct si_powertune_data
{
 ...
 _Bool enable_powertune_by_default;
};
...
struct si_mc_reg_table {
 ...
 u8 num_entries;
 u16 valid_flag;
 struct si_mc_reg_entry mc_reg_table_entry[16];
 SMC_NIslands_MCRegisterAddress mc_reg_address[16];
};
...
{
 u16 count;
 struct si_leakage_voltage_entry entries[4];
};
struct si_ulv_param {
 _Bool supported;
 u32 cg_ulv_control;
...
struct si_power_info {
 ...
 _Bool sclk_deep_sleep_above_low;
 _Bool voltage_control_svi2;
 _Bool vddci_control_svi2;
 u32 sram_end;
       ...
       struct si_dte_data *dte_data)
{
 ...
 if (p_limit2 != 0 && p_limit2 <= p_limit1) {
  ...
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Invalid PL2! DTE will not be updated.\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 }
}
static void si_initialize_powertune_defaults(struct radeon_device *rdev)
{
 ...
 if (rdev->family == CHIP_TAHITI) {
  ...
  case 0x6798:
   si_pi->dte_data.enable_dte_by_default = 1;
   break;
  case 0x6799:
  ...
  case 0x679A:
   ...
   break;
  default:
   if (si_pi->dte_data.enable_dte_by_default == 1)
    printf("error: [" "drm" ":pid%d:%s] *ERROR* " "DTE is not enabled!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   break;
  }
  ...
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Unknown SI asic revision, failed to initialize PowerTune!\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return;
 }
 ...
 ni_pi->driver_calculate_cac_leakage = 1;
 ...
 si_pi->dyn_powertune_data.disable_uvd_powertune = 0;
}
...
static u32 si_calculate_cac_wintime(struct radeon_device *rdev)
{
 ...
 u32 cac_window;
 u32 cac_window_size;
 xclk = (rdev)->asic->get_xclk((rdev));
 if (xclk == 0)
  return 0;
 cac_window = r100_mm_rreg(rdev, (0x8b8), 0) & 0x00ffffff;
          ...
          struct radeon_ps *radeon_state)
{
 ...
 if (ni_pi->enable_power_containment) {
  ...
  if (scaling_factor == 0)
   return -22;
  __builtin_memset((smc_table), (0), (sizeof(SISLANDS_SMC_STATETABLE)));
  ...
  smc_table->dpm2Params.TDPLimit =
   ((__uint32_t)(si_scale_power_for_smc(tdp_limit, scaling_factor) * 1000));
  smc_table->dpm2Params.NearTDPLimit =
   ((__uint32_t)(si_scale_power_for_smc(near_tdp_limit, scaling_factor) * 1000));
  smc_table->dpm2Params.SafePowerLimit =
   ((__uint32_t)(si_scale_power_for_smc((near_tdp_limit * 80) / 100, scaling_factor) * 1000));
  ret = si_copy_bytes_to_smc(rdev,
        (si_pi->state_table_start + __builtin_offsetof(SISLANDS_SMC_STATETABLE, dpm2Params) +
       __builtin_offsetof(PP_SIslands_DPM2Parameters, TDPLimit)),
  ...
  if (si_pi->enable_ppm) {
   ...
   __builtin_memset((papm_parm), (0), (sizeof(PP_SIslands_PAPMParameters)));
   papm_parm->NearTDPLimitTherm = ((__uint32_t)(ppm->dgpu_tdp));
   papm_parm->dGPU_T_Limit = ((__uint32_t)(ppm->tj_max));
   papm_parm->dGPU_T_Warning = ((__uint32_t)(95));
   papm_parm->dGPU_T_Hysteresis = ((__uint32_t)(5));
     ...
     struct radeon_ps *radeon_state)
{
 ...
 if (ni_pi->enable_power_containment) {
  ...
  __builtin_memset((smc_table), (0), (sizeof(SISLANDS_SMC_STATETABLE)));
  smc_table->dpm2Params.NearTDPLimit =
   ((__uint32_t)(si_scale_power_for_smc(rdev->pm.dpm.near_tdp_limit_adjusted, scaling_factor) * 1000));
  smc_table->dpm2Params.SafePowerLimit =
   ((__uint32_t)(si_scale_power_for_smc((rdev->pm.dpm.near_tdp_limit_adjusted * 80) / 100, scaling_factor) * 1000));
  ret = si_copy_bytes_to_smc(rdev,
        (si_pi->state_table_start +
         __builtin_offsetof(SISLANDS_SMC_STATETABLE, dpm2Params) +
         __builtin_offsetof(PP_SIslands_DPM2Parameters, NearTDPLimit)),
...
static _Bool si_should_disable_uvd_powertune(struct radeon_device *rdev,
         struct radeon_ps *radeon_state)
{
 struct si_power_info *si_pi = si_get_pi(rdev);
      ...
      SISLANDS_SMC_SWSTATE *smc_state)
{
 ...
 for (i = 1; i < state->performance_level_count; i++) {
  ...
  smc_state->levels[i].dpm2.MaxPS = (u8)((256 * (max_sclk - min_sclk)) / max_sclk);
  smc_state->levels[i].dpm2.NearTDPDec = 10;
  smc_state->levels[i].dpm2.AboveSafeInc = 5;
  smc_state->levels[i].dpm2.BelowSafeInc = 20;
  smc_state->levels[i].dpm2.PwrEfficiencyRatio = ((__uint16_t)(pwr_efficiency_ratio));
 }
 return 0;
}
 ...
 if (ni_pi->enable_power_containment) {
  if (enable) {
   if (!si_should_disable_uvd_powertune(rdev, radeon_new_state)) {
    smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x59));
   ...
   smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x5A));
   if (smc_result != ((uint8_t)0x01))
    ret = -22;
   ni_pi->pc_enabled = 0;
  }
...
static int si_initialize_smc_dte_tables(struct radeon_device *rdev)
{
 ...
 if (dte_data->k <= 0)
  return -22;
 dte_tables = kzalloc(sizeof(Smc_SIslands_DTE_Configuration), (0x0001 | 0x0004));
 ...
 if (tdep_count > 16)
  tdep_count = 16;
 dte_tables->K = ((__uint32_t)(table_size));
 dte_tables->T0 = ((__uint32_t)(dte_data->t0));
 dte_tables->MaxT = ((__uint32_t)(dte_data->max_t));
 dte_tables->WindowSize = dte_data->window_size;
 dte_tables->temp_select = dte_data->temp_select;
 dte_tables->DTE_mode = dte_data->dte_mode;
 dte_tables->Tthreshold = ((__uint32_t)(dte_data->t_threshold));
 if (tdep_count > 0)
  table_size--;
 for (i = 0; i < table_size; i++) {
  dte_tables->tau[i] = ((__uint32_t)(dte_data->tau[i]));
  dte_tables->R[i] = ((__uint32_t)(dte_data->r[i]));
 }
 dte_tables->Tdep_count = tdep_count;
 for (i = 0; i < (u32)tdep_count; i++) {
  dte_tables->T_limits[i] = dte_data->t_limits[i];
  dte_tables->Tdep_tau[i] = ((__uint32_t)(dte_data->tdep_tau[i]));
  dte_tables->Tdep_R[i] = ((__uint32_t)(dte_data->tdep_r[i]));
 }
 ...
 for (i = 0; i < 16 ; i++) {
  ...
  for (j = 0; j < 32; j++) {
   ...
   if (smc_leakage > 0xFFFF)
    smc_leakage = 0xFFFF;
   cac_tables->cac_lkge_lut[i][32 -1-j] =
    ((__uint16_t)((u16)smc_leakage));
  }
 }
 return 0;
}
...
static int si_initialize_smc_cac_tables(struct radeon_device *rdev)
{
 ...
 u16 t0, t_step;
 u32 load_line_slope, reg64;
 int ret = 0;
 u32 ticks_per_us = (rdev)->asic->get_xclk((rdev)) / 100;
 if (ni_pi->enable_cac == 0)
  return 0;
 cac_tables = kzalloc(sizeof(PP_SIslands_CacConfig), (0x0001 | 0x0004));
 if (!cac_tables)
  return -12;
 reg64 = r100_mm_rreg(rdev, (0x8b8), 0) & ~0x00ffffff;
 reg64 |= ((si_pi->powertune_data->cac_window) << 0);
 r100_mm_wreg(rdev, (0x8b8), (reg64), 0);
 ...
 vddc_min = vddc_max - (vddc_step * (32 - 1));
 t_step = 4;
 t0 = 60;
 ...
 if (ret)
  goto done_free;
 load_line_slope = ((u32)rdev->pm.dpm.load_line_slope << 12) / 100;
 cac_tables->l2numWin_TDP = ((__uint32_t)(si_pi->dyn_powertune_data.l2_lta_window_size));
 cac_tables->lts_truncate_n = si_pi->dyn_powertune_data.lts_truncate;
 cac_tables->SHIFT_N = si_pi->dyn_powertune_data.shift_n;
 cac_tables->lkge_lut_V0 = ((__uint32_t)((u32)vddc_min));
 cac_tables->lkge_lut_Vstep = ((__uint32_t)((u32)vddc_step));
 cac_tables->R_LL = ((__uint32_t)(load_line_slope));
 cac_tables->WinTime = ((__uint32_t)(si_pi->dyn_powertune_data.wintime));
 cac_tables->calculation_repeats = ((__uint32_t)(2));
 cac_tables->dc_cac = ((__uint32_t)(0));
 cac_tables->log2_PG_LKG_SCALE = 12;
 cac_tables->cac_temp = si_pi->powertune_data->operating_temp;
 cac_tables->lkge_lut_T0 = ((__uint32_t)((u32)t0));
 cac_tables->lkge_lut_Tstep = ((__uint32_t)((u32)t_step));
 ret = si_copy_bytes_to_smc(rdev, si_pi->cac_table_start, (u8 *)cac_tables,
       sizeof(PP_SIslands_CacConfig), si_pi->sram_end);
 if (ret)
  goto done_free;
 ret = si_write_smc_soft_register(rdev, 0xA8, ticks_per_us);
done_free:
 if (ret) {
  ni_pi->enable_cac = 0;
  ni_pi->enable_power_containment = 0;
        ...
        const struct si_cac_config_reg *cac_config_regs)
{
 ...
 while (config_regs->offset != 0xFFFFFFFF) {
  ...
  case SISLANDS_CACCONFIG_CGIND:
   offset = 0xc0030000 + config_regs->offset;
   if (offset < 0xc0040000)
    data = tn_smc_rreg(rdev, (offset));
   break;
  default:
  ...
  if (enable) {
   if (!si_should_disable_uvd_powertune(rdev, radeon_new_state)) {
    if (ni_pi->support_cac_long_term_average) {
     smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x6E));
     if (smc_result != ((uint8_t)0x01))
      ni_pi->support_cac_long_term_average = 0;
    }
    smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x53));
    if (smc_result != ((uint8_t)0x01)) {
     ret = -22;
     ni_pi->cac_enabled = 0;
    } else {
    ...
    if (si_pi->enable_dte) {
     smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x87));
     if (smc_result != ((uint8_t)0x01))
      ret = -22;
    }
   }
   ...
   if (si_pi->enable_dte)
    smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x88));
   smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x54));
   ni_pi->cac_enabled = 0;
   if (ni_pi->support_cac_long_term_average)
    smc_result = si_send_msg_to_smc(rdev, ((uint8_t)0x6F));
  }
 }
 return ret;
}
static int si_init_smc_spll_table(struct radeon_device *rdev)
{
 ...
 if (si_pi->spll_table_start == 0)
  return -22;
 spll_table = kzalloc(sizeof(SMC_SISLANDS_SPLL_DIV_TABLE), (0x0001 | 0x0004));
...
static struct si_dpm_quirk si_dpm_quirk_list[] = {
 { 0x1002, 0x6810, 0x1462, 0x3036, 0, 120000 },
 { 0x1002, 0x6811, 0x174b, 0xe271, 0, 120000 },
 { 0x1002, 0x6811, 0x174b, 0x2015, 0, 120000 },
 { 0x1002, 0x6810, 0x174b, 0xe271, 85000, 90000 },
 { 0x1002, 0x6811, 0x1462, 0x2015, 0, 120000 },
 { 0x1002, 0x6811, 0x1043, 0x2015, 0, 120000 },
 { 0x1002, 0x6811, 0x148c, 0x2015, 0, 120000 },
 { 0x1002, 0x6810, 0x1682, 0x9275, 0, 120000 },
 { 0, 0, 0, 0 },
};
     ...
     struct radeon_ps *rps)
{
 ...
 _Bool disable_mclk_switching = 0;
 _Bool disable_sclk_switching = 0;
 u32 mclk, sclk;
...
static _Bool si_is_special_1gb_platform(struct radeon_device *rdev)
{
 _Bool ret = 0;
 u32 tmp, width, row, column, bank, density;
 _Bool is_memory_gddr5, is_special;
 tmp = r100_mm_rreg(rdev, (0x2a00), 0);
 is_memory_gddr5 = (5 == ((tmp & 0xf0000000) >> 28));
 is_special = (1 == ((tmp & 0x0000f000) >> 12))
  & (3 == ((tmp & 0x00000f00) >> 8));
 r100_mm_wreg(rdev, (0x2a44), (0xb), 0);
 width = ((r100_mm_rreg(rdev, (0x2a48), 0) >> 1) & 1) ? 16 : 32;
 tmp = r100_mm_rreg(rdev, (0x2760), 0);
 row = ((tmp & 0x00000038) >> 3) + 10;
 column = ((tmp & 0x000000C0) >> 6) + 8;
 bank = ((tmp & 0x00000003) >> 0) + 2;
...
static void si_get_leakage_vddc(struct radeon_device *rdev)
{
 ...
 int i, ret;
 for (i = 0; i < 4; i++) {
  ret = radeon_atom_get_leakage_vddc_based_on_leakage_idx(rdev, &vddc, 0xff01 + i);
  if (!ret && (vddc > 0) && (vddc != (0xff01 + i))) {
...
static void si_set_dpm_event_sources(struct radeon_device *rdev, u32 sources)
{
 ...
 if (want_thermal_protection) {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x700), 0); tmp_ &= (~(7 << 0)); tmp_ |= ((((dpm_event_src) << 0)) & ~(~(7 << 0))); r100_mm_wreg(rdev, (0x700), (tmp_), 0); } while (0);
  if (pi->thermal_protection)
   do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 2)); tmp_ |= ((0) & ~(~(1 << 2))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
 } else {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 2)); tmp_ |= (((1 << 2)) & ~(~(1 << 2))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
 }
}
...
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
}
static void si_stop_dpm(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
}
static void si_enable_sclk_control(struct radeon_device *rdev, _Bool enable)
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
}
static PPSMC_Result si_send_msg_to_smc_with_parameter(struct radeon_device *rdev,
            PPSMC_Msg msg, u32 parameter)
{
 r100_mm_wreg(rdev, (0x884), (parameter), 0);
 return si_send_msg_to_smc(rdev, msg);
}
static int si_restrict_performance_levels_before_switch(struct radeon_device *rdev)
{
 if (si_send_msg_to_smc(rdev, ((uint8_t)0x41)) != ((uint8_t)0x01))
  return -22;
 return (si_send_msg_to_smc_with_parameter(rdev, ((uint8_t)0x82), 1) == ((uint8_t)0x01)) ?
  0 : -22;
}
int si_dpm_force_performance_level(struct radeon_device *rdev,
       enum radeon_dpm_forced_level level)
{
 ...
 if (level == RADEON_DPM_FORCED_LEVEL_HIGH) {
  if (si_send_msg_to_smc_with_parameter(rdev, ((uint8_t)0x82), levels) != ((uint8_t)0x01))
   return -22;
  if (si_send_msg_to_smc_with_parameter(rdev, ((uint8_t)0x83), 1) != ((uint8_t)0x01))
   return -22;
 } else if (level == RADEON_DPM_FORCED_LEVEL_LOW) {
  if (si_send_msg_to_smc_with_parameter(rdev, ((uint8_t)0x83), 0) != ((uint8_t)0x01))
   return -22;
  if (si_send_msg_to_smc_with_parameter(rdev, ((uint8_t)0x82), 1) != ((uint8_t)0x01))
   return -22;
...
{
 return (si_send_msg_to_smc(rdev, ((uint8_t)0x20)) == ((uint8_t)0x01)) ?
  0 : -22;
}
static int si_halt_smc(struct radeon_device *rdev)
{
 if (si_send_msg_to_smc(rdev, ((uint8_t)0x10)) != ((uint8_t)0x01))
  return -22;
 return (si_wait_for_smc_inactive(rdev) == ((uint8_t)0x01)) ?
  0 : -22;
}
static int si_resume_smc(struct radeon_device *rdev)
{
 if (si_send_msg_to_smc(rdev, ((uint8_t)0x80)) != ((uint8_t)0x01))
  return -22;
 return (si_send_msg_to_smc(rdev, ((uint8_t)0x11)) == ((uint8_t)0x01)) ?
  0 : -22;
}
...
{
 struct si_power_info *si_pi = si_get_pi(rdev);
 si_pi->clock_registers.cg_spll_func_cntl = r100_mm_rreg(rdev, (0x600), 0);
 si_pi->clock_registers.cg_spll_func_cntl_2 = r100_mm_rreg(rdev, (0x604), 0);
 si_pi->clock_registers.cg_spll_func_cntl_3 = r100_mm_rreg(rdev, (0x608), 0);
 si_pi->clock_registers.cg_spll_func_cntl_4 = r100_mm_rreg(rdev, (0x60c), 0);
 si_pi->clock_registers.cg_spll_spread_spectrum = r100_mm_rreg(rdev, (0x620), 0);
 si_pi->clock_registers.cg_spll_spread_spectrum_2 = r100_mm_rreg(rdev, (0x624), 0);
 si_pi->clock_registers.dll_cntl = r100_mm_rreg(rdev, (0x2ba4), 0);
 si_pi->clock_registers.mclk_pwrmgt_cntl = r100_mm_rreg(rdev, (0x2ba0), 0);
 si_pi->clock_registers.mpll_ad_func_cntl = r100_mm_rreg(rdev, (0x2bc0), 0);
 si_pi->clock_registers.mpll_dq_func_cntl = r100_mm_rreg(rdev, (0x2bc4), 0);
 si_pi->clock_registers.mpll_func_cntl = r100_mm_rreg(rdev, (0x2bb4), 0);
 si_pi->clock_registers.mpll_func_cntl_1 = r100_mm_rreg(rdev, (0x2bb8), 0);
 si_pi->clock_registers.mpll_func_cntl_2 = r100_mm_rreg(rdev, (0x2bbc), 0);
 si_pi->clock_registers.mpll_ss1 = r100_mm_rreg(rdev, (0x2bcc), 0);
 si_pi->clock_registers.mpll_ss2 = r100_mm_rreg(rdev, (0x2bd0), 0);
}
...
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 1)); tmp_ |= (((1 << 1)) & ~(~(1 << 1))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
}
static int si_notify_smc_display_change(struct radeon_device *rdev,
         _Bool has_display)
{
 PPSMC_Msg msg = has_display ?
  ((uint8_t)0x5E) : ((uint8_t)0x5D);
 return (si_send_msg_to_smc(rdev, msg) == ((uint8_t)0x01)) ?
  0 : -22;
}
static void si_program_response_times(struct radeon_device *rdev)
{
 ...
 u32 reference_clock;
 si_write_smc_soft_register(rdev, 0x60, 1);
 ...
 si_write_smc_soft_register(rdev, 0xC, vddc_dly);
 si_write_smc_soft_register(rdev, 0x28, acpi_dly);
 si_write_smc_soft_register(rdev, 0x0, vbi_dly);
 si_write_smc_soft_register(rdev, 0x98, 0xAA);
}
static void si_program_ds_registers(struct radeon_device *rdev)
{
 ...
 if (eg_pi->sclk_deep_sleep) {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x670), 0); tmp_ &= (~0xFF); tmp_ |= ((((tmp) << 0)) & ~(~0xFF)); r100_mm_wreg(rdev, (0x670), (tmp_), 0); } while (0);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x62c), 0); tmp_ &= (~(1 << 9)); tmp_ |= (((1 << 9)) & ~(~(1 << 9))); r100_mm_wreg(rdev, (0x62c), (tmp_), 0); } while (0);
 }
}
static void si_program_display_gap(struct radeon_device *rdev)
{
 u32 tmp, pipe;
 int i;
 tmp = r100_mm_rreg(rdev, (0x828), 0) & ~((3 << 0) | (3 << 2));
...
static void si_enable_spread_spectrum(struct radeon_device *rdev, _Bool enable)
{
 struct rv7xx_power_info *pi = rv770_get_pi(rdev);
 if (enable) {
  if (pi->sclk_ss)
   do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 23)); tmp_ |= (((1 << 23)) & ~(~(1 << 23))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
 } else {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x620), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x620), (tmp_), 0); } while (0);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 23)); tmp_ |= ((0) & ~(~(1 << 23))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
 }
}
static void si_setup_bsp(struct radeon_device *rdev)
{
 ...
 r100_mm_wreg(rdev, (0x7fc), (pi->dsp), 0);
}
static void si_program_git(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x804), 0); tmp_ &= (~(0xffff << 0)); tmp_ |= ((((0x200) << 0)) & ~(~(0xffff << 0))); r100_mm_wreg(rdev, (0x804), (tmp_), 0); } while (0);
}
static void si_program_tp(struct radeon_device *rdev)
{
 ...
 enum r600_td td = 0;
 for (i = 0; i < 15; i++)
  r100_mm_wreg(rdev, (0x7c0 + (i * 4)), ((((r600_utc[i]) << 0) | ((r600_dtc[i]) << 10))), 0);
 if (td == R600_TD_AUTO)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 5)); tmp_ |= ((0) & ~(~(1 << 5))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 5)); tmp_ |= (((1 << 5)) & ~(~(1 << 5))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
 if (td == R600_TD_UP)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 6)); tmp_ |= ((0) & ~(~(1 << 6))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
 if (td == R600_TD_DOWN)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x788), 0); tmp_ &= (~(1 << 6)); tmp_ |= (((1 << 6)) & ~(~(1 << 6))); r100_mm_wreg(rdev, (0x788), (tmp_), 0); } while (0);
}
static void si_program_tpp(struct radeon_device *rdev)
{
 r100_mm_wreg(rdev, (0x784), (0x200), 0);
}
static void si_program_sstp(struct radeon_device *rdev)
{
 r100_mm_wreg(rdev, (0x80c), ((((0) << 16) | ((0x00C8) << 0))), 0);
}
static void si_enable_display_gap(struct radeon_device *rdev)
{
 u32 tmp = r100_mm_rreg(rdev, (0x828), 0);
 tmp &= ~((3 << 0) | (3 << 2));
 tmp |= (((R600_PM_DISPLAY_GAP_IGNORE) << 0) |
...
{
 struct rv7xx_power_info *pi = rv770_get_pi(rdev);
 r100_mm_wreg(rdev, (0x7bc), (pi->vrc), 0);
}
static void si_clear_vc(struct radeon_device *rdev)
{
 r100_mm_wreg(rdev, (0x7bc), (0), 0);
}
...
u8 si_get_mclk_frequency_ratio(u32 memory_clock, _Bool strobe_mode)
{
 u8 mc_para_index;
 if (strobe_mode) {
...
static _Bool si_validate_phase_shedding_tables(struct radeon_device *rdev,
           const struct atom_voltage_table *table,
...
static int si_construct_voltage_tables(struct radeon_device *rdev)
{
 ...
 if (pi->mvdd_control) {
  ...
  if (si_pi->mvdd_voltage_table.count > 32)
   si_trim_voltage_table_to_fit_state_table(rdev,
         32,
 ...
 if (si_pi->vddc_phase_shed_control) {
  ret = radeon_atom_get_voltage_table(rdev, 1,
          4, &si_pi->vddc_phase_shed_table);
  if (ret)
   si_pi->vddc_phase_shed_control = 0;
  if ((si_pi->vddc_phase_shed_table.count == 0) ||
      (si_pi->vddc_phase_shed_table.count > 32))
   si_pi->vddc_phase_shed_control = 0;
 }
 return 0;
}
       ...
       SISLANDS_SMC_STATETABLE *table)
{
 ...
 if (si_pi->voltage_control_svi2) {
  si_write_smc_soft_register(rdev, 0x120,
   si_pi->svc_gpio_id);
  si_write_smc_soft_register(rdev, 0x11c,
   si_pi->svd_gpio_id);
  si_write_smc_soft_register(rdev, 0x118,
        2);
 } else {
  ...
  if (si_pi->mvdd_voltage_table.count) {
   ...
   table->voltageMaskTable.lowMask[1] =
    ((__uint32_t)(si_pi->mvdd_voltage_table.mask_low));
  }
  if (si_pi->vddc_phase_shed_control) {
   if (si_validate_phase_shedding_tables(rdev, &si_pi->vddc_phase_shed_table,
             &rdev->pm.dpm.dyn_state.phase_shedding_limits_table)) {
    si_populate_smc_voltage_table(rdev, &si_pi->vddc_phase_shed_table, table);
    table->phaseMaskTable.lowMask[3] =
     ((__uint32_t)(si_pi->vddc_phase_shed_table.mask_low));
    si_write_smc_soft_register(rdev, 0x88,
      ...
      SISLANDS_SMC_VOLTAGE_VALUE *voltage)
{
 ...
 if (pi->mvdd_control) {
  ...
  voltage->value = ((__uint16_t)(si_pi->mvdd_voltage_table.entries[voltage->index].value));
 }
 return 0;
}
static int si_get_std_voltage_value(struct radeon_device *rdev,
 ...
 *std_voltage = ((__uint16_t)(voltage->value));
 if (rdev->pm.dpm.dyn_state.cac_leakage_table.entries) {
  if (rdev->pm.dpm.platform_caps & 0x00040000) {
...
{
 return (si_send_msg_to_smc(rdev, ((uint8_t)0x84)) == ((uint8_t)0x01)) ?
  0 : -22;
}
         ...
         u32 engine_clock)
{
 ...
 u32 mc_arb_rfsh_rate;
 u32 tmp = (r100_mm_rreg(rdev, (0x2760), 0) & 0x00000038) >> 3;
 if (tmp >= 4)
  dram_rows = 16384;
 else
  dram_rows = 1 << (tmp + 10);
 dram_refresh_rate = 1 << ((r100_mm_rreg(rdev, (0x2a00), 0) & 0x3) + 3);
        ...
        unsigned int first_arb_set)
{
 ...
 for (i = 0; i < state->performance_level_count; i++) {
  ...
  ret = si_copy_bytes_to_smc(rdev,
        ...
        __builtin_offsetof(SMC_SIslands_MCArbDramTimingRegisters, data) +
      ...
      SISLANDS_SMC_STATETABLE *table)
{
 ...
 table->initialState.levels[0].mclk.vDLL_CNTL =
  ((__uint32_t)(si_pi->clock_registers.dll_cntl));
 table->initialState.levels[0].mclk.vMCLK_PWRMGT_CNTL =
  ((__uint32_t)(si_pi->clock_registers.mclk_pwrmgt_cntl));
 table->initialState.levels[0].mclk.vMPLL_AD_FUNC_CNTL =
  ((__uint32_t)(si_pi->clock_registers.mpll_ad_func_cntl));
 table->initialState.levels[0].mclk.vMPLL_DQ_FUNC_CNTL =
  ((__uint32_t)(si_pi->clock_registers.mpll_dq_func_cntl));
 table->initialState.levels[0].mclk.vMPLL_FUNC_CNTL =
  ((__uint32_t)(si_pi->clock_registers.mpll_func_cntl));
 table->initialState.levels[0].mclk.vMPLL_FUNC_CNTL_1 =
  ((__uint32_t)(si_pi->clock_registers.mpll_func_cntl_1));
 table->initialState.levels[0].mclk.vMPLL_FUNC_CNTL_2 =
  ((__uint32_t)(si_pi->clock_registers.mpll_func_cntl_2));
 table->initialState.levels[0].mclk.vMPLL_SS =
  ((__uint32_t)(si_pi->clock_registers.mpll_ss1));
 table->initialState.levels[0].mclk.vMPLL_SS2 =
  ((__uint32_t)(si_pi->clock_registers.mpll_ss2));
 table->initialState.levels[0].mclk.mclk_value =
  ((__uint32_t)(initial_state->performance_levels[0].mclk));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_func_cntl));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL_2 =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_func_cntl_2));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL_3 =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_func_cntl_3));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL_4 =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_func_cntl_4));
 table->initialState.levels[0].sclk.vCG_SPLL_SPREAD_SPECTRUM =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_spread_spectrum));
 table->initialState.levels[0].sclk.vCG_SPLL_SPREAD_SPECTRUM_2 =
  ((__uint32_t)(si_pi->clock_registers.cg_spll_spread_spectrum_2));
          ...
          SISLANDS_SMC_STATETABLE *table)
{
 ...
 table->ACPIState.levels[0].mclk.vMPLL_FUNC_CNTL =
  ((__uint32_t)(mpll_func_cntl));
 table->ACPIState.levels[0].mclk.vMPLL_FUNC_CNTL_1 =
  ((__uint32_t)(mpll_func_cntl_1));
 table->ACPIState.levels[0].mclk.vMPLL_FUNC_CNTL_2 =
  ((__uint32_t)(mpll_func_cntl_2));
     ...
     SISLANDS_SMC_SWSTATE *state)
{
 ...
 if (!ret) {
  if (eg_pi->sclk_deep_sleep) {
   if (sclk_in_sr <= 1350)
    state->levels[0].stateFlags |= 0x40;
   else
    state->levels[0].stateFlags |= 0x20;
  }
  if (ulv->one_pcie_lane_in_ulv)
   state->flags |= 0x08;
  state->levels[0].arbRefreshState = (u8)(2);
...
static int si_program_ulv_memory_timing_parameters(struct radeon_device *rdev)
{
 ...
 if (ret)
  return ret;
 si_write_smc_soft_register(rdev, 0x8C,
       ulv->volt_change_delay);
 ret = si_copy_bytes_to_smc(rdev,
       si_pi->arb_table_start +
       __builtin_offsetof(SMC_SIslands_MCArbDramTimingRegisters, data) +
       sizeof(SMC_SIslands_MCArbDramTimingRegisterSet) * 2,
       (u8 *)&arb_regs,
       sizeof(SMC_SIslands_MCArbDramTimingRegisterSet),
       si_pi->sram_end);
 return ret;
...
static int si_init_smc_table(struct radeon_device *rdev)
{
 ...
 if (rdev->pm.dpm.platform_caps & 0x00080000)
  table->extraFlags |= 0x02;
 if (rdev->pm.dpm.platform_caps & 0x00200000) {
  table->systemFlags |= 0x40;
  vr_hot_gpio = rdev->pm.dpm.backbias_response_time;
  si_write_smc_soft_register(rdev, 0x100,
        vr_hot_gpio);
 }
 ...
 if (ulv->supported && ulv->pl.vddc) {
  ...
  if (ret)
   return ret;
  r100_mm_wreg(rdev, (0x878), (ulv->cg_ulv_control), 0);
  r100_mm_wreg(rdev, (0x87c), (ulv->cg_ulv_parameter), 0);
  lane_width = (rdev)->asic->pm.get_pcie_lanes((rdev));
  si_write_smc_soft_register(rdev, 0xF4, lane_width);
 } else {
      ...
      _Bool dll_state_on)
{
 ...
 mclk->vMPLL_FUNC_CNTL = ((__uint32_t)(mpll_func_cntl));
 mclk->vMPLL_FUNC_CNTL_1 = ((__uint32_t)(mpll_func_cntl_1));
 mclk->vMPLL_FUNC_CNTL_2 = ((__uint32_t)(mpll_func_cntl_2));
      ...
      SISLANDS_SMC_HW_PERFORMANCE_LEVEL *level)
{
 ...
 ret = si_populate_mclk_value(rdev,
         ...
         pl->mclk,
         &level->mclk,
         (level->strobeMode & 0x10) != 0, dll_state_on);
 if (ret)
  return ret;
...
static int si_disable_ulv(struct radeon_device *rdev)
{
 ...
 struct si_ulv_param *ulv = &si_pi->ulv;
 if (ulv->supported)
  return (si_send_msg_to_smc(rdev, ((uint8_t)0x63)) == ((uint8_t)0x01)) ?
   0 : -22;
 return 0;
}
static _Bool si_is_state_ulv_compatible(struct radeon_device *rdev,
           struct radeon_ps *radeon_state)
{
             ...
             struct radeon_ps *radeon_new_state)
{
 ...
 if (ulv->supported) {
  if (si_is_state_ulv_compatible(rdev, radeon_new_state))
   return (si_send_msg_to_smc(rdev, ((uint8_t)0x62)) == ((uint8_t)0x01)) ?
    0 : -22;
 }
 return 0;
}
      ...
      SISLANDS_SMC_SWSTATE *smc_state)
{
 ...
 u32 threshold;
 u32 sclk_in_sr = 1350;
 if (state->performance_level_count > 5)
  return -22;
 threshold = state->performance_levels[state->performance_level_count-1].sclk * 100 / 100;
 if (radeon_state->vclk && radeon_state->dclk) {
  eg_pi->uvd_enabled = 1;
  if (eg_pi->smu_uvd_hs)
   smc_state->flags |= 0x02;
 } else {
  eg_pi->uvd_enabled = 0;
 }
 ...
 for (i = 0; i < state->performance_level_count; i++) {
  if (eg_pi->sclk_deep_sleep) {
   if ((i == 0) || si_pi->sclk_deep_sleep_above_low) {
    if (sclk_in_sr <= 1350)
     smc_state->levels[i].stateFlags |= 0x40;
    else
     smc_state->levels[i].stateFlags |= 0x20;
   }
  }
  ...
  if (ni_pi->enable_power_containment)
   ...
   smc_state->levels[i].displayWatermark = (i < 2) ?
    0 : 1;
  if (eg_pi->dynamic_ac_timing)
   smc_state->levels[i].ACIndex = 3 + i;
  else
         ...
         struct radeon_ps *radeon_new_state)
{
 ...
 u32 address = si_pi->state_table_start +
  __builtin_offsetof(SISLANDS_SMC_STATETABLE, driverState);
 u32 state_size = sizeof(SISLANDS_SMC_SWSTATE) +
  ((new_state->performance_level_count - 1) *
   sizeof(SISLANDS_SMC_HW_PERFORMANCE_LEVEL));
 SISLANDS_SMC_SWSTATE *smc_state = &si_pi->smc_statetable.driverState;
 __builtin_memset((smc_state), (0), (state_size));
...
static int si_upload_ulv_state(struct radeon_device *rdev)
{
 ...
 if (ulv->supported && ulv->pl.vddc) {
  u32 address = si_pi->state_table_start +
   __builtin_offsetof(SISLANDS_SMC_STATETABLE, ULVState);
...
static int si_upload_smc_data(struct radeon_device *rdev)
{
 ...
 if (si_write_smc_soft_register(rdev,
           0xC4,
           radeon_crtc->crtc_id) != ((uint8_t)0x01))
  return 0;
 if (si_write_smc_soft_register(rdev,
           0xC8,
           radeon_crtc->wm_high / radeon_crtc->line_time) != ((uint8_t)0x01))
  return 0;
 if (si_write_smc_soft_register(rdev,
           0xCC,
           radeon_crtc->wm_low / radeon_crtc->line_time) != ((uint8_t)0x01))
  return 0;
 return 0;
}
...
static _Bool si_check_s0_mc_reg_index(u16 in_reg, u16 *out_reg)
{
 _Bool result = 1;
 switch (in_reg) {
 case 0x28a0 >> 2:
 ...
 for (i = 0, j = 0; j < si_pi->mc_reg_table.last; j++) {
  if (si_pi->mc_reg_table.valid_flag & (1 << j)) {
   ...
   mc_reg_table->address[i].s0 =
    ((__uint16_t)(si_pi->mc_reg_table.mc_reg_address[j].s0));
   mc_reg_table->address[i].s1 =
    ((__uint16_t)(si_pi->mc_reg_table.mc_reg_address[j].s1));
   i++;
  }
 }
        ...
        struct radeon_ps *radeon_boot_state)
{
 ...
 __builtin_memset((smc_mc_reg_table), (0), (sizeof(SMC_SIslands_MCRegisters)));
 si_write_smc_soft_register(rdev, 0x5C, 1);
      ...
      struct radeon_ps *radeon_new_state)
{
 ...
 u32 address = si_pi->mc_reg_table_start +
  __builtin_offsetof(SMC_SIslands_MCRegisters, data[3]);
 SMC_SIslands_MCRegisters *smc_mc_reg_table = &si_pi->smc_mc_reg_table;
 __builtin_memset((smc_mc_reg_table), (0), (sizeof(SMC_SIslands_MCRegisters)));
 si_convert_mc_reg_table_to_smc(rdev, radeon_new_state, smc_mc_reg_table);
 return si_copy_bytes_to_smc(rdev, address,
        (u8 *)&smc_mc_reg_table->data[3],
        sizeof(SMC_SIslands_MCRegisterSet) * new_state->performance_level_count,
        si_pi->sram_end);
}
static void si_enable_voltage_control(struct radeon_device *rdev, _Bool enable)
{
        if (enable)
                do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 10)); tmp_ |= (((1 << 10)) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
        else
                do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x780), 0); tmp_ &= (~(1 << 10)); tmp_ |= ((0) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x780), (tmp_), 0); } while (0);
}
...
{
 u32 speed_cntl;
 speed_cntl = rdev->pciep_rreg(rdev, (0xa4)) & (0x3 << 13);
 speed_cntl >>= 13;
       ...
       struct radeon_ps *radeon_current_state)
{
 u32 lane_width;
 u32 new_lane_width =
  ((radeon_new_state->caps & 0x000000F8) >> 3) + 1;
 u32 current_lane_width =
  ((radeon_current_state->caps & 0x000000F8) >> 3) + 1;
 if (new_lane_width != current_lane_width) {
  (rdev)->asic->pm.set_pcie_lanes((rdev), (new_lane_width));
  lane_width = (rdev)->asic->pm.get_pcie_lanes((rdev));
  si_write_smc_soft_register(rdev, 0xF4, lane_width);
 }
}
...
static int si_thermal_enable_alert(struct radeon_device *rdev,
       _Bool enable)
{
 u32 thermal_int = r100_mm_rreg(rdev, (0x708), 0);
 if (enable) {
  PPSMC_Result result;
  thermal_int &= ~((1 << 24) | (1 << 25));
  r100_mm_wreg(rdev, (0x708), (thermal_int), 0);
  rdev->irq.dpm_thermal = 0;
  result = si_send_msg_to_smc(rdev, ((uint8_t)0x16));
         ...
         int min_temp, int max_temp)
{
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x708), 0); tmp_ &= (~0x0000FF00); tmp_ |= ((((high_temp / 1000) << 8)) & ~(~0x0000FF00)); r100_mm_wreg(rdev, (0x708), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x708), 0); tmp_ &= (~0x00FF0000); tmp_ |= ((((low_temp / 1000) << 16)) & ~(~0x00FF0000)); r100_mm_wreg(rdev, (0x708), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x700), 0); tmp_ &= (~0x003FC000); tmp_ |= ((((high_temp / 1000) << 14)) & ~(~0x003FC000)); r100_mm_wreg(rdev, (0x700), (tmp_), 0); } while (0);
...
static void si_fan_ctrl_set_static_mode(struct radeon_device *rdev, u32 mode)
{
 ...
 if (si_pi->fan_ctrl_is_in_default_mode) {
  tmp = (r100_mm_rreg(rdev, (0x75C), 0) & (7 << 11)) >> 11;
  si_pi->fan_ctrl_default_mode = tmp;
  tmp = (r100_mm_rreg(rdev, (0x75C), 0) & 0x000000FF) >> 0;
  si_pi->t_min = tmp;
  si_pi->fan_ctrl_is_in_default_mode = 0;
 }
 tmp = r100_mm_rreg(rdev, (0x75C), 0) & ~0x000000FF;
 tmp |= ((0) << 0);
 r100_mm_wreg(rdev, (0x75C), (tmp), 0);
 tmp = r100_mm_rreg(rdev, (0x75C), 0) & ~(7 << 11);
 tmp |= ((mode) << 11);
 r100_mm_wreg(rdev, (0x75C), (tmp), 0);
}
...
{
 struct si_power_info *si_pi = si_get_pi(rdev);
 PP_SIslands_FanTable fan_table = { 0 };
 u32 duty100;
 ...
 duty100 = (r100_mm_rreg(rdev, (0x758), 0) & 0x000000FF) >> 0;
 if (duty100 == 0) {
  rdev->pm.dpm.fan.ucode_fan_control = 0;
  return 0;
 }
 ...
 fan_table.temp_min = ((__uint16_t)((50 + rdev->pm.dpm.fan.t_min) / 100));
 fan_table.temp_med = ((__uint16_t)((50 + rdev->pm.dpm.fan.t_med) / 100));
 fan_table.temp_max = ((__uint16_t)((50 + rdev->pm.dpm.fan.t_max) / 100));
 fan_table.slope1 = ((__uint16_t)(slope1));
 fan_table.slope2 = ((__uint16_t)(slope2));
 fan_table.fdo_min = ((__uint16_t)(fdo_min));
 fan_table.hys_down = ((__uint16_t)(rdev->pm.dpm.fan.t_hyst));
 fan_table.hys_up = ((__uint16_t)(1));
 fan_table.hys_slope = ((__uint16_t)(1));
 fan_table.temp_resp_lim = ((__uint16_t)(5));
 reference_clock = (rdev)->asic->get_xclk((rdev));
 fan_table.refresh_period = ((__uint32_t)((rdev->pm.dpm.fan.cycle_delay * reference_clock) / 1600));
 fan_table.fdo_max = ((__uint16_t)((u16)duty100));
 tmp = (r100_mm_rreg(rdev, (0x710), 0) & (0xff << 20)) >> 20;
...
static int si_fan_ctrl_start_smc_fan_control(struct radeon_device *rdev)
{
 struct si_power_info *si_pi = si_get_pi(rdev);
 PPSMC_Result ret;
 ret = si_send_msg_to_smc(rdev, ((uint8_t)0x5B));
 if (ret == ((uint8_t)0x01)) {
...
static int si_fan_ctrl_stop_smc_fan_control(struct radeon_device *rdev)
{
 struct si_power_info *si_pi = si_get_pi(rdev);
 PPSMC_Result ret;
 ret = si_send_msg_to_smc(rdev, ((uint8_t)0x5C));
 if (ret == ((uint8_t)0x01)) {
          ...
          u32 *speed)
{
 ...
 if (rdev->pm.no_fan)
  return -2;
 duty100 = (r100_mm_rreg(rdev, (0x758), 0) & 0x000000FF) >> 0;
 duty = (r100_mm_rreg(rdev, (0x704), 0) & (0xff << 9)) >> 9;
 if (duty100 == 0)
  return -22;
 tmp64 = (u64)duty * 100;
          ...
          u32 speed)
{
 ...
 duty = (u32)tmp64;
 tmp = r100_mm_rreg(rdev, (0x754), 0) & ~0x000000FF;
 tmp |= ((duty) << 0);
 r100_mm_wreg(rdev, (0x754), (tmp), 0);
 return 0;
}
void si_fan_ctrl_set_mode(struct radeon_device *rdev, u32 mode)
{
...
static void si_fan_ctrl_set_default_mode(struct radeon_device *rdev)
{
 ...
 if (!si_pi->fan_ctrl_is_in_default_mode) {
  tmp = r100_mm_rreg(rdev, (0x75C), 0) & ~(7 << 11);
  tmp |= ((si_pi->fan_ctrl_default_mode) << 11);
  r100_mm_wreg(rdev, (0x75C), (tmp), 0);
  tmp = r100_mm_rreg(rdev, (0x75C), 0) & ~0x000000FF;
  tmp |= ((si_pi->t_min) << 0);
  r100_mm_wreg(rdev, (0x75C), (tmp), 0);
...
static void si_thermal_start_smc_fan_control(struct radeon_device *rdev)
{
 if (rdev->pm.dpm.fan.ucode_fan_control) {
  ...
  si_fan_ctrl_set_static_mode(rdev, 1);
 }
}
static void si_thermal_initialize(struct radeon_device *rdev)
{
 ...
 if (rdev->pm.fan_pulses_per_revolution) {
  tmp = r100_mm_rreg(rdev, (0x770), 0) & ~(0x7 << 0);
  tmp |= ((rdev->pm.fan_pulses_per_revolution -1) << 0);
  r100_mm_wreg(rdev, (0x770), (tmp), 0);
 }
 tmp = r100_mm_rreg(rdev, (0x75C), 0) & ~(0x7f << 25);
 tmp |= ((0x28) << 25);
 r100_mm_wreg(rdev, (0x75C), (tmp), 0);
}
static int si_thermal_start_thermal_controller(struct radeon_device *rdev)
{
 ...
 ret = si_thermal_set_temperature_range(rdev, (90 * 1000), (120 * 1000));
 if (ret)
  return ret;
...
int si_dpm_enable(struct radeon_device *rdev)
{
 ...
 if (pi->voltage_control || si_pi->voltage_control_svi2) {
  ...
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_construct_voltage_tables failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 if (eg_pi->dynamic_ac_timing) {
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_upload_firmware failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_process_firmware_header(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_process_firmware_header failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_initial_switch_from_arb_f0_to_f1(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_initial_switch_from_arb_f0_to_f1 failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_init_smc_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_init_smc_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_init_smc_spll_table(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_init_smc_spll_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_init_arb_table_index(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_init_arb_table_index failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 if (eg_pi->dynamic_ac_timing) {
  ret = si_populate_mc_reg_table(rdev, boot_ps);
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_populate_mc_reg_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 ret = si_initialize_smc_cac_tables(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_initialize_smc_cac_tables failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_initialize_hardware_cac_manager(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_initialize_hardware_cac_manager failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_initialize_smc_dte_tables(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_initialize_smc_dte_tables failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_populate_smc_tdp_limits(rdev, boot_ps);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_populate_smc_tdp_limits failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_populate_smc_tdp_limits_2(rdev, boot_ps);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_populate_smc_tdp_limits_2 failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_notify_smc_display_change failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
...
int si_dpm_set_power_state(struct radeon_device *rdev)
{
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_disable_ulv failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_restrict_performance_levels_before_switch(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_restrict_performance_levels_before_switch failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 if (eg_pi->pcie_performance_request)
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_enable_power_containment failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_enable_smc_cac(rdev, new_ps, 0);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_enable_smc_cac failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_halt_smc(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_halt_smc failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_upload_sw_state(rdev, new_ps);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_upload_sw_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_upload_smc_data(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_upload_smc_data failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_upload_ulv_state(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_upload_ulv_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 if (eg_pi->dynamic_ac_timing) {
  ret = si_upload_mc_reg_table(rdev, new_ps);
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_upload_mc_reg_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 ret = si_program_memory_timing_parameters(rdev, new_ps);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_program_memory_timing_parameters failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 si_set_pcie_lane_width_in_smc(rdev, new_ps, old_ps);
 ret = si_resume_smc(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_resume_smc failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ret = si_set_sw_state(rdev);
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_set_sw_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_set_power_state_conditionally_enable_ulv failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "si_power_control_set_level failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 return 0;
}
          ...
          union pplib_clock_info *clock_info)
{
 ...
 pl->sclk = (__uint16_t)(__builtin_constant_p(clock_info->si.usEngineClockLow) ? (__uint16_t)(((__uint16_t)(clock_info->si.usEngineClockLow) & 0xffU) << 8 | ((__uint16_t)(clock_info->si.usEngineClockLow) & 0xff00U) >> 8) : __swap16md(clock_info->si.usEngineClockLow));
 pl->sclk |= clock_info->si.ucEngineClockHigh << 16;
 pl->mclk = (__uint16_t)(__builtin_constant_p(clock_info->si.usMemoryClockLow) ? (__uint16_t)(((__uint16_t)(clock_info->si.usMemoryClockLow) & 0xffU) << 8 | ((__uint16_t)(clock_info->si.usMemoryClockLow) & 0xff00U) >> 8) : __swap16md(clock_info->si.usMemoryClockLow));
 pl->mclk |= clock_info->si.ucMemoryClockHigh << 16;
 pl->vddc = (__uint16_t)(__builtin_constant_p(clock_info->si.usVDDC) ? (__uint16_t)(((__uint16_t)(clock_info->si.usVDDC) & 0xffU) << 8 | ((__uint16_t)(clock_info->si.usVDDC) & 0xff00U) >> 8) : __swap16md(clock_info->si.usVDDC));
 pl->vddci = (__uint16_t)(__builtin_constant_p(clock_info->si.usVDDCI) ? (__uint16_t)(((__uint16_t)(clock_info->si.usVDDCI) & 0xffU) << 8 | ((__uint16_t)(clock_info->si.usVDDCI) & 0xff00U) >> 8) : __swap16md(clock_info->si.usVDDCI));
 pl->flags = (__uint32_t)(__builtin_constant_p(clock_info->si.ulFlags) ? (__uint32_t)(((__uint32_t)(clock_info->si.ulFlags) & 0xff) << 24 | ((__uint32_t)(clock_info->si.ulFlags) & 0xff00) << 8 | ((__uint32_t)(clock_info->si.ulFlags) & 0xff0000) >> 8 | ((__uint32_t)(clock_info->si.ulFlags) & 0xff000000) >> 24) : __swap32md(clock_info->si.ulFlags));
 ...
 if ((rps->class2 & 0x0002) &&
  ...
  si_pi->ulv.volt_change_delay = 1687;
  si_pi->ulv.cg_ulv_parameter = 0x00040035;
  si_pi->ulv.cg_ulv_control = 0x1f007550;
 }
...
int si_dpm_init(struct radeon_device *rdev)
{
 ...
 int ret;
 u32 mask;
 si_pi = kzalloc(sizeof(struct si_power_info), (0x0001 | 0x0004));
 if (si_pi == ((void *)0))
  return -12;
 ...
 pi->voltage_control =
  radeon_atom_is_voltage_gpio(rdev, 1,
         0);
 if (!pi->voltage_control) {
  si_pi->voltage_control_svi2 =
   radeon_atom_is_voltage_gpio(rdev, 1,
          7);
  if (si_pi->voltage_control_svi2)
   radeon_atom_get_svi2_info(rdev, 1,
        &si_pi->svd_gpio_id, &si_pi->svc_gpio_id);
 }
 pi->mvdd_control =
  radeon_atom_is_voltage_gpio(rdev, 2,
         0);
 eg_pi->vddci_control =
  radeon_atom_is_voltage_gpio(rdev, 4,
         0);
 if (!eg_pi->vddci_control)
          ...
          struct seq_file *m)
{
 ...
 struct rv7xx_pl *pl;
 u32 current_index =
  (r100_mm_rreg(rdev, (0x798), 0) & (0xf << 4)) >>
  4;
