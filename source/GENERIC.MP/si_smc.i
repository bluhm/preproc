...
struct SISLANDS_SMC_STATETABLE
{
    ...
    SISLANDS_SMC_HW_PERFORMANCE_LEVEL dpmLevels[16 - 1];
};
...
struct SMC_SIslands_MCRegisters
{
    uint8_t last;
    uint8_t reserved[3];
    SMC_SIslands_MCRegisterAddress address[16];
    SMC_SIslands_MCRegisterSet data[20];
};
...
_Bool si_is_smc_running(struct radeon_device *rdev);
       ...
       u32 smc_address, u32 limit)
{
 ...
 if ((smc_address + 3) > limit)
  return -22;
 r100_mm_wreg(rdev, (0x200), (smc_address), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x228), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x228), (tmp_), 0); } while (0);
 return 0;
}
int si_copy_bytes_to_smc(struct radeon_device *rdev,
    u32 smc_start_address,
...
void si_start_smc(struct radeon_device *rdev)
{
 u32 tmp = tn_smc_rreg(rdev, (0x80000000));
 tmp &= ~(1 << 0);
 tn_smc_wreg(rdev, (0x80000000), (tmp));
}
void si_reset_smc(struct radeon_device *rdev)
{
 u32 tmp;
 r100_mm_rreg(rdev, (0x9a60), 0);
 r100_mm_rreg(rdev, (0x9a60), 0);
 r100_mm_rreg(rdev, (0x9a60), 0);
 r100_mm_rreg(rdev, (0x9a60), 0);
 tmp = tn_smc_rreg(rdev, (0x80000000));
 tmp |= (1 << 0);
 tn_smc_wreg(rdev, (0x80000000), (tmp));
}
int si_program_jump_on_start(struct radeon_device *rdev)
...
void si_stop_smc_clock(struct radeon_device *rdev)
{
 u32 tmp = tn_smc_rreg(rdev, (0x80000004));
 tmp |= (1 << 0);
 tn_smc_wreg(rdev, (0x80000004), (tmp));
}
void si_start_smc_clock(struct radeon_device *rdev)
{
 u32 tmp = tn_smc_rreg(rdev, (0x80000004));
...
_Bool si_is_smc_running(struct radeon_device *rdev)
{
 u32 rst = tn_smc_rreg(rdev, (0x80000000));
 u32 clk = tn_smc_rreg(rdev, (0x80000004));
 if (!(rst & (1 << 0)) && !(clk & (1 << 0)))
  return 1;
 return 0;
}
PPSMC_Result si_send_msg_to_smc(struct radeon_device *rdev, PPSMC_Msg msg)
{
 ...
 if (!si_is_smc_running(rdev))
  return ((uint8_t)0xFF);
 r100_mm_wreg(rdev, (0x22c), (msg), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  tmp = r100_mm_rreg(rdev, (0x230), 0);
  if (tmp != 0)
   break;
  udelay(1);
 }
...
PPSMC_Result si_wait_for_smc_inactive(struct radeon_device *rdev)
{
 ...
 if (!si_is_smc_running(rdev))
  return ((uint8_t)0x01);
 for (i = 0; i < rdev->usec_timeout; i++) {
  tmp = tn_smc_rreg(rdev, (0x80000004));
  if ((tmp & (1 << 24)) == 0)
   break;
  udelay(1);
 }
...
int si_load_smc_ucode(struct radeon_device *rdev, u32 limit)
{
 ...
 if (rdev->new_fw) {
  ...
  ucode_start_address = (__uint32_t)(__builtin_constant_p(hdr->ucode_start_addr) ? (__uint32_t)(((__uint32_t)(hdr->ucode_start_addr) & 0xff) << 24 | ((__uint32_t)(hdr->ucode_start_addr) & 0xff00) << 8 | ((__uint32_t)(hdr->ucode_start_addr) & 0xff0000) >> 8 | ((__uint32_t)(hdr->ucode_start_addr) & 0xff000000) >> 24) : __swap32md(hdr->ucode_start_addr));
  ucode_size = (__uint32_t)(__builtin_constant_p(hdr->header.ucode_size_bytes) ? (__uint32_t)(((__uint32_t)(hdr->header.ucode_size_bytes) & 0xff) << 24 | ((__uint32_t)(hdr->header.ucode_size_bytes) & 0xff00) << 8 | ((__uint32_t)(hdr->header.ucode_size_bytes) & 0xff0000) >> 8 | ((__uint32_t)(hdr->header.ucode_size_bytes) & 0xff000000) >> 24) : __swap32md(hdr->header.ucode_size_bytes));
  src = (const u8 *)
   (rdev->smc_fw->data + (__uint32_t)(__builtin_constant_p(hdr->header.ucode_array_offset_bytes) ? (__uint32_t)(((__uint32_t)(hdr->header.ucode_array_offset_bytes) & 0xff) << 24 | ((__uint32_t)(hdr->header.ucode_array_offset_bytes) & 0xff00) << 8 | ((__uint32_t)(hdr->header.ucode_array_offset_bytes) & 0xff0000) >> 8 | ((__uint32_t)(hdr->header.ucode_array_offset_bytes) & 0xff000000) >> 24) : __swap32md(hdr->header.ucode_array_offset_bytes)));
 } else {
  switch (rdev->family) {
  case CHIP_TAHITI:
   ucode_start_address = 0x10000;
   ucode_size = 0xf458;
   break;
  case CHIP_PITCAIRN:
   ucode_start_address = 0x10000;
  ...
  default:
   ...
   do { panic("BUG at %s:%d", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/si_smc.c", 255); } while (0);
  }
 ...
 if (ucode_size & 3)
  return -22;
 _spin_lock_irqsave(&rdev->smc_idx_lock, flags );
 r100_mm_wreg(rdev, (0x200), (ucode_start_address), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x228), 0); tmp_ &= (~(1 << 0)); tmp_ |= (((1 << 0)) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x228), (tmp_), 0); } while (0);
 while (ucode_size >= 4) {
  data = (src[0] << 24) | (src[1] << 16) | (src[2] << 8) | src[3];
  r100_mm_wreg(rdev, (0x204), (data), 0);
  src += 4;
