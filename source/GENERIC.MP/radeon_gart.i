...
int radeon_gart_table_vram_pin(struct radeon_device *rdev)
{
 ...
 if (!r) {
  ...
  for (i = 0; i < rdev->gart.num_gpu_pages; i++)
   (rdev)->asic->gart.set_page((rdev), (i), (rdev->gart.pages_entry[i]));
  __asm volatile("membar " "#Sync" ::: "memory");
 ...
 for (i = 0; i < pages; i++, p++) {
  if (rdev->gart.pages[p]) {
   ...
   for (j = 0; j < ((1 << 13) / 4096); j++, t++) {
    ...
    if (rdev->gart.ptr) {
     (rdev)->asic->gart.set_page((rdev), (t), (rdev->dummy_page.entry));
    }
   }
  }
 }
       ...
       uint32_t flags)
{
 ...
 for (i = 0; i < pages; i++, p++) {
  ...
  for (j = 0; j < ((1 << 13) / 4096); j++, t++) {
   page_entry = (rdev)->asic->gart.get_page_entry((page_base), (flags));
   rdev->gart.pages_entry[t] = page_entry;
   if (rdev->gart.ptr) {
    (rdev)->asic->gart.set_page((rdev), (t), (page_entry));
   }
   page_base += 4096;
  }
 }
...
int radeon_gart_init(struct radeon_device *rdev)
{
 ...
 if (rdev->gart.pages_entry == ((void *)0)) {
  radeon_gart_fini(rdev);
  return -12;
 }
 for (i = 0; i < rdev->gart.num_gpu_pages; i++)
