           ...
           unsigned mem_type, int sign)
{
 ...
 u64 size = (u64)bo->tbo.num_pages << 13;
 switch (mem_type) {
 case 1:
  if (sign > 0)
   __sync_fetch_and_add_8(&rdev->gtt_usage, size);
  else
   __sync_fetch_and_sub_8(&rdev->gtt_usage, size);
  break;
 case 2:
  if (sign > 0)
   __sync_fetch_and_add_8(&rdev->vram_usage, size);
  else
   __sync_fetch_and_sub_8(&rdev->vram_usage, size);
  break;
 }
}
...
{
 struct radeon_bo *bo;
 bo = ({ const __typeof( ((struct radeon_bo *)0)->tbo ) *__mptr = (tbo); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, tbo) );});
 radeon_update_memory_usage(bo, bo->tbo.mem.mem_type, -1);
 _rw_enter_write(&bo->rdev->gem.mutex );
 list_del_init(&bo->list);
 _rw_exit_write(&bo->rdev->gem.mutex );
 radeon_bo_clear_surface_reg(bo);
 ({ int __ret = !!(!list_empty(&bo->va)); if (__ret) printf("WARNING %s failed at %s:%d\n", "!list_empty(&bo->va)", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_object.c", 82); __builtin_expect(!!(__ret), 0); });
...
void radeon_ttm_placement_from_domain(struct radeon_bo *rbo, u32 domain)
{
 ...
 if (domain & 0x4) {
  if ((rbo->flags & (1 << 4)) &&
   ...
   rbo->placements[c].fpfn =
    rbo->rdev->mc.visible_vram_size >> 13;
   rbo->placements[c++].flags = (1 << 18) |
           (1 << 17) |
           (1 << 2);
  }
 ...
 if (domain & 0x2) {
  if (rbo->flags & (1 << 1)) {
   rbo->placements[c].fpfn = 0;
   rbo->placements[c++].flags = (1 << 17) |
    (1 << 1);
  } else if ((rbo->flags & (1 << 2)) ||
 ...
 if (!c) {
  rbo->placements[c].fpfn = 0;
  rbo->placements[c++].flags = ((1 << 16) | (1 << 17) | (1 << 18)) |
          (1 << 0);
 }
...
int radeon_bo_create(struct radeon_device *rdev,
       unsigned long size, int byte_align, _Bool kernel,
        ...
        u64 *gpu_addr)
{
 ...
 if (bo->pin_count) {
  ...
  if (max_offset != 0) {
   ...
   ({ static int __warned; int __ret = !!(max_offset < (radeon_bo_gpu_offset(bo) - domain_start)); if (__ret && !__warned) { printf("WARNING %s failed at %s:%d\n", "max_offset < (radeon_bo_gpu_offset(bo) - domain_start)", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_object.c", 349); __warned = 1; } __builtin_expect(!!(__ret), 0); });
  }
  return 0;
 }
 radeon_ttm_placement_from_domain(bo, domain);
 for (i = 0; i < bo->placement.num_placement; i++) {
  if ((bo->placements[i].flags & (1 << 2)) &&
      !(bo->flags & (1 << 4)) &&
      (!max_offset || max_offset > bo->rdev->mc.visible_vram_size))
   bo->placements[i].lpfn =
    bo->rdev->mc.visible_vram_size >> 13;
  else
   bo->placements[i].lpfn = max_offset >> 13;
  bo->placements[i].flags |= (1 << 21);
 }
...
void radeon_bo_fini(struct radeon_device *rdev)
{
 ...
 drm_mtrr_del(0, rdev->mc.aper_base, rdev->mc.aper_size, (1<<1));
}
       ...
       struct list_head *head, int ring)
{
 ...
 r = ttm_eu_reserve_buffers(ticket, head, 1, &duplicates);
 if (__builtin_expect(!!(r != 0), 0)) {
  return r;
 }
 for (lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = ((head)->next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );}); &lobj->tv.head != (head); lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = (lobj->tv.head.next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );})) {
  ...
  retry:
   ...
   bytes_moved += (*(&rdev->num_bytes_moved)) -
           initial_bytes_moved;
   if (__builtin_expect(!!(r), 0)) {
    if (r != -4 &&
 ...
 for (lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = ((&duplicates)->next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );}); &lobj->tv.head != (&duplicates); lobj = ({ const __typeof( ((__typeof(*lobj) *)0)->tv.head ) *__mptr = (lobj->tv.head.next); (__typeof(*lobj) *)( (char *)__mptr - __builtin_offsetof(__typeof(*lobj), tv.head) );})) {
      ...
      struct ttm_mem_reg *new_mem)
{
 ...
 if (!radeon_ttm_bo_is_radeon_bo(bo))
  return;
 rbo = ({ const __typeof( ((struct radeon_bo *)0)->tbo ) *__mptr = (bo); (struct radeon_bo *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo, tbo) );});
...
int radeon_bo_fault_reserve_notify(struct ttm_buffer_object *bo)
{
 ...
 lpfn = rdev->mc.visible_vram_size >> 13;
 for (i = 0; i < rbo->placement.num_placement; i++) {
  if ((rbo->placements[i].flags & (1 << 2)) &&
...
{
 int r;
 r = ttm_bo_reserve(&bo->tbo, 1, no_wait, 0, ((void *)0));
 if (__builtin_expect(!!(r != 0), 0))
  return r;
 if (mem_type)
