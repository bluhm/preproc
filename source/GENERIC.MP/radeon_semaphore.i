...
_Bool radeon_semaphore_emit_signal(struct radeon_device *rdev, int ridx,
             struct radeon_semaphore *semaphore)
{
 struct radeon_ring *ring = &rdev->ring[ridx];
 trace_radeon_semaphore_signale(ridx, semaphore);
 if ((rdev)->asic->ring[(ridx)]->emit_semaphore((rdev), (ring), (semaphore), (0))) {
  --semaphore->waiters;
...
_Bool radeon_semaphore_emit_wait(struct radeon_device *rdev, int ridx,
           struct radeon_semaphore *semaphore)
{
 struct radeon_ring *ring = &rdev->ring[ridx];
 trace_radeon_semaphore_wait(ridx, semaphore);
 if ((rdev)->asic->ring[(ridx)]->emit_semaphore((rdev), (ring), (semaphore), (1))) {
  ++semaphore->waiters;
