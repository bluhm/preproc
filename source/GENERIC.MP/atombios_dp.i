...
void radeon_atom_copy_swap(u8 *dst, u8 *src, u8 num_bytes, _Bool to_le)
{
 ...
 int i;
 u8 align_num_bytes = (((num_bytes)+((4)-1))&(~((4)-1)));
 if (to_le) {
  __builtin_memcpy((src_tmp), (src), (num_bytes));
  for (i = 0; i < align_num_bytes / 4; i++)
   dst_tmp[i] = (__uint32_t)(__builtin_constant_p(src_tmp[i]) ? (__uint32_t)(((__uint32_t)(src_tmp[i]) & 0xff) << 24 | ((__uint32_t)(src_tmp[i]) & 0xff00) << 8 | ((__uint32_t)(src_tmp[i]) & 0xff0000) >> 8 | ((__uint32_t)(src_tmp[i]) & 0xff000000) >> 24) : __swap32md(src_tmp[i]));
  __builtin_memcpy((dst), (dst_tmp), (align_num_bytes));
 } else {
  __builtin_memcpy((src_tmp), (src), (align_num_bytes));
  for (i = 0; i < align_num_bytes / 4; i++)
   dst_tmp[i] = (__uint32_t)(__builtin_constant_p(src_tmp[i]) ? (__uint32_t)(((__uint32_t)(src_tmp[i]) & 0xff) << 24 | ((__uint32_t)(src_tmp[i]) & 0xff00) << 8 | ((__uint32_t)(src_tmp[i]) & 0xff0000) >> 8 | ((__uint32_t)(src_tmp[i]) & 0xff000000) >> 24) : __swap32md(src_tmp[i]));
     ...
     u8 delay, u8 *ack)
{
 ...
 _rw_enter_write(&rdev->mode_info.atom_context->scratch_mutex );
 ...
 *ack = args.v1.ucReplyStatus;
 ...
 _rw_exit_write(&rdev->mode_info.atom_context->scratch_mutex );
...
radeon_dp_aux_transfer_atom(struct drm_dp_aux *aux, struct drm_dp_aux_msg *msg)
{
 struct radeon_i2c_chan *chan =
  ({ const __typeof( ((struct radeon_i2c_chan *)0)->aux ) *__mptr = (aux); (struct radeon_i2c_chan *)( (char *)__mptr - __builtin_offsetof(struct radeon_i2c_chan, aux) );});
 int ret;
 u8 tx_buf[20];
 size_t tx_size;
 u8 ack, delay = 0;
 if (({ int __ret = !!(msg->size > 16); if (__ret) printf("WARNING %s failed at %s:%d\n", "msg->size > 16", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/atombios_dp.c", 170); __builtin_expect(!!(__ret), 0); }))
  return -7;
 ...
 case 0x0:
 case 0x2:
  if (({ static int __warned; int __ret = !!(msg->size > 12); if (__ret && !__warned) { printf("WARNING %s failed at %s:%d\n", "msg->size > 12", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/atombios_dp.c", 188); __warned = 1; } __builtin_expect(!!(__ret), 0); }))
   return -7;
...
{
 struct radeon_encoder *radeon_encoder = ({ const __typeof( ((struct radeon_encoder *)0)->base ) *__mptr = (dp_info->encoder); (struct radeon_encoder *)( (char *)__mptr - __builtin_offsetof(struct radeon_encoder, base) );});
