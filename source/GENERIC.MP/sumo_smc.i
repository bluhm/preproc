...
static void sumo_send_msg_to_smu(struct radeon_device *rdev, u32 id)
{
 ...
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (r100_mm_rreg(rdev, (0x124), 0) & (1 << 1))
   break;
  udelay(1);
 }
 gfx_int_req = ((id) << 1) | (1 << 0);
 r100_mm_wreg(rdev, (0x120), (gfx_int_req), 0);
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (r100_mm_rreg(rdev, (0x120), 0) & (1 << 0))
   break;
  udelay(1);
 }
 for (i = 0; i < rdev->usec_timeout; i++) {
  if (r100_mm_rreg(rdev, (0x124), 0) & (1 << 0))
   break;
  udelay(1);
 }
 for (i = 0; i < rdev->usec_timeout; i++) {
...
void sumo_initialize_m3_arb(struct radeon_device *rdev)
{
 ...
 if (!pi->enable_dynamic_m3_arbiter)
  return;
 for (i = 0; i < 10; i++)
  r600_rcu_wreg(rdev, (0x508 + (i * 4)), (pi->sys_info.csr_m3_arb_cntl_default[i]));
 for (; i < 10 * 2; i++)
  r600_rcu_wreg(rdev, (0x508 + (i * 4)), (pi->sys_info.csr_m3_arb_cntl_uvd[i % 10]));
 for (; i < 10 * 3; i++)
  r600_rcu_wreg(rdev, (0x508 + (i * 4)), (pi->sys_info.csr_m3_arb_cntl_fs3d[i % 10]));
}
static _Bool sumo_is_alt_vddnb_supported(struct radeon_device *rdev)
{
 struct sumo_power_info *pi = sumo_get_pi(rdev);
 _Bool return_code = 0;
          ...
          _Bool powersaving, _Bool force_nbps1)
{
 ...
 if (force_nbps1)
  param |= 2;
 r600_rcu_wreg(rdev, (0x430), (param));
 sumo_send_msg_to_smu(rdev, 27);
}
void sumo_smu_pg_init(struct radeon_device *rdev)
{
 sumo_send_msg_to_smu(rdev, 1);
}
...
void sumo_enable_boost_timer(struct radeon_device *rdev)
{
 ...
 unit = (r600_rcu_rreg(rdev, (0x434)) & (0xf << 4))
  >> 4;
 period = 100 * (xclk / 100 / sumo_power_of_4(unit));
 timer_value = (period << 16) | (unit << 4);
 r600_rcu_wreg(rdev, (0x50C), (timer_value));
 r600_rcu_wreg(rdev, (0x544), (pi->sys_info.sclk_dpm_boost_margin));
 r600_rcu_wreg(rdev, (0x548), (pi->sys_info.sclk_dpm_throttle_margin));
 r600_rcu_wreg(rdev, (0x540), (pi->sys_info.gnb_tdp_limit));
 r600_rcu_wreg(rdev, (0x520), (pi->sys_info.sclk_dpm_tdp_limit_pg));
 sumo_send_msg_to_smu(rdev, 20);
}
void sumo_set_tdp_limit(struct radeon_device *rdev, u32 index, u32 tdp_limit)
{
 u32 regoffset = 0;
 ...
 sclk_dpm_tdp_limit = r600_rcu_rreg(rdev, (regoffset));
 sclk_dpm_tdp_limit &= ~(mask << shift);
 sclk_dpm_tdp_limit |= (tdp_limit << shift);
 r600_rcu_wreg(rdev, (regoffset), (sclk_dpm_tdp_limit));
}
void sumo_boost_state_enable(struct radeon_device *rdev, _Bool enable)
{
 u32 boost_disable = r600_rcu_rreg(rdev, (0x508));
 boost_disable &= 0xFFFFFFFE;
 boost_disable |= (enable ? 0 : 1);
 r600_rcu_wreg(rdev, (0x508), (boost_disable));
}
u32 sumo_get_running_fw_version(struct radeon_device *rdev)
{
 return r600_rcu_rreg(rdev, (0x30c));
}
