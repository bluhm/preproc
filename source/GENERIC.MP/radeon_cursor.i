...
static void radeon_show_cursor(struct drm_crtc *crtc)
{
 ...
 if (radeon_crtc->cursor_out_of_bounds)
  return;
 if (((rdev->family >= CHIP_CEDAR))) {
  r100_mm_wreg(rdev, (0x69a4 + radeon_crtc->crtc_offset), (((u32)(((radeon_crtc->cursor_addr) >> 16) >> 16))), 0);
  r100_mm_wreg(rdev, (0x699c + radeon_crtc->crtc_offset), (((u32)(radeon_crtc->cursor_addr))), 0);
  r100_mm_wreg(rdev, (0x0000), (0x6998 + radeon_crtc->crtc_offset), 0);
  r100_mm_wreg(rdev, (0x0004), ((1 << 0) | (((2) & 0x3) << 8) | (((4) & 0x7) << 24)), 0);
 } else if (((rdev->family >= CHIP_RS600))) {
  if (rdev->family >= CHIP_RV770) {
   if (radeon_crtc->crtc_id)
    r100_mm_wreg(rdev, (0x640c), (((u32)(((radeon_crtc->cursor_addr) >> 16) >> 16))), 0);
   else
    r100_mm_wreg(rdev, (0x6c0c), (((u32)(((radeon_crtc->cursor_addr) >> 16) >> 16))), 0);
  }
  r100_mm_wreg(rdev, (0x6408 + radeon_crtc->crtc_offset), (((u32)(radeon_crtc->cursor_addr))), 0);
  r100_mm_wreg(rdev, (0x0000), (0x6400 + radeon_crtc->crtc_offset), 0);
  r100_mm_wreg(rdev, (0x0004), ((1 << 0) | (2 << 8)), 0);
 } else {
  r100_mm_wreg(rdev, (0x0260 + radeon_crtc->crtc_offset), (radeon_crtc->cursor_addr - radeon_crtc->legacy_display_base_addr), 0);
  switch (radeon_crtc->crtc_id) {
  case 0:
...
static int radeon_cursor_move_locked(struct drm_crtc *crtc, int x, int y)
{
 ...
 if (((rdev->family >= CHIP_CEDAR))) {
  ...
  r100_mm_wreg(rdev, (0x0260 + radeon_crtc->crtc_offset), (radeon_crtc->cursor_addr - radeon_crtc->legacy_display_base_addr + yorigin * 256), 0);
 }
 if (radeon_crtc->cursor_out_of_bounds) {
  radeon_crtc->cursor_out_of_bounds = 0;
 ...
 if (!radeon_crtc->cursor_out_of_bounds) {
  ...
  radeon_crtc->cursor_out_of_bounds = 1;
 }
 return 0;
}
       ...
       int32_t hot_y)
{
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Failed to pin new cursor BO (%d)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , ret);
  drm_gem_object_unreference_unlocked(obj);
  return ret;
 }
 radeon_lock_cursor(crtc, 1);
