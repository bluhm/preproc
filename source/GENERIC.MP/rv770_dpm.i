...
{
 u32 tmp;
 tmp = rdev->pciep_rreg(rdev, (0xa0)) & ~(0xf << 8);
 tmp |= ((3) << 8);
 rdev->pciep_wreg(rdev, (0xa0), (tmp));
}
static void rv770_enable_l1(struct radeon_device *rdev)
{
 u32 tmp;
 tmp = rdev->pciep_rreg(rdev, (0xa0));
 tmp &= ~(0xf << 12);
 tmp |= ((4) << 12);
 tmp &= ~(1 << 16);
 tmp &= ~(1 << 24);
...
{
 u32 tmp;
 tmp = rdev->pciep_rreg(rdev, (0xa0)) & ~(0xf << 12);
 tmp |= ((8) << 12);
 rdev->pciep_wreg(rdev, (0xa0), (tmp));
 tmp = rv370_pcie_rreg(rdev, (0x40));
 tmp |= (1 << 3);
 tmp &= ~(1 << 4);
 tmp &= ~(1 << 9);
 tmp |= (1 << 12);
 rv370_pcie_wreg(rdev, (0x40), (tmp));
}
      ...
      _Bool enable)
{
 ...
 if (enable) {
  ...
  if (rdev->family == CHIP_RV770)
   mgcg_cgtt_local0 = 0x00100000;
  else
   mgcg_cgtt_local0 = 0;
  r100_mm_wreg(rdev, (0x7d0), (mgcg_cgtt_local0), 0);
  r100_mm_wreg(rdev, (0x7d4), ((0xFFFF0000 & 0xFFFFCFFF)), 0);
  if (pi->mgcgtssm)
   r100_mm_wreg(rdev, (0x9150), (0x55940000), 0);
 } else {
  r100_mm_wreg(rdev, (0x7d0), (0xFFFFFFFF), 0);
  r100_mm_wreg(rdev, (0x7d4), (0xFFFFCFFF), 0);
 }
}
void rv770_restore_cgcg(struct radeon_device *rdev)
{
 _Bool dpm_en = 0, cg_en = 0;
 if (r100_mm_rreg(rdev, (0x63c), 0) & (1 << 0))
  dpm_en = 1;
 if (r100_mm_rreg(rdev, (0x644), 0) & (1 << 7))
  cg_en = 1;
 if (dpm_en && !cg_en)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x644), 0); tmp_ &= (~(1 << 7)); tmp_ |= (((1 << 7)) & ~(~(1 << 7))); r100_mm_wreg(rdev, (0x644), (tmp_), 0); } while (0);
}
...
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 2)); tmp_ |= ((0) & ~(~(1 << 2))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 2)); tmp_ |= (((1 << 2)) & ~(~(1 << 2))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
}
void rv770_enable_acpi_pm(struct radeon_device *rdev)
{
    ...
    RV770_SMC_SWSTATE *smc_state)
{
 ...
 for (i = 0; i < (3 - 1); i++) {
  a_t = ((r[i] * pi->bsp / 200) << 0) | ((l[i] * pi->bsp / 200) << 16);
  smc_state->levels[i].aT = ((__uint32_t)(a_t));
 }
 a_t = ((r[3 - 1] * pi->pbsp / 200) << 0) |
  ((l[3 - 1] * pi->pbsp / 200) << 16);
 smc_state->levels[3 - 1].aT =
  ((__uint32_t)(a_t));
 return 0;
}
int rv770_populate_smc_sp(struct radeon_device *rdev,
         ...
         RV7XX_SMC_MCLK_VALUE *mclk)
{
 ...
 mpll_ad_func_cntl &= ~((0x1f << 7) |
          ...
          (0x1f << 12) |
          (0x3ff << 20));
 mpll_ad_func_cntl |= ((encoded_reference_dividers[dividers.ref_div - 1]) << 7);
 mpll_ad_func_cntl |= ((postdiv_yclk) << 17);
 mpll_ad_func_cntl |= ((clkf) << 0);
 mpll_ad_func_cntl |= ((clkfrac) << 12);
 ...
 if (pi->mem_gddr5) {
  ...
  mpll_dq_func_cntl &= ~((0x1f << 7) |
           ...
           (0x1f << 12) |
           (0x3ff << 20));
  mpll_dq_func_cntl |= ((encoded_reference_dividers[dividers.ref_div - 1]) << 7);
  mpll_dq_func_cntl |= ((postdiv_yclk) << 17);
  mpll_dq_func_cntl |= ((clkf) << 0);
  mpll_dq_func_cntl |= ((clkfrac) << 12);
 ...
 mclk->mclk770.mclk_value = ((__uint32_t)(memory_clock));
 mclk->mclk770.vMPLL_AD_FUNC_CNTL = ((__uint32_t)(mpll_ad_func_cntl));
 mclk->mclk770.vMPLL_AD_FUNC_CNTL_2 = ((__uint32_t)(mpll_ad_func_cntl_2));
 mclk->mclk770.vMPLL_DQ_FUNC_CNTL = ((__uint32_t)(mpll_dq_func_cntl));
 mclk->mclk770.vMPLL_DQ_FUNC_CNTL_2 = ((__uint32_t)(mpll_dq_func_cntl_2));
 mclk->mclk770.vMCLK_PWRMGT_CNTL = ((__uint32_t)(mclk_pwrmgt_cntl));
 mclk->mclk770.vDLL_CNTL = ((__uint32_t)(dll_cntl));
 return 0;
}
         ...
         u8 watermark_level)
{
 ...
 level->gen2PCIE = pi->pcie_gen2 ?
  ((pl->flags & 1) ? 1 : 0) : 0;
 level->gen2XSP = (pl->flags & 1) ? 1 : 0;
 level->backbias = (pl->flags & 4) ? 1 : 0;
         ...
         RV770_SMC_SWSTATE *smc_state)
{
 ...
 if (ret)
  return ret;
 smc_state->levels[0].arbValue = 0x0b;
 smc_state->levels[1].arbValue = 0x0c;
 smc_state->levels[2].arbValue = 0x0d;
     ...
     u32 engine_clock)
{
 ...
 u32 mc_arb_rfsh_rate;
 u32 tmp;
 tmp = (r100_mm_rreg(rdev, (0x2760), 0) & 0x00000038) >> 3;
 dram_rows = 1 << (tmp + 10);
 tmp = r100_mm_rreg(rdev, (0x2a00), 0) & 3;
         ...
         struct radeon_ps *radeon_state)
{
 ...
 radeon_atom_set_engine_dram_timings(rdev, high_clock,
         state->high.mclk);
 sqm_ratio =
  ((64 * high_clock / pi->boot_sclk) << 0) |
  ((64 * high_clock / state->low.sclk) << 8) |
  ((64 * high_clock / state->medium.sclk) << 16) |
  ((64 * high_clock / state->high.sclk) << 24);
 r100_mm_wreg(rdev, (0x2770), (sqm_ratio), 0);
 arb_refresh_rate =
  ((rv770_calculate_memory_refresh_rate(rdev, pi->boot_sclk)) << 0) |
...
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 18)); tmp_ |= (((1 << 18)) & ~(~(1 << 18))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~((1 << 19) | (1 << 18))); tmp_ |= ((0) & ~(~((1 << 19) | (1 << 18)))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
}
      ...
      _Bool enable)
{
 ...
 if (enable) {
  if (pi->sclk_ss)
   do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 23)); tmp_ |= (((1 << 23)) & ~(~(1 << 23))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
  if (pi->mclk_ss) {
   if (rdev->family == CHIP_RV740)
    rv740_enable_mclk_spread_spectrum(rdev, 1);
  }
 } else {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x790), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x790), (tmp_), 0); } while (0);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 23)); tmp_ |= ((0) & ~(~(1 << 23))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x798), 0); tmp_ &= (~(1 << 0)); tmp_ |= ((0) & ~(~(1 << 0))); r100_mm_wreg(rdev, (0x798), (tmp_), 0); } while (0);
  if (rdev->family == CHIP_RV740)
...
static void rv770_program_mpll_timing_parameters(struct radeon_device *rdev)
{
 ...
 if ((rdev->family == CHIP_RV770) && !pi->mem_gddr5) {
  r100_mm_wreg(rdev, (0x654), ((((100 * pi->ref_div) << 0) | ((150) << 16))), 0);
 }
}
void rv770_setup_bsp(struct radeon_device *rdev)
{
 ...
 r100_mm_wreg(rdev, (0x6d0), (pi->dsp), 0);
}
void rv770_program_git(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x6d8), 0); tmp_ &= (~(0xffff << 0)); tmp_ |= ((((0x200) << 0)) & ~(~(0xffff << 0))); r100_mm_wreg(rdev, (0x6d8), (tmp_), 0); } while (0);
}
void rv770_program_tp(struct radeon_device *rdev)
{
 int i;
 enum r600_td td = 0;
 for (i = 0; i < 15; i++)
  r100_mm_wreg(rdev, (0x694 + (i * 4)), ((((r600_utc[i]) << 0) | ((r600_dtc[i]) << 10))), 0);
 if (td == R600_TD_AUTO)
...
void rv770_program_tpp(struct radeon_device *rdev)
{
 r100_mm_wreg(rdev, (0x640), (0x200), 0);
}
void rv770_program_sstp(struct radeon_device *rdev)
{
 r100_mm_wreg(rdev, (0x6e8), ((((0) << 16) | ((0x00C8) << 0))), 0);
}
void rv770_program_engine_speed_parameters(struct radeon_device *rdev)
{
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x610), 0); tmp_ &= (~(1 << 5)); tmp_ |= (((1 << 5)) & ~(~(1 << 5))); r100_mm_wreg(rdev, (0x610), (tmp_), 0); } while (0);
}
static void rv770_enable_display_gap(struct radeon_device *rdev)
{
 u32 tmp = r100_mm_rreg(rdev, (0x714), 0);
 tmp &= ~((3 << 24) | (3 << 26));
 tmp |= (((R600_PM_DISPLAY_GAP_IGNORE) << 24) |
  ((R600_PM_DISPLAY_GAP_IGNORE) << 26));
 r100_mm_wreg(rdev, (0x714), (tmp), 0);
}
void rv770_program_vc(struct radeon_device *rdev)
{
 struct rv7xx_power_info *pi = rv770_get_pi(rdev);
 r100_mm_wreg(rdev, (0x690), (pi->vrc), 0);
}
void rv770_clear_vc(struct radeon_device *rdev)
{
      ...
      RV770_SMC_STATETABLE *table)
{
 ...
 mclk_pwrmgt_cntl = ((1 << 16) |
       ...
       (1 << 23));
 dll_cntl = 0xff000000;
 spll_func_cntl |= (1 << 0) | (1 << 1) | (1 << 3);
 spll_func_cntl_2 &= ~(0x1ff << 0);
 spll_func_cntl_2 |= ((4) << 0);
 table->ACPIState.levels[0].mclk.mclk770.vMPLL_AD_FUNC_CNTL = ((__uint32_t)(mpll_ad_func_cntl));
 table->ACPIState.levels[0].mclk.mclk770.vMPLL_AD_FUNC_CNTL_2 = ((__uint32_t)(mpll_ad_func_cntl_2));
 table->ACPIState.levels[0].mclk.mclk770.vMPLL_DQ_FUNC_CNTL = ((__uint32_t)(mpll_dq_func_cntl));
 table->ACPIState.levels[0].mclk.mclk770.vMPLL_DQ_FUNC_CNTL_2 = ((__uint32_t)(mpll_dq_func_cntl_2));
 table->ACPIState.levels[0].mclk.mclk770.vMCLK_PWRMGT_CNTL = ((__uint32_t)(mclk_pwrmgt_cntl));
 table->ACPIState.levels[0].mclk.mclk770.vDLL_CNTL = ((__uint32_t)(dll_cntl));
          ...
          RV770_SMC_VOLTAGE_VALUE *voltage)
{
 ...
 if ((pi->s0_vid_lower_smio_cntl & pi->mvdd_mask_low) ==
      (pi->mvdd_low_smio[0] & pi->mvdd_mask_low) ) {
  voltage->index = 0;
  voltage->value = ((__uint16_t)(0));
 } else {
  voltage->index = 1;
         ...
         RV770_SMC_STATETABLE *table)
{
 ...
 table->initialState.levels[0].mclk.mclk770.vMPLL_AD_FUNC_CNTL =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_ad_func_cntl));
 table->initialState.levels[0].mclk.mclk770.vMPLL_AD_FUNC_CNTL_2 =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_ad_func_cntl_2));
 table->initialState.levels[0].mclk.mclk770.vMPLL_DQ_FUNC_CNTL =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_dq_func_cntl));
 table->initialState.levels[0].mclk.mclk770.vMPLL_DQ_FUNC_CNTL_2 =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_dq_func_cntl_2));
 table->initialState.levels[0].mclk.mclk770.vMCLK_PWRMGT_CNTL =
  ((__uint32_t)(pi->clk_regs.rv770.mclk_pwrmgt_cntl));
 table->initialState.levels[0].mclk.mclk770.vDLL_CNTL =
  ((__uint32_t)(pi->clk_regs.rv770.dll_cntl));
 table->initialState.levels[0].mclk.mclk770.vMPLL_SS =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_ss1));
 table->initialState.levels[0].mclk.mclk770.vMPLL_SS2 =
  ((__uint32_t)(pi->clk_regs.rv770.mpll_ss2));
 table->initialState.levels[0].mclk.mclk770.mclk_value =
  ((__uint32_t)(initial_state->low.mclk));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL =
  ((__uint32_t)(pi->clk_regs.rv770.cg_spll_func_cntl));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL_2 =
  ((__uint32_t)(pi->clk_regs.rv770.cg_spll_func_cntl_2));
 table->initialState.levels[0].sclk.vCG_SPLL_FUNC_CNTL_3 =
  ((__uint32_t)(pi->clk_regs.rv770.cg_spll_func_cntl_3));
 table->initialState.levels[0].sclk.vCG_SPLL_SPREAD_SPECTRUM =
  ((__uint32_t)(pi->clk_regs.rv770.cg_spll_spread_spectrum));
 table->initialState.levels[0].sclk.vCG_SPLL_SPREAD_SPECTRUM_2 =
  ((__uint32_t)(pi->clk_regs.rv770.cg_spll_spread_spectrum_2));
      ...
      RV770_SMC_STATETABLE *table)
{
 ...
 if (pi->mvdd_control) {
  table->lowSMIO[1] |=
   ((__uint32_t)(pi->mvdd_low_smio[1]));
  table->lowSMIO[0] |=
   ((__uint32_t)(pi->mvdd_low_smio[0]));
  table->voltageMaskTable.highMask[1] = 0;
  table->voltageMaskTable.lowMask[1] =
   ((__uint32_t)(pi->mvdd_mask_low));
 }
 return 0;
}
    ...
    struct radeon_ps *radeon_boot_state)
{
 ...
 int ret;
 __builtin_memset((table), (0), (sizeof(RV770_SMC_STATETABLE)));
 ...
 if (rdev->pm.dpm.platform_caps & 32) {
  ...
  if (rdev->pm.dpm.platform_caps & 0x4000)
   table->extraFlags |= 0x08;
  if (rdev->pm.dpm.platform_caps & 0x2000)
   table->extraFlags |= 0x01;
 }
...
static int rv770_construct_vddc_table(struct radeon_device *rdev)
{
 ...
 u8 vddc_index = 0;
 u32 i;
 radeon_atom_get_min_voltage(rdev, 1, &min);
 radeon_atom_get_max_voltage(rdev, 1, &max);
 radeon_atom_get_voltage_step(rdev, 1, &step);
...
u8 rv770_get_memory_module_index(struct radeon_device *rdev)
{
 return (u8) ((r100_mm_rreg(rdev, (0x1734), 0) >> 16) & 0xff);
}
...
{
 if (enable)
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 10)); tmp_ |= (((1 << 10)) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
 else
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(1 << 10)); tmp_ |= ((0) & ~(~(1 << 10))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
}
static void rv770_program_display_gap(struct radeon_device *rdev)
{
 ...
 if (rdev->pm.dpm.new_active_crtcs & 1) {
  tmp |= ((R600_PM_DISPLAY_GAP_VBLANK) << 24);
  tmp |= ((R600_PM_DISPLAY_GAP_IGNORE) << 26);
 } else if (rdev->pm.dpm.new_active_crtcs & 2) {
  tmp |= ((R600_PM_DISPLAY_GAP_IGNORE) << 24);
  tmp |= ((R600_PM_DISPLAY_GAP_VBLANK) << 26);
 } else {
  tmp |= ((R600_PM_DISPLAY_GAP_IGNORE) << 24);
  tmp |= ((R600_PM_DISPLAY_GAP_IGNORE) << 26);
 }
     ...
     struct radeon_ps *radeon_new_state)
{
 ...
 u16 address = pi->state_table_start +
  __builtin_offsetof(RV770_SMC_STATETABLE, driverState);
...
int rv770_halt_smc(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x10)) != ((uint8_t)0x01))
  return -22;
 if (rv770_wait_for_smc_inactive(rdev) != ((uint8_t)0x01))
  return -22;
 return 0;
}
int rv770_resume_smc(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x11)) != ((uint8_t)0x01))
  return -22;
 return 0;
}
int rv770_set_sw_state(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x20)) != ((uint8_t)0x01))
  do { } while( 0);
 return 0;
}
int rv770_set_boot_state(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x40)) != ((uint8_t)0x01))
  return -22;
 return 0;
}
void rv770_set_uvd_clock_before_set_eng_clock(struct radeon_device *rdev,
...
int rv770_restrict_performance_levels_before_switch(struct radeon_device *rdev)
{
 if (rv770_send_msg_to_smc(rdev, (PPSMC_Msg)(((uint8_t)0x41))) != ((uint8_t)0x01))
  return -22;
 if (rv770_send_msg_to_smc(rdev, (PPSMC_Msg)(((uint8_t)0x15))) != ((uint8_t)0x01))
  return -22;
 return 0;
}
int rv770_dpm_force_performance_level(struct radeon_device *rdev,
          enum radeon_dpm_forced_level level)
{
 ...
 if (level == RADEON_DPM_FORCED_LEVEL_HIGH) {
  if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x13)) != ((uint8_t)0x01))
   return -22;
  msg = ((uint8_t)0x42);
 } else if (level == RADEON_DPM_FORCED_LEVEL_LOW) {
  if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x41)) != ((uint8_t)0x01))
   return -22;
  msg = (PPSMC_Msg)(((uint8_t)0x15));
 } else {
  if (rv770_send_msg_to_smc(rdev, ((uint8_t)0x41)) != ((uint8_t)0x01))
   return -22;
  msg = (PPSMC_Msg)(((uint8_t)0x13));
 }
 if (rv770_send_msg_to_smc(rdev, msg) != ((uint8_t)0x01))
  return -22;
...
static void rv770_read_clock_registers(struct radeon_device *rdev)
{
 ...
 pi->clk_regs.rv770.mpll_dq_func_cntl_2 =
  r100_mm_rreg(rdev, (0x630), 0);
 pi->clk_regs.rv770.mclk_pwrmgt_cntl =
  r100_mm_rreg(rdev, (0x648), 0);
 pi->clk_regs.rv770.dll_cntl = r100_mm_rreg(rdev, (0x64c), 0);
}
...
void rv770_reset_smio_status(struct radeon_device *rdev)
{
 ...
 sw_smio_index =
  (r100_mm_rreg(rdev, (0x63c), 0) & (3 << 6)) >> 6;
 switch (sw_smio_index) {
        case 3:
  vid_smio_cntl = r100_mm_rreg(rdev, (0x684), 0);
  break;
        case 2:
  vid_smio_cntl = r100_mm_rreg(rdev, (0x680), 0);
  break;
        case 1:
  vid_smio_cntl = r100_mm_rreg(rdev, (0x67c), 0);
  break;
        case 0:
  return;
        default:
 ...
 r100_mm_wreg(rdev, (0x678), (vid_smio_cntl), 0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x63c), 0); tmp_ &= (~(3 << 6)); tmp_ |= ((((0) << 6)) & ~(~(3 << 6))); r100_mm_wreg(rdev, (0x63c), (tmp_), 0); } while (0);
}
void rv770_get_memory_type(struct radeon_device *rdev)
{
 struct rv7xx_power_info *pi = rv770_get_pi(rdev);
...
static void rv770_get_mclk_odt_threshold(struct radeon_device *rdev)
{
 ...
 if ((rdev->family == CHIP_RV730) || (rdev->family == CHIP_RV710)) {
  ...
  if (radeon_atom_get_memory_info(rdev, memory_module_index, &memory_info))
   return;
  if (memory_info.mem_type == 0x20 ||
      memory_info.mem_type == 0xb0)
...
static void rv770_set_dpm_event_sources(struct radeon_device *rdev, u32 sources)
{
 ...
 if (want_thermal_protection) {
  do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x72C), 0); tmp_ &= (~(7 << 0)); tmp_ |= ((((dpm_event_src) << 0)) & ~(~(7 << 0))); r100_mm_wreg(rdev, (0x72C), (tmp_), 0); } while (0);
            ...
            int min_temp, int max_temp)
{
 ...
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x734), 0); tmp_ &= (~0x0000FF00); tmp_ |= ((((high_temp / 1000) << 8)) & ~(~0x0000FF00)); r100_mm_wreg(rdev, (0x734), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x734), 0); tmp_ &= (~0x00FF0000); tmp_ |= ((((low_temp / 1000) << 16)) & ~(~0x00FF0000)); r100_mm_wreg(rdev, (0x734), (tmp_), 0); } while (0);
 do { uint32_t tmp_ = r100_mm_rreg(rdev, (0x72C), 0); tmp_ &= (~0x003FC000); tmp_ |= ((((high_temp / 1000) << 14)) & ~(~0x003FC000)); r100_mm_wreg(rdev, (0x72C), (tmp_), 0); } while (0);
...
int rv770_dpm_enable(struct radeon_device *rdev)
{
 ...
 if (pi->voltage_control) {
  ...
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "rv770_construct_vddc_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 if (pi->dcodt)
 ...
 if (pi->mvdd_control) {
  ...
  if (ret) {
   printf("error: [" "drm" ":pid%d:%s] *ERROR* " "rv770_get_mvdd_configuration failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
   return ret;
  }
 }
 if (rdev->pm.dpm.platform_caps & 1)
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "rv770_init_smc_table failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
...
int rv770_dpm_late_enable(struct radeon_device *rdev)
{
 ...
 if (rdev->irq.installed &&
  ...
  PPSMC_Result result;
  ret = rv770_set_thermal_temperature_range(rdev, (90 * 1000), (120 * 1000));
  if (ret)
   return ret;
  rdev->irq.dpm_thermal = 1;
  (rdev)->asic->irq.set((rdev));
  result = rv770_send_msg_to_smc(rdev, ((uint8_t)0x16));
  if (result != ((uint8_t)0x01))
   do { } while( 0);
 }
 return 0;
...
int rv770_dpm_set_power_state(struct radeon_device *rdev)
{
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "rv770_restrict_performance_levels_before_switch failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
 ...
 if (ret) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "rv770_upload_sw_state failed\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return ret;
 }
...
void rv770_dpm_setup_asic(struct radeon_device *rdev)
{
 ...
 if (radeon_aspm != 0) {
  if (rdev->pm.dpm.platform_caps & 8)
   rv770_enable_l0s(rdev);
  if (rdev->pm.dpm.platform_caps & 16)
   rv770_enable_l1(rdev);
  if (rdev->pm.dpm.platform_caps & 1024)
      ...
      union pplib_clock_info *clock_info)
{
 ...
 if (rdev->family >= CHIP_CEDAR) {
  ...
  pl->vddc = (__uint16_t)(__builtin_constant_p(clock_info->r600.usVDDC) ? (__uint16_t)(((__uint16_t)(clock_info->r600.usVDDC) & 0xffU) << 8 | ((__uint16_t)(clock_info->r600.usVDDC) & 0xff00U) >> 8) : __swap16md(clock_info->r600.usVDDC));
  pl->flags = (__uint32_t)(__builtin_constant_p(clock_info->r600.ulFlags) ? (__uint32_t)(((__uint32_t)(clock_info->r600.ulFlags) & 0xff) << 24 | ((__uint32_t)(clock_info->r600.ulFlags) & 0xff00) << 8 | ((__uint32_t)(clock_info->r600.ulFlags) & 0xff0000) >> 8 | ((__uint32_t)(clock_info->r600.ulFlags) & 0xff000000) >> 24) : __swap32md(clock_info->r600.ulFlags));
 }
 pl->mclk = mclk;
 pl->sclk = sclk;
 if (pl->vddc == 0xff01) {
...
int rv7xx_parse_power_table(struct radeon_device *rdev)
{
 ...
 for (i = 0; i < power_info->pplib.ucNumStates; i++) {
  ...
  if (power_info->pplib.ucStateEntrySize - 1) {
   u8 *idx;
   ps = kzalloc(sizeof(struct rv7xx_ps), (0x0001 | 0x0004));
   if (ps == ((void *)0)) {
...
int rv770_dpm_init(struct radeon_device *rdev)
{
 ...
 struct atom_clock_dividers dividers;
 int ret;
 pi = kzalloc(sizeof(struct rv7xx_power_info), (0x0001 | 0x0004));
 if (pi == ((void *)0))
  return -12;
 rdev->pm.dpm.priv = pi;
 ...
 pi->sram_end = 0xC000;
 pi->state_table_start = 0xB000;
 pi->soft_regs_start = 0x104;
 return 0;
}
void rv770_dpm_print_power_state(struct radeon_device *rdev,
     struct radeon_ps *rps)
{
 ...
 if (rdev->family >= CHIP_CEDAR) {
  pl = &ps->low;
  printf("\t\tpower level 0    sclk: %u mclk: %u vddc: %u vddci: %u\n", pl->sclk, pl->mclk, pl->vddc, pl->vddci);
  pl = &ps->medium;
  printf("\t\tpower level 1    sclk: %u mclk: %u vddc: %u vddci: %u\n", pl->sclk, pl->mclk, pl->vddc, pl->vddci);
  pl = &ps->high;
  printf("\t\tpower level 2    sclk: %u mclk: %u vddc: %u vddci: %u\n", pl->sclk, pl->mclk, pl->vddc, pl->vddci);
 } else {
  pl = &ps->low;
  printf("\t\tpower level 0    sclk: %u mclk: %u vddc: %u\n", pl->sclk, pl->mclk, pl->vddc);
  pl = &ps->medium;
  printf("\t\tpower level 1    sclk: %u mclk: %u vddc: %u\n", pl->sclk, pl->mclk, pl->vddc);
  pl = &ps->high;
  printf("\t\tpower level 2    sclk: %u mclk: %u vddc: %u\n", pl->sclk, pl->mclk, pl->vddc);
 }
