...
{
 return (((radeon_vm_num_pdes(rdev) * 8) + (4096 - 1)) & ~(4096 - 1));
}
int radeon_vm_manager_init(struct radeon_device *rdev)
{
 int r;
 if (!rdev->vm_manager.enabled) {
  r = (rdev)->asic->vm.init((rdev));
  if (r)
   return r;
       ...
       struct list_head *head)
{
 ...
 list[0].prefered_domains = 0x4;
 list[0].allowed_domains = 0x4;
 ...
 for (i = 0, idx = 1; i <= vm->max_pde_used; i++) {
  ...
  list[idx].prefered_domains = 0x4;
  list[idx].allowed_domains = 0x4;
  list[idx].tv.bo = &list[idx].robj->tbo;
  list[idx].tv.shared = 1;
           ...
           struct radeon_vm *vm, int ring)
{
 ...
 do { panic("BUG at %s:%d", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_vm.c", 220); } while (0);
 return ((void *)0);
}
void radeon_vm_flush(struct radeon_device *rdev,
       struct radeon_vm *vm,
       int ring, struct radeon_fence *updates)
{
 ...
 if (pd_addr != vm_id->pd_gpu_addr || !vm_id->flushed_updates ||
  ...
  (rdev)->asic->ring[(&rdev->ring[ring])->idx]->vm_flush((rdev), (&rdev->ring[ring]), (vm_id->id), (vm_id->pd_gpu_addr));
 }
}
void radeon_vm_fence(struct radeon_device *rdev,
       struct radeon_vm *vm,
...
{
 struct radeon_bo_va *bo_va;
 bo_va = kzalloc(sizeof(struct radeon_bo_va), (0x0001 | 0x0004));
 if (bo_va == ((void *)0)) {
  return ((void *)0);
 }
 bo_va->vm = vm;
    ...
    uint32_t incr, uint32_t flags)
{
 ...
 if ((flags & ( (1 << 5) | (1 << 6) | (1 << 1) | (1 << 0) )) == ( (1 << 5) | (1 << 6) | (1 << 1) | (1 << 0) )) {
  uint64_t src = rdev->gart.table_addr + (addr >> 12) * 8;
  ((rdev)->asic->vm.copy_pages((rdev), (ib), (pe), (src), (count)));
 } else if ((flags & (1 << 1)) || (count < 3)) {
         ...
         struct radeon_bo *bo)
{
 ...
 r = radeon_ib_get(rdev, 3, &ib, ((void *)0), 256);
 if (r)
  goto error_unreserve;
 ib.length_dw = 0;
 radeon_vm_set_pages(rdev, &ib, addr, 0, entries, 0, 0);
 ((rdev)->asic->vm.pad_ib((&ib)));
 ({ int __ret = !!(ib.length_dw > 64); if (__ret) printf("WARNING %s failed at %s:%d\n", "ib.length_dw > 64", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_vm.c", 414); __builtin_expect(!!(__ret), 0); });
     ...
     uint32_t flags)
{
 ...
 if (soffset) {
  ...
  last_pfn = eoffset / 4096;
  if (last_pfn >= rdev->vm_manager.max_pfn) {
   printf("drm:pid%d:%s *ERROR* " "va above limit (0x%08X >= 0x%08X)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , last_pfn, rdev->vm_manager.max_pfn);
   r = -22;
   goto error_unreserve;
  }
 } else {
 ...
 if (soffset || eoffset) {
  ...
  if (it && it != &bo_va->it) {
   ...
   tmp = ({ const __typeof( ((struct radeon_bo_va *)0)->it ) *__mptr = (it); (struct radeon_bo_va *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo_va, it) );});
   printf("drm:pid%d:%s *ERROR* " "bo %p va 0x%010llx conflict with " "(bo %p 0x%010lx 0x%010lx)\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , bo_va->bo, soffset, tmp->bo, tmp->it.start, tmp->it.last);
   _rw_exit_write(&vm->mutex );
   r = -22;
   goto error_unreserve;
  }
 ...
 if (bo_va->it.start || bo_va->it.last) {
  struct radeon_bo_va *tmp;
  tmp = kzalloc(sizeof(struct radeon_bo_va), (0x0001 | 0x0004));
  if (!tmp) {
   _rw_exit_write(&vm->mutex );
   r = -12;
   goto error_unreserve;
  ...
  __mtx_enter(&vm->status_lock );
  bo_va->it.start = 0;
  bo_va->it.last = 0;
  list_del_init(&bo_va->vm_status);
  list_add(&tmp->vm_status, &vm->freed);
  __mtx_leave(&vm->status_lock );
 }
 if (soffset || eoffset) {
  __mtx_enter(&vm->status_lock );
 ...
 ((!(eoffset >= radeon_vm_num_pdes(rdev))) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_vm.c", 532, "!(eoffset >= radeon_vm_num_pdes(rdev))"));
 ...
 for (pt_idx = soffset; pt_idx <= eoffset; ++pt_idx) {
  ...
  if (vm->page_tables[pt_idx].bo)
   continue;
  _rw_exit_write(&vm->mutex );
  r = radeon_bo_create(rdev, (1 << radeon_vm_block_size) * 8,
         4096, 1,
         0x4, 0,
         ((void *)0), ((void *)0), &pt);
  if (r)
...
{
 uint64_t result;
 result = rdev->gart.pages_entry[addr >> 12];
 result &= ~(4096 - 1);
 return result;
}
...
{
        uint32_t hw_flags = 0;
        hw_flags |= (flags & (1 << 0)) ? (1 << 0) : 0;
        hw_flags |= (flags & (1 << 1)) ? (1 << 5) : 0;
        hw_flags |= (flags & (1 << 2)) ? (1 << 6) : 0;
        if (flags & (1 << 3)) {
                hw_flags |= (1 << 1);
                hw_flags |= (flags & (1 << 4)) ? (1 << 2) : 0;
        }
        return hw_flags;
}
int radeon_vm_update_page_directory(struct radeon_device *rdev,
        struct radeon_vm *vm)
{
 ...
 uint32_t incr = (1 << radeon_vm_block_size) * 8;
 ...
 if (ndw > 0xfffff)
  return -12;
 r = radeon_ib_get(rdev, 3, &ib, ((void *)0), ndw * 4);
 if (r)
  return r;
 ib.length_dw = 0;
 ...
 if (ib.length_dw != 0) {
  ((rdev)->asic->vm.pad_ib((&ib)));
  radeon_sync_resv(rdev, &ib.sync, pd->tbo.resv, 1);
  ({ int __ret = !!(ib.length_dw > ndw); if (__ret) printf("WARNING %s failed at %s:%d\n", "ib.length_dw > ndw", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_vm.c", 704); __builtin_expect(!!(__ret), 0); });
  r = radeon_ib_schedule(rdev, &ib, ((void *)0), 0);
  if (r) {
   radeon_ib_free(rdev, &ib);
   return r;
    ...
    uint64_t addr, uint32_t flags)
{
 ...
 uint64_t frag_start = (((pe_start)+((frag_align)-1))&(~((frag_align)-1)));
...
{
 uint64_t mask = (1 << radeon_vm_block_size) - 1;
 ...
 for (addr = start; addr < end; ) {
  ...
  uint64_t pte;
  int r;
  radeon_sync_resv(rdev, &ib->sync, pt->tbo.resv, 1);
  ...
  if ((addr & ~mask) == (end & ~mask))
   ...
   nptes = (1 << radeon_vm_block_size) - (addr & mask);
   ...
   struct ttm_mem_reg *mem)
{
 ...
 int r;
 if (!bo_va->it.start) {
  printf("drm:pid%d:%s *ERROR* " "bo %p don't has a mapping in vm %p\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__ , bo_va->bo, vm);
  return -22;
 }
 __mtx_enter(&vm->status_lock );
 if (mem) {
 ...
 ((rdev)->asic->vm.pad_ib((&ib)));
 ({ int __ret = !!(ib.length_dw > ndw); if (__ret) printf("WARNING %s failed at %s:%d\n", "ib.length_dw > ndw", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/radeon/radeon_vm.c", 1019); __builtin_expect(!!(__ret), 0); });
 r = radeon_ib_schedule(rdev, &ib, ((void *)0), 0);
 if (r) {
  radeon_ib_free(rdev, &ib);
  return r;
     ...
     struct radeon_vm *vm)
{
 ...
 while (!list_empty(&vm->freed)) {
  bo_va = ({ const __typeof( ((struct radeon_bo_va *)0)->vm_status ) *__mptr = ((&vm->freed)->next); (struct radeon_bo_va *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo_va, vm_status) );});
  __mtx_leave(&vm->status_lock );
  r = radeon_vm_bo_update(rdev, bo_va, ((void *)0));
        ...
        struct radeon_vm *vm)
{
 ...
 while (!list_empty(&vm->invalidated)) {
  bo_va = ({ const __typeof( ((struct radeon_bo_va *)0)->vm_status ) *__mptr = ((&vm->invalidated)->next); (struct radeon_bo_va *)( (char *)__mptr - __builtin_offsetof(struct radeon_bo_va, vm_status) );});
  __mtx_leave(&vm->status_lock );
  r = radeon_vm_bo_update(rdev, bo_va, ((void *)0));
  if (r)
   return r;
        ...
        struct radeon_bo *bo)
{
 struct radeon_bo_va *bo_va;
 for (bo_va = ({ const __typeof( ((__typeof(*bo_va) *)0)->bo_list ) *__mptr = ((&bo->va)->next); (__typeof(*bo_va) *)( (char *)__mptr - __builtin_offsetof(__typeof(*bo_va), bo_list) );}); &bo_va->bo_list != (&bo->va); bo_va = ({ const __typeof( ((__typeof(*bo_va) *)0)->bo_list ) *__mptr = (bo_va->bo_list.next); (__typeof(*bo_va) *)( (char *)__mptr - __builtin_offsetof(__typeof(*bo_va), bo_list) );})) {
  __mtx_enter(&bo_va->vm->status_lock );
  if (list_empty(&bo_va->vm_status) &&
      (bo_va->it.start || bo_va->it.last))
   list_add(&bo_va->vm_status, &bo_va->vm->invalidated);
  __mtx_leave(&bo_va->vm->status_lock );
 }
}
int radeon_vm_init(struct radeon_device *rdev, struct radeon_vm *vm)
{
 const unsigned align = min(32768,
  (1 << radeon_vm_block_size) * 8);
 ...
 _rw_init_flags(&vm->mutex, "vmlk", 0, ((void *)0));
 vm->va = (struct rb_root) { ((void *)0) };
 do { (void)(((void *)0)); (void)(0); __mtx_init((&vm->status_lock), ((((6)) > 0 && ((6)) < 12) ? 12 : ((6)))); } while (0);
 ...
 vm->page_tables = kzalloc(pts_size, (0x0001 | 0x0004));
 if (vm->page_tables == ((void *)0)) {
  printf("error: [" "drm" ":pid%d:%s] *ERROR* " "Cannot allocate memory for page table array\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
  return -12;
 }
...
void radeon_vm_fini(struct radeon_device *rdev, struct radeon_vm *vm)
{
 ...
 int i, r;
 if (!((&vm->va)->rb_node == ((void *)0))) {
  printf("drm:pid%d:%s *ERROR* " "still active bo inside vm\n", (__curcpu->ci_self)->ci_curproc->p_p->ps_pid, __func__);
 }
 for ((bo_va) = (linux_root_RB_MINMAX((struct linux_root *)&vm->va, -1) ? ({ const __typeof( ((__typeof(*bo_va) *)0)->it.rb ) *__mptr = (linux_root_RB_MINMAX((struct linux_root *)&vm->va, -1)); (__typeof(*bo_va) *)( (char *)__mptr - __builtin_offsetof(__typeof(*bo_va), it.rb) );}) : ((void *)0)); ((bo_va) != ((void *)0)) && ({(tmp) = (linux_root_RB_NEXT(&bo_va->it.rb) ? ({ const __typeof( ((typeof(*bo_va) *)0)->it.rb ) *__mptr = (linux_root_RB_NEXT(&bo_va->it.rb)); (typeof(*bo_va) *)( (char *)__mptr - __builtin_offsetof(typeof(*bo_va), it.rb) );}) : ((void *)0)); 1; }); (bo_va) = (tmp)) {
  interval_tree_remove(&bo_va->it, &vm->va);
