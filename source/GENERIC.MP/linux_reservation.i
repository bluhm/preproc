...
_Bool reservation_object_test_signaled_rcu(struct reservation_object *obj,
       _Bool test_all);
struct ww_class reservation_ww_class = { .stamp = 0, .name = "reservation_ww_class" };
;
...
int reservation_object_reserve_shared(struct reservation_object *obj)
{
 ...
 if (old && old->shared_max) {
  ...
  max = 4;
 fobj = kmalloc(__builtin_offsetof(typeof(*fobj), shared[max]), (0x0001 | 0x0004));
 if (!fobj)
  return -12;
 if (obj->staged != ((void *)0))
  __builtin_memcpy((fobj), (obj->staged), (__builtin_offsetof(typeof(*fobj), shared[max])));
 kfree(obj->staged);
 obj->staged = fobj;
 fobj->shared_max = max;
 return 0;
          ...
          struct fence *fence)
{
 ...
 for (i = 0; i < fobj->shared_count; ++i) {
  ...
  old_fence = (fobj->shared[i]);
  if (old_fence->context == fence->context) {
   do { (fobj->shared[i]) = (fence); } while(0);
 ...
 do { (fobj->shared[fobj->shared_count]) = (fence); } while(0);
 fobj->shared_count++;
 write_seqcount_end(&obj->seq);
 ;
}
...
{
 unsigned i;
 struct fence *old_fence = ((void *)0);
 fence_get(fence);
 if (!old) {
  do { (fobj->shared[0]) = (fence); } while(0);
  fobj->shared_count = 1;
  goto done;
 }
 ...
 for (i = 0; i < old->shared_count; ++i) {
  ...
  if (!old_fence && check->context == fence->context) {
   ...
   do { (fobj->shared[i]) = (check); } while(0);
 }
 if (!old_fence) {
  do { (fobj->shared[fobj->shared_count]) = (fence); } while(0);
  fobj->shared_count++;
 ...
 ;
 write_seqcount_begin(&obj->seq);
 do { (obj->fence) = (fobj); } while(0);
 write_seqcount_end(&obj->seq);
 ;
 if (old)
  do { free((void *)old, 145, 0); } while(0);
 if (old_fence)
  fence_put(old_fence);
}
void reservation_object_add_shared_fence(struct reservation_object *obj,
      struct fence *fence)
{
 ...
 obj->staged = ((void *)0);
 if (!fobj) {
  ((!(old->shared_count >= old->shared_max)) ? (void)0 : __assert("diagnostic ", "/home/bluhm/github/preproc/openbsd/src/sys/arch/sparc64/compile/GENERIC.MP/obj/../../../../../dev/pci/drm/linux_reservation.c", 199, "!(old->shared_count >= old->shared_max)"));
           ...
           struct fence *fence)
{
 ...
 ;
 write_seqcount_begin(&obj->seq);
 do { (obj->fence_excl) = (fence); } while(0);
 if (old)
  old->shared_count = 0;
 write_seqcount_end(&obj->seq);
 ;
          ...
          struct fence ***pshared)
{
 ...
 unsigned retry = 1;
 struct fence **shared = ((void *)0), *fence_excl = ((void *)0);
 int ret = 0;
 while (retry) {
  struct reservation_object_list *fobj;
  unsigned seq;
  ...
  if (fobj) {
   ...
   nshared = kmalloc(sz, 0x0002 | 0);
   if (nshared != ((void *)0) && shared != ((void *)0))
    __builtin_memcpy((nshared), (shared), (sz));
   kfree(shared);
   if (!nshared) {
    ;
    nshared = kmalloc(sz, (0x0001 | 0x0004));
    if (nshared != ((void *)0) && shared != ((void *)0))
     __builtin_memcpy((nshared), (shared), (sz));
    kfree(shared);
    if (nshared) {
   ...
   shared = nshared;
   __builtin_memcpy((shared), (fobj->shared), (sz));
...
retry:
 ...
 if (wait_all) {
  ...
  if (read_seqcount_retry(&obj->seq, seq))
   goto unlock_retry;
  for (i = 0; i < shared_count; ++i) {
   struct fence *lfence = (fobj->shared[i]);
 ...
 if (!shared_count) {
  struct fence *fence_excl = (obj->fence_excl);
  if (read_seqcount_retry(&obj->seq, seq))
   goto unlock_retry;
  if (fence_excl &&
